...
class Agent:
    ...
    # Default is empty list - we'll handle termination differently
    terminating_tools: list[Callable] = field(default_factory=list, repr=False)
    ...
    async def step(
        ...
        **kwargs,
        ...
        # No special-casing for send_message or other tools
        # Simply yield the response as is
        yield response
    ...
    async def run(
        ...
        **kwargs,
        ...
        while True:
            ...
            # If the response is a plain text message (not a tool call), treat it as a final response
            # This handles both direct string content and TextBlock content
            if isinstance(response.content, str) or isinstance(
                response.content, TextBlock
            ):
                return

            # Also handle case where content is a sequence containing only TextBlocks
            if (
                isinstance(response.content, Sequence)
                and not isinstance(response.content, str)
            ) and all(isinstance(item, TextBlock) for item in response.content):
                return
            ...
            async for output in self.modify_tool_results(
                ...
                if isinstance(output, Message):
                    followup_messages.append(output)
                else:
                    yield output
            ...
            # Yield any followup messages generated by the capabilities.
            for followup_message in followup_messages:
                yield followup_message
                messages.append(followup_message)

            # Check if we should terminate the tool usage loop, but don't modify the tool call
            if (
                not followup_messages
                and isinstance(last_tool_call, ToolCall)
                and any(
                    last_tool_call.name == tool.__name__
                    for tool in self.terminating_tools
                )
            ):
                # End the loop but preserve the original tool call structure
                break
