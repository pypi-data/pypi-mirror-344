...
@click.option(
    "--forward-tail",
    help="5' tail sequence (5'-3' direction) to remove from forward primers"
)
@click.option(
    "--reverse-tail",
    help="5' tail sequence (5'-3' direction) to remove from reverse primers"
)
@click.option(
    "--gff",
    type=click.Path(exists=True),
...
def main(
    ...
    output: str,
    remove_primers: bool,
    forward_tail: Optional[str],
    reverse_tail: Optional[str],
    gff: Optional[str],
