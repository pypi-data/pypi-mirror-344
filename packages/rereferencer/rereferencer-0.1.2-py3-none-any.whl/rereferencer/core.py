...
def extract_region(
    ...
    forward_matches = find_primer_matches(str(seq_obj), str(forward_obj))
    reverse_matches = find_primer_matches(str(seq_obj.reverse_complement()), str(reverse_obj))
    
    # If no matches in forward orientation, try reverse orientation
    if not forward_matches or not reverse_matches:
        forward_matches = find_primer_matches(str(seq_obj.reverse_complement()), str(forward_obj))
        reverse_matches = find_primer_matches(str(seq_obj), str(reverse_obj))
        if forward_matches and reverse_matches:
            # Convert positions for reverse orientation
            forward_matches = [len(seq) - pos - len(forward_primer) for pos in forward_matches]
            reverse_matches = [pos for pos in reverse_matches]
    ...
    for f_pos in forward_matches:
        for r_pos in reverse_matches:
            # Convert reverse position to original sequence coordinates
            r_pos_orig = len(seq) - r_pos - len(reverse_primer) if not forward_matches else r_pos
            if r_pos_orig > f_pos:
                start = f_pos
                end = r_pos_orig + len(reverse_primer)
                if remove_primers:
                    start += len(forward_primer)
                    end -= len(reverse_primer)
                return (start, end, True, original_forward)
