...
def test_sequential():
    ...
    composed = generic.ComposedPipeline(transform=pipeline, pre=pre, post=post)
    processed = _apply_pipeline(composed, raw)
    assert processed == [10, 20, 30]

    composed = generic.ComposedPipeline(transform=pipeline, pre=pre)
    processed = _apply_pipeline(composed, raw)
    assert processed == [2, 4, 6]

    composed = generic.ComposedPipeline(transform=pipeline, post=post)
    processed = _apply_pipeline(composed, raw)
    assert processed == [5, 10, 15]

    composed = generic.ComposedPipeline(transform=pipeline)
