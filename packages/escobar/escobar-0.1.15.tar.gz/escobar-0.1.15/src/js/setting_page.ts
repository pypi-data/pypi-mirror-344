...
export class SettingsPage {
  ...
  private createContainer(): HTMLDivElement {
    ...
    // UI URL field
    const uiUrlGroup = document.createElement('div');
    uiUrlGroup.className = 'escobar-settings-group';

    const uiUrlLabel = document.createElement('label');
    uiUrlLabel.textContent = 'UI URL';
    uiUrlLabel.htmlFor = 'escobar-ui-url';
    uiUrlGroup.appendChild(uiUrlLabel);

    const uiUrlDescription = document.createElement('div');
    uiUrlDescription.className = 'escobar-settings-description';
    uiUrlDescription.textContent = 'The URL to open in the iframe tab.';
    uiUrlGroup.appendChild(uiUrlDescription);

    const uiUrlInput = document.createElement('input');
    uiUrlInput.id = 'escobar-ui-url';
    uiUrlInput.className = 'escobar-settings-input';
    uiUrlInput.type = 'text';
    uiUrlInput.value = this.currentSettings.uiUrl || 'localhost:3000';
    uiUrlGroup.appendChild(uiUrlInput);

    form.appendChild(uiUrlGroup);
    ...
    this.settingsRegistry.load('escobar:plugin')
      .then(settings => {
        ...
        this.currentSettings = {
          ...
          uiUrl: latestSettings.uiUrl || this.currentSettings.uiUrl || 'localhost:3000'
        };
  ...
  private updateFormFields(): void {
    ...
    const uiUrlInput = document.getElementById('escobar-ui-url') as HTMLInputElement;
    if (uiUrlInput) uiUrlInput.value = this.currentSettings.uiUrl || 'localhost:3000';
  }
  ...
  private saveSettings(): void {
    ...
    const newSettings: IChatSettings = {
      ...
      apiKey,
      username,
      uiUrl: uiUrlInput ? uiUrlInput.value.trim() : 'localhost:3000'
    };
    ...
    this.settingsRegistry.load('escobar:plugin')
      .then(settings => {
        ...
        settings.set('uiUrl', newSettings.uiUrl);
