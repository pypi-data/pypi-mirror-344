...
export function createTopButtons(
  ...
  uiButton.addEventListener('click', () => {
    ...
    alert(`JupyterLab is running at: ${window.location.href}`);

    // Get the UI URL from settings
    const settings = getSettings();
    const uiUrl = settings.uiUrl || 'localhost:3000';

    // Format the URL properly with protocol
    let formattedUrl = uiUrl;
    if (!formattedUrl.startsWith('http://') && !formattedUrl.startsWith('https://')) {
      // Check if it's localhost and use the same protocol as the parent page
      if (formattedUrl.includes('localhost') || formattedUrl.includes('127.0.0.1')) {
        const currentProtocol = window.location.protocol;
        formattedUrl = currentProtocol + '//' + formattedUrl;
        console.log(`Using current protocol (${currentProtocol}) for localhost URL: ${formattedUrl}`);
      } else {
        formattedUrl = 'http://' + formattedUrl;
      }
    }
    alert(`Opening UI at URL: ${formattedUrl}`);
    ...
    if (existingWidget) {
      ...
      try {
        ...
        alert(`Could not update iframe sandbox settings: ${e}`)
      ...
      alert(`Iframe created: ${iframe}`);

      // Add event listeners for debugging iframe loading issues
      iframe.node.addEventListener('load', () => {
        console.log('Iframe loaded successfully');
        try {
          // Try to access iframe content to verify it loaded properly
          const iframeWindow = (iframe.node as HTMLIFrameElement).contentWindow;
          if (iframeWindow) {
            alert('Iframe window accessible')
            console.log('Iframe window accessible');
            try {
              // This might fail due to cross-origin restrictions
              const iframeDoc = iframeWindow.document;
              console.log('Iframe document accessible:', iframeDoc !== null);
            } catch (e) {
              alert(`'Cannot access iframe document due to security restrictions:', ${e}`)
              console.warn('Cannot access iframe document due to security restrictions:', e);
            }
          }
        } catch (e) {
          console.error('Error accessing iframe content:', e);
          alert(`Error accessing iframe content:', ${e}`)
        }
      });

      iframe.node.addEventListener('error', (e) => {
        console.error('Error loading iframe:', e);
        alert(`Error loading iframe:', ${e}`)
        alert('Failed to load iframe content. This might be due to mixed content restrictions.');
      });
      ...
      // Set up a message channel for communication testing
      window.addEventListener('message', (event) => {
        if (event.origin === new URL(formattedUrl).origin) {
          console.log('Received message from iframe:', event.data);
          alert(`Received message from iframe', ${event.data}`)
        }
      });

      // Try to send a message to the iframe after a short delay
      setTimeout(() => {
        try {
          (iframe.node as HTMLIFrameElement).contentWindow?.postMessage('ping', '*');
