...
def SyncStockTradeInfoUSA(shCode='', sDate='20140101', eDate='99991231', truncInd='N', splitAdjInd = False):
    SyncEtfTradeInfoUSA(shCode=shCode, sDate=sDate, eDate=eDate, truncInd=truncInd, splitAdjInd=splitAdjInd)

def SyncEtfTradeInfoUSA(shCode='', sDate='20140101', eDate='99991231', truncInd='N', splitAdjInd = False):
    ''' 
    **kwargs
    ...
    for idx, [shCode, shName] in enumerate(dfCmpList[['종목코드','종목명']].values.tolist()):
        ...
        newData = yahoo.GetEtfTradeInfoUSA(shCode=shCode, frDt = frDt, toDt = toDt, splitAdjInd=splitAdjInd).values.tolist()
