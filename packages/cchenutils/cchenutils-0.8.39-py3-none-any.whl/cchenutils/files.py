...
def read_id(fp, ids, *, agg=None, filter=None) -> Dict | list:
    ...
    Args:
        ...
        filter (dict, optional): A dict specifying filtering conditions for rows. The dictionary should contain
    ...
    if filter is not None:
        dtype |= {k: str for k, v in filter.items() if agg is not None and k not in agg}
    ...
    if agg is None:
        if len(ids) == 1:
            ...
            return ddf[ids].drop_duplicates().compute().apply(tuple, axis=1).tolist()
    else:
...
if __name__ == '__main__':
    ...
    a = read_id(r"D:\weibo_data\comments.csv", ('mid', 'cid'), agg={'cid': 'range'})
