...
def read_id_agg(fp, ids, agg) -> Dict:
    """
    Aggregate values from a CSV file by specified ID field(s).

    Args:
        fp (str): The file path to the CSV file.
        ids (str | list of str): Field name(s) used to group the rows.
        agg (dict): Aggregation to apply, specified as a {field: aggregation} dictionary.
                    Currently only one field-aggregation pair is supported.
                    Use 'range' to return (min, max) tuples.

    Returns:
        Dict: A dictionary mapping each unique ID value (or tuple of values if multiple IDs)
              to the aggregation result. If 'range' is used, values are (min, max) tuples;
              otherwise, single aggregated values.

    Example:
        >>> read_id_agg('data.csv', 'mid', {'cid': 'range'})
        {'A': (1, 5), 'B': (2, 8)}

        >>> read_id_agg('data.csv', ['mid', 'uid'], {'likes': 'sum'})
        {('A', '1'): 10, ('B', '2'): 15}
    """
    if not os.path.exists(fp):
        return Dict()
    ...
    if isinstance(ids, str):
        ids = [ids]
    ddf = dd.read_csv(fp, dtype={idf: str for idf in ids}, encoding='utf-8')
...
if __name__ == '__main__':
    agg = {'cid': 'range'}
    id_fields = 'mid'
    a = read_id_agg(r"D:\weibo_data\comments.csv", 'mid', {'cid': 'range'})
