...
def read_id(fp, ids, *, agg=None, filter=None) -> Dict | list:
    """
    ...
    Args:
        ...
        ids (str | list of str): Field name(s) used to group the rows.
        ...
        filter (dict, optional): A dictionary specifying filtering conditions for rows. The dictionary should contain
            conditions in the form {field: expected_value}, where each field will be filtered
            to match the `expected_value`. Filters can include exact matches, boolean checks
            (True or False), lists of possible values, regex patterns, or custom functions.
            If not provided, no filtering is applied.

    Returns:
        Dict | list:
            - If no aggregation is specified, returns a list of unique ID values.
            - If agg is specified, returns a dictionary mapping each unique ID (or tuple of IDs) to the agg result.
    ...
    Example:
        ...
        >>> read_id('data.csv', 'mid', filter={'status': 'active'})
        ['A', 'B', 'C']
    """
    ...
    if filter is not None:
        cols |= set(filter.keys())

    ddf = dd.read_csv(fp,
                      dtype={idf: str for idf in ids},
                      usecols=list(cols),
                      encoding='utf-8')
        ...
        for field, expected in filter.items():
            match expected:
                case bool():  # True or False
                    ddf = ddf[ddf[field].notna()] if expected else ddf[ddf[field].isna()]
                case list() | set() | tuple():  # Multiple possible values
                    ddf = ddf[ddf[field].isin(expected)]
    ...
    if agg is None:
        if len(ids) == 1:
            return ddf[ids[0]].drop_duplicates().compute().tolist()
        else:
            return ddf[ids].drop_duplicates().compute().values.tolist()
    else:
...
if __name__ == '__main__':
    ...
    a = read_id(r"D:\weibo_data\comments.csv", 'mid',
                agg=agg,
                filter={'cid': lambda x: x > 5160212470431950})
