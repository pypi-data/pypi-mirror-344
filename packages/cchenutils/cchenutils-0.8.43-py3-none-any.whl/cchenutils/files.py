...
def read_id(fp, ids, dtype=None, *, agg=None, filter=None, agg_filter=None) -> Dict | OrderedSet:
    """
    ...
    Args:
        ...
        filter (dict, optional): Filter conditions on individual rows, as {field: condition}.
            Only one condition is applied (first item).
        ...
        agg_filter (dict, optional): Filter rows based on agg'd data, as {field: (agg, condition)}.
            Only one condition is applied (first item).
    ...
    Example:
        ...
        >>> read_id('data.csv', 'mid', agg_filter={'likes': ('count', lambda x: x > 5)})
        ['A', 'C']
    """
    ...
    if agg_filter is not None:
        cols |= set(agg_filter.keys())

    if dtype is None:
        dtype = {}
    dtype |= {idf: str for idf in ids if idf not in dtype}
    if filter is not None:
        dtype |= {k: 'object' for k, v in filter.items()
                  if k not in dtype and (agg is None or k not in agg) and (agg_filter is None or k not in agg_filter)}
    ...
    if agg_filter is not None:
        ...
        k, (a, v) = agg_filter.popitem()
        ddf = ddf.groupby(ids).agg({k: a})
        ddf = _apply_filter(ddf, k, v).reset_index()
...
if __name__ == '__main__':
    ...
    a = read_id(r"D:\weibo_data\comments.csv", 'mid', filter={'cid': lambda x: int(x) > 5159662870334851},
                agg_filter={'cid': ('count', lambda x: x > 99)})
