...
def read_id_agg(fp, id_fields, agg_field, mode) -> Dict:
    """
    Reads a CSV file and obtains the minimum and maximum for a specified range field, grouped by an ID field.
    ...
    Args:
        fp (str): The file path to the CSV file.
        id_fields (list | str): The names of the field used to group the rows.
        agg_field (str): The name of the field whose minimum and maximum values are calculated for each group.
                         The values in this field must be convertible to integers.
        mode (str): Mode of extraction. Options:
            - 'range' (default): Return (min, max) tuple of values.
    ...
    if isinstance(id_fields, str):
        id_fields = [id_fields]
    ddf = dd.read_csv(fp, dtype={idf: str for idf in id_fields}, encoding='utf-8')
    match mode:
        case 'range':
            result = (ddf.groupby(id_fields).agg({agg_field: ['min', 'max']}).compute()
                      .apply(lambda row: (row[(agg_field, 'min')], row[(agg_field, 'max')]), axis=1).to_dict())
        case _:
            result = (ddf.groupby(id_fields).agg({agg_field: mode}).compute()
                      .apply(lambda row: row['cid'], axis=1).to_dict())
    return Dict(result)
