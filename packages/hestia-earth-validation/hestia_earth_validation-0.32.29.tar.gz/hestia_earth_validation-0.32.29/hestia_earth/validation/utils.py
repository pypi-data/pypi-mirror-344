...
def _blank_node_properties_group(blank_node: dict):
    def property_group(property: dict):
        lookup = download_lookup(f"{TermTermType.PROPERTY.value}.csv")
        return get_table_value(lookup, 'termid', property.get('term', {}).get('@id'), column_name('blankNodesGroup'))
...
def group_blank_nodes(nodes: list, by_sum: bool = True):
    ...
    def group_by(group: dict, values: tuple):
        ...
        properties_group = _blank_node_properties_group(blank_node)
