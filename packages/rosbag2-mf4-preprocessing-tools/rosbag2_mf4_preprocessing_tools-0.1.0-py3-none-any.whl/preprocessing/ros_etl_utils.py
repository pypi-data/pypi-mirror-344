...
def generate_extract_task_kwargs(subfolder: str, config: Dict[str, Any]) -> Dict[str, Any]:
    """
    Generates the kwargs dictionary for a mapped extract_task instance.
    Needs to be a top-level function for pickling.
    """
    return {"config": config, "subfolder_path": subfolder}
...
def _extract_ros_data_logic(config: Dict[str, Any], subfolder_path: str) -> Optional[Dict[str, Any]]:
    #for some for the function to work in the PythonVirtualEnvOperator we need to import every needed module inside the function
    #Even the ones defined in the same file
    import logging
    import os
    import time
    from pathlib import Path
