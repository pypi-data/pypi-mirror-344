...
class MLPImageClassifier(BaseModel):
    ...
    def fit(self, x: datasets.Dataset, y: datasets.Dataset):
        ...
        self.model = MLP(self.input_dim, self.output_dim, self.hidden_dims).to(self.device)
    ...
    def save(self, filename: str) -> None:
        """Guarda el checkpoint completo del modelo."""
        checkpoint = {
    ...
    def load(cls, filename: str):
        """Carga el checkpoint completo del modelo."""
        checkpoint = torch.load(filename, map_location=torch.device("cpu"))
        model = MLP(checkpoint['input_dim'], checkpoint['output_dim'], checkpoint['hidden_dims'])
