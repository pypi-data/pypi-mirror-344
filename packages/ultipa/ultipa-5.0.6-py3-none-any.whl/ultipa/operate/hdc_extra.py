...
class HDCExtra(BaseExtra):
    ...
    def createHDCGraphBySchema(self,
        ...
        nodeSchemas, edgeSchemas,builder.direction.value,str(builder.loadId).lower(),builder.syncType,str(builder.isDefault).lower())
    ...
    def showHDCGraph(self,config: RequestConfig = RequestConfig())->List[HDCGraph]:
        ...
        if res.status.code == ULTIPA.ErrorCode.SUCCESS:
            if res.aliases:
                ...
                proj = convertTableToDict(res.alias(res.aliases[0].name).entities.rows,res.alias(res.aliases[0].name).entities.headers)
