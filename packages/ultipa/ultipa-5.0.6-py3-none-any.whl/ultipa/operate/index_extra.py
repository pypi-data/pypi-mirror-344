...
class IndexExtra(BaseExtra):
    ...
    def createIndex(self, dbType: DBType, source: str, indexName: str,
        ...
        '''
        response = Response()
        if FormatType.checkNone(dbType) or FormatType.checkNone(source) or FormatType.checkNone(indexName) or dbType == DBType.DBGLOBAL:
            message = 'The dbType cannot be None' if FormatType.checkNone(dbType) \
                else 'The source cannot be None' if FormatType.checkNone(source) else 'The indexName cannot be None' if FormatType.checkNone(source) else 'The dbType cannot be DBTYPE.DBGLOBAL'
