    ...
    if isinstance(res.data, list) and len(res.data) > 1:
        ...
        for graph in graphdata:
            graphlist.append(GraphSet(
                id=graph.id,
                name=graph.name,
                totalNodes=int(graph.total_nodes) if hasattr(graph,
                                                             'total_nodes') and graph.total_nodes.isdigit() else 0,
                totalEdges=int(graph.total_edges) if hasattr(graph,
                                                             'total_edges') and graph.total_edges.isdigit() else 0,
    ...
    for data in schemas:
        if data.name in count:
            ...
            data.stats = [SchemaStat(fromSchema=pairiter.get('from_schema'),
                                     ...
                                     type=DBType.DBNODE if pairiter.get('type') in 'node' else DBType.DBEDGE  if pairiter.get('type') in 'edge' else None ,
