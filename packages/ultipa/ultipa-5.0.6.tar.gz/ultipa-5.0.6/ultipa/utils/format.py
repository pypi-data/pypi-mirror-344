...
from ultipa import Edge, UltipaPropertyType
...
class FormatType:
		...
		for edge_row in edgeTableData.edge_table.entity_rows:
			...
			if edge_row.values:
				for index, uvalue in enumerate(edge_row.values):
					...
					value = seria.unserialize()
					data.update({schemaHeaderGet(schema_name).get(index): value})
			edge = ULTIPA.Edge(fromId=_from_id, toId=_to_id, values=data, uuid=uuid,fromUuid=_from_uuid,toUuid=_to_uuid,
							   schema=schema_name)
			tables.append(edge)
		return tables
	...
	def attrEntity(entityRows, type: str):
		...
		for data in entityRows.entity_rows:
			...
			if type == "node":
				...
				entityList.append(Edge(schema=data.schema_name, fromId=data.from_id, fromUuid=data.from_uuid, toId=data.to_id, toUuid = data.to_uuid, values=values, uuid=data.uuid))
		return entityList
	...
	def makeEntityNodeTable(schema: ULTIPA_REQUEST.Schema, entity_rows: List[ULTIPA.Node],
		...
		for prop in schema.properties:
			if prop.isIdType() or prop.isIgnore():
				continue
			prop.type=prop.type.value if isinstance(prop.type,Enum) else prop.type if isinstance(prop.type,int) else prop.getPropertyTypeByString(prop.type).value if isinstance(prop.getPropertyTypeByString(prop.type),Enum) else prop.getPropertyTypeByString(prop.type)
			if prop.subType :
				prop.subType[0] = prop.subType[0].value if isinstance(prop.subType[0],Enum)  else prop.subType[0] if isinstance(prop.subType[0],int) else  prop.getPropertyTypeByString(prop.subType[0]).value if isinstance(prop.getPropertyTypeByString(prop.subType[0]),Enum) else prop.getPropertyTypeByString(prop.subType[0])
	...
	def makeEntityEdgeTable(schema: ULTIPA_REQUEST.Schema, rows: List[ULTIPA.Edge],
		...
		for prop in schema.properties:
			...
			if prop.subType :
				prop.subType[0] =  prop.subType[0].value if isinstance(prop.subType[0],Enum)  else prop.subType[0] if isinstance(prop.subType[0],int) else  prop.getPropertyTypeByString(prop.subType[0]).value if isinstance(prop.getPropertyTypeByString(prop.subType[0]),Enum) else prop.getPropertyTypeByString(prop.subType[0])
			edgetable.schemas[0].get("properties").append({"property_name": prop.name, "property_type": prop.type,"sub_types":prop.subType})
	...
	def mergeUqlResponse(mergeRes: ULTIPA_RESPONSE.UltipaResponse, res: ULTIPA_RESPONSE.UltipaResponse):
		...
		if res.data.explainPlan:
			mergeRes.data.explainPlan.planNodes = DataMerge.concat(mergeRes.data.explainPlan.planNodes, res.data.explainPlan.planNodes)
	...
	@staticmethod
	def response(uql_response: ULTIPA_RESPONSE.UltipaResponse, uqlReply, timeZone,
				 timeZoneOffset) -> ULTIPA_RESPONSE.UltipaResponse:
		attrs = FormatType.attrAlias(uqlReply.attrs, timeZone, timeZoneOffset)

		attrs_attrs = []
		attrs_map = {}

		def addAttributes(alias: str, type: ULTIPA.ResultType,propertyType:UltipaPropertyType, values: any, dataMap: HasDataMap) -> None:
		...
		for attrAlias in attrs:
			...
			if attrAlias.attr.type == ULTIPA.UltipaPropertyType.LIST.value or attrAlias.attr.type == ULTIPA.UltipaPropertyType.SET.value:
				if only_attr_list:
					addAttributes(alias, ULTIPA.ResultType.RESULT_TYPE_ATTR,Property.getPropertyByInt(attrAlias.attr.type), attrAlias.attr.values, dataMap)
					continue

				for row in attrAlias.attr.values:
					if not row or not row.type:
						addAttributes(alias, ULTIPA.ResultType.RESULT_TYPE_ATTR,Property.getPropertyByInt(attrAlias.attr.type), row, dataMap)
						continue
					...
					if row.type == ULTIPA.ResultType.RESULT_TYPE_NODE.value:
						addAttributes(alias, ULTIPA.ResultType.RESULT_TYPE_NODE, Property.getPropertyByInt(attrAlias.attr.type),row.nodes, dataMap)

					elif row.type == ULTIPA.ResultType.RESULT_TYPE_EDGE.value:
						addAttributes(alias, ULTIPA.ResultType.RESULT_TYPE_EDGE,Property.getPropertyByInt(attrAlias.attr.type), row.edges, dataMap)

					elif row.type == ULTIPA.ResultType.RESULT_TYPE_PATH.value:
						addAttributes(alias, ULTIPA.ResultType.RESULT_TYPE_PATH, Property.getPropertyByInt(attrAlias.attr.type),row.paths, dataMap)
