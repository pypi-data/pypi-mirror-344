...
def configure_proxy_settings(settings: ProxySettingsManager, args: argparse.Namespace, only_missing: bool = False):
    ...
    first_time = all(not x for x in [ current_id, current_name, current_description, current_affinity, current_api_key, current_base_url])

    if any(not x for x in [current_id, current_name, current_description, current_affinity, current_api_key, current_base_url]):
    ...
    while True:
        if args.configure or not all([
            ...
            if args.alias not in settings.config:
                sys.stdout.write(f"Alias '{args.alias}' not found in configuration file. Adding it...\n")
                settings.config.add_section(args.alias)

            geai_api_key = settings.get_api_key(args.alias)
            geai_base_url = settings.get_base_url(args.alias)
