...
class JuliusApp:
    ...
    def register_input(self, input_fun: callable, id: Optional[str], collection_from: Optional[str], collections_from: Optional[List[str]], extra_collection_from: Optional[str], extra_collections_from: Optional[List[str]], by_args: bool, query: Optional[str], finish_msg: Optional[str], cpu_bound: bool, tags: Dict[str, str], mode: InputWrapper):
        ...
        if self.__mode == "info":
            ...
            if id not in self.__info.inputs:
                try:
                    input_function_info = InputFunctionInfo(id=id, name=input_fun.__name__, arguments=get_fun_info_verbose(input_fun), doc=input_fun.__doc__, collectionsNames=collections_names, extraCollectionsNames=extra_collections_names, query=query, finishMsg=finish_msg, cpu_bound=cpu_bound, tags=tags, mode=mode.value)
                except:
                    self.logs_buffer.write(f"wrong input: {id}")
                    input_function_info = InputFunctionInfo(id=id, name=input_fun.__name__, arguments=get_fun_info_verbose(input_fun), doc=input_fun.__doc__, collectionsNames=collections_names, extraCollectionsNames=extra_collections_names, query=query, finishMsg=finish_msg, cpu_bound=cpu_bound, mode=mode.value)
    ...
    def register_processor(self, processor_fun: callable, id: Optional[str], finish_msg: Optional[str], cpu_bound: bool, tags: Dict[str, str], drop_internal: bool, get_scale_part_all: bool = False):
        if id is None:
        ...
        if self.__mode == "info":
            if id not in self.__info.processors:
                ...
                try:
                    processor_function_info = ProcessorFunctionInfo(id=id, name=processor_fun.__name__, arguments=get_fun_info_verbose(processor_fun), doc=processor_fun.__doc__, finishMsg=finish_msg, cpu_bound=cpu_bound, tags=tags, contextClass=get_context_info(schemes_info[0]))
                except:
                    self.logs_buffer.write(f"wrong processor: {id}")
                    processor_function_info = ProcessorFunctionInfo(id=id, name=processor_fun.__name__, arguments=get_fun_info_verbose(processor_fun), doc=processor_fun.__doc__, finishMsg=finish_msg, cpu_bound=cpu_bound, contextClass=get_context_info(schemes_info[0]))
    ...
    def register_output(self, output_fun: callable, id: Optional[str], collection_out_name: Optional[str], collection_out_names: Optional[List[str]], finish_msg: Optional[str], cpu_bound: bool, tags: Dict[str, str], drop_internal: bool):
        ...
        if self.__mode == "info":
            if id not in self.__info.outputs:
                try:
                    output_function_info = OutputFunctionInfo(id=id, name=output_fun.__name__, arguments=get_fun_info_verbose(output_fun), doc=output_fun.__doc__, collectionOutNames=collection_out_names, finishMsg=finish_msg, cpu_bound=cpu_bound, tags=tags)
                except:
                    self.logs_buffer.write(f"wrong output: {id}")
                    output_function_info = OutputFunctionInfo(id=id, name=output_fun.__name__, arguments=get_fun_info_verbose(output_fun), doc=output_fun.__doc__, collectionOutNames=collection_out_names, finishMsg=finish_msg, cpu_bound=cpu_bound)
    ...
    def register_condition(self, condition_fun: callable, id: Optional[str], finish_msg: Optional[str], cpu_bound: bool, tags: Dict[str, str], drop_internal: bool):
        ...
        if self.__mode == "info":
            if id not in self.__info.conditions:
                try:
                    condition_function_info = ConditionFunctionInfo(id=id, name=condition_fun.__name__, arguments=get_fun_info_verbose(condition_fun), doc=condition_fun.__doc__, finishMsg=finish_msg, cpu_bound=cpu_bound,tags=tags)
                except:
                    self.logs_buffer.write(f"wrong condition: {id}")
                    condition_function_info = ConditionFunctionInfo(id=id, name=condition_fun.__name__, arguments=get_fun_info_verbose(condition_fun), doc=condition_fun.__doc__, finishMsg=finish_msg, cpu_bound=cpu_bound)
    ...
    def register_init(self, init_fun: callable, id: Optional[str], enable: bool, tl: Optional[int], prepare: bool, cpu_bound: bool, tags: Dict[str, str]):
        ...
        if self.__mode == "info":
            try:
                init_function_info = InitInfo(id=id, enable=enable, tl=tl, prepare=prepare, argname=get_context_argname(init_fun), doc=init_fun.__doc__, cpu_bound=cpu_bound, tags=tags)
            except:
                self.logs_buffer.write(f"wrong init: {id}")
                init_function_info = InitInfo(id=id, enable=enable, tl=tl, prepare=prepare, argname=get_context_argname(init_fun), doc=init_fun.__doc__, cpu_bound=cpu_bound)
