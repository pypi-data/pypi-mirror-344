...
class JuliusRegistry(JuliusEmptyRegistry):
    ...
    def info(self) -> AppFunctionsInfo:
        return AppFunctionsInfo(
            inputs={id: self.__function_info_with_tags(InputFunctionInfo(id=id, name=args[0].__name__, arguments=get_fun_info_verbose(args[0]), doc=args[0].__doc__, finishMsg=kwargs["finish_msg"], mode=kwargs["mode"].value, cpu_bound=kwargs["cpu_bound"]), tags=kwargs["tags"]) for id, (args, kwargs) in self.__inputs.items()},
            processors={id: self.__function_info_with_tags(ProcessorFunctionInfo(id=id, name=args[0].__name__, arguments=get_fun_info_verbose(args[0]), doc=args[0].__doc__, finishMsg=kwargs["finish_msg"], contextClass=get_context_info(get_fun_info(args[0])[0]), cpu_bound=kwargs["cpu_bound"]), tags=kwargs["tags"]) for id, (args, kwargs) in self.__processors.items()},     # TODO contextClass
            outputs={id: self.__function_info_with_tags(OutputFunctionInfo(id=id, name=args[0].__name__, arguments=get_fun_info_verbose(args[0]), doc=args[0].__doc__, finishMsg=kwargs["finish_msg"], collectionOutNames=kwargs["collection_out_names"] if (collection_out_name := kwargs["collection_out_name"]) is None else [collection_out_name], cpu_bound=kwargs["cpu_bound"]), tags=kwargs["tags"]) for id, (args, kwargs) in self.__outputs.items()},
            conditions={id: self.__function_info_with_tags(ConditionFunctionInfo(id=id, name=args[0].__name__, arguments=get_fun_info_verbose(args[0]), doc=args[0].__doc__, finishMsg=kwargs["finish_msg"], cpu_bound=kwargs["cpu_bound"]), tags=kwargs["tags"]) for id, (args, kwargs) in self.__conditions.items()},
            schemes={id: json_schema(cl) for id, cl in self.__schemes.items()},
            inits={id: self.__function_info_with_tags(InitInfo(id=id, enable=kwargs["enable"], tl=kwargs["tl"], prepare=kwargs["prepare"], argname=get_context_argname(args[0]), doc=args[0].__doc__, cpu_bound=kwargs["cpu_bound"]), tags=kwargs["tags"]) for id, (args, kwargs) in self.__inits.items()},
