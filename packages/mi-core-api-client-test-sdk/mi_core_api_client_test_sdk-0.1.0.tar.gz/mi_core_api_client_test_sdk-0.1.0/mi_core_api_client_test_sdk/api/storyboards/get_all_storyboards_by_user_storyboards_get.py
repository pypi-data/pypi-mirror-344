...
def sync_detailed(
    *,
    ...
    """Get All Storyboards By User

     Retrieve a paginated list of storyboards for the authenticated user.

    Args:
    - **page**: Page number for pagination (default: 1, minimum: 1).
    - **per_page**: Number of items per page for pagination (default: 5, maximum: 10).
    - **status[StoryboardGenerationStatusEnum | None]**: Filter storyboards by generation status.

    Returns:
    - **PaginatedResponse[StoryboardWithScenesDTO]**: A paginated list of storyboards.
