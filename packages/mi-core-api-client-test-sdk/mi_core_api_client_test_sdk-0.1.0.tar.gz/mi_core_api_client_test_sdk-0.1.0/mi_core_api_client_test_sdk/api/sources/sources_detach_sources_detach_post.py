...
def sync_detailed(
    *,
    ...
    """Sources Detach
     ...
     Detach sources from collections.

    This endpoint detaches all sources from all provided collections if they are attached.

    Args:
    - **source_ids**: List of source ids to detach from collections.
    - **collection_ids**: List of collection ids to detach sources from.
    ...
    - **401 Unauthorized (CollectionNotOwnedByUser)**: Raised if the user provided collection that is
    not owned by him.

    Args:
        source_ids (Union[Unset, list[UUID]]): List of source Ids to detach to collections
        collection_ids (Union[Unset, list[UUID]]): List of collection Ids to detach source to
        project_ids (Union[Unset, list[UUID]]): List of project Ids to detach source to
