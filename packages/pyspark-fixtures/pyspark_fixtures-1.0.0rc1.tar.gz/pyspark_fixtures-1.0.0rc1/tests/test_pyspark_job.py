...
class TestPysparkJob:
    TEST_DIR: Final[str] = os.path.dirname(os.path.abspath(__file__))
    PYSPARK_WAREHOUSE_DIR: Final[str] = f"{TEST_DIR}/pytest-spark-warehouse"

    _spark: SparkSession

    @classmethod
    def setup_class(cls) -> None:
        spark = get_spark_session(cls.PYSPARK_WAREHOUSE_DIR)

        if os.path.exists(cls.PYSPARK_WAREHOUSE_DIR):
            shutil.rmtree(cls.PYSPARK_WAREHOUSE_DIR)
    ...
    def test__transformation(self) -> None:
        ...
        fixtures = PySparkFixtures(
            f"{self.TEST_DIR}/test_data_pyspark_job.md", self._spark
        )
    ...
    def test_populated_input_tables(self) -> None:
        ...
        """
        fixtures = PySparkFixtures(
            f"{self.TEST_DIR}/test_data_pyspark_job.md",
            self._spark,
        )
        generic_populated_input_tables_test(
            self._spark,
    ...
    @classmethod
    def schemas_fetcher(cls, dataset_name: str) -> StructType:
        db_name, table_name, *_ = dataset_name.split(".")
        base_path = f"{cls.TEST_DIR}/schemas"
    ...
    def test_populated_input_tables_with_schemas_fetcher(self) -> None:
        ...
        """
        fixtures = PySparkFixtures(
            f"{self.TEST_DIR}/test_data_pyspark_job_schemas_fetcher.md",
            self._spark,
