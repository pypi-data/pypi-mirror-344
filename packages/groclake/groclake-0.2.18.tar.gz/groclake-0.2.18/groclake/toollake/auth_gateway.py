...
class OAuthGateway(AuthGateway):
    ...
    def __init__(self, tool_name: str, tool_id: int, auth_config: Dict[str, Any]):
        ...
        self.state = auth_config.get('state')  # Store the state from config

    def _connect(self) -> Dict[str, Any]:
        """Connect using OAuth and return the authorization URL or exchange code for token"""
        ...
        if self.auth_config.get('code'):
            return self._exchange_code_for_token()
        else:
            # Generate state if not provided
            if not self.state:
                self.state = self._generate_state_token()
            # Return authorization URL
            print(f"Generating authorization URL with state: {self.state}")
            return self._get_authorization_url()

    def _get_authorization_url(self) -> Dict[str, Any]:
        """Get the OAuth authorization URL"""
        params = {
            "client_id": self.client_id,
            "scope": " ".join(self.scopes) if self.scopes else "",  # Empty string if no scopes
        ...
        # Build URL with parameters, omitting empty values
        query_params = "&".join(f"{k}={v}" for k, v in params.items() if v)
        auth_url = f"{self.auth_url}?{query_params}"
    ...
    def _exchange_code_for_token(self) -> Dict[str, Any]:
        ...
        received_state = self.auth_config.get('state')
        
        if not code:
            raise ValueError("Authorization code is required for token exchange")
            
        # Validate state if it was provided
        if self.state and received_state and self.state != received_state:
