...
def get_attr_jira(issue_code: str, jira):
  dict_attr = {}
...
def get_attr_subtask(subtasks_code: str, jira):
  ...
  for subtask in subtasks_code:
    aux_dicct = {}
    for key, value in get_attr_jira(subtask.key, jira).items():
...
def clean_values(dict_jira_prev, jira):
...
def validate_ticket(username: str, token: str):
  ...
  subtasks_attr = get_attr_jira(issue_code, jira)
  subtasks_code = subtasks_attr['subtasks']
  subtasks_value = get_attr_subtask(subtasks_code, jira)
  ...
  dict_final = clean_values(dict_jira_prev, jira)
