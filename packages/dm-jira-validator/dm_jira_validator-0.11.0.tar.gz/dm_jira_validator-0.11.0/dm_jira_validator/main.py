...
#   print(f"\nRevisando el ticket {dict_final['summary']}" + "(" + issue_code + ")" + "\n")
...
def validate_history(dict_final, issue_code):
    ...
    <h3>Revisando el ticket: {dict_final['summary']} ({issue_code})</h3>
    ...
    if dict_final['issuetype'].upper() == 'HISTORIA':
        ...
        html += f"""
        <tr style="background-color: #f8d7da; color: white;">
            <td>Tipo de Issue</td>
    ...
    for subtask in subtasks:
        ...
        if subtask['resolution'].upper() == 'NUEVO':
            ...
            <tr style="background-color: #d4edda; color: black;">
                <td>{subtask['summary']}</td>
                <td>Resolución</td>
                <td>CORRECTO - Resolución: Nuevo</td>
            </tr>
            """
        else:
            html += f"""
