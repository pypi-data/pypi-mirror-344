...
def smb_traverse_delete_file(username, password, server_name, share_name, report_save_path, port=445, exception_str='', need_smbclient=True):
    """ This function is used to copy local file to public folder
        ...
        for traverse_item_dict in traverse_result_list:
            ...
            if traverse_item_dict['is_folder']:
                if not any(keyword in item_name_upper for keyword in except_str_list):
                    ...
                    smb_traverse_delete_file(username, password, server_name, share_name, folder_path, port, exception_str, False)
            else:
                if not any(keyword in item_name_upper for keyword in except_str_list):
