...
class Balkenschwingungen:
    def __init__(self,_l = 16.40, _A = 1.00,_rho = 5063,_xi = 0.0257,_E = 2.1e11,_I = 0.0288, _BC = "Hinged-Hinged", _cross_section = None):
        ## Calculation Parameters

        self.a_tmax = 1.3  # Faktor, der die Gesamtzeit der Berechnung
        # steuert. Für a_tmax=1 ist die
        #  Berechnungsdauer gleich der
        # Überquerungszeit des Trägers.

        self.ndt = 20  # Anzahl der Unterteilungen der kleinsten

        self.x0 = 8.20  # Stelle der Ausgabe [m]

        self.n = 4  # Anzahl der berücksichtigten Eigenformen

        self.v_zug_kmh = 142  # Zuggeschwindigkeit in [km/h]
        ...
        self.t_ber = 20  # Dauer der Berechnung
    ...
    def load_database(self):
        ...
        data = self.cursor.fetchall()
        for i, row in enumerate(data):
            self.a_lambda_cantilever[i], self.a_lambda_clamped_hinged[i], self.a_lambda_clamped_clamped[i] = row
    ...
    def load_definition(self):
        if self.loading == "train":
            Kraftdefinition = pd.read_csv("Loading/Inputdatei_1.txt", delim_whitespace=True)
    ...
    def numeric_eigenvalues(self):
        ...
        if (self.boundary_condition == "Hinged-Hinged-Numeric"):
            self.Deformation_Num = DeformationMethod(30,self.E, self.I , self.A, self.rho, self.l, "SPG", [1,1], 1000)
    ...
    def load_conditions(self):
        ...
        if self.loading == "train":
            ...
            self.t_ber = self.T_u * self.a_tmax  # Dauer der Berechnung
            ...
            for i in range(self.nt):
                self.F_Mat[i] = self.F_0_H * np.sin(self.OMEGA * self.dt * i - self.PHI_0)
            ...
            for i in range(self.nt):
                if (self.v_pedestrian * self.t[i] > 0) and (
                    ...
                    self.F_Mat[i] = (1+ coeff_1  * np.sin(2  *np.pi * 2.45 * self.t[i])
                                    + coeff_2  * np.sin(4  *np.pi * 2.45* self.t[i] -np.pi/2	)
                                     +coeff_3 * np.sin(6  *np.pi * 2.45* self.t[i] -np.pi/2	))  * g_pedestrian
                else:
                    self.F_Mat[i] = 0
    ...
    def hinged_hinged_numeric(self,x_load,n_eigen):
        interpolator = interp1d(self.Deformation_Num.len_plotting, self.Deformation_Num.eigenmodes_matrix[:, n_eigen], kind='linear', fill_value=0)
    ...
    def generalized_load_function(self):
        ...
        if self.loading == "train":
                ...
                for i in range(self.nt):
                    for j in range(len(self.x_k)):
                        if self.boundary_condition == "Hinged-Hinged":
                            ...
                            try:
                                self.Gen_K[u][i] += self.F_Mat[j][i] * self.hinged_hinged_numeric(
                                    -self.x_k[j] + self.v_zug * self.t[i], u
                                )
                            except:
                                self.Gen_K[u][i] += 0
            ...
            for u in range(self.n):
                for i in range(self.nt):
                    if (self.boundary_condition == "Hinged-Hinged"):
                        self.Gen_K[u][i] = self.F_Mat[i] * self.hinged_hinged(self.x0, u) 
                    elif (self.boundary_condition == "Hinged-Hinged-Numeric"):
                        self.Gen_K[u][i] = self.F_Mat[i] * self.hinged_hinged_numeric(self.x0, u) 
                    elif (self.boundary_condition == "Clamped-Hinged"):
                        self.Gen_K[u][i] = self.F_Mat[i] * self.clamped_hinged(self.x0, u)
                    elif (self.boundary_condition == "Clamped-Clamped"):
                        self.Gen_K[u][i] = self.F_Mat[i] * self.clamped_clamped(self.x0, u)
                    elif (self.boundary_condition == "Cantilever"):
            ...
            for u in range(self.n):
                for i in range(self.nt):
                    if (self.boundary_condition == "Hinged-Hinged"):
                        self.Gen_K[u][i] = self.F_Mat[i] * self.hinged_hinged(self.v_pedestrian * self.t[i], u) 
                    elif (self.boundary_condition == "Hinged-Hinged-Numeric"):
                        try:
                            self.Gen_K[u][i] = self.F_Mat[i] * self.hinged_hinged_numeric(self.v_pedestrian * self.t[i], u) 
                        except:
                            self.Gen_K[u][i] = 0
                    elif (self.boundary_condition == "Clamped-Hinged"):
                        self.Gen_K[u][i] = self.F_Mat[i] * self.clamped_hinged(self.v_pedestrian * self.t[i], u)
                    elif (self.boundary_condition == "Clamped-Clamped"):
                        self.Gen_K[u][i] = self.F_Mat[i] * self.clamped_clamped(self.v_pedestrian * self.t[i], u)
                    elif (self.boundary_condition == "Cantilever"):
                        self.Gen_K[u][i] = self.F_Mat[i] * self.cantilever(self.v_pedestrian * self.t[i], u)
    ...
    def fourier_transformation(self):
        ...
        self.Tges = len(self.t) * self.delta_t                   # Gesamtzeit: Letztes Element des Arrays - Erstes Element des Arrays
