...
class SAP2Assembler:
    ...
    def defineVariable(self, assignment):
        ...
        variable_location = expression[2]
    ...
    def setVariable(self, variable_set_expression):
        ...
        variable_value = variable_specs[2]
    ...
    def identifyLabels(self):
        ...
        assemblyCodeLines = []
        self.address = "00"
        for codeLine in self.assemblyCodeLines:
    ...
    def assemble(self, fileToAssemble, fileToWrite="", row_width=16, hex_data=False, n_bytes=256, print_data=False):
        ...
        if not os.path.exists(fileToWrite) and self.fileToWrite != "":
            raise assemblerError(f"File {fileToWrite} not found")
        ...
        for line_idx, line in enumerate(self.assemblyCodeLines):
            if (".org" not in line) and (".word" not in line):
