...
class BenchmarkTimer:
    ...
    def _print_console_table(self, data: list[list]):
        """
        ...
        data : list[list]
            The prepared data (list of rows).
        """
        headers = ["Section", "Measure", "Last Time (ms)", "Min (ms)", "Max (ms)", "Mean (ms)", "Median (ms)", "Stdev (ms)"]

        formatted_data = []
        for row in data:
            if not row:
                formatted_data.append([""] * len(headers)) #empty row
            elif row[1] == "total_time":  #bold in console
    ...
    def print_results(self, title: str = None, show_total: bool = True):
        """
        ...
        if is_notebook():
            ...
            self._print_console_table(data)
