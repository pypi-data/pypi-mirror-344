...
// __device__ calculate_refractive_index                        //
// __device__ interp1d                                          //
// __device__ interp2d                                          //
...
// Air
__constant__ float B_AIR[2] = {0.05792105f, 0.00167917f};
__constant__ float C_AIR[2] = {238.0185f, 57.362f};

// Fused silica
__constant__ float B_FS[3] = {0.6961663f, 0.4079426f, 0.8974794f};
__constant__ float C_FS[3] = {0.00467914825849f, 0.01351206307396f, 97.934002537921f}; 
...
__constant__ float DIST_EPS = 1e-9;
 ...
 *                       is expected to have a size of at least NUM_ASPHERIC_COEFFS.
...
)
{
    ...
    // Calculate aspheric contribution
    float tot_aspher = 0.f;
    float rn = normalized_radius_squared;

    #pragma unroll
    for (int i = 0; i < NUM_ASPHERIC_COEFFS; i++) {
        tot_aspher += aspheric_coeffs[i] * rn;
        rn *= normalized_radius_squared;
    }
...
)
{
    ...
    // Calculate aspheric contribution to the derivative
    float tot_aspher_deriv = 0.f;
    float rn = normalized_radius;

    #pragma unroll
    for (int i = 0; i < NUM_ASPHERIC_COEFFS; i++) {
        tot_aspher_deriv += (i + 1) * aspheric_coeffs[i] * rn;
        rn *= normalized_radius_squared;
    }
    tot_aspher_deriv *= 2.f;
...
)
{
    ...
    // Divide this result by half_aperture^2 since since ai = Ai*half_aperture^(2*(i+1))
    float normalized_r2 = r2 * inv_half_aperture2;
    float total_aspheric_term = 0.f;
    float pow_norm_r2 = inv_half_aperture2;

    #pragma unroll
    for (int i = 0; i < NUM_ASPHERIC_COEFFS; i++) {
        total_aspheric_term += 2.f * (i+1) * aspheric_coeffs[i] * pow_norm_r2;
        pow_norm_r2 *= normalized_r2;
    }

    float arg_sqrt = 1.f - (1.f + conic_constant) * curvature * curvature * r2;

    // This function is called after the intersection point has been founded
...
)
{
    ...
    // if (iteration>25) printf("%d\n", iteration);
    return new_t;
}

/**
 * @brief Calculates the refractive index of a material based on wavelength.
 *
 * This function calculates the refractive index of either air or fused silica
 * based on the provided wavelength and material index.
 *
 * @param wavelength The wavelength of light in nanometers (nm).
 * @param material_index An integer representing the material:
 *                       - 0: Air
 *                       - 1: Fused Silica
 *                       - Other values: treated in the else condition (currently returns 1.0)
 *
 * @return The calculated refractive index. Returns 1.0 if an invalid material index is provided.
 */
__device__ float 
...
{
    float inv_wavelength_squared = 1e6f / wavelength / wavelength;
    float refractive_index;

    if (material_index == AIR) {
        float inv_c0 = 1.f / (C_AIR[0] - inv_wavelength_squared);
        float inv_c1 = 1.f / (C_AIR[1] - inv_wavelength_squared);
        refractive_index = fmaf(B_AIR[1], inv_c1, fmaf(B_AIR[0], inv_c0, 1.f));
    } else if (material_index == FUSED_SILICA) {
        float inv_c0 = 1.f / (1.f - C_FS[0] * inv_wavelength_squared);
        float inv_c1 = 1.f / (1.f - C_FS[1] * inv_wavelength_squared);
        float inv_c2 = 1.f / (1.f - C_FS[2] * inv_wavelength_squared);
        refractive_index = sqrtf(fmaf(B_FS[2], inv_c2, fmaf(B_FS[1], inv_c1, fmaf(B_FS[0], inv_c0, 1.f))));
    } else {
...
)
{
    ...
    else if (aperture_shape == EXAGONAL) {
    ...
    // Aspherical surface
        ...
        //     max_radius
        // );
        // initial_guess = (0.5f*sag - p.z) * inv_v2;
    ...
    if (inner_aperture>0.f) {
        ...
        else if (central_hole_shape == EXAGONAL) {
            float px = fabsf(xa);
            float py = fabsf(ya);
            // [inner exagon radius]/cos(30/180*pi)=[outer exagon radius]
...
)
{
    ...
    float t, surface_rotation_matrix[9], aspheric_coefficients[10];
    for (int i = 0; i < num_surfaces; i++) {
        ...
        // Surface rotation
        for (int j = 0; j < 9; j++) {
            surface_rotation_matrix[j] = surface_rotation_matrices[i*9 + j];
        }
        ...
        if (shapes[i] == CYLINDRICAL) {
            ...
            for (int j = 0; j < 10; j++) {
                aspheric_coefficients[j] = aspheric_coeffs[i * 10 + j];
            }
            distance_to_aspherical_surface(
                t,
                p,
...
)
{
    ...
    while (true) {
        ...
        if ((surface_types[next_surface_index]==OPAQUE) || (next_surface_index==-1) || bounces_counter>1000) {
        ...
        for (int j = 0; j < 9; j++) {
            surface_rotation_matrix[j] = surface_rotation_matrices[current_surface*9+j];
        }

        // After this call we are in the surface local coordinate system
        ...
        // Update time
        float ri = calculate_refractive_index(wls[k], material_in);
        times[k] += next_surface_distance * ri / C_LIGHT; // C_LIGHT should be defined
        ...
        float A[10];
        for (int j = 0; j < 10; j++) {
            A[j] = aspheric_coefficients[current_surface*10 + j];
        }
        ...
        {
            float refractive_index_ratio = ri / calculate_refractive_index(wls[k], material_out);
...
)
{
    ...
    ts[k] += d_to_module / C_LIGHT * calculate_refractive_index(wls[k], AIR);
