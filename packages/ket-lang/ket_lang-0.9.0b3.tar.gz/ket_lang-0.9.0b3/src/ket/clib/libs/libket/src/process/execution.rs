...
fn submit_execution(
    ...
    match execution_strategy {
        ...
        ExecutionStrategy::MeasureFromSample => {
            let (data, measures, qubit_map) =
                if let (Some(physical_circuit), Some(u2_gates)) = (physical_circuit, u2_gates) {
                    let measures: Vec<_> = physical_circuit
                        .get_measures()
                        .iter()
                        .map(|(qs, i)| (qs.iter().map(|q| q.index()).collect::<Vec<_>>(), *i))
                        .collect();
                    let qubits: Vec<_> = measures.iter().flat_map(|(q, _)| q).unique().collect();
                    let qubit_map: HashMap<_, _> =
                        qubits.iter().enumerate().map(|(i, q)| (**q, i)).collect();

                    let mut circuit = physical_circuit.clone();
                    circuit.clear_measure();
                    circuit.gate_map(u2_gates, parameters);

                    execution.submit_execution(&circuit.final_instructions(), parameters);

                    (execution.get_results(), measures, qubit_map)
                } else {
                    let measures: Vec<_> = logical_circuit
                        .get_measures()
                        .iter()
                        .map(|(qs, i)| (qs.iter().map(|q| q.index()).collect::<Vec<_>>(), *i))
                        .collect();
    ...
    rounds
        ...
        .map(|round| {
            ...
            for (i, measure) in round.iter().enumerate() {
                match measure {
                    Pauli::PauliX => circuit.gate(QuantumGate::Hadamard, qubits[i], &[]),
                    Pauli::PauliY => {
...
impl Process {
    ...
    pub fn execute(&mut self) -> Result<()> {
        assert!(self.execution_strategy.is_none());
        self.execution_strategy = Some(match &self.execution_target.execution_protocol {
            ...
            ExecutionProtocol::SampleBased(exp_value_strategy) => {
                if !self.measurements.is_empty() {
                    ...
                    match exp_value_strategy {
                        Some(ExpValueStrategy::ClassicalShadows {
                            bias,
                            samples,
                            shots,
                        }) => ExecutionStrategy::ClassicalShadows {
                        ...
                        Some(ExpValueStrategy::DirectSample(shots)) => {
                            ExecutionStrategy::DirectSample(*shots)
                        }
                        _ => unreachable!(),
                    }
