...
def rotatelist(data, num_columns, value_idx = 1):
...
def main():
    ...
    if 'OPC_GATE_PORT' in os.environ:    open_port = int(environ['OPC_GATE_PORT'])
    ...
    if opc_mode == 'dcom' and not OpenOPC.opcda.win32com_found:
    ...
    # ACTION: Server Info
        ...
        try:
            output(rotatelist(opc.info(), num_columns), style)
        except Exception as error_msg:
            if opc_mode == 'open': error_msg = error_msg[0]
            print(error_msg)
    ...
    # ACTION: List Servers
        ...
        try:
            output(rotatelist(opc.servers(opc_host), num_columns), style)
        except Exception as error_msg:
            if opc_mode == 'open': error_msg = error_msg[0]
