...
from carate.loss.scaled_loss import ScaledMAELoss, ScaledMSELoss
    ...
    def cv(
        ...
        for i in range(num_cv):
            ...
            for epoch in range(1, num_epoch + 1):
                train_mae_loss, train_loss, total_mae, len_train_loader = self.train(
                    model_net=model_net,
                    epoch=epoch,
                    optimizer=optimizer,
                ...
                logger.log({
                    "Epoch": epoch,
                    "Train_Loss Scaled": "{:16f}".format(train_loss),
                    "Train_MAE_Loss Scaled": "{:16f}".format(train_mae_loss),
                    "Total MAE Train Scaled": "{:16f}".format(total_mae),
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        for data in train_loader:
            ...
            loss = loss(output_probs, data.y, loss_factor)
            mae += loss.item()
            ...
            optimizer.step()
            torch.cuda.empty_cache()
        return mae / len(train_loader), loss, mae, len(train_loader)
