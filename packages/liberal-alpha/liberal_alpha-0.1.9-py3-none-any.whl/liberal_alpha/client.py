        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("status", "").lower() == "success":
                    ...
                    # Create output directory for saving decrypted data (if needed)
                    ...
                    for entry in entries:
                        # Check if this entry has encrypted data and we have a private key
                        if self.private_key and "data" in entry and isinstance(entry["data"], dict):
                            encrypted_data = entry["data"]
                            try:
                                # Decrypt the data using our crypto module
                                decrypted_data = self.crypto.decrypt_alpha_message(self.private_key, encrypted_data)
                                
                                if decrypted_data:
                                    # Save the decrypted data to the entry
                                    ...
                                    filename = output_dir / f"history_{timestamp}_{entry_id}.json"
                                    ...
                                    logger.info(f"Saved decrypted historical data to {filename}")
                                else:
                                    logger.warning(f"Failed to decrypt data for entry {entry.get('id', 'unknown')}")
                                    # Mark as failed decryption
                                    entry["decryption_failed"] = True
                            except Exception as e:
                                logger.error(f"Error decrypting historical data: {e}")
                                entry["decryption_error"] = str(e)
                        
                        decrypted_entries.append(entry)
                    
                    # Replace entries with our processed ones
