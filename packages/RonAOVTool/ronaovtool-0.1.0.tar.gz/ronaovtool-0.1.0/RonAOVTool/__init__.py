...
class editAssetbundle:
	...
	def changePath(self, original_path, new_path):
		
		if not self.data:
			raise ValueError("No data to change")
		
		if not isinstance(self.data, bytes):
			raise TypeError("Incorrect data type to change")
			
		if original_path_hashcode not in self.data:
		...
		original_path_hashcode = b"\x00" * 3 + hascode(original_path, "byte")
		new_path_hashcode = b"\x00" * 3 + hascode(new_path, "byte")
	...
	def save(self):
		if not self.filename:
			raise ValueError("No filename specified to save")
