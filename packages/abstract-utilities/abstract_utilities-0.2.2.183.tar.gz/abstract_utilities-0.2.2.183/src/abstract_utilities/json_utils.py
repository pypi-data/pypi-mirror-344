...
def safe_update_json_datas(json_data,
                           update_data,
                           valid_keys=None,
                           invalid_keys=None
                           ):
    valid_keys=make_list(valid_keys or [])
    invalid_keys=make_list(invalid_keys or [])
    for key,value in update_data.items():
        json_data_value = json_data.get(key)
        if invalid_keys or valid_keys:
            if valid_keys:
                if key in valid_keys:
                    json_data[key] = json_data.get(key) or value
            
            elif invalid_keys:
                if key in invalid_keys and json_data_value != None:
                    del json_data[key]
            else:
                json_data[key] = json_data.get(key) or value
        else:
            json_data[key] = json_data_value or value
...
def safe_updated_json_data(data,
                            file_path,
                            valid_keys=None,
                            invalid_keys=None
                            ):
    ...
    new_data = safe_update_json_datas(json_data=json_data,
