...
def get_package_root(
    ...
    if agent_dir:
        package_root = Path(package_root, agent_dir)
    visited: set[Path] = set()
    while package_root not in visited:
        visited.add(package_root)
        pyproject_path = Path(package_root, config_file)
        if pyproject_path.exists():
            return package_root
        package_root = package_root.parent
    raise FileNotFoundError(
        f"No {config_file} found. Please ensure you are in the resource folder's directory."
    )
...
def select_resource_by_index(value: str, available_resources: list[str]) -> str | None:
    """Attempts to return the resource based on the provided index (as a string)."""
    try:
        index = int(value) - 1
        if index < 0:
            return None
