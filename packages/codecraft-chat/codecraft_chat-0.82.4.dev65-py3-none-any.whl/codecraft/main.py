...
def main_with_utf8():
    ...
    if not sys.flags.utf8_mode:
        # If not already in UTF-8 mode, we need to restart the process with -X utf8
        import subprocess
        
        # Check if we're running as a module or as an installed script
        module_name = os.path.basename(sys.argv[0])
        if module_name.lower() == "codecraft" or module_name.lower() == "codecraft.exe":
            # We're running as an installed script, restart with sys.executable
            cmd = [sys.executable, "-X", "utf8", "-m", "codecraft"] + sys.argv[1:]
            os.environ["CODECRAFT_RESTARTED_WITH_UTF8"] = "1"
            os.execv(sys.executable, cmd)
        else:
            # We're already running as a module, just add the UTF-8 flag
            cmd = [sys.executable, "-X", "utf8"] + sys.argv
            os.environ["CODECRAFT_RESTARTED_WITH_UTF8"] = "1"
            os.execv(sys.executable, cmd)
            
    # On Windows, try to set console to UTF-8 mode
    if sys.platform == "win32":
        try:
            ...
            # Ignore exceptions, as this is just an optimization
            pass
