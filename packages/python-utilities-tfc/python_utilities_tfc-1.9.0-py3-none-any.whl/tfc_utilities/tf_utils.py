...
def get_workspace_runs(workspace_id, api_token=TERRAFORM_CLOUD_TOKEN, url="https://app.terraform.io"):
    """
    Retrieve all runs for a specific Terraform Cloud workspace, using manual pagination (page[number]).
    ...
    Returns:
        list: All runs for the given workspace.
    """
    all_runs = []
    page = 1
    ...
    while True:
        full_url = (
            f"{url}/api/v2/workspaces/{workspace_id}/runs"
            f"?page[number]={page}&filter[operation]={all_operations}"
        )
        response = requests.get(full_url, headers=headers)

        if response.status_code != 200:
            logging.error(f"❌ Error retrieving runs (page {page}): {response.status_code} - {response.text}")
            return None
        ...
        # Stop if fewer than 20 results (end of pages)
        if len(runs) < 20:
            break
