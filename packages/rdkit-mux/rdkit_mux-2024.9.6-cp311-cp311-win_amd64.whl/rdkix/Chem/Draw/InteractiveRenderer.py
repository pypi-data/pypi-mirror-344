...
#
""" Interactive molecule rendering through rdkix-structure-renderer.js """
import base64
import json
import logging
import re
...
rdkixStructureRendererJsUrl = "https://unpkg.com/rdkix-structure-renderer/dist/rdkix-structure-renderer-module.js"
minimalLibJsUrl = "https://unpkg.com/rdkix-structure-renderer/public/RDKix_minimal.js"
...
def setEnabled(shouldEnable=True, quiet=False):
  ...
  loadingMsg = "Loading rdkix-structure-renderer.js..."
  failedToLoadMsg = "Failed to load rdkix-structure-renderer.js:"
...
if (window.rdkStrRnr) {{
  ...
  window.rdkStrRnr = new Promise(resolve => {{
    try {{
      fetch('{rdkixStructureRendererJsUrl}').then(
        r => r.text().then(
