...
class TKCanvas(tk.Canvas, rdkix.sping.pid.Canvas):
  ...
  def __init__(
    ...
    rdkix.sping.pid.Canvas.__init__(self, size=size, name=size)
  ...
  def _getTkColor(self, color, defaultColor):
    if color is None:
      color = defaultColor
    if color is rdkix.sping.pid.transparent:
...
#
  ...
  def _drawRotatedString(self, s, x, y, font=None, color=None, angle=0):
    ...
    try:
      ...
      import rdkix.sping.PIL.pidPIL
      pp = rdkix.sping.PIL.pidPIL
    except ImportError:
    ...
    if color == rdkix.sping.pid.transparent:
      return
...
try:
  import rdkix.sping.PIL

  class TKCanvasPIL(rdkix.sping.PIL.PILCanvas):
    ...
    def __init__(self, size=(300, 300), name='TKCanvas', master=None, **kw):
      rdkix.sping.PIL.PILCanvas.__init__(self, size=size, name=name)
      self._tkcanvas = TKCanvas(size, name, master, **kw)

    def flush(self):
      rdkix.sping.PIL.PILCanvas.flush(self)  # call inherited one first
