...
from rdkix.Chem.Draw.rdMolDraw2D import *


def _sip_available():
  try:
    from rdkix.Chem.Draw.rdMolDraw2DQt import rdkixQtVersion
  except ImportError:
    return False
  pyqt_pkg = f'PyQt{rdkixQtVersion[0]}'
...
def MolToImage(mol, size=(300, 300), kekulize=True, wedgeBonds=True, fitImage=False, options=None,
  ...
  if not hasattr(rdMolDraw2D, 'MolDraw2DCairo'):
    raise RuntimeError("MolToImage requires that the RDKix be built with Cairo support")
...
def ShowMol(mol, size=(300, 300), kekulize=True, wedgeBonds=True, title='RDKix Molecule',
...
def _MolsToGridImage(mols, molsPerRow=3, subImgSize=(200, 200), legends=None,
  ...
  if not hasattr(rdMolDraw2D, 'MolDraw2DCairo'):
    raise RuntimeError("MolsToGridImage requires that the RDKix be built with Cairo support")
...
def ReactionToImage(rxn, subImgSize=(200, 200), useSVG=False, drawOptions=None, returnPNG=False,
  ...
  if not useSVG and not hasattr(rdMolDraw2D, 'MolDraw2DCairo'):
    raise RuntimeError("PNG output requires that the RDKix be built with Cairo support")
...
def DrawRDKixBits(tpls, **kwargs):
  ...
  return DrawRDKixEnvs(envs, **kwargs)


def DrawRDKixBit(mol, bitId, bitInfo, whichExample=0, **kwargs):
  bondPath = bitInfo[bitId][whichExample]
  return DrawRDKixEnv(mol, bondPath, **kwargs)
...
def DrawRDKixEnvs(envs, molsPerRow=3, subImgSize=(150, 150), baseRad=0.3, useSVG=True,
  ...
  for mol, bondpath in envs:
    menv = _getRDKixEnv(mol, bondpath, baseRad, aromaticColor, extraColor, nonAromaticColor,
                        **kwargs)
    submols.append(menv.submol)
    highlightAtoms.append(menv.highlightAtoms)
    atomColors.append(menv.atomColors)
...
def DrawRDKixEnv(mol, bondPath, molSize=(150, 150), baseRad=0.3, useSVG=True,
                 aromaticColor=(0.9, 0.9, 0.2), extraColor=(0.9, 0.9, 0.9), nonAromaticColor=None,
                 drawOptions=None, **kwargs):
  menv = _getRDKixEnv(mol, bondPath, baseRad, aromaticColor, extraColor, nonAromaticColor, **kwargs)
