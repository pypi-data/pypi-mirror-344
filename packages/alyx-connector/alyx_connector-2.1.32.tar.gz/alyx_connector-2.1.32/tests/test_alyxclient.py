...
from alyx_connector import params
...
class TestAuthentication(unittest.TestCase):
    ...
    def test_authentication(self):
        ...
        ac._par = params.from_dict({k: v for k, v in ac._par.as_dict().items() if k not in login_keys})
        ac.logout()
        with mock.patch("builtins.input") as mock_input:
    ...
    def test_auth_errors(self):
        ...
        rep.status_code = 500
        with mock.patch("one.webclient.requests.post", return_value=rep), self.assertRaises(requests.HTTPError):
            self.ac.authenticate()
...
class TestJsonFieldMethods(unittest.TestCase):
    ...
    def _json_field_write(self):
        written1 = self.ac.json_field_write(self.endpoint, self.eids[0], self.field_name, self.data_dict)
        written2 = self.ac.json_field_write(self.endpoint, self.eids[1], self.field_name, self.data_dict)
    ...
    def _json_field_update(self):
        data_field = next(filter(lambda x: x.startswith("low"), self.data_dict))
        modified = self.ac.json_field_update(self.endpoint, self.eids[0], self.field_name, {data_field: 0.6})
...
class TestDownloadHTTP(unittest.TestCase):
    ...
    def test_rest_endpoint_write(self):
        ...
        wa = {"subject": "flowers", "date_time": "2018-06-30T12:34:57", "weight": 22.2, "user": "olivier"}
        ...
        data = {"birth_date": "2018-04-01", "death_date": "2018-09-10"}
        ...
        data = {"birth_date": "2018-04-02", "death_date": "2018-09-09"}
        sub = self.ac.rest("subjects", "partial_update", id="flowers", data=data)
        self.assertEqual(sub["birth_date"], data["birth_date"])
        self.assertEqual(sub["death_date"], data["death_date"])
    ...
    def test_rest_endpoint_read_only(self):
        ...
        # Test with full URL
        d = self.ac.rest("labs", "read", f'{TEST_DB_1["base_url"]}/labs/mainenlab')
    ...
    def test_rest_all_actions(self):
        ...
        newsub = self.ac.rest("subjects", "partial_update", id=nickname, data={"description": "hey"})
