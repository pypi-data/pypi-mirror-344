...
SPEC_DESCRIPTION = {
    "root": (
        "The root path, extracted either from the repository registered in the Alyx database,"
        " or the local path provided by the user."
    ),
    "lab": "The name of the lab where the data were collected (optional).",
    "Subjects": (
        "An optional directory to indicate that the experiment data are divided by "
    ...
    "number": (
        "The sequential session number of the day, optionally zero-padded to be three " "numbers, e.g. 001, 002, etc."
    ),
    "collection": (
        "An optional folder to group data by modality, device, etc.  This is necessary "
    ...
    "revision": (
        "An optional folder to organize data by version.  The version label is arbitrary, "
    ...
    "namespace": (
        "An option filename prefix for data that are not not expected to be a community "
    ...
    "object": (
        "Every file describing a given object has the same number of rows (i.e. the 1st "
    ...
    "attribute": (
        "Together with the object, the attribute represents the type of data in the "
    ...
    "timescale": (
        "If you want to represent times relative to another (non-universal) timescale, "
    ...
    "extra": (
        "File names could have as many optional parts as you like: "
    ...
    "extension": (
        "ALF can deal with any sort of file, as long as it has a concept of a number of "
...
def describe(part=None, width=99):
    ...
    for part in parts:
        ...
        # Split by max width
        lines = flatten(
            textwrap.wrap(ln, width, replace_whitespace=False) for ln in SPEC_DESCRIPTION[part].splitlines()
        )
...
def to_alf(object, attribute, extension, namespace=None, timescale=None, extra=None):
    ...
    # Construct ALF file
    parts = (object, attribute, *extra, extension)
    return ".".join(parts)
