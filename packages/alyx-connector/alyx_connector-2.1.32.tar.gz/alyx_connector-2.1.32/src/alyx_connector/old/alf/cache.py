...
from alyx_connector.alf.io import iter_sessions, iter_datasets
from alyx_connector.alf.files import session_path_parts, get_alf_path
from alyx_connector.converters import session_record2path
...
def _ids_to_uuid(df_ses, df_dsets):
    ...
    df_dsets["eid"] = df_ses.set_index("id_").loc[df_dsets["eid"], "id"].values
    # Check that the session int IDs in both frames match
    ses_id_set = df_ses.set_index("id_")["id"]
    assert (
        df_dsets.set_index("eid_")["eid"].drop_duplicates().equals(ses_id_set)
    ), "session int ID mismatch between frames"
...
def _make_datasets_df(root_dir, hash_files=False) -> pd.DataFrame:
    ...
    for session_path in iter_sessions(root_dir):
        ...
        df = pd.concat((df, pd.DataFrame(rows, columns=DATASETS_COLUMNS)), ignore_index=True, verify_integrity=True)
    return df
