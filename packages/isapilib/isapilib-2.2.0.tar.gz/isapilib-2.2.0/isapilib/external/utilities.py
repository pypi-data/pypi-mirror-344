...
def get_sucursal(mov='Servicio', sucursal=0):
    ...
    if mov in ['Cita Servicio'] and sucursal % 2 == 0:
        sucursal += 1
...
def get_almacen(mov='Servicio', sucursal=0):
    ...
    if (mov in ['Venta Perdida', 'Hist Refacc', 'Reservar'] or 'Nota' in mov) and sucursal % 2 == 1:
        sucursal -= 1
        return Almacen.objects.get(Q(sucursal=sucursal), Q(almacen='R') | Q(almacen__istartswith='RS'))
    else:
        return Almacen.objects.get(sucursal=sucursal, almacen__istartswith='S')
