...
def move_note_or_folder(source, destination):
  ...
  try:
    ...
    print(f"\n[bold green]Note '{source}' moved to '{destination}'.[/bold green]\n")
...
def run():
 ...
 | ||  __/ |  | | | | | | | | | (_) | ||  __/\__ \
  ...
  if quick_note_opened is False:
    if "Quick notes" not in [f for f in os.listdir(BASE_DIR) if os.path.isdir(os.path.join(BASE_DIR, f))]:
      create_folder("Quick notes")
    in_folder = "Quick notes"
    list_notes(in_folder)
    name = f'Untitled {datetime.now().strftime("%H:%M:%S")}'
    tags = ""
  ...
  while True:
    ...
    if choice.startswith("o "):  # Open a folder or note
      ...
      if " " not in specification:
        ...
        seperation = specification.index(" ")
        note_or_folder_to_move = specification[:seperation]
        destination_folder = specification[seperation + 1:]
        move_note_or_folder(note_or_folder_to_move, destination_folder)
