...
def run():
 ...
 | ||  __/ |  | | | | | | | | | (_) | ||  __/\__ \
  ...
  if quick_note_opened is False:
    ...
    print("Note content (press 'Escape' to cancel, or type 'save' to finish):")
    content = ""
    while True:
      if keyboard.is_pressed("esc"):  # Check if the Escape key is pressed
          print("\n[bold red]Note creation canceled.[/bold red]\n")
          return  # Exit the note creation process
      line = input()
      if line.lower() == "save":  # Stop when the user types "save"
          break
