...
import alphagram
from alphagram.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~alphagram.Client.start`, :meth:`~alphagram.idle` and :meth:`~alphagram.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that Alphagram's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`alphagram.compose`.
