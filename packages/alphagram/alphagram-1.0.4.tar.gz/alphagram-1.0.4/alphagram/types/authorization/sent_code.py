...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~alphagram.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~alphagram.Client.sign_in` or :meth:`~alphagram.Client.sign_up`).

        next_type (:obj:`~alphagram.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~alphagram.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~alphagram.Client.resend_code`.
    """
