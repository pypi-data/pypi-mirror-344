...
DEFAULT_API_KEY = os.environ.get("API_KEY", "sk-KohA4NPkpBqo6mmNO175JaGsJMiKplTmemchA64oUW1CCy9q")
DEFAULT_BASE_URL = os.environ.get("API_BASE_URL", "https://api.ssopen.top")
# 图床API配置
DEFAULT_IMAGE_HOSTING_TOKEN = os.environ.get("IMAGE_HOSTING_TOKEN", "357|NE5P2BQu5EECD0t3DO2ThUyxy6zViDBAP17J3JSv")
DEFAULT_IMAGE_HOSTING_URL = os.environ.get("IMAGE_HOSTING_URL", "https://tu.my/api/v1/upload")
# 是否禁用本地保存
DISABLE_LOCAL_SAVE = os.environ.get("DISABLE_LOCAL_SAVE", "false").lower() == "true"

# 服务器启动时打印配置信息（不显示敏感信息）
print(f"配置信息:", file=sys.stderr)
print(f"  API_BASE_URL: {DEFAULT_BASE_URL}", file=sys.stderr)
print(f"  API_KEY: {'已设置' if DEFAULT_API_KEY else '未设置，如没有key可到https://api.ssopen.top/ 注册'}", file=sys.stderr)
print(f"  IMAGE_HOSTING_URL: {DEFAULT_IMAGE_HOSTING_URL}", file=sys.stderr)
print(f"  IMAGE_HOSTING_TOKEN: {'已设置' if DEFAULT_IMAGE_HOSTING_TOKEN else '未设置，如没有token可到https://tu.my/ 注册'}", file=sys.stderr)
print(f"  DISABLE_LOCAL_SAVE: {DISABLE_LOCAL_SAVE}", file=sys.stderr)
