...
class WebApp:
	...
	@route(config.get_relative_url(config.DB_EVENTS),methods=[_A])
	def list_db_events(self,request:Request)->ListEventsResponse:
		try:B=self._event_storage_service.list_db_events()
		except Exception as A:LOG.error(f"Failed to list db events: {A}");return ListEventsResponse(status=400,error=f"Error occurred while fetching all db events: {A}")
		return ListEventsResponse(status=200,events=B)
