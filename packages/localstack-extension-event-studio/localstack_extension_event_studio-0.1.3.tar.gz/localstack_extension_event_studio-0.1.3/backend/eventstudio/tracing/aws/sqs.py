...
class SQSInstrumentation(Instrumentation):
	...
	def _put_message_sqs_queue(I,fn,self_,message:Message,**J)->dict:
		H='Send';F=message;A=self_
		try:C=get_trace_context();K=C.parent_id;D=C.trace_id;L=C.version;E=F.copy();M,N=load_sqs_message_body(E['Body']);O=pars_timestamp_ms(list(E['Attributes'].values())[1]);G=get_request_context();P=G.account_id;Q=G.region;B=is_lambda_helper_queue(A.arn,connect_to(region_name=Q,aws_access_key_id=P).lambda_);R=InputEventModel(parent_id=K,trace_id=D,event_id=E.pop('MessageId'),version=L,account_id=A.account_id,region=A.region,service=ServiceName.SQS,resource_name=A.name,arn=A.arn,operation_name=H,is_replayable=B if B else True,is_hidden=B if B else False,event_data={'body':M},event_metadata={'queue_arn':A.arn,'body_type':N,'original_time':O});S,D=I.event_storage_service.add_event(InputEventModel.model_validate(R));T=TraceContext(trace_id=D,parent_id=S);set_trace_context(T)
		except Exception as U:log_event_studio_error(logger=LOG,service=ServiceName.SQS,operation=H,error=str(U))
		return fn(A,message=F,**J)