...
from eventstudio.types.errors import ErrorModel
from eventstudio.types.services import ServiceName
RegionName=str
SQSMessageBodyType=Literal[_F,_G,'XML']
S3DataType=Literal[_F,_H]
APIGatewayBodyType=Literal[_F,_H,_G,'XML','EMPTY']
class APIGatewayEventData(BaseModel):model_config=ConfigDict(extra=_B);body:dict|str;request:dict
class APIGatewayEventMetadata(BaseModel):model_config=ConfigDict(extra=_B);api_type:str;api_name:str;deployment_id:str;stage_name:str;request_type:str;body_type:APIGatewayBodyType
...
class EventsEventData(BaseModel):model_config=ConfigDict(extra=_B);version:str;detail_type:str;source:str;resources:list[str]|_A;detail:dict|str
class EventsEventPartialData(BaseModel):model_config=ConfigDict(extra=_B);version:str;detail_type:str|_A;source:str|_A;resources:list[str]|_A;detail:dict|str|_A
...
class ExternalEventMetadata(BaseModel):model_config=ConfigDict(extra=_B);user_agent:str;browser:str|_A=_A;language:str|_A=_A;platform:str|_A=_A;version:str|_A=_A
...
class SQSEventMetadata(BaseModel):model_config=ConfigDict(extra=_B);queue_arn:str;body_type:SQSMessageBodyType;message_attributes:dict|_A=_A;message_system_attributes:dict|_A=_A;original_time:datetime|_A=_A
class S3EventData(BaseModel):model_config=ConfigDict(extra=_B);body:str|_A=_A
class S3EventMetadata(BaseModel):model_config=ConfigDict(extra=_B);bucket:str;key:str;data_type:S3DataType
class InputEventModel(BaseModel):
	model_config=ConfigDict(extra='ignore');parent_id:str|_A=_A;trace_id:str|_A=_A;event_id:str|_A=_A;version:int=0;status:str='OK';is_deleted:bool=_E;is_replayable:bool=_E;is_edited:bool=_E;is_hidden:bool=_E;account_id:str;region:str;service:ServiceName;resource_name:str;arn:str;operation_name:str;creation_time:datetime|_A=_A;SERVICE_MAPPINGS:ClassVar[Dict[ServiceName,Dict[str,tuple]]]={ServiceName.APIGATEWAY:{_D:(APIGatewayEventData,),_C:(APIGatewayEventMetadata,)},ServiceName.DYNAMODB:{_D:(DynamoDBEventData,),_C:(DynamoDBEventMetadata,)},ServiceName.EVENTS:{_D:(EventsEventData,EventsEventPartialData),_C:(EventsEventMetadata,)},ServiceName.EXTERNAL:{_C:(ExternalEventMetadata,)},ServiceName.LAMBDA:{_D:(LambdaEventData,),_C:(LambdaEventMetadata,)},ServiceName.INTERNAL:{_C:(InternalEventMetadata,)},ServiceName.SNS:{_D:(SNSEventData,),_C:(SNSEventMetadata,)},ServiceName.SQS:{_D:(SQSEventData,),_C:(SQSEventMetadata,)},ServiceName.S3:{_D:(S3EventData,),_C:(S3EventMetadata,)}}
...
class EventModel(InputEventModel):model_config=ConfigDict(from_attributes=True);span_id:str;errors:list['ErrorModel']=[];children:list['EventModel']=[]
