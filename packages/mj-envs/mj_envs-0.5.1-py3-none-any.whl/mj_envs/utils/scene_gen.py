...
def _construct_scene(
    ...
    for i, camera in enumerate(camera_extras):
        ...
        camera_info = CameraInfo(
            name = camera[0],
            pos = camera[1].get("pos", [0, 0, 0]),
            euler = camera[1].get("euler", [0, 0, 0]),
            fovy = camera[1].get("fovy", 70),
            attach_to = camera[1].get("attach_to", None),
            attach_type = camera[1].get("attach_type", None)
        )
