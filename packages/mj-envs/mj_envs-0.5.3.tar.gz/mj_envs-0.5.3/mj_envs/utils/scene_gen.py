...
def construct_scene(
    ...
    camera_extras: List[Tuple[str, dict]] = (),
):
    ...
    model, root = _construct_scene(task_cfg, robot_cfg, thirdperson, \
                                   worldbody_extras, camera_extras)
    return model
...
def _construct_scene(
    ...
    for i, camera in enumerate(camera_extras):
        # convert camera (which is dictionary) to a CameraInfo object
        camera_info = CameraInfo(
            name = camera[0],
            pos = camera[1]['init'].get("pos", [0, 0, 0]),
            euler = camera[1]['init'].get("euler", [0, 0, 0]),
            fovy = camera[1]['init'].get("fovy", 70),
            attach_to = camera[1]['init'].get("attach_to", None),
            attach_type = camera[1]['init'].get("attach_type", None)
        )

        if camera_info.attach_to is not None: 
            assert camera_info.attach_type is not None 

            print(f"Attaching camera {i} to {camera_info.attach_to} of type {camera_info.attach_type}")

            mat = R.from_euler("xyz", camera_info.euler, degrees = False).as_matrix() 
            mat = mat @ rot_y(-90)[:3, :3] @ rot_z(-90)[:3, :3]
            quat = R.from_matrix(mat).as_quat(scalar_first = True)


            root, m = attach_body_to_body(root, body_name = camera_info.attach_to, mocap_name = camera_info.name, \
                                    pos = camera_info.pos, quat = quat, size = 1)
            m.add("camera", name=camera_info.name, fovy=camera_info.fovy)

        else: 

            cam_body = add_mocap_body_to_model(root, camera_info.name) 
            cam_body.pos = camera_info.pos
            cam_body.quat = R.from_euler("xyz", camera_info.euler, degrees = True).as_quat(scalar_first = True)

            cam_body.add("camera", name=camera_info.name, fovy=camera_info.fovy)
