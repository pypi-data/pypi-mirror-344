...
def _construct_scene(
    ...
    for i, camera in enumerate(camera_extras):
        ...
        camera.pos = camera[1].get("pos", [0, 0, 0])
        camera.euler = camera[1].get("euler", [0, 0, 0])
        camera.fovy = camera[1].get("fovy", 70)
        camera.attach_to = camera[1].get("attach_to", None)
        camera.attach_type = camera[1].get("attach_type", None)
