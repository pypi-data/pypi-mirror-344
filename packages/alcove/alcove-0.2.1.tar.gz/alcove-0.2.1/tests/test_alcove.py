...
def test_add_file(setup_test_environment):
    ...
    # check if data path is added to data/.gitignore
    data_gitignore_file = tmp_path / "data" / ".gitignore"
    assert data_gitignore_file.exists()
    with open(data_gitignore_file, "r") as f:
        gitignore_content = f.read()
        assert "tables/" in gitignore_content
        assert f"snapshots/{uri.path}.txt" in gitignore_content
...
def test_shelve_directory(setup_test_environment):
    ...
    with open(data_gitignore_file, "r") as f:
        gitignore_content = f.read()
        assert "tables/" in gitignore_content
        assert f"snapshots/{path}" in gitignore_content
