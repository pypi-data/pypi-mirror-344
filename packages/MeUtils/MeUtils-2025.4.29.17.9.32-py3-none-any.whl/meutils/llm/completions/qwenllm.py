...
async def create(request: CompletionRequest, token: Optional[str] = None):  # ChatCompletionRequest 重构
    ...
    if not request.stream:
        ...
        chunk.choices[0].delta.content = nostream_content
        chunk.choices[0].delta.reasoning_content = nostream_reasoning_content

        yield chat_completion_chunk.construct(**chunk.model_dump(exclude_none=True))
