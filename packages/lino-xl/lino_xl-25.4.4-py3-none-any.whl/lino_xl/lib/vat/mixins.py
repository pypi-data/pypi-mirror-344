...
class VatDocument(Voucher, ProjectRelated, VatTotal):
    ...
    def get_vat_subtotals(self):
        ...
        for k, amounts in sums.items():
            (cat, rate) = k
            yield cat, rate, myround(amounts[0]), myround(amounts[1])
...
dd.update_field(
    VatItemBase, 'total_incl', verbose_name=_("Price to pay"), decimal_places=4)
dd.update_field(
    VatItemBase, 'total_base', verbose_name=_("Price excl. VAT"), decimal_places=4)
dd.update_field(
    VatItemBase, 'total_vat', verbose_name=_("VAT to pay"), decimal_places=4)
