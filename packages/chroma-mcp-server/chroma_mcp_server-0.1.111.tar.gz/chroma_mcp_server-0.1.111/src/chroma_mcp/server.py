...
def config_server(args: argparse.Namespace) -> None:
    ...
    try:
        # --- Load .env --- >
        # ADD DEBUG: Print received args
        print(f"DEBUG [config_server]: Received args: {args}", file=sys.stderr)
        ...
        # ADD DEBUG: Print logger name
        print(f"DEBUG [config_server]: Configuring logger named '{BASE_LOGGER_NAME}'", file=sys.stderr)
        logger.setLevel(log_level)

        # ADD DEBUG: Check handlers
        print(f"DEBUG [config_server]: logger.hasHandlers() before adding? {logger.hasHandlers()}", file=sys.stderr)
        ...
        if not logger.hasHandlers():
            ...
            if log_dir:
                # ADD DEBUG: Log attempt to add file handler
                print(
                    f"DEBUG [config_server]: Attempting to create/add file handler for dir: {log_dir}", file=sys.stderr
                )
                ...
                print(f"DEBUG [config_server]: File handler added for {log_file}", file=sys.stderr)
            else:
                print(f"DEBUG [config_server]: log_dir is None or empty, skipping file handler.", file=sys.stderr)
        else:
            print(f"DEBUG [config_server]: Logger already has handlers, skipping handler setup.", file=sys.stderr)
