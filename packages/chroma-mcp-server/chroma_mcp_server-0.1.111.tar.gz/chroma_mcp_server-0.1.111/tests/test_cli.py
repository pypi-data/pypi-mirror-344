...
def test_cli_main_stdio_keyboard_interrupt(mock_asyncio_run, mock_parse_args):
...
@patch("src.chroma_mcp.cli.parse_args")
@patch("asyncio.run")  # Patch asyncio.run used in stdio path
def test_cli_main_stdio_generic_exception(mock_asyncio_run, mock_parse_args):
    """Test cli.main() handles generic Exceptions and returns 1 in stdio mode."""
    # Simulate parse_args returning args for stdio mode
    mock_args = MagicMock()
    mock_args.mode = "stdio"
    mock_parse_args.return_value = mock_args

    # Simulate generic Exception during asyncio.run
    error_message = "Stdio failed"
...
def test_cli_main_http_keyboard_interrupt(mock_server_main, mock_config_server, mock_parse_args):
    ...
    # Simulate parse_args returning args for http mode
    mock_args = MagicMock()
    mock_args.mode = "http"
    mock_parse_args.return_value = mock_args
    ...
    result = cli_main()
    assert result == 0  # Clean exit on Ctrl+C
    mock_config_server.assert_called_once_with(mock_args)  # Ensure config was called
    mock_server_main.assert_called_once()  # Ensure http path was attempted
...
def test_cli_main_http_generic_exception(mock_server_main, mock_config_server, mock_parse_args):
    """Test cli.main() handles generic Exceptions and returns 1 in http mode."""
    # Simulate parse_args returning args for http mode
    mock_args = MagicMock()
    mock_args.mode = "http"
    mock_parse_args.return_value = mock_args

    # Simulate generic Exception during server_main
    error_message = "HTTP server failed"
    mock_server_main.side_effect = Exception(error_message)
...
@patch("src.chroma_mcp.cli.parse_args")
@patch("src.chroma_mcp.cli.config_server")  # Patch config_server called in http path
# No need to patch server_main as the exception happens earlier
def test_cli_main_http_config_exception(mock_config_server, mock_parse_args):
    """Test cli.main() handles exceptions during config_server in http mode."""
    # Simulate parse_args returning args for http mode
    mock_args = MagicMock()
    mock_args.mode = "http"
    mock_parse_args.return_value = mock_args

    # Simulate exception during config_server
    error_message = "Config failed"
    mock_config_server.side_effect = Exception(error_message)
