...
def mock_mcp_client():
    ...
    client.initialize = AsyncMock(return_value=None)  # Simulate successful initialization

    # Mock call_tool to return different values based on tool name
    async def mock_call_tool(*args, **kwargs):
        name = kwargs.get("name")
        # Default empty result
        result_text = "{}"
        if name == "chroma_sequential_thinking":
            # Return session_id only for the first thought in a chain/branch for simplicity
            arguments = kwargs.get("arguments", {})
            thought_num = arguments.get("thought_number", 0)
            if thought_num == 1:
                result_text = json.dumps({"session_id": "mock-session-id"})
            else:
                result_text = json.dumps({})  # Empty for subsequent thoughts
        ...
        # Create a mock TextContent object for the result
        text_content_mock = MagicMock(spec=types.TextContent)
        text_content_mock.text = result_text
        # Return the list containing the TextContent directly
        return [text_content_mock]
