...
from brocc.browser.chrome_process import REMOTE_DEBUG_PORT
...
async def get_tabs() -> List[ChromeTab]:
    ...
    try:
        ...
        logger.debug(f"[get_tabs] Getting tab list from: {url_to_check}")  # DEBUG
...
async def get_chrome_info():
    ...
    try:
        url_to_check = f"http://localhost:{REMOTE_DEBUG_PORT}/json/version"
        logger.debug(f"[get_chrome_info] Getting version from: {url_to_check}")  # DEBUG
        async with httpx.AsyncClient() as client:
            response = await client.get(
                url_to_check,  # Use variable
...
async def _get_html_using_dom(ws_url: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        logger.debug(f"[_get_html_using_dom] Connecting to WebSocket: {ws_url}")  # DEBUG
