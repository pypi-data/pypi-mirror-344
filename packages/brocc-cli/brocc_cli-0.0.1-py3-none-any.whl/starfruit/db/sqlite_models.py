...
class Item(Base):
    ...
    def create_or_update(
        ...
        original_url = proc_item.url
        normalized_url = normalize_url(original_url)

        db_item = session.query(cls).filter(cls.url == normalized_url).first()
        ...
        if db_item:
            ...
            session.flush()
            logger.debug(f"Added new item, assigned ID {new_db_item.id} ")
