...
from brocc.tasks.model_const import DEFAULT_LM, NOMIC_EMBED_TEXT_V2

logger = get_logger(__name__)
HF_HOME_ENV = os.getenv("HF_HOME")
HF_CACHE_DIR = Path(HF_HOME_ENV if HF_HOME_ENV else Path.home() / ".cache" / "huggingface") / "hub"
logger.info(f"[ModelLoader] HF_HOME env var: {HF_HOME_ENV}")  # DEBUG
logger.info(f"[ModelLoader] Calculated HF_CACHE_DIR: {HF_CACHE_DIR}")  # DEBUG
