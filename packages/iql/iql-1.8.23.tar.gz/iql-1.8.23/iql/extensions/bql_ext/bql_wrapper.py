            ...
            try:
                ...
                if r is None:
                    ...
                    if q._output_filename is not None:
                        result_table: pa.Table = _to_data_arrow(r)  # type: ignore
...
def _to_data_arrow(response) -> DataFrame:  # type: ignore
    if type(response).__name__ == "ArrowExecuteResponse":
        return _to_data_arrow_execute_response(response).to_pandas()
    else:
        return _to_data_nonarrow_execute_response(response).to_pandas()
