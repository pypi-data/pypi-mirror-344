...
MCP-Router-Use provides flexible options for building custom agents that can utilize MCP tools. This guide will show you how to create your own agents by leveraging the existing adapters, particularly focusing on the LangChain adapter.

## Overview

MCP-Router-Use allows you to:
...
While MCP-Router-Use provides a built-in `MCPAgent` class, you may want to create your own custom agent implementation for more flexibility or to integrate with other frameworks.
...
from mcp_router_use.adapters import LangChainAdapter
...
MCP-Router-Use welcomes contributions for integrating with different agent frameworks. The adapter architecture is designed to make this process straightforward.

### Adapter Architecture

MCP-Router-Use provides a `BaseAdapter` abstract class that handles most of the common functionality:
...
from mcp_router_use.adapters.base import BaseAdapter
...
Building custom agents with MCP-Router-Use offers tremendous flexibility while leveraging the power of MCP tools. By combining different connectors and adapters, you can create specialized agents tailored to specific tasks or integrate MCP capabilities into existing agent frameworks.

The adapter architecture makes it easy to extend MCP-Router-Use to support new frameworks - you just need to implement the `_convert_tool` method to bridge between MCP tools and your framework of choice.
