...
@LastEditTime: 2025-04-28 18:02:59
@LastEditors: yy
@Description: 
"""
    ...
    def save_user_by_openid(self, app_id, act_id, open_id, user_nick, avatar, union_id="", login_ip="", is_update_user_nick=True, mix_nick='', plat_type=0, continue_request_expire=1, is_acquire_lock=False, is_refresh_token=True):
        ...
        try:
            ...
            if not user_info:
                ...
                user_id = redis_init.incr(f"user_info_userid:{app_id}")
                user_info = UserInfo()
                user_info.id_md5 = self._get_user_info_id_md5(act_id, user_id, app_id)
                ...
                user_info.is_new = 1
                user_info.user_nick = CryptographyHelper.emoji_to_emoji_base64(user_nick)
                user_info.user_nick_encrypt = CryptographyHelper.base64_encrypt(user_nick) if user_nick else ""
                ...
                if user_nick and user_info.user_nick != user_nick and is_update_user_nick == True:
                    user_info.user_nick = CryptographyHelper.emoji_to_emoji_base64(user_nick)
                    user_info.user_nick_encrypt = CryptographyHelper.base64_encrypt(user_nick)
                if avatar and user_info.avatar != avatar and is_update_user_nick == True:
                    user_info.avatar = avatar
                if union_id and user_info.union_id != union_id:
                    user_info.union_id = union_id
