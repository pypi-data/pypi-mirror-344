...
@LastEditTime: 2025-04-28 16:46:39
@LastEditors: yy
@Description: 
"""
...
class FrameBaseHandler(FrameBaseApiHandler):
    ...
    def is_api_encrypt(self):
        ...
        :last_editors: yy
        """
        c_encrypt_type = share_config.get_value("c_encrypt_type", 0)  #客户端加密类型 0-无，1-aes加密
        s_encrypt_type = share_config.get_value("s_encrypt_type", 0)  #千牛端或后台加密类型 0-无，1-aes加密
        ...
        if is_encrypt == True and ((request_source_type == 2 and s_encrypt_type == 1) or (request_source_type == 1 and c_encrypt_type == 1)):
            return True
        else:
            return False
    ...
    def _request_source_type(self):
        """
        :Description: 请求来源类型 0-未知 1-c(客户端) 2-s(服务端)
        :last_editors: yy
        """
    ...
    def _decrypt_request_params(self, app_id, spm, spv, encrypt_key):
        """
        :description: 解密请求参数
        :param app_id: app_id
        :param spm: spm
        ...
        try:
            ...
            if share_config.get_value("plat_type", PlatType.tb.value) == PlatType.jd.value:
                spm = CodingHelper.url_decode(spm)
            password = str(encrypt_key).replace("1", "l")
            iv = app_id[0:10] + str(spv)[0:6] if len(str(spv)) >= 6 else ""
            invoke_result_data.data = json.loads(CryptographyHelper.aes_decrypt(spm, password, iv))
            return invoke_result_data
        except Exception as ex:
            invoke_result_data.success = False
    ...
    def _convert_request_params(self):
        ...
        try:
            ...
            if self.is_api_encrypt() is True:
                encrypt_key = self.encrypt_key if hasattr(self, "encrypt_key") else share_config.get_value("encrypt_key", "")
                json_params = None
                if is_post_request is True:
                ...
                invoke_result_data = self._decrypt_request_params(app_id, self.request_params.get("spm", ""), self.request_params.get("spv", ""), encrypt_key)
    ...
    def get_app_id(self):
        ...
        plat_type = share_config.get_value("plat_type", PlatType.tb.value)  # 平台类型
        if plat_type == PlatType.tb.value:
            template_id = share_config.get_value("c_template_id")
            if not app_id:
                ...
                elif request_source_type == 1: # 1(客户端)
