...
@LastEditTime: 2025-04-28 16:44:08
...
logger_error = Logger(f"{log_file_path.rstrip('/')}/log_error", "ERROR", "log_error", HostHelper.get_host_ip(), project_name, log_config).get_logger()

logger_info = Logger(f"{log_file_path.rstrip('/')}/log_info", "INFO", "log_info", HostHelper.get_host_ip(), project_name, log_config).get_logger()

logger_sql = Logger(f"{log_file_path.rstrip('/')}/log_sql", "SQL", "log_sql", HostHelper.get_host_ip(), project_name, log_config).get_logger()

logger_http = Logger(f"{log_file_path.rstrip('/')}/log_http", "HTTP", "log_http", HostHelper.get_host_ip(), project_name, log_config).get_logger()
...
if nacos_config:
    ...
    try:
        nacos_client.add_config_watcher(data_id=nacos_data_id, group=nacos_group, cb=lambda cfg: config.init_config_from_nacos(config_file, cfg["content"]))
    except Exception as ex:
        pass
