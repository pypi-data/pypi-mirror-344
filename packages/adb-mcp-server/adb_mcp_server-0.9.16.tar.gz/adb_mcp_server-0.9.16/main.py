...
def input(command: str, sn: str = '') -> str:
    ...
    return __internal_adb_shell(f'input {command}', sn)
...
def uiautomator_dump(sn: str = '') -> str:
    ...
    if os.path.exists("window_dump.xml"):
        debug_msg.append("remove old xml file")
        os.remove("window_dump.xml")

    debug_msg.append("adb wait for device")
    __internal_adb_wait_for_device(sn)
    debug_msg.append("adb wait for device done")

    command = "uiautomator dump /sdcard/window_dump.xml"

    debug_msg.append(f"adb shell {command}")
    __internal_adb_shell(command, sn)
    debug_msg.append("adb shell uiautomator dump done")
    
    # wait for the file to be created
    debug_msg.append("wait for the file to be created")
    adb_pull("/sdcard/window_dump.xml", sn)
    debug_msg.append("adb pull /sdcard/window_dump.xml done")
    debug_msg.append(f"check if the file exists: {os.path.exists('window_dump.xml')}")

    # remove the xml file in android phone
    debug_msg.append("remove the xml file in android phone")
    __internal_adb_shell("rm /sdcard/window_dump.xml", sn)
    debug_msg.append("remove the xml file in android phone done")
    
    try:
        # read the xml file
        debug_msg.append("read the xml file")
        with open("window_dump.xml", "r", encoding="utf-8") as f:
            content = f.read()
        debug_msg.append("read the xml file done")
    except Exception as e:
        raise Exception(f"read window_dump.xml failed, error information: {e}; debug information: {debug_msg}")

    # remove the xml file
    debug_msg.append("remove the xml file")
    if os.path.exists("window_dump.xml"):
        os.remove("window_dump.xml")
        debug_msg.append("remove the xml file done")
...
def adb_shell(command: str, sn: str = '') -> str:
    ...
    """
    return __internal_adb_shell(command, sn)


def __internal_adb_shell(command: str, sn: str = '') -> str:
    """
        run adb shell command
    Args:
        command (str): a adb shell command to run. for example,  'input tap 100 200' is to run 'adb shell "input tap 100 200"' 
...
def adb_wait_for_device(sn: str = '') -> str:
    ...
    """
    return __internal_adb_wait_for_device(sn)

def __internal_adb_wait_for_device(sn: str = '') -> str:
    """
        wait for the android device to be connected
    Args:
        sn: android phone serial number which be connected to run command. default is empty, to connected default android phone.
