...
def uiautomator_dump(sn: str = '') -> str:
    ...
    result = __internal_adb_shell(command, sn)

    # get generated xml file path
    dump_file = result.split(":")[1].strip().replace("\n", "")


    if sn != '':
        pull_command = f"adb -s {sn} pull {dump_file}"
    else:
        pull_command = f"adb pull {dump_file}"
    result = __run_command_in_shell(pull_command)

    try:
        # get file name
        pull_file = result.split(":")[0].strip().replace("\n", "")
        # get file name
        pull_file_name = os.path.basename(pull_file)
    except Exception as e:
        raise Exception(f"uiautomator dump failed, error information: {result}")

    if(not os.path.exists(pull_file_name)):
        raise Exception(f"pull file {pull_file_name} failed, error information: {result};")
