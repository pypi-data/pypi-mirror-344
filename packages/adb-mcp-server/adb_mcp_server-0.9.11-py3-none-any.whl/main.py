...
def uiautomator_dump(sn: str = "") -> str:
    ...
    # if window_dump.xml exists, remove it
    if os.path.exists("window_dump.xml"):
        os.remove("window_dump.xml")

    if sn != "":
        os.system(f'adb -s {sn} shell "rm -rf /sdcard/window_dump.xml"')
        os.system(f'adb -s {sn} shell "uiautomator dump /sdcard/window_dump.xml"')
        os.system(f'adb -s {sn} pull /sdcard/window_dump.xml')
    else:
...
if __name__ == "__main__":
    #  main()
    print(uiautomator_dump(sn='e38c6a01'))
