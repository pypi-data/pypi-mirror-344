...
class SyncWebDevice(SyncWebBrowser,
    ...
    def web_assertion_element(self, name, ope_key, ope_value):
        ...
        try:
            if is_method:
                ...
                Mango.a_e(SyncWebAssertion(self.base_data), ope_key, ope_value)
            elif is_method_public:
                self.base_data.log.debug(f'2-开始断言-2，方法：{ope_key}，断言值：{ope_value}')
                Mango.a_e(PublicAssertion, ope_key, ope_value)
            else:
    ...
    def web_find_ele(self, name, _type, exp, loc, sub, is_iframe) \
            -> tuple[int, Locator] | tuple[int, list[Locator]]:
        self.base_data.log.debug(
            f'查看元素，名称：{name},_type:{_type},exp:{exp},loc:{loc},sub:{sub},is_iframe:{is_iframe}')
