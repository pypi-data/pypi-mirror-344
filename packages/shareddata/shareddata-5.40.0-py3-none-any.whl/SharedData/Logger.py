...
class Logger:
        ...
        if Logger.logfilepath.is_file():
            ...
            with open(Logger.logfilepath, 'r') as file:
                ...
                if Logger._sorted_until == 0 or max_asctime is None:
                    need_full_sort = True
                elif dfnewlines['asctime'].min() < max_asctime:
                    need_full_sort = True

                dfnewlines_sorted = dfnewlines.sort_values('asctime')
                if need_full_sort:
                    Logger.dflogs = pd.concat([Logger.dflogs, dfnewlines_sorted])
                    Logger.dflogs['asctime'] = pd.to_datetime(Logger.dflogs['asctime'])
                    Logger.dflogs = Logger.dflogs.sort_values('asctime', ignore_index=True)                        
                    Logger._sorted_until = len(Logger.dflogs)                        
                    # update max asctime and sorted output
                    dfnewlines_sorted = dfnewlines_sorted[dfnewlines_sorted['asctime']>=max_asctime]
    ...
    def handleHeartbeats(keep_latest=500):
        ...
        Logger.dfheartbeats.loc[newidx,dflasthb.columns] = dflasthb.values
