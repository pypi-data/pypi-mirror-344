...
export const initializeClient = async (
  ...
  try {
    ...
    console.debug(`Loaded settings: backendUrl=${backendUrl}, authToken=${localAuthToken ? '***' : '<empty>'}`);

    // If token is missing in settings, try fetching from the backend
    if (!localAuthToken) {
      console.debug('Auth token missing in settings, attempting to fetch from backend...');
    ...
    jovyanClientInstance = new JovyanClient(backendUrl, localAuthToken || '', '');
