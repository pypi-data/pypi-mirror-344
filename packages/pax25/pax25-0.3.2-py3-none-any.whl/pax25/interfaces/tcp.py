...
class TCPInterface(Interface[TCPSettings]):
    ...
    async def start_server(self) -> None:
        ...
        """
        if not self.settings.get("allow_inbound"):
            return
        try:
    ...
    def build_connections(self) -> None:
        ...
        if not connections:
            return
        call_sign = self.settings.get("call_sign", "")
        password = self.settings.get("password", 0)
        if not (call_sign and password):
            raise ConfigurationError(
                "TCP settings must include both call_sign and password.",
            )
        for value in self.settings.get("connections", []):
            self.connections.add(
                TCPConnection(
                    remote_station=ResolvedConnectionSpec(
