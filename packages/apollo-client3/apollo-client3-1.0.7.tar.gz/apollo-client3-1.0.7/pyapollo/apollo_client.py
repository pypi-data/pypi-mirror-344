...
class ApolloClient(object):
        ...
        for listener in self.listen_configs:
            try:
                ...
                if isinstance(listener,type):
                    self.logger.error(f"Apollo更新{listener.__class__.__name__}配置失败: {e}")
                else:
                    self.logger.error(f"Apollo更新{listener}配置失败: {e}")
    ...
    def _get_config_by_namespace(self, namespace: str = "application") -> None:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                self._update_listener(self._cache[namespace])
    ...
    def _listener(self) -> None:
        ...
        """
        while True:
            self.logger.info("Entering listener loop...")
            self._long_poll()
