...
# from synalinks.src.utils.module_utils import dmtree
...
class TreeTest(testing.TestCase):
    ...
    def test_flatten_with_path(self, t):
        ...
        self.assertEqualStrict(
            t.flatten_with_path(defaultdict(default_value, [("b", 2), ("a", 1)])),
            [(("a",), 1), (("b",), 2)],
        )
    ...
    def test_pack_sequence_as(self, t):
        ...
        self.assertEqualStrict(t.pack_sequence_as(deque([10, 20]), [1, 2]), deque([1, 2]))
        ...
        self.assertEqualStrict(t.pack_sequence_as(OrderedDict(), []), OrderedDict())
        self.assertEqualStrict(
        ...
        # Keras tracking wrappers.
        self.assertEqualStrict(t.pack_sequence_as(TrackedList([]), []), TrackedList([]))
        self.assertEqualStrict(
        ...
        self.assertEqualStrict(t.pack_sequence_as(TrackedSet([]), []), TrackedSet([]))
        self.assertEqualStrict(t.pack_sequence_as(TrackedSet([10]), [1]), TrackedSet([1]))
        self.assertEqualStrict(
            t.pack_sequence_as(TrackedSet([10, 20]), [1, 2]), TrackedSet([1, 2])
        )
        self.assertEqualStrict(t.pack_sequence_as(TrackedDict({}), []), TrackedDict({}))
        self.assertEqualStrict(
    ...
    def test_map_structure_with_one_structure(self, t):
        ...
        self.assertEqualStrict(t.map_structure(f1, deque([1, 2])), deque([11, 12]))
        self.assertEqualStrict(t.map_structure(f1, Empty()), Empty())
        self.assertEqualStrict(t.map_structure(f1, Point(y=2, x=1)), Point(x=11, y=12))
        self.assertEqualStrict(
            t.map_structure(f1, {}),
            {},
        )
        ...
        self.assertEqualStrict(
            t.map_structure(f1, defaultdict(default_value, [("b", 2), ("a", 1)])),
            defaultdict(default_value, [("a", 11), ("b", 12)]),
        )

        # Keras tracking wrappers.
        self.assertEqualStrict(t.map_structure(f1, TrackedList([])), TrackedList([]))
        self.assertEqualStrict(t.map_structure(f1, TrackedList([1])), TrackedList([11]))
        self.assertEqualStrict(
            t.map_structure(f1, TrackedList([1, 2])), TrackedList([11, 12])
        )
        self.assertEqualStrict(t.map_structure(f1, TrackedSet([])), TrackedSet([]))
        self.assertEqualStrict(t.map_structure(f1, TrackedSet([1])), TrackedSet([11]))
        self.assertEqualStrict(
