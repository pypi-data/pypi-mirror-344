...
class ProdRequestsGitlabRepo:
    def __init__(self, *, with_auth: bool, project_name: str = "lhcb-dpa/prod-requests"):
        cred_path = self.cache_path
        cred = {}
        if cred_path.exists():
                ...
                try:
                    ...
                    cred = {
                        "private_token": prompt_toolkit.prompt(
                            f"Go to https://gitlab.cern.ch/{project_name}/-/settings/access_tokens"
        ...
        self.project = self.api.projects.get(project_name)

    @property
    def cache_path(self):
        if not CACHE_DIR.exists():
            CACHE_DIR.mkdir(parents=True, exist_ok=True)
        return CACHE_DIR / "gitlab.json"
    ...
    @staticmethod
    def _create_issue(issue: gitlab.v4.objects.ProjectIssue) -> RepoIssue:
        return RepoIssue(issue)

    def poll(self, *, states=ISSUE_STATE_RESPONSIBLE, do_status_update=False) -> dict[str, list[RepoIssue]]:
        all_tids = set()
        ...
        with Progress(*PROGRESS_COLUMNS) as progress:
            ...
            for state in states:
                ...
                for issue in issues:
                    issue = self._create_issue(issue)
