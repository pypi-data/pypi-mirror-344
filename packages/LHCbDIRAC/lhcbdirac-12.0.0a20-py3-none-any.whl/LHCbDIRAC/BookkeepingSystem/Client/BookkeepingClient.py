...
class BookkeepingClient(Client):
    ...
    #############################################################################
    def getFileDescendentsTree(self, lfns, depth=0):
        """Retrieve file(s) descendents in tree form.

        :param list lfns: list of LFNs
        :param int depth: depth of the processing chane
        :return: descendents of a file or a list of files.
        """
        if isinstance(lfns, str):
            lfns = lfns.split(";")

        return self._getRPC().getFileDescendentsTree(lfns, depth)
