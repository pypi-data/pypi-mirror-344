...
def calc_enstrophy_3d(subset, variables_names, order, dims, nz, ny, nx):
    """
    Calculate enstrophy structure function in 3D: ((delta omega_x)^2 + (delta omega_y)^2 + (delta omega_z)^2)^(order/2)
    ...
    subset : xarray.Dataset
        Subset of the dataset containing required variables
    variables_names : list
        List of variable names (should contain three vorticity components: omega_x, omega_y, omega_z)
    order : int
        Order of the structure function (typically 2 for standard enstrophy)
    dims : list
        List of dimension names
    nz, ny, nx : int
        Array dimensions
    ...
    """
    if len(variables_names) != 3:
        raise ValueError(f"3D enstrophy structure function requires exactly 3 vorticity components, got {len(variables_names)}")
    
    # Assume variables are provided in order: omega_x, omega_y, omega_z
    omega_x, omega_y, omega_z = variables_names
    ...
    omega_x_var = subset[omega_x].values
    omega_y_var = subset[omega_y].values
    omega_z_var = subset[omega_z].values
    ...
    print(f"Using 3D enstrophy structure function with vorticity components {omega_x}, {omega_y}, {omega_z}")
    ...
    for iz in range(nz):
        for iy in range(ny):
            for ix in range(nx):
                ...
                domega_x = fast_shift_3d(omega_x_var, iz, iy, ix) - omega_x_var
                domega_y = fast_shift_3d(omega_y_var, iz, iy, ix) - omega_y_var
                domega_z = fast_shift_3d(omega_z_var, iz, iy, ix) - omega_z_var
                ...
                # Calculate enstrophy: (domega_x^2 + domega_y^2 + domega_z^2)^(order/2)
                enstrophy = (domega_x**2 + domega_y**2 + domega_z**2)**(order/2)
                results[idx] = bn.nanmean(enstrophy)
                
                idx += 1
                
    return results, dx_vals, dy_vals, dz_vals

def calc_advective_vorticity_3d(subset, variables_names, order, dims, nz, ny, nx):
    """
    Calculate advective vorticity structure function in 3D: (δω_x·δadv_ω_x + δω_y·δadv_ω_y + δω_z·δadv_ω_z)^n
    ...
    subset : xarray.Dataset
        Subset of the dataset containing required variables
    variables_names : list
        List of variable names (should contain six vorticity components: 
        omega_x, omega_y, omega_z, adv_omega_x, adv_omega_y, adv_omega_z)
    order : int
        Order of the structure function
    dims : list
        List of dimension names
    ...
    """
    if len(variables_names) != 6:
        raise ValueError(f"3D advective vorticity structure function requires exactly 6 components, got {len(variables_names)}")
    
    # Extract vorticity and advective vorticity components
    # Assuming first three are vorticity and last three are advective vorticity
    omega_vars = variables_names[:3]
    adv_omega_vars = variables_names[3:]
    ...
    # Get the vorticity components
    omega_x_var = subset[omega_vars[0]].values
    omega_y_var = subset[omega_vars[1]].values
    omega_z_var = subset[omega_vars[2]].values
    
    # Get the advective vorticity components
    adv_omega_x_var = subset[adv_omega_vars[0]].values
    adv_omega_y_var = subset[adv_omega_vars[1]].values
    adv_omega_z_var = subset[adv_omega_vars[2]].values
    ...
    print(f"Using 3D advective vorticity structure function with components {omega_vars} and {adv_omega_vars}")
    ...
    for iz in range(nz):
        for iy in range(ny):
            for ix in range(nx):
                ...
                # Calculate advective vorticity differences
                dadv_omega_x = fast_shift_3d(adv_omega_x_var, iz, iy, ix) - adv_omega_x_var
                dadv_omega_y = fast_shift_3d(adv_omega_y_var, iz, iy, ix) - adv_omega_y_var
                dadv_omega_z = fast_shift_3d(adv_omega_z_var, iz, iy, ix) - adv_omega_z_var
                ...
                # Calculate advective vorticity structure function: (δω·δadv_ω)^n
                advective_term = domega_x * dadv_omega_x + domega_y * dadv_omega_y + domega_z * dadv_omega_z
                sf_val = advective_term ** order
                results[idx] = bn.nanmean(sf_val)
...
def calculate_structure_function_3d(ds, dims, variables_names, order, fun='longitudinal', 
    ...
    # Add new function types here
        ...
        results, dx_vals, dy_vals, dz_vals = calc_enstrophy_3d(
            subset, variables_names, order, dims, nz, ny, nx)
    elif fun == 'advective_vorticity':
        results, dx_vals, dy_vals, dz_vals = calc_advective_vorticity_3d(
            subset, variables_names, order, dims, nz, ny, nx)
    else:
