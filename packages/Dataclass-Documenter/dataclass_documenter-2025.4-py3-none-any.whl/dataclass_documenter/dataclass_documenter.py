...
class DataclassDocumenter:
    ...
    def _handle_lists(self, datacls, level, commented, field):
        in_list = get_origin(field.type) is list
        dado = self._get_nested_dado(datacls)
        blocks = dado.get_yaml_blocks(
            header=datacls.__name__,
            level=level + 1,
            commented=commented,
        )
        if in_list:
            ...
            for block in blocks:
                ...
                if stripped_block and not stripped_block.startswith("#"):
                    yield f"{self.indent_unit * (level)}-{self.indent_unit[1:]}{stripped_block}"
                    added_marker = True
                else:
                    yield block
        else:
    ...
    def get_yaml_blocks(self, level=0, header=None, commented=False, in_list=False):
        """
        ...
        Args:
            ...
            in_list:
                If True, emit this class as the first item in a list.

        Returns:
            A generator over blocks of YAML.
        """
        _, indent = self._get_indents(level, commented, in_list, False)
        empty_line = ""
