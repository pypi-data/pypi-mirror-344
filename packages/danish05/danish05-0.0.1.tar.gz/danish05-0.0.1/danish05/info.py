...
def bgrem(pathstart, pathsave, name, alpha):
...
def mbgrem(input1, output1, alpha):
    try:
        def watermark():
            baris1 = [
                f" ",
        ...
        with open('output.txt', 'w') as file_output:
            ...
            for filename in os.listdir(input1):
                if filename.endswith(('.jpg', '.jpeg', '.png')):
                    result = bgrem(input1, output1, filename, alpha)
        ...
        beautifullline("*", 30)
        print(f"Error: {e}")
        print("Command: mbgrem(folder input gambar-gambar, folder output gambar-gambar)")
...
beautifullline("*", 30)
print("Package danish05 is running!")
info()
