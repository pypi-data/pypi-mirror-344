...
from x4util.conversion import x4_x4toj4
from x4util.extract import x4_refbib

def main():
  args=get_args(ver)
  get_doi(*get_input(args))


def get_doi(file_x4,file_dict,file_doi,key_anal,email,force0):
...
# Conversion to J4 format by X4TOJ4
  x4_x4toj4.convert(file_x4,file_dict,file_tmp,key_keep,force,chkrid,add19,keepflg,outstr)
...
# Check with REFDOI
  ...
  for ansan, keyword, code in zip(ansans,keywords,codes):
    x4_refbib.convert(code,fauthor,file_dict,file_tmp,format,email,force,strip)
    g=open(file_tmp, "r")
    lines=g.readlines()
    g.close()
    os.remove(file_tmp)
