...
class TabuSearch:
    ...
    def _select_best_candidate(self, neighbors):
        best_indexes = np.sort([n.representation._value for n in neighbors])
        for i in best_indexes[-1:]:
            if neighbors[i] not in self.tabu_list or (self.aspiration_criteria is not None and True in [crit(neighbors) for crit in self.aspiration_criteria] ):
                return neighbors[i]
        return neighbors[best_indexes[-1]]