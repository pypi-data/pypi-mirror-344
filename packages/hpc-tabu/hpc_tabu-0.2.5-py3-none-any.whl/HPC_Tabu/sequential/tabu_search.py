...
class TabuSearch:
    ...
    def _select_best_candidate(self, neighbors):
        best_neighbors = np.argsort([n._value for n in neighbors])
        for i in best_neighbors[-1:]:
            if best_neighbors[i] not in self.tabu_list or (self.aspiration_criteria is not None and True in [crit(best_neighbors[i]) for crit in self.aspiration_criteria] ):
                return best_neighbors[i]
        return best_neighbors[-1]