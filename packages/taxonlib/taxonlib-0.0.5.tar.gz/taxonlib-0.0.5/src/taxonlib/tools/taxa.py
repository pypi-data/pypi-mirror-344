from src.canonical_name import canonize

ranks = ["kingdom", "division", "class", "order", "family", "genus"]
remove_modifiers = []

taxa_path = r"D:\users\laurens\code\taxonmap\data\taxa\obs_taxa.csv"
...
def remove_author(name):
    ...
    for e, element in enumerate(elements):
        ...
        if e > 0:
            ...
            if start_parenthesis and (element.endswith(")") or element.endswith("),") or element.endswith(")]")):
                parenthesized_content += element
                end_parenthesis = e
            elif (
                element[0].isupper()
