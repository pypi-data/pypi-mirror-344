...
def canonize(taxon_name: str):
    ...
    for t, token in enumerate(tokens):
        ...
        if subgenus_found:
            if len(tokens) > 2:
                is_subgenus_spec = "." in tokens[2]  # e.g. Chorthippus (Chorthippus) spec., Arion (Arion) agg.
            else:
                is_subgenus_spec = True  # e.g. Chorthippus (Chorthippus)
        for interpunction in interpunction_to_separate:
            if interpunction in token:
                new_tokens.extend(token.replace(interpunction, f" {interpunction} ").split())
                interpunction_found = True
                break
        if (not interpunction_found  # interpunction attached to tokens is dealt with above
                and (
