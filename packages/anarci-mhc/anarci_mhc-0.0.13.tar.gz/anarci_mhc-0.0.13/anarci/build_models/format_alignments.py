...
from .fasta_parser import chunkify
    ...
    for record in chunkify(handle):
        ...
        try:
            if fields["accession_number"] == "None":
                continue
            if (
                fields["functionality"] == "F"
                and not fields["partial"].strip()
                and not fields["reverse"].strip()
            ):
                if read_all:
                    pass
                ...
                if fields["region"] == region_name:
                    ...
                    ), "The region for some the entries is different"
...
def read_fasta(filename):
    ...
    with open(filename, "r") as f:
        records = [[s.description, s.seq.replace(" ", "")] for s in chunkify(f)]
    return records


def write_fasta(sequences, curated_dir):
    """
    Write a fasta file containing all sequences
    """
    filename = os.path.join(curated_dir, "muscle_alignments", "all_js.fasta")
...
def format_c_genes(calignments, gene_name=""):
    ...
    for entry in calignments:
        ...
        for seq in calignments[entry]:
            ...
            new_calignments[entry][new_name] = (
                sequence[:max_length].ljust(max_length).replace(" ", ".")
            )

    return new_calignments


def format_j_genes(jalignments, curated_dir):

    reference = ("WFAYWGQGTLVTVSA", 4, 19)
    #                 seq           start  end

    ffile = write_fasta(jalignments, curated_dir)
    al_filename = os.path.join(curated_dir, "muscle_alignments", "all_js_aligned.fasta")
    ...
    for name, sequence in aligned:
        ...
        new_jalignments[(species, chain_type)][(id1, id2)] = (
            sequence[START:END][-14:].rjust(20).replace(" ", ".")
        )
...
def format_v_genes(valignments):
    ...
    for entry in valignments:
        ...
        for seq in valignments[entry]:
            ...
            # if (
            #     new_valignments[entry][seq][103] != "C"
            #     or new_valignments[entry][seq][22] != "C"
            # ):
            #     sys.stderr.write(
...
def combine_sequences(vsequences, jsequences):
    ...
    for v in vsequences:
        ...
        for j in jsequences:
            _, jallele = j
            combined_sequences[
                ("%s_%s_%s" % (vspecies, vallele, jallele)).replace(" ", "_")
            ] = (vsequences[v] + jsequences[j])
    return combined_sequences


def make_putative_alignments(vsequences, jsequences, curated_dir, calignments=None):
    all_sequences = {}
    for species, chain_type in vsequences:
        if (species, chain_type) not in jsequences or (
            species,
            chain_type,
        ) not in vsequences:
            continue
        combined_sequences = combine_sequences(
            vsequences[(species, chain_type)], jsequences[(species, chain_type)]
        )
    ...
    output_stockholm_all(all_sequences, curated_dir)

    # Write the V and J combinations and the c-domains
    if calignments:
        output_stockholm_all_and_C(all_sequences, calignments, curated_dir)


def write_germlines(vsequences, jsequences, curated_dir):
    """
    ...
    for species, chain_type in vsequences:
        for (_, gene), seq in vsequences[(species, chain_type)].items():
            assert len(seq) == 108, (
                species + _ + gene + chain_type + _ + seq + str(len(seq))
            )
            try:
                all_gene_alignments["V"][chain_type][translations[species]][gene] = (
                    seq.replace(".", "-") + "-" * 20
                )
            except KeyError:
                try:
                    all_gene_alignments["V"][chain_type][translations[species]] = {
                        gene: seq.replace(".", "-") + "-" * 20
                    }
                except KeyError:
                    try:
        ...
        for (_, gene), seq in jsequences.get((species, chain_type), {}).items():
            assert len(seq) == 20
            try:
                all_gene_alignments["J"][chain_type][translations[species]][gene] = (
                    "-" * 108 + seq.replace(".", "-")
                )
            except KeyError:
                try:
                    all_gene_alignments["J"][chain_type][translations[species]] = {
        ...
        # # Save germlines as json
        # filename = os.path.join(curated_dir, "germlines.json")
        # with open(filename, "w") as f:
        #     json.dump(all_gene_alignments, f)
        output_python_lookup(all_gene_alignments, curated_dir)
...
def output_python_lookup(all_gene_alignments, path=None):
    ...
    # filename = os.path.join(curated_dir, "germlines.json")
    # with open(filename, "w") as outfile:
    #     json.dump(all_gene_alignments, file=f)
    # print("all_germlines = "+repr(all_gene_alignments), file=outfile)
    if path is None:
        path = "curated_alignments"
...
def output_C_alignments(alignments, c_name, curated_dir):
    """
    Write a stockholm for all C domains.
    """
    for species, chain_type in alignments:
        output_stockholm(
            alignments[(species, chain_type)],
            "%s_%s_%s" % (c_name, translations[species], chain_type),
            curated_dir,
        )


def output_stockholm_all_and_C(all_sequences, all_C_alignments, curated_dir):
    """
    Output a minimal stockholm alignment file for all sequences.
    """

    filename = os.path.join(curated_dir, "ALL_AND_C.stockholm")
    with open(filename, "w") as outfile:
        for species, chain_type in all_sequences:
            sequences = all_sequences[(species, chain_type)]
            l = len(list(sequences.values())[0])
            assert all(
                [1 if l == len(sequences[s]) else 0 for s in sequences]
            ), "Not all sequences in alignment are the same length"
            write_stockholm(
                sequences, "%s_%s" % (translations[species], chain_type), outfile
            )

        for c_name in all_C_alignments:
            for species, chain_type in all_C_alignments[c_name]:
                write_stockholm(
                    all_C_alignments[c_name][(species, chain_type)],
                    "%s_%s_%s" % (translations[species], chain_type, c_name),
                    outfile,
                )
    return filename


def output_stockholm_all(all_sequences, curated_dir):
    """
    Output a minimal stockholm alignment file for all sequences.
    """

    filename = os.path.join(curated_dir, "ALL.stockholm")
    with open(filename, "w") as outfile:
        for species, chain_type in all_sequences:
            sequences = all_sequences[(species, chain_type)]
            l = len(list(sequences.values())[0])
...
def output_stockholm(sequences, name, curated_dir):
    """
    Output a minimal stockholm alignment file.
    """

    filename = os.path.join(curated_dir, "%s.stockholm" % name)
    l = len(list(sequences.values())[0])
...
def format_alignments(fasta_dir, curated_dir):
    """
    ...
    for species in all_species:
        for chain_type in "HKL":
            if not os.path.isfile(
                os.path.join(fasta_dir, "%s_%sV.fasta" % (species, chain_type))
            ):
                continue
            ...
            jalignments[(species, chain_type)] = read_alignment(
                os.path.join(fasta_dir, "%s_%sJ.fasta" % (species, chain_type)),
                region_name="J-REGION",
            )

            ### Comment out if you want constant regions?
            # if chain_type == "H":
            #    c1alignments[ (species, chain_type) ]   = read_alignment( os.path.join( fasta_dir , "%s_%sC.fasta"%(species, chain_type) ), region_name="CH1")
            #    c2alignments[ (species, chain_type) ]   = read_alignment( os.path.join( fasta_dir , "%s_%sC.fasta"%(species, chain_type) ), region_name="CH2")
            #    c3alignments[ (species, chain_type) ]   = read_alignment( os.path.join( fasta_dir , "%s_%sC.fasta"%(species, chain_type) ), region_name="CH3")
            # else:
            #    ccalignments[ (species, chain_type) ]   = read_alignment( os.path.join( fasta_dir , "%s_%sC.fasta"%(species, chain_type) ), region_name="C-REGION")
    ...
    jalignments = format_j_genes(jalignments, curated_dir)
    ...
    all_jalignments = format_j_genes(all_jalignments, curated_dir)

    # all_C_alignments = { "CC":ccalignments,"C1":c1alignments,"C2":c2alignments,"C2":c2alignments}

    write_germlines(all_valignments, all_jalignments, curated_dir)
    ...
    combined_sequences = make_putative_alignments(valignments, jalignments, curated_dir)
