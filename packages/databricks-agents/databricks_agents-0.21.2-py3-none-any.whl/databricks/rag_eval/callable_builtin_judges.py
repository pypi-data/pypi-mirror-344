        ...
        eval_item = entities.EvalItem.from_dict(
            {
                ...
                schemas.GUIDELINES_CONTEXT_COL: guidelines_context,
            }
        )
...
def _validate_guidelines_context(guidelines_context: Optional[Dict[str, str]]) -> None:
    ...
    for key, value in guidelines_context.items():
        ...
        if not isinstance(value, str):
            raise ValueError(
                f"guidelines_context values must be strings. Got: {type(value)}"
            )
...
def guideline_adherence(
    ...
    Args:
        ...
        guidelines_context: Mapping of a string (context field name) to string (content) containing context the guidelines can apply to.
