    ...
    async def get_task(
        ...
        try:
            if return_field_codes and return_fields_id:
                aggregated_by_codes = await self.extractor.extract_task_fields(task_details, return_field_codes)
                aggregated_by_ids = await self.extractor.search_fields_by_codes_with_ids(task_details, return_fields_id)
                aggregated_fields = {**aggregated_by_codes, **aggregated_by_ids}
                return aggregated_fields
            elif return_field_codes:
                return await self.extractor.extract_task_fields(task_details, return_field_codes)
            elif return_fields_id:
                return await self.extractor.search_fields_by_codes_with_ids(task_details, return_fields_id)
            else:
                return task_details
        except Exception as e:
