...
class ProjectManager:
    ...
    def run(self, name: str = "") -> None:
        ...
        if len(self.projects) > 1:
            if not name:
                logging.error(f"存在多个项目, 请输入名称: {self.projects}")
                raise RunExecutableError("存在多个项目")

            project_run = [p for p in self.projects if name.lower() in p.normalized_name.lower()]
            if len(project_run):
                project_run = project_run[0]
            else:
                logging.error(f"未找到 [red]{name}[/] 对应项目")
                raise RunExecutableError("未找到项目")
        else:
            project_run = list(self.projects)[0]

        if not project_run.exe_file.exists():
            logging.error(f"项目可执行文件不存在: {project_run}")
            raise RunExecutableError("项目可执行文件不存在")
