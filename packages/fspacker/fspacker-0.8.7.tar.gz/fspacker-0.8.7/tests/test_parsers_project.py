...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("ex09_poetry_py310", ["pygame>=2.6.1,<3.0.0", "tomli>=2.2.1,<3.0.0", "typer>=0.15.2"]),
...
def test_project_opt_deps(dir_ex00_simple):
    info = Project(dir_ex00_simple)
    assert "plot" in info.optional_dependencies
    assert "excel" in info.optional_dependencies
...
def test_project_error_invalid_source_ast(dir_ex94_error_invalid_source_ast, caplog):
    ...
    assert f"源文件解析语法错误: {dir_ex94_error_invalid_source_ast / 'main.py'}" in caplog.text
...
def test_project_error_invalid_pep621(dir_ex96_error_invalid_pep621, caplog):
    ...
    assert "可选依赖项格式错误" in caplog.text


def test_project_error_invalid_optional_deps(dir_ex97_error_invalid_optional_deps, caplog):
    """测试 pyproject.toml 解析可选依赖失败"""

    Project(dir_ex97_error_invalid_optional_deps)
    assert "可选依赖项格式错误" in caplog.text
