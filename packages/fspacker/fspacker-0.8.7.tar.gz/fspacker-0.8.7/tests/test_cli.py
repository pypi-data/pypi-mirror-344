...
def test_build_command(typer_runner, dir_ex00_simple):
    # Build normal
    result = typer_runner.invoke(app, ["b", str(dir_ex00_simple)])
    ...
    result = typer_runner.invoke(app, ["b", "--debug", "--simplify", str(dir_ex00_simple)])
    assert result.exit_code == 0
    assert get_settings().mode.debug is True
    assert get_settings().mode.simplify is True

    # Build with archive and rebuild
    result = typer_runner.invoke(app, ["b", "--archive", "--rebuild", str(dir_ex00_simple)])
    assert result.exit_code == 0
    assert get_settings().mode.archive is True
    assert get_settings().mode.rebuild is True

    # Build with use_tk
    result = typer_runner.invoke(app, ["b", "--use-tk", str(dir_ex00_simple)])
    assert result.exit_code == 0
...
def test_run_command(typer_runner, dir_ex00_simple):
    ...
    # Run
    result = typer_runner.invoke(app, ["r", "ex00_simple"])
    assert result.exit_code == 0
