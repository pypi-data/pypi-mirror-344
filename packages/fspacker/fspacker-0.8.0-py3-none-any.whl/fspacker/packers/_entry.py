...
class EntryPacker(BasePacker):
    ...
    def pack(self):
        ...
        if not self.info.source_file or not self.info.source_file.exists():
            raise ProjectPackError(f"入口文件{self.info.source_file}无效")
        ...
        dst_exe_path = self.parent.dist_dir / f"{name}.exe"

        logging.info(f"打包目标类型: {'[green bold]窗口' if self.info.is_gui else '[red bold]控制台'}[/]")
        logging.info(
            f"复制可执行文件: [green underline]{src_exe_path.name} -> "
            f"{dst_exe_path.relative_to(self.parent.project_dir)}[/] [bold green]:heavy_check_mark:"
        )
        shutil.copy(src_exe_path, dst_exe_path)

        dst_int_path = self.parent.dist_dir / f"{name}.int"

        logging.info(
            f"创建 int 文件: [green underline]{name}.int -> {dst_int_path.relative_to(self.parent.project_dir)}"
            f"[/] [bold green]:heavy_check_mark:"
        )

        for lib_name in ["PySide2", "PyQt5", "PySide6", "PyQt6"]:
            if self.info.contains_lib(lib_name):
