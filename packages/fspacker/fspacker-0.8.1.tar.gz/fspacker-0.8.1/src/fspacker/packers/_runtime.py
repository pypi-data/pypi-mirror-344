...
class RuntimePacker(BasePacker):
    ...
    def pack(self):
        ...
        if parse(self.python_ver) not in specs:
            logging.error(
                f"当前环境python版本: [green bold]{self.python_ver}[/], 与项目要求"
    ...
    def _fetch_runtime(self):
        ...
        archive_url = f"{fastest_embed_url}{self.python_ver}/{self.embed_filename}"
