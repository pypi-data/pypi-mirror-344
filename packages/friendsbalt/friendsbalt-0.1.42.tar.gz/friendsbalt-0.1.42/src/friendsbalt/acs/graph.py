...
class Digraph:
    ...
    Methods:
        ...
        __str__():
            Returns a string representation of the graph, including the number of vertices and edges.
        from_input_string(input_string):
            Creates a Digraph instance from an input string.
            The input string should contain the number of vertices on the first line,
            followed by lines of edges in the format "u v", where `u` and `v` are vertex indices.
    """
    ...
    def __str__(self):
        ...
        Returns:
            str: A string representation of the graph.
        """
        result = f"Digraph with {self.vertex_count} vertices and {self.edge_count} edges:\n"
        for vertex, edges in self.adjacency_list.items():
            result += f"{vertex}: {edges}\n"
        return result.strip()
    
    def from_input_string(input_string: str) -> 'Digraph':
        """
        Creates a Digraph instance from an input string.
        ...
        Returns:
            Digraph: A Digraph instance created from the input string.
        """
        lines = input_string.strip().split('\n')
        v = int(lines[0])
        graph = Digraph(v)
...
class Topological:
    """
    A class to perform topological sorting on a directed acyclic graph (DAG).
    This class uses depth-first search to compute the topological order of the vertices.
    Attributes:
        order (list[int]): A list containing the vertices in topological order.
    Methods:
        __init__(g: Digraph):
            Initializes the Topological object and computes the topological order.
        is_dag() -> bool:
            Checks if the graph is a directed acyclic graph (DAG).
        get_order() -> list[int]:
            Returns the vertices in topological order.
        __str__() -> str:
            Returns a string representation of the topological order.
    """
    def __init__(self, g: Digraph):
        self.order = []
        dfs = DepthFirstOrder(g)
        self.order = dfs.reverse_post_order().to_list()
    ...
    def is_dag(self) -> bool:
        ...
        """
        return len(self.order) == self.graph.V()
    
    def get_order(self) -> list[int]:
        """
        Returns the vertices in topological order.
        Returns:
            list[int]: A list of vertices in topological order.
        """
        return self.order.copy()
    
    def __str__(self):
        return f"Topological order: {self.order}"
