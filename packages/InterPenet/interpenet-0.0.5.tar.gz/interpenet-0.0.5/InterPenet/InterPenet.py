...
class Base():
    ...
    def Calc(self,df2):
        ...
        ##################################################
        print('Validation',self.formula.Validation(df))
    ...
    def CalcDict(self,df2):
        ...
        #つまり、クラスGに特殊処理があるとき、貫通公式にはgDict関数を、クラスGには、setDict関数をもつ必要がある。
        ##################################################
        res['Formula']=self.formula.title
        self.formula.i_Valid=True
        if 'Material' in df.keys():
            self.formula.setMaterial(df['Material'])
            res['Material']=df['Material']
        if 'shape' in df.keys():
            res['shape']=df['shape']
        res['Validation']=self.formula.Validation(df)
        #res[form]['unsatisfied']=self.formula.Validation(df)
        res['variable']=self.formula.variable
        mu,sig,dist=self.formula.makeData(df)
        res['g_value']=self.formula.gcalc(mu)
        #self.formula.Reliability(df,dict=dict)#信頼性評価
        self.formula.Calc(df)#信頼性評価
        res['beta']=self.formula.GetBeta()
        res['Alpha']=self.formula.GetAlpha()
        res['Pf']=self.formula.GetPOF()

        df['v']['mean']=0
        df['Me']['mean']=1.0
        mu,sig,dist=self.formula.makeData(df)
        res['Vbl']=self.formula.gcalc(mu)
        return res
    def Vbl(self,df):
        """
    ...
    def CalcPenet(self,fname):
        ...
        self.Calc(df)
        print('File=',fname)
    def CalcPenet2Df(self,fname):
        """
        CalcPenetと同じであるが，出力は行わず，結果を辞書型データとして戻す
        """
        df = self.read_pickle(fname)
        res=self.CalcDict(df)
        res['File']=fname
        return res      
