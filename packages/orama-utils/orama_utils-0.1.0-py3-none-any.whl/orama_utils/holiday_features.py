...
def _load_holiday_data(country_code: str) -> pd.DataFrame:
    ...
    if country_code == 'ES':
        file_path = current_dir / 'db' / 'spain_holidays.csv'
...
def add_holiday_features(
    ...
    county_column: Optional[str] = 'county',
    ...
    county_column : str, optional, default='county'
    ...
    for country in VALID_COUNTRIES:
        ...
        # Process each date in the holidays DataFrame
        for _, holiday in holidays_df.iterrows():
            ...
            if counties and county_column and county_column in result_df.columns:
                local_mask = result_df[county_column].isin(counties)
                result_df.loc[combined_mask & local_mask, 'is_local_holiday'] = True
