...
# Description
# --------------------------------------------------------------
# The porpuse of this script is to run simulations using the calibration outputs, so we can obtain
# the curves we need to plotting (so we just load them when we want to plot).
# It also plots (does not save png) the results so you can check that everything is correct.

# Inputs
# --------------------------------------------------------------
file_out = 'pcalibration'  # p_calibraiton =processed calibration
...
for p, name in enumerate(names):
    ...
    if name == 'RNAP':
        ...
        ax2.plot(x_gene, output['results']['kde_gene'][name], colors_dict[name], lw=lw, label=name)
    else:
        ax.plot(x_system, output['results']['FE_dict'][name], color=colors_dict[name], lw=lw, label=name)
