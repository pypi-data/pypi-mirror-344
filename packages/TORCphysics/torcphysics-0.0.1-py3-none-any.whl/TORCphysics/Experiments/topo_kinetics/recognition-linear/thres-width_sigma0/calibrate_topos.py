...
# Here, sigma0 is fixed for Gyrase, and width/threshold are coupled.
# When sigma=0 (Rx), Gyrase efficiency is at 30%, and topo I at 5%
# ----------------------------------------------------------------------------------------------------------------------
# Initial conditions
# ----------------------------------------------------------------------------------------------------------------------
# Units:
...
# -----------------------------------
topoI_ef = 0.05 # Efficiency at relaxed sigma=0
topoI_A = np.log((1.0-topoI_ef)/topoI_ef)  # threshold = width * topo_A. topo_A is just a constant
# TopoI ranges
...
gyrase_ef = 0.3 # Efficiency at sigma=0
gyrase_A = -np.log((1.0-gyrase_ef)/gyrase_ef)  # threshold = width * gyrase_A. gyrase_A is just a constant
# Gyrase ranges
...
def objective_function(params):
    ...
    # Topoisomerase I
    name = topoI_name
    topoI_threshold = params['width_topoI'] * topoI_A
    object_type = topoI_type
    binding_model_name = topoI_binding_model_name
    binding_oparams = {'k_on': params['k_on_topoI'], 'width': params['width_topoI'],
                       'threshold': topoI_threshold}
    ...
    # Gyrase
    name = gyrase_name
    gyrase_threshold = params['width_gyrase'] * gyrase_A
    object_type = gyrase_type
    binding_model_name = gyrase_binding_model_name
    binding_oparams = {'k_on': params['k_on_gyrase'], 'width': params['width_gyrase'],
                       'threshold': gyrase_threshold}
...
space = {
    ...
    'width_topoI': hp.uniform('width_topoI', width_min_topoI, width_max_topoI),
    ...
    'width_gyrase': hp.uniform('width_gyrase', width_min_gyrase, width_max_gyrase)
}
...
both_df = pd.DataFrame(columns=['k_on_topoI', 'k_off_topoI', 'k_cat_topoI', 'width_topoI', 'threshold_topoI',
                                'k_on_gyrase', 'k_off_gyrase', 'k_cat_gyrase',
...
both_df['width_gyrase'] = best_df['width_gyrase']
both_df['threshold_gyrase'] = best_df['width_gyrase'] * gyrase_A
both_df['width_topoI'] = best_df['width_topoI']
both_df['threshold_topoI'] = best_df['width_topoI'] * topoI_A
...
params_df = pd.DataFrame(columns=['test', 'loss',
                                  'k_on_topoI', 'k_off_topoI', 'k_cat_topoI',
                                  'width_topoI', 'threshold_topoI',
...
for n in range(tests):
    ...
    new_row = pd.DataFrame({
        ...
        'width_topoI': va['width_topoI'], 'threshold_topoI': float(va['width_topoI'][0])*topoI_A,
        'k_on_gyrase': va['k_on_gyrase'], 'k_off_gyrase': va['k_off_gyrase'], 'k_cat_gyrase': va['k_cat_gyrase'],
        'width_gyrase': va['width_gyrase'], 'threshold_gyrase': float(va['width_gyrase'][0]) * gyrase_A,
...
topo_df['threshold'] = best_df['width_topoI'] * topoI_A
...
gyrase_df['threshold'] = best_df['width_gyrase'] * gyrase_A
