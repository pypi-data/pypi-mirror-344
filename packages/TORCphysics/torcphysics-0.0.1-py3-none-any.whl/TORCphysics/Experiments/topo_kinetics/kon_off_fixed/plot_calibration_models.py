...
n_simulations = 96#10#84  # 60 #48 #120
...
params_dict = pd.read_csv(params_file).to_dict()
poi_objective, poi_sim_superhelicals = objective_function(params=params_dict)
...
for n in range(3):
    ...
    formatted_sum_err = "{:.2f}".format(sum_err_rec)  # Formatting to three significant figures
    rec_label = r'Recognition $\epsilon={}$'.format(formatted_sum_err)

    formatted_sum_err = "{:.2f}".format(sum_err_poi)  # Formatting to three significant figures
    poi_label = r'Poisson $\epsilon={}$'.format(formatted_sum_err)

    ax.plot(time, exp_sigmas[n], lw=lw, color=experiment_color, label='Kinetic')
    ax.plot(time, rec_sim_superhelicals[n], lw=lw, color=rec_model_color, label=rec_label)
    ...
    ax.text(-0.12, 0.99, outside_label[n], transform=ax.transAxes,
