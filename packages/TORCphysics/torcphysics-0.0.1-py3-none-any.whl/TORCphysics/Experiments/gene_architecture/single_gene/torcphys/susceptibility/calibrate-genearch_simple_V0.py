...
# It varies gamma.
# This file uses a model based on Sam Meyer's model, where binding/transcription completely depends on the
# free energy required for melting

# **********************************************************************************************************************
# Inputs/Initial conditions - At least the ones you need to change before each run
# **********************************************************************************************************************
#model_code='GB-Stages-trials-avgx1-01-'
model_code = 'genearch_V0-01'
...
reporter_type = 'site'
reporter_binding_model_name = 'SISTBasedBinding'
reporter_oparams = {
...
RNAP_unbinding_model_name = 'RNAPSimpleUnbinding'
...
#    'width': presponse['width'].iloc[0], 'threshold': presponse['threshold'].iloc[0]
        ...
        binding_oparams = {
            # GaussianBinding
            'k_on': params['kon'], 'width': reporter_oparams['width'], 'threshold': reporter_oparams['threshold']
        }
...
complete_df = pd.DataFrame(columns=['k_on', 'width', 'threshold',
                                    'gamma', 'kappa', 'velocity', 'stall_torque'])
...
for n in range(tests):
    ...
    new_row = pd.DataFrame({
        'test': n, 'loss': lo,
        'k_on': va['kon'], 'width': reporter_oparams['width'], 'threshold': reporter_oparams['threshold'],
        'gamma': RNAP_oparams['gamma'], 'kappa': RNAP_oparams['kappa'],
