...
class PvCheckbox(PvText):
    def __init__(self, container,
                 ...
                 checkmark_color=(0, 0, 0, 1), checkmark_size=10, checkmark_type="❤️",
                 checkbox_corner_radius=0, is_checked=False, checked=None, toggle_callback=None,
                 border_color=(50, 50, 50, 1), border_thickness=1, corner_radius=2, border_style="solid",
    ...
    def configure_style(self):
        ...
        # Always apply corner radius regardless of border
        if self._corner_radius > 0:
            style += f" border-radius: {self._corner_radius}px;"

        # Add border styling using CSS
        if self._border_thickness > 0 and self._border_color is not None:
            border_r, border_g, border_b, border_a = self._border_color
            style += f" border: {self._border_thickness}px {self._border_style} rgba({border_r}, {border_g}, {border_b}, {border_a});"
            # Add padding to create gap between border and content
            # style += f" padding: 12px;"
...
if __name__ == "__main__":
    ...
    checkbox1 = PvCheckbox(window,
                           ...
                           on_change=lambda checked: print(
                               f"Checkbox 3 is {'checked' if checked else 'unchecked'} now"))
