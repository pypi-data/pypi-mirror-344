    ...
    async def run_browser_agent(ctx: Context, task: str, add_infos: str = "") -> str:
        ...
        try:
            (
                ...
                keep_browser_open=get_env_bool("MCP_KEEP_BROWSER_OPEN", True),
                headless=get_env_bool("MCP_HEADLESS", False),
...
def playwright_install_chromium()->int:

    try:
        completed_process = subprocess.run(["playwright", "install", "chromium"])
...
def set_browser_use_settings():
    os.environ["ANONYMIZED_TELEMETRY"] = "false"

server = serve()

def main():
    set_browser_use_settings()
