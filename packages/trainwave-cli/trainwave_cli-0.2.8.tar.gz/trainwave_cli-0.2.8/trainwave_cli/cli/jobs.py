...
from trainwave_cli.utils import async_command, ensure_api_key, to_dict_recursive, truncate_string
...
async def launch(  # noqa
    ...
    setup_command: Annotated[str, typer.Option(help="The command to run before the job")],
    run_command: Annotated[str, typer.Option(help="The command to run the job")],
    # Optional params
    organization: Annotated[str | None, typer.Option(help="ID or RID of organization")] = None,
    expires: Annotated[
        str,
        typer.Option(
    ...
    exclude_regex: Annotated[
        str | None,
        typer.Option(help="Files matching this pattern will be ignored when packaging up the job"),
    ] = None,
    # Harware parameters
    image: Annotated[str, typer.Option(help="The docker image to use as base for the job")] = "",
    ...
    gpus: Annotated[int, typer.Option(help="Number of GPUs to allocate of the `gpu_type`")] = 0,
    gpu_type: Annotated[str, typer.Option(help="Canonical name of the GPU to use")] = "",
    ...
    try:
        ...
        tarball = create_tarball(
            Path().cwd(), exclude_gitignore, exclude_regex, show_progress_bar=not json
        )
        await api_client.upload_code(Path(tarball.name), job.upload_url, show_progress_bar=not json)
        ...
        if not json:
            ...
            print(_json.dumps(to_dict_recursive(job), indent=4))
...
async def list_jobs(
    ...
    api_client = Api(config.api_key, config.endpoint, project=project, organization=organization)
