...
class SystemLogsAssistant:
    ...
    def getLogMessages(self, webPage:str) -> str:
        ...
        """
        logObj = DB.name.getDocuments(collectionName=Vars.collectionName, fields={webPage: {'$exists': True}}, 
                                      includeFields={'_id':0})
        count = deepcopy(logObj)
        if len(list(count)) > 0:
            messages = logObj[0]
    ...
    def log(self, user:str, webPage:str, action:str, msgType:str, msg:str, forDetailLogs='') -> None:
        ...
        try:
            ...
            result = DB.name.updateDocument(collectionName=Vars.collectionName, 
    ...
    def deletePastLogs(self):
        ...
        # ['results', 'accountMgmt', 'modules', 'pipelines']
        allLogCategories = self.getLogTitles()
        today = datetime.now()
        
        for logCategory in allLogCategories:
            logs = DB.name.getDocuments(collectionName=Vars.collectionName, fields={logCategory: {'$exists': True}}, 
                                        includeFields={'_id':0})
