...
class KeystackServices:
    ...
    def isServiceRunning(self, service, showStdout=True):
        """
        ...
        if process:
            ...
            if match:
                print(f'\nisServiceRunning: {service} is running')
                return True
            else:
                print(f'\nisServiceRunning: {service} is not running')
                return False
    ...
    def stopService(self, service, showStdout=False):
        ...
        """ 
        buffer = execSubprocessInShellMode(f'ps -aux | grep {service}', showStdout=True)
