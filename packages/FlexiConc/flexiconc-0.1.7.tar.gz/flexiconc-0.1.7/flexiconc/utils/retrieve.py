...
def retrieve_from_sketchengine(
    ...
    info = requests.get(f'{base_url}/search/corp_info', params={
        'corpname': corpus,
        'format': 'json'
    }, auth=auth).json()
    attrs = ['word'] + [attr['name'] for attr in info['attributes'] if attr['name'] != 'word']

    page_size, fromp, all_lines = 1000, 0, []

    while True:
        response = requests.get(f'{base_url}/search/concordance', params={
            'corpname': corpus,
            'q': f'q{query}',
            'format': 'json',
            'attrs': ','.join(attrs),
    ...
    for line_id, line in enumerate(all_lines):
        ...
        def collect_tokens(toklist, line_id, id_in_line):
            ...
            for tok in toklist:
                ...
                attr_values = parse_attr_string(tok.get('attr', ''))
                result.append({
        ...
        id_in_line = 0
        left_tokens, id_in_line = collect_tokens(line.get('Left', []), line_id, id_in_line)
        kwic_tokens, id_in_line = collect_tokens(line.get('Kwic', []), line_id, id_in_line)
        right_tokens, id_in_line = collect_tokens(line.get('Right', []), line_id, id_in_line)
        ...
        if fetch_metadata:
            md = {'line_id': line_id}
            try:
                meta_response = requests.get(f'{base_url}/search/fullref', params={
                    'corpname': corpus,
                    'pos': line.get('toknum')
                }, auth=auth).json()
                for ref in meta_response.get('Refs', []):
                    key = ref.get('id', ref.get('name')).replace('.', '_').lower()
                    md[key] = ref.get('val')
            except Exception:
                pass
            metadata_rows.append(md)
        else:
            metadata_rows.append({'line_id': line_id})
