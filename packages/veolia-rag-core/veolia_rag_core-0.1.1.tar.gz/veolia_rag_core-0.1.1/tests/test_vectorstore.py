...
async def test_mock_vectorstore():
    ...
    results = await vectorstore.search(
        query_vector=query_vector, k=2, filters={"tenant_id": "test_tenant"}
    )
...
async def test_vectorstore_event_handler():
    ...
    # Criar handler
    handler = VectorStoreEventHandler(event_bus=event_bus, vectorstore=vectorstore)
...
async def test_vectorstore_error_handling():
    ...
    event_data = {"embeddings": None}  # Embeddings inválidos
