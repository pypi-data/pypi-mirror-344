...
async def test_result_combination():
    ...
    doc1 = Document(
        page_content="Conteúdo 1", metadata={"source": "doc1", "score": 0.8}
    )
    doc2 = Document(
        page_content="Conteúdo 2", metadata={"source": "doc2", "score": 0.9}
    )
    doc3 = Document(
        page_content="Conteúdo 3", metadata={"source": "doc3", "score": 0.7}
    )
...
async def test_custom_base_retriever():
    ...
    retriever = MockMultiQueryRetriever(base_retriever=base_retriever, num_queries=2)
    await retriever.initialize()
