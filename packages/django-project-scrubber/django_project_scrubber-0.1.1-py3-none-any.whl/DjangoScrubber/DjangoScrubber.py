                    ...
                    if file.endswith(('.html', '.txt')):
                        ...
                        try:
                            with open(file_path, 'r', encoding='utf-8') as f:
                                ...
                                extend_matches = re.findall(r'{%\s*extends\s*[\'"]([^\'"]+)[\'"]\s*%}', content)
                ...
                for file in files:
                    if file.endswith('.py'):
                        ...
                        try:
                            with open(file_path, 'r', encoding='utf-8') as f:
                                content = f.read()
                                # Check for template rendering in views
                                render_matches = re.findall(r'render\([^)]*template_name\s*=\s*[\'"]([^\'"]+)[\'"]', content)
