...
async def get_file(file_id: str | ObjectId, user: UserDB) -> FileDB:
    ...
    access_from_file_share = False
    if user.email is not None:
        access_from_file_share = bool(user.email.lower() not in [user.lower() for user in file.file_users])

    if file.owner_id != user.sub and file.owner_id != str(user.id) and not access_from_file_share and not access_from_project:
