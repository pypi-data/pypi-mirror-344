...
async def get_file(file_id: str | ObjectId, user: UserDB) -> FileDB:
    ...
    if file.project_id is not None:
        ...
        if project is not None and project.user_has_access(user):
            access_from_project = True

    if file.owner_id != user.sub and file.owner_id != str(user.id) and user.email not in file.file_users and not access_from_project:
