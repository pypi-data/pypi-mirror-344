...
class BaseDapiValidator(BaseValidator):
    ...
    def generated_file_state(self) -> Dict[str, Dict]:
        ...
        for filepath, gen_dapi in gen_files.items():
            ...
            field_name_to_sort_key = {
                field["name"]: (i, field["name"])
                for i, field in enumerate(og_dapi.get("fields", []))
            }
            for field in gen_dapi["fields"]:
                if field["name"] in field_name_to_sort_key:
                    continue
                # all new fields have inf, so will go to end, and then sorted by their name
                field_name_to_sort_key[field["name"]] = (math.inf, field["name"])

            # now we have to order the fields in gen, but make sure that we
            # only include the generated fields - not the ones in original
            gen_dapi["fields"] = sorted(
                gen_dapi["fields"],
                # the lambda is invoked only in the scope of the same loop iteration,
                # so this wont lead to closer weirdness with other iterations
                key=lambda f: field_name_to_sort_key[  # pylint: disable=cell-var-from-loop
                    f["name"]
                ],
            )
        return gen_files
