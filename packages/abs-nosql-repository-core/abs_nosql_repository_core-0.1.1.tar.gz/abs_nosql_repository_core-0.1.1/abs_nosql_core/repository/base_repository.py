...
from ..exceptions import BadRequestError, NotFoundError, MongoError
...
class BaseRepository:
    ...
    def __init__(self, document:Type[BaseDocument]):
    ...
    async def create(self,data:T):
        ...
        await model_instance.insert()  # Inserting the object to the DB
    ...
    async def update(self,id:str,obj:T):
        ...
        """
        # Only include fields that are not None (assuming None means "not provided")
    ...
    async def get_all(self,find:ListFilter):
        ...
        if find.filter:
            filter_dict = find.filter.model_dump()
        ...
        # Return the data and search options (pagination details)
        response = {
            "founds": data, 
            "search_options": {
    ...
    def build_mongo_filter(self, filter_dict: Dict,operator:LogicalOperator=LogicalOperator.AND) -> Dict[str, Any]:
        ...
        if "operator" in filter_dict and "conditions" in filter_dict:
            ...
            try:
                ...
                return apply_condition(comp_operator,filter_dict["field"], filter_dict["value"])
            except ValueError:
    ...
    async def delete(self, id: str) -> bool:
        ...
        if result:
            ...
            raise NotFoundError("Youre trying to delete a non-existing object")
    ...
    async def get_unique_values(self, schema: FindUniqueByFieldInput):
        try:
            ...
            if schema.sort_order:
                pipeline.append({
                    "$sort": {
                        "_id": 1 if schema.sort_order == SortDirection.ASC else -1
                    }
                })

            # Count total
            count_pipeline = pipeline + [{"$count": "total"}]
            count_result = await self.model.get_motor_collection().aggregate(count_pipeline).to_list(length=1)
            total_count = count_result[0]["total"] if count_result else 0

            # Execute aggregation
            results = await self.model.get_motor_collection().aggregate(pipeline).to_list(length=None)
