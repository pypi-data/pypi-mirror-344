...
def run(args, is_async=False):
    ...
    if sock_path is None or config_file_resolved == False:
        ...
        if pid > 0:  # pid == child process id
            ...
            _diag_log('INFO', '[run-proxy]', os.path.join(root_dir, 'bin'), config_path, log_dir, sock_path)
...
def start_diagnostics_thread():
    ...
    _diag_log('INFO', 'Activate start_diagnostics_thread', 'parent process id:', os.getppid())
