...
class SvgParser(SvgHandler):
    ...
    def parse_transform(self, element, group, dest_trans):
        ...
        if not bb.isnull():
            ...
            itcy = self.qualified("inkscape", "transform-center-y")
            if itcx in element.attrib and itcy in element.attrib:
                cx = float(element.attrib[itcx])
                cy = float(element.attrib[itcy])
                bbx, bby = bb.center()
                cx += bbx
                cy = bby - cy
                tentative_ap = NVector(cx, cy)
            #else:
                #c = bb.center()
        ...
        if "transform" not in element.attrib:
            if tentative_ap:
                dest_trans.anchor_point.value += tentative_ap
                dest_trans.position.value += tentative_ap
            return
