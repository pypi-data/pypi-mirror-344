...
class ResultsAnalyzer:
    ...
    def print_results(self, limit=None, schema_id=None):
        """
        ...
        schema_id : str, optional
            Specific schema_id to filter for
        """
        query = "SELECT * FROM results"
        where_clauses = []
        
        if schema_id:
            where_clauses.append(f"schema_id = '{schema_id}'")
    ...
    def search_by_keyword(self, keyword, context=False):
        ...
        if context:
            select_clause += """,
            answer_generator_input->>'schema_text' as context
            """
