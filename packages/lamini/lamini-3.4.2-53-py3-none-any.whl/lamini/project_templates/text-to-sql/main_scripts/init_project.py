...
from models.project.projectdb import ProjectDB  # initialize all tables including datasets
current_dir = os.path.dirname(os.path.abspath(__file__))
...
def update_project(project_name):
    ...
    # Determine base project name by stripping any "<n>.0" suffix
    match = re.match(r'^(.*?)(\d+)\.0$', project_name)
    base_name = match.group(1) if match else project_name
    # Path to the project directory under the base name
    project_dir = base_dir / "projects" / base_name
    ...
    # Select all rows for this project (base or versioned names)
    pattern = rf'^{re.escape(base_name)}(?:\d+\.0)?$'
    ...
    # Prepare new project_name suffix and version
    new_version = latest_version + 1
    new_name = f"{base_name}{new_version}.0"
    # Generate a unique project ID
    project_id = str(uuid.uuid4())
...
def main():
    ...
    if args.update:
        ...
        print("2. update experiment.yml for your first experiment. HINT: This repo considers each run of the pipeline as an \"experiment\"")
        print("3. (optional) update prompts.yml. The default prompts will work well for your initial run. but definitely update them to shape the pipeline output to more closely resemble your eval data and how your users expect the data.")
