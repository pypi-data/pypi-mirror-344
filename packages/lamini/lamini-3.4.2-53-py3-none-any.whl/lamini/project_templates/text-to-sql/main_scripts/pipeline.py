...
def main(args,config,project_dir,experiment_config):
    ...
    def get_experiment_config(experiment_name, project_name, description,
                                data_dir='data',results_dir='experiment_results',
    ...
    project_glossary_df = glossary_df[glossary_df["project_id"] == ExperimentDefinition["project_name"]]
    glossary = {row["input"]: row["output"] for _, row in project_glossary_df.iterrows()}


    pipeline = BaseAgenticPipeline.from_json(ExperimentDefinition["pipeline"]["path"])
    ...
    # --------------------------------------------------------------------- #
    # Process the refset through the pipeline                             #
    # --------------------------------------------------------------------- #
    refset_path = Path(__file__).parent.parent / "local-db" / "refset.parquet"
    
    try:
        refset_df = pd.read_parquet(refset_path)
    except Exception as e:
        print(f"Error loading refset: {e}")
        refset_df = pd.DataFrame()
    project_refset_df = refset_df[refset_df["project_id"] == project_name]
    if project_refset_df.empty:
        print(f"No refset entries found for project '{project_name}'")
    else:
        # Prepare full prompt DataFrame with all required input keys for pipeline
        prompts_df = project_refset_df.rename(columns={"input": "question", "output": "sql"})
        ...
        extract_cols = [
            "question", "sql", "original_question", "original_sql",
            "schema", "glossary", "db_type", "db_params",
            "sub_question", "error_sql", "error_message", "error_explanation"
        ]
        ...
        # Save pipeline outputs for refset
        save_results(results_df, ExperimentDefinition["experiment_name"])
        print(f"Processed {len(results_df)} refset entries through pipeline.")
        # Create dataset of valid QA pairs and register experiment
        try:
            valid_pairs = results_df[results_df.get('is_valid') == True][['question', 'sql']]
            if not valid_pairs.empty:
                ...
                print(f"Created dataset '{dataset_id}' with {len(valid_pairs)} valid pairs and linked to experiment.")
        except Exception as e:
