    ...
    while True:
        ...
        if choice == '1':
            ...
            if 'LAMINI_API_KEY' not in os.environ:
                ...
                main_topic = input("Enter the main topic for the project (this will be used for baseline roles and prompts):  ").strip()
                project_description = input("Enter a description for the project (optional): ").strip()
                subprocess.run(["python", "main_scripts/init_project.py", "--project_name", project_name,"--topic", main_topic])
                ...
                project_file['Lamini']['api_key']['value']=os.environ['LAMINI_API_KEY'] 
                project_file['Project']['topic']['value']=main_topic
                if not project_description=='':
                    project_file['Project']['topic']['description']=project_description
            ...
            if 'LAMINI_API_KEY' not in os.environ:
                with open(os.path.join('projects',project_name_org,'ymls','project.yml'), 'r') as file:
                    project_file = yaml.safe_load(file)
                os.environ['LAMINI_API_KEY']=project_file['Lamini']['api_key']['value'] 
            ...
            while True:
                ...
                action_choice = input("Enter your choice [d/ge/t/i/m/r/q]: ").strip().lower()
                ...
                if action_choice == 'd':
                    ...
                    mode = exp_file['Experiment']['loading_mode']['value'].strip().lower()
                    chunk_strategy = exp_file['Experiment']['chunk_strategy']['value'].strip().lower()
                    ...
                    with open(os.path.join(Path(__file__).parent,'projects',project_name_org,'ymls','experiment.yml'), 'w') as file:
                    ...
                    while True:
                        ...
                        if experiment_name not in list_experiments:
                            ...
                            print(f"✅  Using experiment “{experiment_name}” for evaluation set generation")
                            break

                    sample_frac_input = input("Enter fraction of chunks to sample [default 0.2]: ").strip()
                    try:
                        sample_frac = float(sample_frac_input) if sample_frac_input else 0.2
                    except ValueError:
                        print("Invalid fraction. Defaulting to 0.2")
                        sample_frac = 0.2

                    generate_eval_cmd = [
                        "python", "main_scripts/generate_eval_set.py",
                        "--project_name", project_name_org,
                        "--experiment_name", experiment_name,
