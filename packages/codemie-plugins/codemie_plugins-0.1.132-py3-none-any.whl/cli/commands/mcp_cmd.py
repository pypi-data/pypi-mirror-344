...
SERVERS_WIN_JSON = "servers-win.json"
...
def get_toolkit_config_path() -> Path:
    """
    Get the path to the MCP toolkit configuration file based on the OS.
    
    Returns:
        Path: Path to the configuration file
    """
    config_file = SERVERS_WIN_JSON if os.name == "nt" else SERVERS_JSON
    root_dir = Path(__file__).parent.parent.parent
    return root_dir / "toolkits" / "mcp" / config_file
...
def print_config_paths(config_path: Path) -> None:
    ...
    Args:
        config_path: Path to the toolkit configuration file
    """
    console.print("\n[dim]Configurations loaded from:[/]")
    if config_path.exists():
        console.print(f"[dim]- Toolkit: {config_path}[/]")
    console.print(f"[dim]- Global: {GLOBAL_CONFIG_PATH}[/]")
...
def print_server_sources(toolkit_config_path: Path, temp_config_path: Path) -> None:
    ...
    """
    console.print("[dim]Using servers from:[/]")
    if toolkit_config_path.exists():
        console.print(f"[dim]- Toolkit config: {toolkit_config_path}[/]")
    console.print(f"[dim]- Global config: {GLOBAL_CONFIG_PATH}[/]")
    console.print(f"[dim]- Combined config: {temp_config_path}[/]")
...
@click.pass_context
def mcp_run(ctx, servers: str, env: List[str], timeout: Optional[int]):
