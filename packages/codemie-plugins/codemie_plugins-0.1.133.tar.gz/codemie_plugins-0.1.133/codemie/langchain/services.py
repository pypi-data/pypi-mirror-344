...
def format_tool_content(content: Any, max_length: int = 500, is_json: bool = True) -> str:
...
def format_tool_output(tool_name: str, output: Any, max_length: int = 500) -> str:
    """Format and truncate tool output for logging.
    ...
    Returns:
        Formatted and potentially truncated output string
    """
    formatted_output = format_tool_content(output, max_length)
    separator = "-" * 60
    header = f"▼ Tool '{tool_name}' RESULT ▼"
    centered_header = header.center(60)
    footer = "▲" + " END RESULT ".center(56, "─") + "▲"
    
    return f"\n{separator}\n{centered_header}\n{formatted_output}\n{footer}\n{separator}"


def format_tool_input(tool_name: str, input_data: Any, max_length: int = 300) -> str:
    """Format and truncate tool input for logging.
    
    Args:
        tool_name: Name of the tool
        input_data: The input to the tool
        max_length: Maximum length of the formatted input
        
    Returns:
        Formatted and potentially truncated input string
    """
    formatted_input = format_tool_content(input_data, max_length)
    separator = "-" * 60
    header = f"▼ Tool '{tool_name}' INPUT ▼"
    centered_header = header.center(60)
    footer = "▲" + " END INPUT ".center(56, "─") + "▲"
...
class ToolService:
    ...
    async def _on_error(self, e):
        separator = "!" * 50
        logger.error(f"\n{separator}\nNATS error for tool '{self.tool.name}': {str(e)}\n{separator}")

    async def _on_disconnect(self):
        separator = "!" * 50
        logger.warning(f"\n{separator}\nNATS disconnected for tool '{self.tool.name}'\n{separator}")

    async def _on_reconnect(self):
        separator = "=" * 50
        logger.info(f"\n{separator}\nNATS reconnected for tool '{self.tool.name}'\n{separator}")

    async def _on_close(self):
        separator = "=" * 50
        logger.info(f"\n{separator}\nNATS connection closed for tool '{self.tool.name}'\n{separator}")
        self._running = False
    ...
    async def execute_tool_with_timeout(self, query, timeout):
        ...
        try:
            ...
            logger.info(format_tool_input(self.tool.name, tool_input))
            tool_response = await asyncio.wait_for(
                self.tool.arun(tool_input, tool_call_id=str(uuid.uuid4())), timeout
            )
            logger.info(format_tool_output(self.tool.name, tool_response))
    ...
    async def subscribe(self):
        ...
        async def _run_handler(msg, query: str = None):
            logger.info(f"Running tool '{self.tool.name}' with query")
            response = service_pb2.ServiceResponse()
            response.meta.subject = self.subject
        ...
        if sub:
            ...
            separator = "=" * 50
            logger.info(f"\n{separator}\nTool '{self.tool.name}' subscribed to '$PLUGIN_KEY.{'.'.join(parts[1:])}'\n{separator}")
        while True:
            await asyncio.sleep(1)
