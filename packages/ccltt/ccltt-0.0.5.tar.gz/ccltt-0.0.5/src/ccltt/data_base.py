...
class MyDatabase:
    ...
    def find_child_by_name(self, tree_name, table_name, parent_id, target_name):
        ...
        select_nodes_sql = f'''
        SELECT id, node_name, node_type, node_value
        FROM {table_name}
        WHERE id IN ({placeholder}) AND node_name = ?
        UNION
        SELECT r.child_id AS id, n.node_name, n.node_type, n.node_value
        FROM relations r
