...
def create_security_summary(results: Dict) -> str:
    ...
    if "network" in results.get("modules", {}):
        ...
        ssh_ports = [port for port in network_data.get("metrics", {}).get("open_ports", [])
                    if port.get("port") == 22]
        if ssh_ports:
            table.add_row(
                "SSH Access",
                "[yellow]OPEN[/yellow]",
                f"SSH service is running and accessible"
            )
    ...
    if "system" in results.get("modules", {}):
        ...
        suspicious_processes = system_data.get("metrics", {}).get("processes", {}).get("high_resource_processes", [])
        if suspicious_processes:
            for proc in suspicious_processes[:5]:  # Show top 5 suspicious processes
                if proc.get("cpu_percent", 0) > 50 or proc.get("memory_percent", 0) > 50:
                    table.add_row(
                        "Suspicious Process",
