...
from simplyblock_core.services.health_check_service import nodes_ports_blocked
...
class CLIWrapperBase:
    ...
    def storage_node__configure(self, sub_command, args):
        ...
        if args.sockets_to_use:
            try:
                sockets_to_use = [int(x.strip()) for x in args.sockets_to_use.split(',')]
            except ValueError:
                self.parser.error(
        ...
        return storage_ops.generate_automated_deployment_config(args.max_lvol, args.max_prov, args.sockets_to_use,
                                                                sockets_to_use)
