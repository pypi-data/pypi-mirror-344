...
def generate_configs(max_lvol, max_prov, sockets_to_use, nodes_per_socket):
    ...
    print(cores_by_numa)  # Print for debugging
    nics = detect_nics()
    nvmes = detect_nvmes()
    ...
    for nid in sockets_to_use:
        ...
        for idx, core_group in enumerate(node_cores.get(nid, [])):
            ...
            print(core_group)
            number_of_distribs_cores = core_group["distribution"][5]
            number_of_poller_cores = core_group["distribution"][2]


            if len(number_of_distribs_cores) > 2:
                number_of_distribs = len(number_of_distribs_cores)
            node_info = {
                "socket": nid,
                "sys_memory": f"{calculate_sys_memory(max_prov)}",
                ...
                "number_of_alcemls": 0,
                "distribution": core_group["distribution"],
                "ssd": [],
                "nic_ports": system_info[nid]["nics"]
            }
            for nvme_name in nvme_per_core_group[idx]:
                node_info["ssd"].append({
                    "device": nvme_name,
                    "pci_address": nvmes[nvme_name]["pci_address"]
                })
            number_of_alcemls = len(node_info["ssd"])
            node_info["number_of_alcemls"] =number_of_alcemls
            small_pool_count, large_pool_count = calculate_pool_count(number_of_alcemls, 2 * number_of_distribs,len(core_group["isolated"]), len(number_of_poller_cores) or len(core_group["isolated"]))
