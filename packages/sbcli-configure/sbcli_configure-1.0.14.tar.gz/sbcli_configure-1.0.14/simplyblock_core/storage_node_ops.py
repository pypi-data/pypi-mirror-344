...
def add_node(cluster_id, node_ip, iface_name, data_nics_list,
    ...
    for node in nodes:
        ...
        req_cpu_count = len(node.get("isolated"))

        if req_cpu_count >= 64:
            logger.error(
                f"ERROR: The provided cpu mask {req_cpu_count} has values greater than 63, which is not allowed")
            return False
        poller_cpu_cores = node.get("distribution").get("poller_cpu_cores")
        alceml_cpu_cores = node.get("distribution").get("alceml_cpu_cores")
        distrib_cpu_cores = node.get("distribution").get("distrib_cpu_cores")
        alceml_worker_cpu_cores = node.get("distribution").get("alceml_worker_cpu_cores")
        jc_singleton_core = node.get("distribution").get("jc_singleton_core")
        app_thread_core = node.get("distribution").get("app_thread_core")
        jm_cpu_core = node.get("distribution").get("jm_cpu_core")
        number_of_distribs = node.get("number_of_distribs")

        pollers_mask = utils.generate_mask(poller_cpu_cores)
        app_thread_mask = utils.generate_mask(app_thread_core)

        if jc_singleton_core:
            jc_singleton_mask = utils.decimal_to_hex_power_of_2(jc_singleton_core)
        ...
        max_prov = int(utils.parse_size(node.get("max_size")))

        if max_prov <= 0:
            logger.error(f"Incorrect max-prov value {max_prov}")
            return False

        number_of_alceml_devices = node.get("number_of_alcemls")
        # for jm
        number_of_alceml_devices += 1
        small_pool_count = node.get("small_pool_count")
        large_pool_count = node.get("large_pool_count")

        minimum_hp_memory = node.get("huge_page_memory")
