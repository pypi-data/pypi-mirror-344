...
from quantables.table_model import NO_UNIT_STRING
...
class DataFrameView(QTableView):
    ...
    def _cleanup_data(
        ...
        df.columns = measures
        df = df.drop(columns=SELECT_MEASURE, errors="ignore")
        ...
        for col, unit in zip(measures, units):
            ...
            # create pint series
            ignore_units = [SELECT_UNIT, NO_UNIT_STRING, ""]
...
class CSVPreviewWidget(QWidget):
    ...
    def _populate(self) -> None:
        ...
        for col in range(df.shape[1]):
            ...
            populate_combo_box(
                measure_combo, self.measures, title=SELECT_MEASURE, parent=self
            )
            self.table_view.setIndexWidget(self.model.index(0, col), measure_combo)

            unit_combo = self._create_combo()
            populate_combo_box(unit_combo, self.units, title=SELECT_UNIT, parent=self)
    ...
    def _prepare_units(self, units: list[str]) -> list[str]:
        ...
        if NO_UNIT_STRING not in units:
            units = [NO_UNIT_STRING] + units
        return units
