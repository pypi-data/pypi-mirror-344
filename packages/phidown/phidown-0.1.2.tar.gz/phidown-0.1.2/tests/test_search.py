...
def test_searcher_invalid_orbit_direction():
    with pytest.raises(ValueError, match="Invalid orbit direction"):
        CopernicusDataSearcher(config_path=CONFIG_PATH, collection_name='SENTINEL-1', product_type='SLC', orbit_direction='INVALID')

# Test invalid cloud cover threshold (below 0)
def test_searcher_invalid_cloud_cover_low():
    with pytest.raises(ValueError, match="must be between 0 and 100"):
        CopernicusDataSearcher(config_path=CONFIG_PATH, collection_name='SENTINEL-2', product_type='S2MSI1C', cloud_cover_threshold=-10)

# Test invalid cloud cover threshold (above 100)
def test_searcher_invalid_cloud_cover_high():
    with pytest.raises(ValueError, match="must be between 0 and 100"):
        CopernicusDataSearcher(config_path=CONFIG_PATH, collection_name='SENTINEL-2', product_type='S2MSI1C', cloud_cover_threshold=110)
...
def test_searcher_invalid_aoi_wkt_format():
    with pytest.raises(ValueError, match="must be a valid WKT POLYGON"):
        CopernicusDataSearcher(config_path=CONFIG_PATH, collection_name='SENTINEL-1', product_type='SLC', aoi_wkt='INVALID WKT')
...
def test_searcher_invalid_aoi_wkt_polygon():
    with pytest.raises(ValueError, match="must start and end with the same point"):
        CopernicusDataSearcher(config_path=CONFIG_PATH, collection_name='SENTINEL-1', product_type='SLC', aoi_wkt='POLYGON((0 0, 1 0, 1 1, 0 1))')
...
def test_searcher_invalid_top_high():
    with pytest.raises(ValueError, match="must be between 1 and 1000"):
        CopernicusDataSearcher(config_path=CONFIG_PATH, collection_name='SENTINEL-1', product_type='SLC', top=1001)
...
def test_searcher_invalid_order_by_format():
    with pytest.raises(ValueError, match="Invalid order_by format"):
        CopernicusDataSearcher(config_path=CONFIG_PATH, collection_name='SENTINEL-1', product_type='SLC', order_by='InvalidFormat')
...
def test_searcher_invalid_order_by_field():
    with pytest.raises(ValueError, match="Invalid order_by value"):
        CopernicusDataSearcher(config_path=CONFIG_PATH, collection_name='SENTINEL-1', product_type='SLC', order_by='InvalidField desc')

# Test invalid 'order_by' direction
def test_searcher_invalid_order_by_direction():
    with pytest.raises(ValueError, match="Invalid order_by value"):
        CopernicusDataSearcher(config_path=CONFIG_PATH, collection_name='SENTINEL-1', product_type='SLC', order_by='PublicationDate invalid')

# Test building a basic filter
def test_build_filter_basic():
    searcher = CopernicusDataSearcher(config_path=CONFIG_PATH, collection_name='SENTINEL-1', product_type='SLC')
    searcher._build_filter()
    expected_filter = ("(Collection/Name eq 'SENTINEL-1') and "
...
def test_build_filter_complex():
    ...
    expected_parts = [
        ...
        "Attributes/OData.CSC.StringAttribute/any(att:att/Name eq 'platformSerialIdentifier' and att/OData.CSC.StringAttribute/Value eq 'B')"
    ]
...
def test_build_query():
    ...
    expected_url = (f"{searcher.base_url}?$filter={expected_filter}"
