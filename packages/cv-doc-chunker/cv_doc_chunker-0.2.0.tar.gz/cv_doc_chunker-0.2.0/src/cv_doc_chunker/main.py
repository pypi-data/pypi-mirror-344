...
class PDFParser: 
    ...
    def __init__(self, yolo_model_path = None, debug_mode = False, container_threshold = None, ocr=False, azure_endpoint=None, azure_key=None, hierarchy=True): 
        ...
        Args:
            ...
            hierarchy (bool, optional): Enable hierarchy generation.
                Defaults to True.
            azure_endpoint (str, optional): Azure Document Intelligence endpoint URL.
                Required if ocr is True. Defaults to env var AZURE_DOC_INTEL_ENDPOINT.
            azure_key (str, optional): Azure Document Intelligence API key.
        ...
        if self.ocr:
            ...
            self.azure_endpoint = azure_endpoint or os.getenv("AZURE_DOC_INTEL_ENDPOINT")
            self.azure_key = azure_key or os.getenv("AZURE_DOC_INTEL_KEY")
            ...
            try:
                ...
                self.document_client = DocumentIntelligenceClient(endpoint=self.azure_endpoint, credential=AzureKeyCredential(self.azure_key))
    ...
    def parse_document(self, pdf_path, output_dir=None, generate_annotations=True, iou_threshold=None, save_bounding_boxes=True, use_tesseract = False, tesseract_cmd = None):
        ...
        # Check tesseract cmd path if provided
        if use_tesseract and tesseract_cmd and not os.path.exists(tesseract_cmd):
             logger.warning(f"Provided Tesseract command path does not exist: {tesseract_cmd}")
        ...
        if self.hierarchy: 
            ...
            for tag, text in flattened_doc_texts.items(): 
                ...
                if 'title' in tag: 
                    ...
                    chunks[tag] = {'heading':heading, 'page':page}
                    heading = tag
        ...
        if self.ocr: 
            ...
            try:
                ...
                sorted_doc_texts = self._run_ocr(output_dir, file_basename, result_path, use_tesseract, tesseract_cmd)
        ...
        if self.ocr and self.hierarchy: 

            return {'structure':structure_info, 'text':sorted_doc_texts, 'hierarchy':chunks}
            
        elif self.ocr and not self.hierarchy: 

            return {'structure':structure_info, 'text':sorted_doc_texts, 'hierarchy':None}
            
        elif not self.ocr and self.hierarchy: 

            return {'structure':structure_info, 'text':None, 'hierarchy':chunks}
            
        else: 

            return {'structure':structure_info, 'text':None, 'hierarchy':None}
              
    def _ocr_with_tesseract(self, image_path, tesseract_cmd=None):
        ...
        try:
            ...
            # Temporarily set tesseract command path if provided
            if tesseract_cmd:
            ...
            logger.error(
                ...
                "Please install Tesseract OCR engine or provide the correct path via tesseract_cmd."
            )
            ...
            # Restore original tesseract command path if it was changed
    ...
    def _run_ocr(self, output_dir, doc_name, structure_json_path, use_tesseract = False, tesseract_cmd = None):
        ...
        for page_dir_name in page_dirs:
            ...
            for box_file in box_files:
                ...
                try:
                    ...
                    text = self._ocr_image_box(image_path)
                    
                    if use_tesseract:
                        text = self._ocr_with_tesseract(image_path, tesseract_cmd=tesseract_cmd)
