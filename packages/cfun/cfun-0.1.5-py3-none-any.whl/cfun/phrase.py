...
class Phrase:
    ...
    def __init__(self, userdb: str = "", use_corrector: bool = True):
        """
        根据短语，返回正确的语序
        :param userdb: 用户自定义的数据库， 如果不传入，则使用默认的数据库, 数据库应该有两列，word 和 count列
        :param use_corrector: 是否使用纠错器
        """
        ...
        self.corrector = self._loadbert() if self.use_corrector else None
    ...
    def haserror(self, candidates: list[str]) -> tuple[bool, str, str]:
        """
        检查是否有错误
    ...
    def get_yuxu(self, fragment: str) -> tuple[str, str, bool]:
        ...
        :param userdb: 可选的用户数据库文件路径
        :param use_corrector: 是否启用 pycorrector 检测错别字
        ...
        if matched == "":
            ...
            candidates = [self.get_ordinal_string(fragment, i) for i in match_df["word"]] if len(match_df) > 1 else []
            if candidates:
                has_err, wrong, right = self.haserror(candidates)
                # 如果有错误的，则改成正确的
                if has_err:
