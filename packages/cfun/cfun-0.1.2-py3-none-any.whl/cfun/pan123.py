...
class Pan123Uploader:
    def __init__(self, api: pan123openAPI):
        self.api = api
    ...
    def upload_file_data(self, filename, preuploadID, start_seek, length, idx):
        ...
        with open(filename, "rb") as f:
            while True:
                data_response = self.api.file.get_upload_url(preuploadID, idx + 1)
                presignedURL = data_response.data["presignedURL"]
                f.seek(start_seek)
                try:
                    requests.put(
    ...
    def upload(self, filename, upload_name, parentFileID):
        ...
        """
        f = open(filename, "rb")
        file_etag = self.api.file.md5(f)
        f.seek(0, 2)
        file_size = f.tell()
        f.seek(0)

        # 创建文件
        data_response = self.api.file.create(
            parentFileID=parentFileID,
            filename=upload_name,
            etag=file_etag,
            size=file_size,
        ...
        # 完成上传
        data_response = self.api.file.upload_complete(preuploadID)
        if data_response.data["completed"]:
            return data_response.data["fileID"]
        ...
        if data_response.data["async"]:
            while True:
                time.sleep(1)
                data_response = self.api.file.upload_async_result(preuploadID)
                if data_response.data["completed"]:
                    return data_response.data["fileID"]
...
__all__ = ["pan123openAPI", "Pan123Uploader"]
