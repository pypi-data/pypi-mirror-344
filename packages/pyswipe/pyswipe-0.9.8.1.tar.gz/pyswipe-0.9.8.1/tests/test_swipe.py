...
import pyswipe
from pyswipe.swipe import SWIPE, get_B_space, get_B_ground
from pyswipe import model_utils

test_coeff_fn = os.path.abspath(os.path.join(model_utils.basepath,'coefficients','test_model.txt'))

@pytest.fixture()
def swipe_model(model_coeff):
    model_args = [
        0.4,    # v
        100.6,  # By
        200.7,  # Bz
    ...
    try:
        model = SWIPE(*model_args, **model_kwargs)
    except Exception:
...
class Test_SWIPE(object):
    ...
    def test_init(self, swipe_model):
        model, m_args, m_kwargs = swipe_model

        model = SWIPE(*m_args, **m_kwargs)
        model_vectors = pyswipe.model_utils.get_model_vectors(*m_args, coeff_fn = m_kwargs['coeff_fn'])
    ...
    def test_update_model(self, swipe_model):
        model, m_args, m_kwargs = swipe_model
    ...
    def test__get_vectorgrid(self, swipe_model):
        model, _, _ = swipe_model

        mlat, mlt = model._get_vectorgrid()

        mlat_, mlt_, mlt_res = pyswipe.plot_utils.equal_area_grid(dr=model.dr, M0=model.M0)
    ...
    def test__get_scalargrid(self, swipe_model):
        model, _, m_kwargs = swipe_model
    ...
    def test_calculate_matrices(self, swipe_model):
        model, _, m_kwargs = swipe_model
    ...
    def test_toroidal_scalar(self, swipe_model, mlat, mlt):
        model, _, m_kwargs = swipe_model
        mlt2r = np.pi / 12

        P, dP = pyswipe.sh_utils.legendre(model.N, model.M, 90 - mlat)
    ...
    @pytest.mark.parametrize("mlat, mlt", [(np.array([60.]), np.array([0.])),
                                           (np.array([71.]), np.array([6.]))])
    def test_poloidal_scalar(self, swipe_model, mlat, mlt):
        model, _, m_kwargs = swipe_model
        mlt2r = pi / 12
        REFRE = 6371.2
    ...
    @pytest.mark.parametrize("mlat, mlt", [(np.array([[60.]]), np.array([[0.]])),
                                           (np.array([[71.]]), np.array([[6.]]))])
    def test_get_divergence_free_current_function(self, swipe_model, mlat, mlt):
        model, _, m_kwargs = swipe_model
        mlt2r = pi / 12
        REFRE = 6371.2
        MU0 = pi * 4e-7
    ...
    @pytest.mark.parametrize("mlat, mlt", [(np.array([[60.]]), np.array([[0.]])),
                                           (np.array([[71.]]), np.array([[6.]]))])
    def test_get_upward_current(self, swipe_model, mlat, mlt):
        model, _, m_kwargs = swipe_model
        mlt2r = pi / 12
        REFRE = 6371.2
        MU0 = pi * 4e-7
    ...
    @pytest.mark.parametrize("mlat, mlt", [(np.array([[60.]]), np.array([[0.]])),
                                           (np.array([[71.]]), np.array([[6.]]))])
    def test_get_curl_free_current_potential(self, swipe_model, mlat, mlt):
        model, _, m_kwargs = swipe_model
        mlt2r = pi / 12
        REFRE = 6371.2
        MU0 = pi * 4e-7
    ...
    @pytest.mark.parametrize("mlat, mlt", [(np.array([[60.]]), np.array([[0.]])),
                                           (np.array([[71.]]), np.array([[6.]]))])
    def test_get_divergence_free_current(self, swipe_model, mlat, mlt):
        model, _, m_kwargs = swipe_model
        mlt2r = pi / 12
        lat2r = pi / 180
        REFRE = 6371.2
    ...
    @pytest.mark.parametrize("mlat, mlt", [(np.array([[60.]]), np.array([[0.]])),
                                           (np.array([[71.]]), np.array([[6.]]))])
    def test_get_curl_free_current(self, swipe_model, mlat, mlt):
        model, _, m_kwargs = swipe_model
        mlt2r = pi / 12
        lat2r = pi / 180
        REFRE = 6371.2
    ...
    @pytest.mark.parametrize("mlat, mlt", [(np.array([[60.]]), np.array([[0.]])),
                                           (np.array([[71.]]), np.array([[6.]]))])
    def test_get_total_current(self, swipe_model, mlat, mlt):
        model, _, m_kwargs = swipe_model
    ...
    def test_get_integrated_upward_current(self, swipe_model):
        model, _, m_kwargs = swipe_model
        mlt2r = pi / 12
        lat2r = pi / 180
        REFRE = 6371.2
    ...
    @pytest.mark.parametrize("mlat, mlt", [(np.array([[60.]]), np.array([[0.]])),
                                           (np.array([[71.]]), np.array([[6.]]))])
    def test_get_ground_perturbation(self, swipe_model, mlat, mlt):
        model, _, m_kwargs = swipe_model
        mlt2r = pi / 12
        lat2r = pi / 180
        REFRE = 6371.2
    ...
    def test_get_AE_indices(self, swipe_model):
        model, _, m_kwargs = swipe_model
    ...
    def test_plot_currents(self, swipe_model):
        pass
