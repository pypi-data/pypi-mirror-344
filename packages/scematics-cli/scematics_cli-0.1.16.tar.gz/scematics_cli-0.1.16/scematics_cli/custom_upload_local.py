...
def process_directory_structure(base_url, token, project_id, input_path):
    ...
    if is_video_directory(input_path):
        ...
        # Summary table
        summary = [["(root)", "Success" if result else "Failed", f"{len(json_files)} JSON files"]]
        # print("\nFinal Summary:")
        # print(tabulate(summary, headers=["Folder", "Status", "Details"], tablefmt="grid"))
        ...
        for folder_index, folder in folder_pbar:
            ...
            summary.append([folder_name, status, f"- Successfully processed files"])
