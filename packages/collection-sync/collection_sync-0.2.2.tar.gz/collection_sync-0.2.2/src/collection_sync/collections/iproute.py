...
class IPRoute(ReadWriteCollection):
    ...
    def add(self, item: str) -> None:
        subprocess.run(self.build_cmd('add', item, self.via, self.dev, self.table), shell=True, check=True)

    def add_batch(self, items: list[str]) -> None:
        cmd = '; '.join(self.build_cmd('add', item, self.via, self.dev, self.table) for item in items)
        subprocess.check_call(cmd, shell=True)

    def delete_by_key(self, key: str, key_func: Callable = identity_func) -> None:
        for item in self:
            if key_func(item) == key:
                subprocess.run(self.build_cmd('del', item, self.via, self.dev, self.table), shell=True, check=True)
                break
    ...
    def delete_by_key_batch(self, keys: list[str], key_func: Callable = identity_func) -> None:
        ...
        cmd = '; '.join(self.build_cmd('del', item, self.via, self.dev, self.table) for item in _items)
        subprocess.check_call(cmd, shell=True)
