...
def create_lessons(
    ...
    for idx, lesson_dict in enumerate(lessons):
        ...
        if create_folders:
            ...
            if include_video:
                # ① Pick the desired container *first*
                video_ext = lesson_dict.get("video_ext", DEFAULT_VIDEO_EXT)
            
                # ② Pull the season (“04”) from the parent folder name
                season_folder = Path(parent_path).name
                m             = re.search(r'(\d{2})$', season_folder)
                season_prefix = m.group(1) if m else "01"
            
                # ③ Find the next free episode number inside season_##/
                episode_number = get_next_numeric_prefix(
                    expanded_lesson_path,
                    file_extension=f".{video_ext}"
                )
