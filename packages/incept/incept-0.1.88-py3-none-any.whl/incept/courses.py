...
def addLessons(lesson_payload: dict, course_filter: str, templates_dir: Path, db=DEFAULT_DB, include_video: bool = False, **kwargs):
    ...
    parent_path = (
        target_chapter.get("video_path")    # season_##
        if include_video else target_chapter.get("path")

    if not parent_path:
        raise Exception("Target chapter does not have a valid path/video_path.")
