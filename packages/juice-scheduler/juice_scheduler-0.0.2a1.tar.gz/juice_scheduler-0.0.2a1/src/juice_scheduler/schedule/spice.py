...
class SpiceRunner:
    ...
    def run(self, runner_configuration):
        ...
        logger.info("Getting ptr snippets")
        ...
        logger.info("Dissolving gaps < %.3f seconds", gap_size)
        ...
        logger.info("Dissolving slots < %.3f seconds", gap_size)
        ...
        if not dry_run:
            ...
            with self.ptr_path.open("w") as f:
                f.write(self.ptr)
            logger.info("PTR written to %s", self.ptr_path)
            ...
            logger.info("JSON written to %s", self.json_path)
