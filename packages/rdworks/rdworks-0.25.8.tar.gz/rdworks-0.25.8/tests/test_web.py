...
class TestRdwork(unittest.TestCase):
    ...
    def test_80(self):
        ...
        for mol in libr:
            print(mol.serialize(decimal_places=2))
    ...
    def test_96(self):
        from rdworks import fix_decimal_places_in_dict, fix_decimal_places_in_list
        state_mol = rdworks.Mol('Cc1nc2cc(Cl)nc(Cl)c2nc1C', 'A-1250')
        state_mol = state_mol.make_confs(method='RDKit_ETKDG', optimize='MMFF94')
        ...
        state_mol = state_mol.cluster_confs('QT', 
                                            threshold=1.0, 
                                            sortby='energy')
        print(state_mol.name, {k:v for k,v in state_mol.props.items()})
        for conf in state_mol.confs:
            conf.props = fix_decimal_places_in_dict(conf.props, decimal_places=2)
