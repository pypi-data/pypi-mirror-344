    ...
    if _tf32_enabled is None:
        ...
        if torch.cuda.is_available() and (detect_default_tf32() or torch.backends.cuda.matmul.allow_tf32):
            try:
                ...
                _tf32_enabled = (
                    a_full.float() @ b_full.float() - a_full @ b_full
                ).abs().max().item() > 0.001  # 0.1713
...
class DistCall:
    ...
    def __call__(self, obj):
        if not torch.distributed.is_available():
            return unittest.skipIf(True, "Skipping distributed tests because not torch.distributed.is_available()")(
                obj
            )
