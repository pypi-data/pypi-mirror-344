...
class XpanderEventListener:
    ...
    def __init__(
        ...
        self.base_url = f"{xpander_client.configuration.base_url}"
    ...
    def is_not_inbound(self) -> bool:
        ...
        """
        return "inbound.xpander" not in self.base_url and \
               "inbound.stg.xpander" not in self.base_url
    ...
    async def _make_heartbeat(self, worker_id: str, base_url: str, api_key: str):
        """
        Sends an HB request to the server.

        Args:
            worker_id (str): The ID of the worker to be reported as alive.
        """
        try:
            url = f"{base_url}/{worker_id}"
            async with httpx.AsyncClient() as client:
                await client.post(
                    url,
            ...
            raise Exception(f"Failed to report hb for worker - {str(e)}")
        ...
        try:
            ...
            async for event in aiosseclient(
                ...
                if event.event == EventType.WorkerRegistration:
                    ...
                    agent = agent_worker.metadata
                    logger.info(f"Worker registered - {agent_worker.id}")
                    
                    if agent:
                        is_stg = "stg." in self.get_events_base() or "localhost" in self.get_events_base()
                        agent_name = agent.get("name", agent_id)
                        chat_url = "https://"+agent.get("unique_name", agent_id)+".agents"+(".stg" if is_stg else "")+".xpander.ai"
                        logger.info(f"Agent Name: {agent_name}\n Chat URL: {chat_url}")
                    
                    await self._start_heartbeat_loop(worker_id=agent_worker.id,base_url=self.get_events_base(),api_key=self.api_key)
    ...
    async def _start_heartbeat_loop(self, worker_id: str, base_url: str, api_key: str):
        async def heartbeat_loop():
            while True:
                try:
                    await self._make_heartbeat(worker_id=worker_id, base_url=base_url, api_key=api_key)
                except Exception as e:
