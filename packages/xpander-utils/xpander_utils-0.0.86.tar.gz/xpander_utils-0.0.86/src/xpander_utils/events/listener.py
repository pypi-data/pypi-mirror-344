...
class XpanderEventListener:
    ...
    async def _make_heartbeat(self, worker_id: str):
        """
        ...
        try:
            ...
            async for event in aiosseclient(
                ...
                if event.event == EventType.WorkerRegistration:
                    ...
                    await self._start_heartbeat_loop(worker=agent_worker)
    ...
    async def _start_heartbeat_loop(self, worker: DeployedAsset):
        async def heartbeat_loop():
            while True:
                try:
                    await self._make_heartbeat(worker_id=worker.id)
                except Exception as e:
