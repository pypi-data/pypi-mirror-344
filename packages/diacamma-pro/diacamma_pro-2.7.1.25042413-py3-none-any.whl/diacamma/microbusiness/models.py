...
class SocialDeclaration(LucteriosModel):
    ...
    def calcul(self):
        ...
        amount_sell = 0
        for line in EntryLineAccount.objects.filter(periode & Q(account__code__in=Params.getvalue("microbusiness-sellaccount"))):
            amount_sell += float(line.amount)
        amount_service = 0
        for line in EntryLineAccount.objects.filter(periode & Q(account__code__in=Params.getvalue("microbusiness-serviceaccount"))):
...
def microbusiness_checkparam():
    Parameter.check_and_create(name="microbusiness-simple-socialdeclaration", typeparam=Parameter.TYPE_BOOL, title=_("microbusiness-simple-socialdeclaration"), args="{}", value='True')
    Parameter.check_and_create(name="microbusiness-sellaccount", typeparam=Parameter.TYPE_STRING, title=_("microbusiness-sellaccount"), args="{'Multi':True}", value='',
    ...
    Parameter.check_and_create(name="microbusiness-serviceaccount", typeparam=Parameter.TYPE_STRING, title=_("microbusiness-serviceaccount"), args="{'Multi':True}", value='',
                               meta='("accounting","ChartsAccount","import diacamma.accounting.tools;django.db.models.Q(code__regex=diacamma.accounting.tools.current_system_account().get_revenue_mask()) & django.db.models.Q(year__is_actif=True)", "code", True)')
