...
from myshare.xq import get_xq_symbol
from myshare.utils import get_working_days
...
def get_stock_spot(symbol: str) -> Dict[str, Any]:
    """
    ...
    with SessionLocal() as db:
        ...
        if 'item' not in spot_data or 'value' not in spot_data:
            raise Exception(f"Malformed response from AKShare: missing 'item' or 'value' keys. Response content: {spot_data}")
        ...
        existing_spot = db.query(StockSpot).filter(StockSpot.symbol == symbol).first()
        if existing_spot:
            db.query(StockSpot).filter(StockSpot.symbol == symbol).update({
