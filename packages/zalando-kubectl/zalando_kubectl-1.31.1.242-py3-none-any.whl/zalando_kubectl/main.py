...
def request_cluster_access(
    ...
    if emergency:
        ...
        make_emergency_access_request(
            env, reference_url, reason, okta_auth_client_id, wait_for_approval, timeout, force_new_request
        )
    else:
        make_manual_access_request(env, reason, okta_auth_client_id, wait_for_approval, timeout, force_new_request)
...
def make_manual_access_request(
    env: Environment, reason, okta_auth_client_id, wait_for_approval, timeout, force_new_request
):
    access_request.create_with_okta(
        env,
        "privileged",
...
@click.pass_obj
def list_cluster_requests(env: Environment, request_key, okta_auth_client_id, output):
