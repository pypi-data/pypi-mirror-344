...
class bitslicedpe:
    ...
    def __init__(
        self, 
        HGS=1e-5, LGS=1e-7, g_level=16, var=0.05, vnoise=0.05, wire_resistance=2.93, 
    ...
    def __QuantMap(self, mat, blk=[1, 1, 2, 4]):
        ...
        matq = np.zeros_like(mat)
        max_mat=np.zeros(mat.shape[0])
        
        for i in range(mat.shape[0]):
            max_mat[i] = np.max(np.abs(mat[i,:]))
            if max_mat[i]==0:
                matq[i,:] = 0*mat[i,:]
            else:
                matq[i] = np.round((mat[i,:] / max_mat[i])*(2**(bits-1)-1))
        ...
        for i in range(n_blk):
            data_int[i,:,:] = (((matq-matq%2**bias)%2**(bias+blk[-1-i])))>>int(bias)
            bias+=blk[-1-i]
        
        return max_mat, data_int
    
    def BitSliceVMM(self, xin, matin, xblk=[1,1,2,4], mblk=[1,1,2,4]):     
        ...
        assert mat.shape[1]==x.shape[0]

        max_x, xdata = self.__QuantMap(x, blk=xblk)
        max_m, mdata = self.__QuantMap(mat.T, blk=mblk)
        ...
        return np.dot(np.diag(max_x), np.dot(out, np.diag(max_m)))/(2**(mbits-1)-1)/(2**(xbits-1)-1)
    ...
    def __dot(self, x, mat, xblk=[1,1,2,4], mblk=[1,1,2,4], wire_factor=False):
        ...
        max_x, xint = self.__QuantMap(x, blk=xblk) 
        max_m, matint = self.__QuantMap(mat.T, blk=mblk) 
        matint = matint.transpose(0,2,1)
