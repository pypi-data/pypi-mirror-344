...
from sdeux.defs import (S2_CURR_LIMIT_MODE_GLOBAL, S2_PACKET_QUERY_CONFIGURATION, S2_CURR_LIMIT_MODE_STORED,
                        S2_PULSING_OFF, S2_PACKET_INFO, S2_PACKET_SET_SETTINGS,
                        ...
                        S2_PULSING_BURST_EXTERNAL_TRIGGER, S2_PULSING_EXTERNAL_TRIGGER)
...
class S2Settings(S2Payload):
    __slots__ = ('pulse_period', 'pulse_width', 'output_voltage_set', 'output_current_limit',
                 'pulsing_mode', 'external_trigger_mode_nb_of_pulse_repetition',
                 'unused', 'burst_ON', 'burst_OFF', 'output_voltage_set_A', 'output_voltage_set_B',
                 'pulse_width_A', 'pulse_width_B', 'current_limit_mode')
    _STRUCT = struct.Struct('<IIffHHHIIffIIH')
    ...
    def default(cls):
        ...
        return S2Settings(pulse_period=500, pulse_width=1, output_voltage_set=1.0,
                          output_current_limit=0.1, pulsing_mode=S2_PULSING_OFF,
                          external_trigger_mode_nb_of_pulse_repetition=0,
                          unused=0,
                          burst_ON=0, burst_OFF=0,
...
class BitRegister:
    ...
    def to_dict(self):
        ...
        for a in dir(self):
            if a.startswith('_'):
                continue
            if a not in ['to_dict', 'update_value', 'to_user_message']:
...
class S2(S2Base):
    ...
    PULSING_MODES_LABELS = {S2_PULSING_OFF: 'off',
                            ...
                            S2_PULSING_BURST_EXTERNAL_TRIGGER: 'burst_external_trigger'
                            }
    ...
    def is_overcurrent(self):
        return self._statusRegister.isOverCurrent or self._statusRegister.isFastOverCurrent

    @property
    def is_undervoltage(self):
        return self._statusRegister.isUnderVoltage
    ...
    @property
    def external_trigger_pulse_repetitions(self):
        return self._settings.external_trigger_mode_nb_of_pulse_repetition
    ...
    def __init__(self, th, max_voltage=25):
    ...
    def reload_info(self, expected_response_time=5.0):
        ...
        self._statusRegister.update_value(self.info.status)

    def reload_settings(self, expected_response_time=5.0):
        self._query_packet(_settings_query_packet, self._settings,
                           expected_header=S2_PACKET_QUERY_SETTINGS,
                           expected_response_time=expected_response_time)

    def reload_advanced_settings(self):
        self._query_packet(_info_advanced_settings, self._advancedSettings)
    ...
    def set_settings(self, pulsing_mode=None, voltage=None, pulse_period=None, pulse_width=None, current_limit=None,
                     ...
                     burst_ON=None, burst_OFF=None, current_limit_mode =None,
                     external_trigger_pulse_repetitions=None,
                     persistent = False):
        ...
        with self._lock:
            ...
            if pulsing_mode == S2_PULSING_BURST_EXTERNAL_TRIGGER:
                if external_trigger_pulse_repetitions is not None:
                    self._settings.external_trigger_mode_nb_of_pulse_repetition = external_trigger_pulse_repetitions
