...
from mhm_setup import create_header, create_nml, create_output_nml, create_parameter_nml
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    """
    Parses command-line arguments for the Hydroland MHM/MRM run wrapper script.
    ...
    """
    parser = argparse.ArgumentParser(description="Hydroland MHM/MRM run wrapper")
    parser.add_argument(
        "--ini_date", help="Initialization date (YYYYMMDD) for restart files"
    )
    parser.add_argument("--end_date", help="End date (YYYYMMDD) for output files")
    parser.add_argument(
        "--previous_date", help="Previous run date (YYYYMMDD) for restart lookup"
    )
    parser.add_argument(
        "--stat_freq",
        choices=["daily", "hourly"],
        help="Frequency of model output statistics (daily or hourly)",
    )
    parser.add_argument(
        "--init_files", help="Directory containing initial restart files"
    )
    parser.add_argument("--app_outpath", help="Application output base directory")
    parser.add_argument("--resolution", type=int, help="Model resolution identifier")
    parser.add_argument("--current_mhm_dir", help="Current run directory for MHM")
    parser.add_argument("--forcings_dir", help="Directory to store forcing files")
    parser.add_argument("--mhm_restart_dir", help="Directory for MHM restart files")
    parser.add_argument("--current_mrm_dir", help="Current run directory for MRM")
    parser.add_argument("--mrm_restart_dir", help="Directory for MRM restart files")
