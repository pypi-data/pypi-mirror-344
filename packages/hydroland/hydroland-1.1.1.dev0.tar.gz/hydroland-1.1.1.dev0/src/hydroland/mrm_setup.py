...
def merge_files(
    ...
    while num_pairs > 0:
        if j <= num_in:
            ...
            mosaic_two_tiles(file1, file2, outfile)
        i += 2
        j += 2
        num += 1
        num_pairs -= 1
...
def mrm_fluxes_merge(current_mrm_dir: str, mrm_out_file: str) -> None:
    # --- Merging mRM fluxes ---
    ...
    while j <= 52:
        ...
        mosaic_two_tiles(infile1, infile2, outfile)
        i += 2
        j += 2
        num += 1

    # --- Subsequent merge operations using merge_files() ---
    merge_files("temp_first_loop", 26, "temp_second_loop", 13, current_mrm_dir)
    merge_files("temp_second_loop", 14, "temp_third_loop", 7, current_mrm_dir)
    merge_files("temp_third_loop", 7, "temp_fourth_loop", 4, current_mrm_dir)
    ...
    merge_files("temp_fourth_loop", 4, "temp_fifth_loop", 2, current_mrm_dir)
    ...
    mosaic_two_tiles(infile1, infile2, final_out)
...
time {executable_mrm} > mrm_{ini_date}_to_{end_date}.log 2>&1
""")
