...
def execute_mrm(
    ...
    # executing mRM
    logger.debug("Starting mRM parallel subdomain execution.")
    pRun = ParallelRun(
        exe=run_script,
        args_list=arg_list,
    ...
    logger.debug("Merging all mRM subdomains into one.")
    mrm_fluxes_merge(
        current_mrm_dir=current_mrm_dir,
        mrm_out_file=mrm_out_file,
    )

    logger.debug("Adding DGOV data to mRH output file.")
    prepare_data(
