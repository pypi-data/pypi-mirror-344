...
class A2AClient:
        ...
        try:
            async with self.session.get(f"{self.server_url}/.well-known/agent.json") as response:
                if response.status != 200:
                    ...
                    raise Exception(f"Failed to get agent card: {response.status} - {error_text}")
    ...
    async def send_task(self, 
                        ...
                        session_id: Optional[str] = None) -> SendTaskResponse:
        """
        ...
        if isinstance(message, str):
            message = Message(
                ...
                parts=[TextPart(type="text", text=message)]
            )
        ...
        payload = {
            ...
            "params": {
                ...
                "message": message.dict(exclude_none=True) if hasattr(message, 'dict') else message
            }
        }
        ...
        try:
            async with self.session.post(
                ...
                if response.status != 200:
                    error_text = await response.text()
                    raise Exception(f"Failed to send task: {response.status} - {error_text}")
    ...
    async def cancel_task(self, task_id: str) -> CancelTaskResponse:
        ...
        try:
            async with self.session.post(
                f"{self.server_url}/tasks/cancel",
                json=payload
            ) as response:
                if response.status != 200:
                    error_text = await response.text()
                    raise Exception(f"Failed to cancel task: {response.status} - {error_text}")
    ...
    async def wait_for_task_completion(self, 
                                      ...
                                      timeout: Optional[float] = None) -> GetTaskResponse:
        """
        ...
        while True:
            ...
            if response.result and response.result.status and response.result.status.state in ["COMPLETED", "FAILED", "CANCELED", "completed", "failed", "canceled"]:
                return response
