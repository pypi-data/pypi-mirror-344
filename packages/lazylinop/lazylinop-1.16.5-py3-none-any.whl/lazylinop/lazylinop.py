        ...
        **Notations**:
        ...
        The function verifies:
            ...
            - Linearity:
                ...
                6. ``L @ (a1 * u1 + a2 * u2) == a1 * (L @ u1) + a2 * (L @ u2)``.
        ...
        a1 = np.random.randn()
        u1 = np.random.randn(self.shape[1])
        a2 = np.random.randn()
        u2 = np.random.randn(self.shape[1])
...
#             raise Exception("Wrong operator adjoint on matrix product")
        ...
        # Check linearity L @ (a1 * u1 + a2 * u2) = a1 * L @ u1 + a2 * L @ u2.
        y = self @ (a1 * u1 + a2 * u2)
        z = a1 * (self @ u1) + a2 * (self @ u2)
