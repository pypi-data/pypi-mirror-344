...
class CalendarsResource:
    ...
    async def check(self,
        ...
        Example:
            >>> response = await client.calendars.check("calendar_id", 1672531200)
        """
        params = {"timestamp": timestamp}
        return await self._client._request("GET", self.root_path + "/" + calendar_id + "/check", params=params, headers=self.headers)
    ...
    async def delete_exclusions(self, calendar_id: str, exclusions: list[str]):
        ...
        Example:
            >>> response = await client.calendars.delete_exclusion("calendar_id", ["2023-01-01", "2023-01-02"])
        """
        if len(exclusions) > 1000:
            raise NaxaiValueError("You can only delete up to 1000 exclusions at a time.")

        return await self._client._request("POST", self.root_path + "/" + calendar_id + "/exclusions/remove", json={"exclusions": exclusions}, headers=self.headers)
    ...
    async def add_exclusions(self, calendar_id: str, exclusions: list[str]):
        ...
        Example:
            >>> response = await client.calendars.add_exclusion("calendar_id", ["2023-01-01", "2023-01-02"])
        """
        if len(exclusions) > 1000:
            raise NaxaiValueError("You can only add up to 1000 exclusions at a time.")
        
        return await self._client._request("POST", self.root_path + "/" + calendar_id + "/exclusions/add", json={"exclusions": exclusions}, headers=self.headers)
    ...
    async def update(self, calendar_id: str, data: CreateCalendarRequest):
        ...
        Example:
            >>> updated_calendar = await client.calendars.update("calendar_id", CreateCalendarRequest(
            ...     name="Updated Calendar",
            ...     ...
            ... ))
        """
        return await self._client._request("PUT", self.root_path + "/" + calendar_id, json=data.model_dump(by_alias=True, exclude_none=True), headers=self.headers)
    ...
    async def get(self, calendar_id: str):
        ...
        Example:
            >>> calendar = await client.calendars.get("calendar_id")
        """
        return await self._client._request("GET", self.root_path + "/" + calendar_id, headers=self.headers)
