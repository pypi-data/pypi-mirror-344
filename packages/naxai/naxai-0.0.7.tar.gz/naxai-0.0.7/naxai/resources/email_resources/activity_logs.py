...
class ActivityLogsResource:
    ...
    def get(self, message_id: str, email: str):
        ...
        Example:
            >>> response = client.email.activity_logs.get(message_id="XXX", email="example@example.com")
        """


        return self._client._request("GET", self.root_path + "/" + message_id + "/" + email, headers=self.headers)
    ...
    def list(self,
        ...
        Example:
            >>> response = client.email.activity_logs.list(start=1625097600, stop=1627689600)
        """
        params = {
            "page": page,
            "pagesize": page_size,
