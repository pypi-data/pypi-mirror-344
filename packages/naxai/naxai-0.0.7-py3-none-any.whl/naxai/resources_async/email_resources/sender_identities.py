...
class SenderIdentitiesResource:
    ...
    async def update(self, sender_id: str, name: str, email: str):
        ...
        Example:
            >>> response = await client.email.sender_identities.update("sender123", "Johnny Tuch")
        """
        payload = {
            "name": name,
            "email": email
        ...
        return await self._client._request("PUT", self.root_path + "/" + sender_id, json=payload, headers=self.headers)
    ...
    async def delete(self, sender_id: str):
        ...
        Example:
            >>> response = await client.email.sender_identities.delete("sender123")
        """
    ...
    async def get(self, sender_id: str):
        ...
        Example:
            >>> response = await client.email.sender_identities.get("sender123")
        """
        self._client.logger.debug("sender_id url: %s", self.root_path + "/" + sender_id)
        return await self._client._request("GET", self.root_path + "/" + sender_id, headers=self.headers)
    ...
    async def create(self, domain_id: str, email: str, name: str):
        ...
        Example:
            >>> response = await client.email.sender_identities.create("domain123", "sender@example.com", "Johnny Tuch")
        """
        payload = {
            "domainId": domain_id,
            "email": email,
        ...
        return await self._client._request("POST", self.root_path, json=payload, headers=self.headers)
    ...
    async def list(self,
        ...
        Example:
            >>> response = await client.email.sender_identities.list()
        """
        params = {}
