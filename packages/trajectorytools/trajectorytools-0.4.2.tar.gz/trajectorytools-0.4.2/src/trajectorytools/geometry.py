...
def matrix_dot(matrix: np.ndarray, data: np.ndarray):
    if matrix.ndim == 2:
    ...
    elif data.ndim == matrix.ndim - 1:
        # Each frame, individual (and maybe neighbour) has a matrix
        return np.einsum("...ij,...j->...i", matrix, data)
    elif data.ndim == matrix.ndim == 4:
...
def norm(a: np.ndarray, keepdims=False) -> np.ndarray:
...
def curvature(v: np.ndarray, a: np.ndarray):
    assert (v.shape[-1]) == 2
    assert np.all(v.shape == a.shape)
    return (v[..., 0] * a[..., 1] - v[..., 1] * a[..., 0]) / np.power(norm(v), 3)


def distance_travelled(s) -> np.ndarray:
...
def matrix_rotate_to_normalised_vector(e_y) -> np.ndarray:
    e_x = _ey_to_ex(e_y)
    return np.stack((e_x, e_y), axis=-2)


def matrix_rotate_to_vector(v) -> np.ndarray:
    e_y = normalise(v)
...
def angle_between_vectors(x, y) -> np.ndarray:
