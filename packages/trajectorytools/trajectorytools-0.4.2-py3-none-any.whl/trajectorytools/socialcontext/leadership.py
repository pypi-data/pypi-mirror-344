...
def sliding_average_dot_product_with_delays(
    ...
    if end_frame is None:
        end_frame = data.shape[0] - window_size - 1
...
def sliding_average_dot_product_with_delays2(
    ...
    if end_frame is None:
        end_frame = len(data) - num_frames_to_average - 1
    ...
    for i in range(end_frame - start_frame):
        ...
        assert not isinstance(sum_fleshout, int)  # they should be np.ndarray
        assert not isinstance(sum_connections, int)  # they should be np.ndarray
