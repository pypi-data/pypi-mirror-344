...
def find_enclosing_circle_simple(t: np.ndarray):
    min_x, min_y = np.nanmin(t[..., 0]), np.nanmin(t[..., 1])
    max_x, max_y = np.nanmax(t[..., 0]), np.nanmax(t[..., 1])
...
def find_enclosing_circle(t: np.ndarray):
    ...
    try:
        ...
        logging.error(
            "Miniball failed to find enclosing circle with exception %s: %s",
            type(exc).__name__,
            exc,
            exc_info=exc,
        )
        logging.info("Falling back to simple algorithm")
