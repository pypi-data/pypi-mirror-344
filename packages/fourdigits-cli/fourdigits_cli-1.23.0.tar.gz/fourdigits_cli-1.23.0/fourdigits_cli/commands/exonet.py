...
def minio_create(environment, refresh_access_token=False, name="", token=""):
    ...
    # User
    storage_user = get_resource_by_name(client, "object_storage_users", minio_name)
    if not storage_user:
        storage_user = ApiResource("object_storage_users")
        storage_user.attribute("name", minio_name)
