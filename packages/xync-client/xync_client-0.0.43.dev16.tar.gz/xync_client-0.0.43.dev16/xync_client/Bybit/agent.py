...
class AgentClient(BaseAgentClient):  # Bybit client
    ...
    async def battle(
        ...
        while self.actor.person.user.status > 0:
            ...
            rprice = cprice if (cprice - ceil) * k > 0 else mprice
            mew_price = str(round(rprice - k * step(mad, cad), cur.scale))
...
def step(mad, cad) -> float:
    return (
        ...
        or (cad.recentExecuteRate == mad.recentExecuteRate and cad.orderNum > mad.orderNum)
        else 0
    )
...
async def main():
    ...
    # await cl.set_creds()
    await gather(
        cl.battle(coin, cur, False, [volet], 500, 86.9, 0),
