...
class Ultrasonic:
    ...
    """
    SPEED_OF_SOUND = 343.2  # m/s  (20 °C 기준)
    ...
    def __init__(self, trig_pin, echo_pin):
        ...
        self.echo.irq(trigger=machine.Pin.IRQ_RISING | machine.Pin.IRQ_FALLING, handler=self.__echo_cb)

    def __echo_cb(self, pin):
        """
        Callback function for echo pin interrupt.
        
        :param pin: Pin object for echo
        """
        if pin.value():           # Rising edge
            self._start = utime.ticks_us()
            self._done = False
        else:                     # Falling edge
            self._end = utime.ticks_us()
            self._done = True
    ...
    def read(self, timeout_ms:int=60) -> float:
        ...
        dur = utime.ticks_diff(self._end, self._start)  # μs
        return (dur * self.SPEED_OF_SOUND / 10_000) / 2
