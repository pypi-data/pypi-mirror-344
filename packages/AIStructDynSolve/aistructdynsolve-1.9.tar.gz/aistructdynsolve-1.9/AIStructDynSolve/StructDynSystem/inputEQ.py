...
class inputEQ:
    def __init__(self, StructDynSystemParm, duration_time, eq_time, eq_acc):
        ...
        if not isinstance(self.eq_acc, np.ndarray):
            ...
            self.dimensionsless_Pt = self.eq_acc.abs().mean() / StructDynSystemParm.get_dimensionsless_input()
        else:
            # 如果是np.array矩阵
            self.dimensionsless_Pt = np.abs(self.eq_acc).mean() / StructDynSystemParm.get_dimensionsless_input()
