...
class inputPt:
    def __init__(self, StructDynSystemParm, duration_time, pt_time, pt_value):
        ...
        # 返回所有均值中的均值
        mean_value = np.mean([np.abs(Pt).mean() if isinstance(Pt, np.ndarray) else Pt.abs().mean() for Pt in self.pt_value])
        self.dimensionsless_Pt = mean_value / StructDynSystemParm.get_dimensionsless_input() /StructDynSystemParm.get_dimensionsless_unit()
