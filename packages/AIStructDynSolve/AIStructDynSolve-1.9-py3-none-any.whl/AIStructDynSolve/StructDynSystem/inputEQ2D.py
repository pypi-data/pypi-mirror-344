...
class inputEQ2D:
    def __init__(self, StructDynSystemParm, duration_time, eq_time, eq_accX, eq_accY):
        ...
        if not isinstance(self.eq_accX, np.ndarray):
            ...
            self.dimensionsless_Pt = self.eq_accX.abs().mean() / StructDynSystemParm.get_dimensionsless_input()
        else:
            # 如果是np.array矩阵
            self.dimensionsless_Pt = np.abs(self.eq_accX).mean() / StructDynSystemParm.get_dimensionsless_input()
