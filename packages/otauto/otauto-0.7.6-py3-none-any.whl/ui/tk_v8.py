    ...
    def find_windows(self):
        ...
        for i in range(1, 201):
            ...
            try:
                ...
                ip = data["vnc_ip"]
                targets_dict.update({ip:section_name})
            except Exception as e:
                pass

        # logger.info(f"正在查找窗口: {targets_dict}")

        target_class_name = "TscShellContainerClass"

        all_windows = Desktop(backend="uia").windows()
        found_windows = [
            (window.class_name(), window.window_text().strip(), window.handle)
            for window in all_windows
            if window.class_name() == target_class_name
        ]
        # print(found_windows)
        ip_ls=list(targets_dict.keys())
        if found_windows:
            for i in found_windows:
                socket_ip=i[1].split(' - ')[0]
                if socket_ip in ip_ls:
                    res_ls.append((i[0],targets_dict[socket_ip], i[2]))
        #[('TscShellContainerClass', '192.168.110.122 - 远程桌面连接', 7738306)]
        res_ls.sort(key=lambda x: int(x[1]))
        logger.success(f"找到 {len(res_ls)} 个窗口: {res_ls}")
        return res_ls
