...
class MainApp:
    ...
    def insert_data_to_tree(self):
        ...
        logger.info(f"找到的窗口数据: {data}")
        # [('VNCMDI_Window', '001', 1378852)]
    ...
    def find_windows(self):
        ...
        for i in range(1, 201):
            section_name = f"{i:03}"
            try:
                data = self.ini_handler.get_section_items(section_name)
                ip = data["vnc_ip"]
                port = int(data["vnc_port"])
                win_num = data["vnc_window"]
                numeric_id = generate_uuid_numeric_id()
                targets.append((ip, port, win_num, numeric_id))
            except Exception as e:
                pass

        for ip, port, vnc_num, unique_id in targets:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(0.1)  # 可选：设置连接超时时间为 2 秒
            result = sock.connect_ex((ip, port))
            if result == 0:
                successful_connections.append((f"{ip}:{port}", vnc_num, unique_id))
            else:
