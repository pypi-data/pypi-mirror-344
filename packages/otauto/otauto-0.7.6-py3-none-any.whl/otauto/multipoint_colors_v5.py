...
def is_color_within_tolerance(color1, color2, tolerance):
    """判断两个颜色是否在允许的偏差范围内"""
    return cp.all(cp.abs(color1 - color2) <= tolerance)


# 优化：使用 CuPy 的布尔索引提前过滤主颜色
def filter_main_color(image_array, main_rgb, tolerance):
    """使用布尔索引提前过滤出与主颜色接近的像素"""
    # 计算每个像素与主颜色的差异
    diff = cp.abs(image_array - main_rgb)
...
def find_points_with_tolerance(image_array, colors, relative_positions, tolerance=10, offset_x=0, offset_y=0):
    ...
    main_rgb = cp.array(get_rgb_from_hex(main_color))
    ...
    for y, x in main_color_candidates:
        if is_surrounding_colors_match(image_array, x, y, relative_positions, tolerance):
            matching_positions.append((x + offset_x, y + offset_y))
...
def is_surrounding_colors_match(image_array, x, y, relative_positions, tolerance):
    ...
    for color, (dx, dy) in relative_positions.items():
        ...
        expected_rgb = cp.array(get_rgb_from_hex(color))
...
def multipoint_colors(image_array, colors, tolerance=10, x=0, y=0):
    """主函数：在图像中查找符合条件的颜色点"""
...
if __name__ == "__main__":
    ...
    image_array_gpu = cp.array(image_array)

    # 调用函数查找匹配的坐标
    res = multipoint_colors(image_array_gpu, colors, tolerance=10, x=0, y=0)

    # 将结果从 GPU 移回 CPU 并打印结果
