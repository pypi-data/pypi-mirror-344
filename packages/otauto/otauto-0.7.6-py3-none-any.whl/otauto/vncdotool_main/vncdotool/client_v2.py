...
class VNCDoToolClient(rfb.RFBClient):
    ...
    def captureScreenAsArray(self) -> np.ndarray:
        """Capture the current display and return it as a NumPy array."""
        log.debug("captureScreenAsArray")
        d = self.refreshScreen()
        d.addCallback(self._captureSaveAsArray)
        return d

    def captureRegionAsArray(self, x: int, y: int, w: int, h: int) -> np.ndarray:
        """Capture a region of the current display and return it as a NumPy array."""
        log.debug("captureRegionAsArray (%d, %d, %d, %d)", x, y, w, h)
        d = self.refreshScreen()
    ...
    def _captureSaveRegionAsArray(self, data: object, x: int, y: int, w: int, h: int) -> np.ndarray:
        ...
        log.debug("captureSaveRegionAsArray (%d, %d, %d, %d)", x, y, w, h)
        assert self.screen is not None
