...
def template_match_area(np_img, template_params,area_tupe:(), debug=False):
    """
    区域判断
    np_img : numpy数组 rgb格式
    template_params : 图片参数
    area_tupe : 截图参数,元组(0,0,100,100)
    debug : 是否保存图片
    """
    img = cv2.cvtColor(np_img, cv2.COLOR_RGB2BGR) # 转成OpenCV BGR格式
    x1_area, y1_area, x2_area, y2_area = area_tupe # 截图区域
    ...
    for param in template_params.values():
        if param.get("enable", True) is False:
            continue
        x1_par,y1_par,x2_par,y2_par=param['scope']
        scope_tupe=(x1_par-x1_area,y1_par-y1_area,x2_par-x1_area,y2_par-y1_area)
        logger.info(f"相对区域:{scope_tupe}")

        rois.append(scope_tupe)
    ...
    for i, (box, score, tid) in enumerate(zip(boxes, scores, template_ids)):
        ...
        center_x = int((x1 + x2) / 2) + offset[0]
        center_y = int((y1 + y2) / 2) + offset[1]
        res_dict[path]["boxes"].append((center_x+x1_area, center_y+y1_area))
        res_dict[path]["scores"].append(score)
    return res_dict
