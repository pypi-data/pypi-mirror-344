...
def match_features(image_path1, image_path2, match_threshold=0.75, debug=False):
    ...
    # 加载图片
    img1 = cv2.imdecode(np.fromfile(image_path1, dtype=np.uint8), cv2.IMREAD_GRAYSCALE)
    img2 = cv2.imdecode(np.fromfile(image_path2, dtype=np.uint8), cv2.IMREAD_GRAYSCALE)
