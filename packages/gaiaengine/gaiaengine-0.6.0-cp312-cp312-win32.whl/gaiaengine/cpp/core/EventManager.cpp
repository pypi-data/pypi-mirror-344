...
void EventManager::processEvent(std::shared_ptr<Window> window, SDL_Event& event) {
  ...
  if (event.type == SDL_FINGERDOWN)
    _lastTouchID = event.tfinger.touchId;
...
void EventManager::pythonBindings(py::module& m) {
  py::class_<EventManager, std::shared_ptr<EventManager>, Component, ManagerBase>(m, "EventManager", py::multiple_inheritance())
    ...
    .def_readwrite("lastTouchID", &EventManager::_lastTouchID);
