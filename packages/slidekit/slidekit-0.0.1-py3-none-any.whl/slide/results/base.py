...
class Results:
    ...
    def filter(
        ...
        windows: Optional[Union[List[int], range]] = None,
    ) -> "Results":
        """Filter the result DataFrame by p-value, FDR, and optional window indices.

        Args:
            max_p_value (float): Maximum p-value threshold. Defaults to 1.0.
            ...
            windows (Optional[Union[List[int], range]]): Restrict to these windows. Defaults to None.
        ...
        self._clusters = None  # Reset clusters when filter is called
    ...
    def describe(self) -> None:
        ...
        log_describe("Results Summary", self.summary(), key_width=25)
    ...
    def label_df(self) -> pd.DataFrame:
        ...
        """
        if self._filtered_entries is None:
            raise RuntimeError("You must call `.filter()` before accessing `label_df`.")
...
class ResultStore:
    ...
    def get_filtered_arrays(
        ...
        Args:
            ...
            windows (Optional[set]): Set of window indices to restrict results to.
        ...
        if windows is not None:
            valid = np.isin(self.windows, list(windows))
            mask[~valid, :] = False
...
class FilteredEntries:
    ...
    def df(self) -> pd.DataFrame:
        ...
        for (
            ...
            windows=(set(self.filter_state["windows"]) if self.filter_state["windows"] else None),
        ):
        ...
        if dfs:
            ...
            self._cached_df[cols[1]] = self._cached_df[cols[1]].astype(np.int32)
            self._cached_df[cols[2]] = self._cached_df[cols[2]].astype(np.int32)
            self._cached_df[cols[3]] = self._cached_df[cols[3]].astype(np.int32)
            for num_col in cols[4:]:
                # downcast floats to 32-bit
                self._cached_df[num_col] = pd.to_numeric(self._cached_df[num_col], downcast="float")
        else:
