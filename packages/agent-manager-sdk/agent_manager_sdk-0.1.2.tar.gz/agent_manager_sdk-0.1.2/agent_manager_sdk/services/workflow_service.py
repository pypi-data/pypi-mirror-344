...
class WorkflowService:
    ...
    def get_draft_workflow(self, workflow_id: int) -> Optional[Workflow]:
        """
        Get draft workflow
        """
        # fetch draft workflow by workflow_id
        workflow = db.session.query(Workflow).filter(
    ...
    def sync_draft_workflow(self, 
        ...
        """
        # fetch draft workflow by app_model
        workflow = self.get_draft_workflow(workflow_id=workflow_id)
        ...
        #     features=features
        # )
        user = db.session.query(Account).filter(Account.user_id == user_id).first()
        if not user:
            db.session.add(Account(user_id=user_id))
            db.session.commit()
        ...
        if not workflow:
            workflow = Workflow(
                ...
                id=str(uuid.uuid4()) if not workflow_id else workflow_id,
                user_id=user_id,
                version='draft',
                graph=json.dumps(graph),
                output_keys=json.dumps(output_keys) if output_keys else None,
        ...
        # update draft workflow if found
            ...
            workflow.output_keys = output_keys
