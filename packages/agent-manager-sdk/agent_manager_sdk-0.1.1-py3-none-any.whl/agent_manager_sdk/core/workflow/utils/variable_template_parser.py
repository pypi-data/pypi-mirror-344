...
def populate_fstring(template: str, variables: dict) -> str:
    # Regular expression to find placeholders in the template
    placeholders = re.findall(r'{(.*?)}', template)
    
    # Check if all placeholders have corresponding keys in the variables dictionary
    for placeholder in placeholders:
        try:
            get_nested_value(variables, placeholder)
        except KeyError:
            raise KeyError(f"Missing key for placeholder: {placeholder}")
    
    # Populate the template with values from the dictionary
    populated_template = template
    for placeholder in placeholders:
        value = get_nested_value(variables, placeholder)
        populated_template = populated_template.replace(f'{{{placeholder}}}', str(value))
