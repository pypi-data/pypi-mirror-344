...
from .....core.model_runtimes.openai.base import OpenAIModelRuntimeSingleton
...
class LLMNode(BaseNode):
    ...
    def _run(self, variable_pool: VariablePool) -> NodeRunResult:
        ...
        try:
            ...
            # prompt_messages = self._fetch_prompt_messages(
            #     node_data=node_data,
            ...
            #     stop=stop
            # )
            # prompt_messages = [{'role': msg.role, 'content':msg.content} for msg in prompt_messages]
            
            vars = {**variable_pool.user_inputs, **variable_pool.outputs}
            
            system = populate_fstring(node_data.system, variable_pool.system_variables)
            user = populate_fstring(node_data.user, vars)
            assistant = populate_fstring(node_data.assistant, vars)
        ...
        output_key = 'text' if not node_data.output_key else node_data.output_key
    ...
    def _invoke_llm(self, 
        ...
        invoke_result = OpenAIModelRuntimeSingleton().run(messages=prompt_messages)
        # invoke_result = "TEST OUTPUT"
