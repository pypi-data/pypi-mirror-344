...
from ...core.workflow.nodes.image_gen.image_gen_node import (
    ImageGenNode,
)
...
# )
from ...core.workflow.nodes.workflow_node.workflow_node import WorkflowNode
from ...core.workflow.utils.variable_template_parser import get_nested_value
...
class WorkflowEngineManager:
    ...
    def _run_workflow(self, workflow: Workflow,
        ...
        """
        graph = workflow.graph_dict
        print(f'found graph {workflow.graph_dict}')
        outputs = {}
        try:
            predecessor_node: BaseNode = None
            current_iteration_node: BaseIterationNode = None
            has_entry_node = False
            max_execution_steps = current_app.config.get("WORKFLOW_MAX_EXECUTION_STEPS", 4)
            max_execution_time = current_app.config.get("WORKFLOW_MAX_EXECUTION_TIME", 4)
            while True:
                ...
                print(f'Running Workflow Node {next_node}')
                node_run_result = self._run_workflow_node(
                    workflow_run_state=workflow_run_state,
                    node=next_node,
                    predecessor_node=predecessor_node,
                    # callbacks=callbacks
                )
                outputs.update(node_run_result.outputs)
                workflow_run_state.variable_pool.outputs.update(node_run_result.outputs)
                print(f'Finished Running Workflow Node {next_node}')
        ...
        print('Output_keys:', eval(workflow.output_keys.replace('\n', '')))
        try:
            if workflow.output_keys:
                output_keys = eval(workflow.output_keys.replace('\n', '')) # convert string to list
                if isinstance(output_keys, str):
                    output_keys = eval(output_keys)
                # output_keys.append('text')
                out = {}
                for key in output_keys:
                    try:
                        out.update(_extract_nested_dict(key, outputs))
                        # out[key] = get_nested_value(outputs, key)
                    except KeyError:
                        raise ValueError(f'Output key {key} not found in outputs')
                outputs = out
        except Exception as e:
            raise e
    ...
    def _get_next_overall_node(self, workflow_run_state: WorkflowRunState,
        ...
        if not predecessor_node:
            print(f'Predecessor Node {predecessor_node}')
            for node_config in nodes:
                ...
                if start_at:
                    ...
                    if node_config.get('data', {}).get('type', '') == NodeType.START.value:
                        print(f'Found Start Node {node_config.get("data", {}).get("type")}')
                        node_cls = StartNode
                if node_cls:
                    print(f'Node Class {node_cls}')
                    node = node_cls(
    ...
    def _run_workflow_node(self, workflow_run_state: WorkflowRunState,
        ...
        try:
            ...
            print(f'Node Run Result {node_run_result}')
        except GenerateTaskStoppedException as e:
...
def _extract_nested_dict(path: str, dictionary: dict) -> dict:
    keys = path.split('.')
    result = current = {}
    ...
    current[keys[-1]] = get_nested_value(dictionary, path)
