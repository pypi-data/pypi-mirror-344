...
class ParameterExtractorNode(LLMNode):
    ...
    def _run(self, variable_pool: VariablePool) -> NodeRunResult:
        ...
        prompt_messages = self._generate_prompt_engineering_prompt(node_data, query, variable_pool, 
                                                                #    model_config, memory
                                                                   )
        prompt_message_tools = []
        process_data = {
        ...
        if not result:
            result = self._generate_default_result(node_data)
            error = "Failed to extract result from text response, using empty result."
    ...
    def _invoke_llm(self, node_data_model: ModelConfig,
        ...
        invoke_result = AzureModelRuntimeSingleton().invoke_llm(
            prompt_messages=prompt_messages,
            # model_parameters=node_data_model.completion_params,
            # tools=tools,
            # stop=stop,
    ...
    def _generate_prompt_engineering_chat_prompt(self,
        ...
        """
        prompt_transform = AdvancedPromptTransform(with_variable_tmpl=True)
        # rest_token = self._calculate_rest_token(node_data, query, variable_pool, 
        #                                         model_config, 
        #                                         '')
