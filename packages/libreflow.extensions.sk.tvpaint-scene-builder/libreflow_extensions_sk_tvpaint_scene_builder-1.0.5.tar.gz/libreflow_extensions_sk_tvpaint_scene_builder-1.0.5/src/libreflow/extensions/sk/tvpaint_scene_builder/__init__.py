...
class CreateTvPaintFile(flow.Action):
    ...
    def get_source_path(self, task_name):
        ...
        if task_name == "storyboard" :
            if not self.animatic_task.files.has_file('animatic', 'mp4'):
                return None
            f = self.animatic_task.files['animatic_mp4']
    ...
    def run(self, button):
        ...
        if sources_ver is None :
            sources_ver = {}
        sources_ver.update({default_file.name() : [layout_source_name, color_source_name]})
...
class OpenWithTvPaintAction(OpenWithAction):
    ...
    def execute_update_tvpaint_script(self, layout_path, color_path):
    ...
    def run(self, button):
        ...
        if button == 'Update':
            ...
            for sp in (
                ...
                if (
                    sp["is_running"]
                    and sp["label"] == "Update TvPaint Project"
                ):
                    while sp["is_running"]:
                        time.sleep(1)
            ...
            if f.get_working_copy() is not None:
                ...
                if to_update["bg_color"]:
                    ...
                    sources_ver[self._file.name()][0] = color_source_name
