...
class PlaywrightTool(BaseTool):
    ...
    @tool_method(description="填写信息流栏目表单",prompt="信息流ID、栏目标题、栏目描述、模板名称、发布开始时间、发布结束时间必须由用户提供")
    async def fill_column_form(
        ...
        Args:
            ...
            hide_title: 是否隐藏标题
            enable_sub: 二级栏目 是或否 默认为否
            img_title: 图片标题 是或否 默认为否
            is_recommend: 使用推荐算法 是或否 默认为否
            sort_number: 栏目序号 
            is_top: 置顶 是或否 默认为否
            publish_start_time: 发布开始时间 格式: YYYY-MM-DD HH:mm:ss
            publish_end_time: 发布结束时间 格式: YYYY-MM-DD HH:mm:ss
            
        Returns:
            表单填写结果
            注意：该工具的必填参数必须从对话中获取，不允许自动生成，且该工具调用完成后，需
        """
        ...
        with RequestTimer(f"fill_column_form - {title}"):
            try:
                ...
                try:
                    ...
                    publish_end_time = (now + timedelta(days=30)).strftime("%Y-%m-%d %H:%M:%S")
                ...
                result = {
                    "success": True,
                    "message": "信息流栏目配置填写完成，你必须暂定，提示用户输入select_column_resource工具所所需要的必填参数",
                }
    ...
    @tool_method(description="保存栏目",prompt="保存类型必须由用户提供")
    ...
    async def login_to_system(
        ...
        try:
            ...
            if login_success:
                ...
                # 导航到新增信息流页面
                await self.navigate_after_login(module="查询信息流列表")
    ...
    async def navigate_after_login(
        ...
        Args:
            target_url: 模块名称，例如新增信息流 新增栏目 查询信息流列表，信息流首页，默认为新增信息流
        ...
        try:
            ...
            # 尝试导航到目标页面            
            logger.info(f"登录后导航到目标页面: {target_url}")
    ...
    @tool_method(description="创建信息流",prompt="信息流标题、频道、下发渠道必须由用户提供")
    ...
    @tool_method(description="保存信息流",prompt="提交动作必须由用户提供")
    ...
    @tool_method(description="为栏目选择资源",prompt="频道类型、媒资形式、搜索关键字、搜索关键字信息、资源类型必须由用户提供，且调用完该工具时，必须停止，等待用户下一步指令")
        ...
        try:
            ...
            try:
                ...
                if has_results:
                    ...
                    return {
                        "success": True,
                        "message": "资源选择成功，请询问用户下一次调用select_column_resource工具所需要的必填参数，方便进行下一个资源的填充",
                    }
                else:
    ...
    @tool_method(description="查看信息流列表页")
    async def view_flow_detail(self) -> None:
        """查看信息流列表页
        ...
        # 确保浏览器已启动
                ...
                # 导航到信息流编辑页面
                column_url = f"https://idsaas-qcl.test.leiniao.com/page/cms-lite-launcher/#/page/infoFlow"
                logger.info(f"导航到栏目编辑页 {column_url}")
