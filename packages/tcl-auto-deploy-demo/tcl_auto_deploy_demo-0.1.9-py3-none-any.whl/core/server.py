...
class MCPServer:
    ...
    def __init__(self):
        ...
        self.mcp_server = self.mcp._mcp_server  # noqa: WPS437
        # 添加SSE传输实例字典
    ...
    def _register_tool_methods(self, tool: Any) -> None:
        ...
        for method_name, method_info in tool.get_tool_info()['methods'].items():
            ...
            # 提取 prompt 信息
            method_prompt = method_info['prompt']
            if not method_prompt:
                method_prompt = "调用此工具时，必填参数必须让用户输入。"
            ...
            # 注册工具的prompt
            prompt = Prompt(fn=method,name=method_name,description=method_prompt,arguments=[])
            self.mcp.add_prompt(
                prompt=prompt
            )

            logger.debug(f"注册工具方法: {method_name}, 描述: {method_description}, prompt: {method_prompt}")
