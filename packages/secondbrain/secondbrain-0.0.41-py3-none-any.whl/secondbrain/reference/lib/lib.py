...
def wrapped_func(func, module_path, execution_path_type=None, ui_datas=None):
    def _wrapped(*args, **kwargs):
        old_ui_datas = utils.params["uiDatas"]
        if ui_datas is not None:
            params = utils.params
            params["uiDatas"] = ui_datas
            # params["uiDatas"] = current_node["data"]["uiDatas"]
            # # py_params_file = os.path.join(self.path, "params_" + str(uuid.uuid4()) + ".json")
            # # with open(self.py_params_file, "w", encoding="utf-8") as f:
            # #     json.dump(params, f)
            
        if execution_path_type == 'contextFolder':
            try:
