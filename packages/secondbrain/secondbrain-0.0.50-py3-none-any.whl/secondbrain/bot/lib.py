...
def ref_bot_cached(item_id, config=None, workspace_path=None):
    return ref_bot(item_id, config, workspace_path)


@lru_cache(maxsize=128)
def ref_mcp_cached(item_id, config=None, workspace_path=None):
    return ref_mcp(item_id, config, workspace_path)

@lru_cache(maxsize=128)
def ref_tools_cached(item_id, config=None, working_directory=None, workspace_path=None):
    return ref_tools(item_id, config, working_directory, workspace_path)

@lru_cache(maxsize=128)
def ref_workflow_cached(item_id, config=None, workspace_path=None):
    return ref_workflow(item_id, config, workspace_path)


def get_abilities(cwd, bot_setting):
    config = bot_setting["abilitiesConfig"]
    abilities = []
    
    bot_config = config["botConfig"]
    for item_id in bot_config:
        item = ref_bot_cached(item_id)
        if item:
            abilities.append(item)
            
    mcp_config = config["mcpConfig"]
    for item_id in mcp_config:
        item = ref_mcp_cached(item_id)
        if item:
            abilities.append(item)
    ...
    for item_id in tool_config:
        ...
        items = ref_tools_cached(item_id, item_config, cwd)
    ...
    for item_id in workflow_config:
        ...
        item = ref_workflow_cached(item_id, item_config)
        if item:
            abilities.append(item)
    return abilities
