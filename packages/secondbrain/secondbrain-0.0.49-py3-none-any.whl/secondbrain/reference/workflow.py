...
@lru_cache()
def _load_workflows(workflow_id, config=None, workspace_path=None):
...
def load_workflow(workflow_id, config=None, workspace_path=None):
    tools = _load_workflows(workflow_id, workspace_path=workspace_path)
    if len(tools) > 0:
        return tools[0]
    else:
        return None
