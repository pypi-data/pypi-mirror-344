...
from secondbrain.reference.bot import load_bot
from secondbrain.reference.mcp import ref_mcp
from secondbrain.reference.tool import load_tools
from secondbrain.reference.workflow import load_workflow
...
def get_abilities(cwd, bot_setting):
    ...
    for bot_id in bot_config:
        bot = load_bot(bot_id)
        if bot:
            abilities.append(bot)
            
    mcp_config = config["mcpConfig"]
    for mcp_id in mcp_config:
        mcp = ref_mcp(mcp_id)
        if mcp:
            abilities.append(mcp)

    tool_config = config["toolConfig"]
    for tool_id in tool_config:
        item_config = tool_config[tool_id]
        abilities.extend(load_tools(tool_id, item_config, cwd))
    
    workflow_config = config["workflowConfig"]
    for workflow_id in workflow_config:
        item_config = workflow_config[workflow_id]
        workflow = load_workflow(workflow_id, item_config)
        if workflow:
