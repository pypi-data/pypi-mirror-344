...
import argparse
import sys
from cms.common import read_and_correct_parquet, fig_add_text, get_df_with_correction
import pandas as pd
...
def ijazz_kin_plot_cmd():
    ...
    parser.add_argument('--HDNA', action='store_true', help="use HDNA mode")
    ...
    ijazz_kin_plot(config, config_plot, args.HDNA)

def ijazz_kin_plot(config: Dict=None, config_plot: Dict=None, HDNA: bool=False):
    ...
    Args:
        ...
        HDNA (bool, optional): load systematics from different folders instead of looking for mass_scale_up and mass_smear_up branches. Defaults to False.
    """
    ...
    df_mc = get_df_with_correction(config['file_mc'], mll_name=mll_name, is_mc=True, do_syst=not HDNA, merge_files=not HDNA)
...
def main():
    ...
    parser.add_argument('--AN', action='store_true', help="use AN mode")
    parser.add_argument('--add_mass', type=str, default='', help='add mass to the mll2 variable')
