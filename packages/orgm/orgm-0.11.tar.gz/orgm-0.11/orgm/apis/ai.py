...
if __name__ == "__main__":
    ...
    print("Cargando variables de entorno para prueba local...")
    load_dotenv(override=True)

    # Example usage:
    test_project_name = "Proyecto de Prueba"
    print(f"Generando descripción para: '{test_project_name}'")
    description = generate_project_description(test_project_name)

    if description:
        console.print(f"\n[bold green]Descripción generada:[/bold green]\n{description}")
    else:
        console.print("\n[bold red]No se pudo generar la descripción.[/bold red]")

    # Example direct call to generate_text
    test_prompt = "Explain the importance of AI in modern software development."
    test_config = "general_explanation"
    print(f"\nGenerando texto para prompt: '{test_prompt}' con config: '{test_config}'")
    generated_text = generate_text(test_prompt, test_config)

    if generated_text:
        console.print(f"\n[bold green]Texto generado:[/bold green]\n{generated_text}")
    else:
        console.print("\n[bold red]No se pudo generar el texto.[/bold red]")
