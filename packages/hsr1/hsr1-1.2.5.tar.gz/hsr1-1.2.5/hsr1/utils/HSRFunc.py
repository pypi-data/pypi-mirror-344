...
def calc_aot_direct(ed, eds, sza, e_solar=None, sed=None, aod_type=["total_od", "aod_microtops", "aod_wood_2017"], et_wavelengths=None):
    ...
    offsets_wl = interpolate.interp1d(wl_e, offset_wl, kind = 'linear', axis = 0, fill_value = "extrapolate")(wl) # linearly extrapolated outside wl range
    slopes_wl = interpolate.interp1d(wl_e, slope_wl, kind = 'linear', axis = 0, fill_value = "extrapolate")(wl)
    ...
    offsets_am = offset_am(airmasses)
    ...
    tau_corr = (tau_a - offsets_am - offsets_wl) *slopes_wl
...
def load_et_spectrum(filepath=None, wavelengths=None):
    ...
    smoothed_e_solar = e_solar.rolling(3, min_periods=1, center=True).mean().T
