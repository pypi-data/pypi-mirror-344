...
@mcp.tool()
def delete_data(sql: str, ack_msg: str) -> list[types.TextContent]:
    """
        执行指定删除数据语句(只执行DELETE语句)，而且DELETE语句必须带WHERE条件。执行前需要用户确认。用户确认后传入的ack_msg将会是"确认"。
        函数输入：
            sql: 删除数据的SQL语句
            ack_msg: 用户确认消息，用户确认后，传入的ack_msg将会是"确认"
        函数输出：
            包含执行删除语句的结果
    """
    if ack_msg != "确认":
