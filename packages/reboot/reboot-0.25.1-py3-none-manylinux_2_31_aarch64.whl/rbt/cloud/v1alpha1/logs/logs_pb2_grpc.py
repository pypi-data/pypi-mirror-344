...
class ApplicationLogsMethodsServicer(object):
    ...
    def RegisterConsensus(self, request, context):
        """*
        ApplicationLogs does not store logs itself; they are stored per-consensus.
        This method registers a consensus, ensuring that logs for that consensus
        will be retrieved when `Logs()` is called.
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')
