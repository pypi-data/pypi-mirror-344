...
from localstack.services.plugins import PLUGIN_NAMESPACE as SERVICE_PROVIDER_NAMESPACE
...
def set_default_providers_to_pro():
	E='pro';from localstack.services.plugins import SERVICE_PLUGINS as A;D=get_licensed_environment()
	if not config_ext.PROVIDER_FORCE_EXPLICIT_LOADING:
		for(B,F)in localstack_config.SERVICE_PROVIDER_CONFIG._provider_config.items():
			G=A.api_provider_specs[B];C=next((A for A in G if A==f"{F}_pro"),None)
			if C and D.has_product_license(f"{SERVICE_PROVIDER_NAMESPACE}/{B}:{C}"):localstack_config.SERVICE_PROVIDER_CONFIG.set_provider(B,C)
	H=[B for B in A.apis_with_provider(E)if localstack_config.SERVICE_PROVIDER_CONFIG.default_value not in A.api_provider_specs[B]or D.has_product_license(f"{SERVICE_PROVIDER_NAMESPACE}/{B}:pro")];localstack_config.SERVICE_PROVIDER_CONFIG.bulk_set_provider_if_not_exists(H,E)
