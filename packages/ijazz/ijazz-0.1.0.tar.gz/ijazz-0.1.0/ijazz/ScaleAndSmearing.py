...
def ijazz_sas(config:dict=None, do_fit=True, save_corrlib=True):
    ...
    - corrlib: configuration for the correction library
    ...
    - save_corrlib: if True, save the correction library
    """
    ...
    if config.get('syst', False):
        ...
        for syst_name, syst_cfg  in config['syst'].items():
            ...
            if config['fitter'].get('double_gaussian', False):
                ...
                syst_dict[syst_name] = str(dir_results / f'SAS{dset_name}_syst-{syst_name}.json')
        ...
        #plot each syst contribution
        figs, _ = ijp.plot_syst_from_jsons(nominal_json, syst_dict, scale_flat_syst=scale_flat_syst, fit_2g=fit_2g_json, #nominal_syst=nominal_syst_json,
                                               jsons_mode='compare', param_to_plot='resp', resp_range=[0.0,0.01],reso_range=[0.0,0.01], 
                                               cat_latex=cat_latex, hide_legend=False, leg_ncol=3, leg_fontsize='small', leg_title_fontsize='medium')
        for ifig, fig in enumerate(figs):
            fig_name = dir_results / f'SAS{dset_name}_syst-contribution-resp.jpg'
            print(f" --> Plot Nominal with Syst : {fig_name.with_suffix(f'.fig{ifig}.jpg')}")
            fig.savefig(fig_name.with_suffix(f'.fig{ifig}.jpg'))
        
        figs, _ = ijp.plot_syst_from_jsons(nominal_json, syst_dict, scale_flat_syst=scale_flat_syst, fit_2g=fit_2g_json, #nominal_syst=nominal_syst_json,
                                               jsons_mode='compare', param_to_plot='reso', resp_range=[0.0,0.01],reso_range=[0.0,0.01], 
                                               cat_latex=cat_latex, hide_legend=False, leg_ncol=3, leg_fontsize='small', leg_title_fontsize='medium')
        for ifig, fig in enumerate(figs):
            fig_name = dir_results / f'SAS{dset_name}_syst-contribution-reso.jpg'
            print(f" --> Plot Nominal with Syst : {fig_name.with_suffix(f'.fig{ifig}.jpg')}")
            fig.savefig(fig_name.with_suffix(f'.fig{ifig}.jpg'))
