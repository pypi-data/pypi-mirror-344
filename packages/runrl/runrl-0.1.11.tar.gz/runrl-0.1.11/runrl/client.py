...
DEFAULT_BASE_URL = "https://rrluc.com"
...
class RunRL:
    ...
    def __init__(self, api_key: str, base_url: str = DEFAULT_BASE_URL):
        ...
        self.websocket_base_url = base_url.replace("http", "ws", 1)
    ...
    def list_runs(self) -> List[Dict[str, Any]]:
        ...
        # Expect response_data to be a dict like {"runs": [...] }
        # or potentially just the list [...] in some cases? Using .get handles both.
        # If it's a dict, get "runs"; if key missing or not a dict, default to empty list.
        # If it's already a list, .get will fail (AttributeError), so add fallback.
        try:
            runs_list = response_data.get("runs", [])
        except AttributeError:
             # If response_data doesn't have .get (e.g., it's already a list), use it directly
             # or default to empty list if it's None or something else unexpected.
             runs_list = response_data if isinstance(response_data, list) else []


        # Process the extracted/provided list to remove verbose fields
        processed_runs = []
        # Ensure runs_list is actually a list before iterating
        if isinstance(runs_list, list):
            # Keep debug prints for now, can be removed later
            print(f"DEBUG: Processing {len(runs_list)} runs...") # Debug print
            for i, run in enumerate(runs_list):
                ...
                print(f"DEBUG: Run {i} - Keys before copy: {list(run.keys())}") # Debug print
                processed_run = run.copy()
                print(f"DEBUG: Run {i} - 'metrics' in copy before pop: {'metrics' in processed_run}") # Debug print
                metrics_val = processed_run.pop('metrics', '__KEY_NOT_FOUND__') # Remove metrics if exists
                print(f"DEBUG: Run {i} - 'metrics' pop result: {type(metrics_val)}") # Debug print
                print(f"DEBUG: Run {i} - 'metrics' in copy AFTER pop: {'metrics' in processed_run}") # Debug print

                processed_run.pop('logs', None)    # Remove logs if exists
                processed_runs.append(processed_run)
                print(f"DEBUG: Run {i} - Keys in final appended dict: {list(processed_runs[-1].keys())}") # Debug print
        else:
            # This case should be less likely now with the .get() and fallback logic,
            # but keep a warning just in case response_data was truly weird.
            print(f"Warning: Could not extract a list of runs from response: {type(response_data)}")
            # Return empty list as per type hint
            return []
