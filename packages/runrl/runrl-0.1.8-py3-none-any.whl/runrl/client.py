...
class RunRL:
    ...
    def list_runs(self) -> List[Dict[str, Any]]:
        ...
        runs_data = self._handle_response(response)
        
        # Process the data to remove verbose fields before returning
        processed_runs = []
        if isinstance(runs_data, list):
            print(f"DEBUG: Processing {len(runs_data)} runs...") # Debug print
            ...
            # Handle unexpected response format (e.g., error dict)
             print(f"Warning: Unexpected response format from /runs: {type(runs_data)}")
             return runs_data # Return original data if not a list
    ...
    def generate_api_key(self, name: str) -> Dict[str, Any]:
        response = self._request("POST", f"/api-keys/{name}")
        return self._handle_response(response)
    ...
    def delete_api_key(self, name: str) -> Dict[str, Any]:
        response = self._request("DELETE", f"/api-keys/{name}")
        return self._handle_response(response) 