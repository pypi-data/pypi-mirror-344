...
def compute_risk_table(prices: pd.DataFrame,
    ...
    for asset in sampled_prices:
        ...
        if len(sampled_price.index) > 1:
            ...
            max_dd, last_dd = compute_max_last_drawdown(prices=dd_sampled_prices[asset].dropna())
            rel_returns = sampled_price.pct_change().dropna()
            asset_dict.update({
                PerfStat.MAX_DD.to_str(): max_dd,
                PerfStat.CURRENT_DD.to_str(): last_dd,
...
def compute_max_last_drawdown(prices: Union[pd.DataFrame, pd.Series]) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    if isinstance(prices, pd.DataFrame):
        last_dds = max_dd_data.iloc[-1, :].to_numpy()
    else:
        last_dds = max_dd_data.iloc[-1]
    return max_dds, last_dds
