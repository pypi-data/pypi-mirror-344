...
class MooredataDet:
    ...
    def __init__(self, nuscenes_root: str, output_dir: str, oss_root: str, predata: bool = False, json_file_name: str = None, lidar_name: str = None):
        """
        初始化NuScenes数据集转换工具
        ...
        for token in sample_tokens:
            ...
            if frame_data:
                sorted_images = sorted(
                    frame_data,
                    key=lambda x: camera_order.get(camera_to_calibrated.get(x['calibrated_sensor_token'], ''), float('inf'))
                )
