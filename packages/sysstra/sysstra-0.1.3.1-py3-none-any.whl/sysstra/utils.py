...
def fetch_available_funds(credential_id):
    ...
    try:
        ...
        response = requests.get(config.orders_url+"get_broker_fund?credential_id={}".format(credential_id))
...
def send_order_alert(alert_dict):
    ...
    try:
        print("Sending Alert")
        response = requests.post(url=config.orders_url+"send_notification", json=alert_dict)
...
def check_order_status(credential_id, order_id, exchange):
    ...
    try:
        ...
        last_order = requests.post(url=config.orders_url+"get_order_by_id", params={"credential_id": credential_id, 'order_details': json.dumps(get_order_params)}).json()
