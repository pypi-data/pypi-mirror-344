...
def download_tiktok_video(
    ...
    not provided). If 'output_filename' is not provided,
    the file is returned as a file in memory (BytesIO).
    """
    if not PythonValidator.is_string(url):
        raise Exception('The provided "url" parameter is not a string.')
    ...
    filename = FileWriter.write_binary_file(
    ...
    # TODO: This below is old, I want to download and return it
    # always, at least by now
    return (
        file_in_memory
        if output_filename is False else
        FileWriter.write_binary_file(
            file_in_memory.getvalue(),
            Output.get_filename(output_filename, FileTypeX.VIDEO)
        )
    )
