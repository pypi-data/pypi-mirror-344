...
class DistancesCoordination(Validator):
    ...
    # This is a combination of get_voronoi_polyhedra and _extract_cell_info 
    # from VoronoiNN in pymatgen
    def get_site_statistics(self, site_idx, struct, sliced_df):
        # Get the coordinates of the central site
        center = struct[site_idx]
        center_coords = center.coords
        center_voro_idx = 0

        neighbors = struct.get_sites_in_sphere(center_coords, r=5)
        neighbors = [ngbr[0] for ngbr in sorted(neighbors, key=lambda s: s[1])]
        #nhbr_idx = [idx for neighbor.idx in sorted()]
        ...
        for nn, vind in voro.ridge_dict.items():
            if center_voro_idx in nn:
                other_site = nn[0] if nn[1] == center_voro_idx else nn[1]
                ...
                true_other_site = neighbors[other_site].index
    ...
    def get_coordination(self, move_indices, voro, sliced_df, points, struct):
        ...
        for move_index in move_indices:
            ...
            nns = self.get_site_statistics(true_move_index, struct, sliced_df)
            # vnn = VoronoiNN(weight="solid_angle")
            # nns = vnn.get_voronoi_polyhedra(struct, true_move_index) 
