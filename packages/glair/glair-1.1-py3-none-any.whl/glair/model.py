...
def build_sequences(slices, SEQ_LEN):
    X, Y = [], []
...
def hybrid_loss(y_true, y_pred):
    ...
    ssim_l = 1 - tf.reduce_mean(tf.image.ssim(y_true, y_pred, max_val=1.0))
    return mse + 0.1 * ssim_l
...
def train(model, loss_func, EPOCHS, BATCH_SIZE):
    if model=="vae":
        ...
        model.save("glair.keras")
        print("Model saved as: 'glair.keras' to root directory")

def predict(DATA_DIR, SEQ_IN, SEQ_OUT, NUM_PATIENTS, INTERP_STEPS):
