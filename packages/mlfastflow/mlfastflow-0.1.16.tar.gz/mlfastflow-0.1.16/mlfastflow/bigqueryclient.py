...
class BigQueryClient:
    ...
    def run_sql(self, sql=None):
        """
        Run a SQL query without waiting for completion.
        
        Args:
            sql (str): SQL query to execute
            
        Returns:
            str: Job ID that can be used to check status later, or None if there was an error
        """
        if sql is None:
        ...
        try:
            # Create a job configuration that doesn't wait
            job_config = bigquery.QueryJobConfig(
                priority=bigquery.QueryPriority.BATCH  # Use BATCH mode to ensure async behavior
            )
            
            # Prepare the job using the low-level API
            query_job = self.client.create_job(
                job_config.to_api_repr(), 
                bigquery.job.QueryJob,
                job_id_prefix="async_query_",
                location=self.client.location,
                query=sql
            )
            
            # Get the job ID without waiting
            job_id = query_job.job_id
            
            print(f"Query submitted as batch job: {job_id}")
            print(f"You can check the status in the BigQuery console or use check_job_status('{job_id}')")
    ...
    def check_job_status(self, job_id):
        """
        Check the status of a previously submitted BigQuery job.
        
        Args:
            job_id (str): The job ID returned by run_sql
            
        Returns:
            dict: Job information including status, errors if any, and statistics
        """
        try:
            # Get the job by ID
            ...
            # Add statistics if the job is complete
            if job.done():
                result["statistics"] = {
                    ...
                    "total_rows": job.total_rows if hasattr(job, "total_rows") else None,
                }
