...
class BigQueryClient:
    ...
    def run_sql(self, sql: str) -> None:
        """Submits a SQL query to BigQuery to run asynchronously.
        ...
        Raises:
            ...
            Exception: If there's an error submitting the query job.

        Note:
            This method submits the job and does not wait for completion.
            For safety, it prevents DELETE, TRUNCATE TABLE, or DROP TABLE operations.
        """
        if not sql: # More Pythonic check for empty or None string
            raise ValueError("sql must be a non-empty string")

        # Check if SQL contains potentially destructive operations
        sql_upper = sql.upper()
        # Using word boundaries/more specific terms to avoid matching substrings
        # Also check for DROP TABLE
        if ("DELETE " in sql_upper or
            "TRUNCATE TABLE" in sql_upper or # Be more specific for TRUNCATE
            "DROP TABLE" in sql_upper): # Add DROP TABLE check
            print("ERROR: Cannot execute DELETE, TRUNCATE TABLE, or DROP TABLE operations for safety reasons.")
            return

        try:
            print(f"Submitting SQL to BigQuery: {sql[:100]}...") # Log start and beginning of SQL
            # Submit the query job - does not wait for results
            self.client.query(sql)
            print(f"Submission call completed. Job object received: {query_job.job_id}") # Check if this line is reached
            # Log the job ID for reference
            print(f"Query job {query_job.job_id} submitted successfully. It will run in the background.")
        except Exception as e:
            # Provide more context in the error message
            print(f"Error submitting query job: {str(e)}")
