...
import logging
from abstract_utilities.json_utils import try_json_dumps
...
def make_request(url, data=None, headers=None, get_post=None, endpoint=None, status_code=False, retry_after=False, raw_response=False, response_result=None, load_nested_json=True,auth=None):
    ...
    dump_data = try_json_dumps(data)
    if dump_data:
        values = get_values_js(url=url,endpoint=endpoint,data=dump_data,headers=headers)
