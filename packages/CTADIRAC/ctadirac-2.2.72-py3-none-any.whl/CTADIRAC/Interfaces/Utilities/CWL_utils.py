        ...
        if isinstance(input_value, list):
            for file in input_value:
                if isinstance(file, File):
                    original = file.path
                    file.path, lfn = translate_sandboxes_and_lfns(file)
                    if lfn:
                        input_lfns.append(
                            original.replace(LFN_PREFIX, LFN_DIRAC_PREFIX)
                        )
                    else:
                        input_sandboxes.append(file.path)

        elif isinstance(input_value, File):
            path = input_value.path
            input_value.path, lfn = translate_sandboxes_and_lfns(path)
            if lfn:
                input_lfns.append(path.replace(LFN_PREFIX, LFN_DIRAC_PREFIX))
            else:
                input_sandboxes.append(input_value.path)
