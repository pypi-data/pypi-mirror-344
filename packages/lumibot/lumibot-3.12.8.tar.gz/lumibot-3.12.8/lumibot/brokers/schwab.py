...
class Schwab(Broker):
    ...
    def __init__(
        ...
        # ---- Show every candidate env-var at INFO level ----
        logging.info(f"[Schwab] SCHWAB_REDIRECT_URI        = {os.getenv('SCHWAB_REDIRECT_URI')}")
        logging.info(f"[Schwab] REPLIT_APP_URL            = {os.getenv('REPLIT_APP_URL')}")
        logging.info(f"[Schwab] REPLIT_WEBVIEW_URL        = {os.getenv('REPLIT_WEBVIEW_URL')}")
        logging.info(f"[Schwab] REPL_SLUG / REPL_OWNER    = {os.getenv('REPL_SLUG')} / {os.getenv('REPL_OWNER')}")
        logging.info(f"[Schwab] RENDER_EXTERNAL_HOSTNAME  = {os.getenv('RENDER_EXTERNAL_HOSTNAME')}")

        # Order of precedence for redirect URI
        redirect_uri = (
            os.getenv("SCHWAB_REDIRECT_URI")                                               # manual override
            or (os.getenv("REPLIT_APP_URL")      and os.getenv("REPLIT_APP_URL") + "/schwab-login")
            or (os.getenv("REPLIT_WEBVIEW_URL")  and os.getenv("REPLIT_WEBVIEW_URL") + "/schwab-login")
            or (os.getenv("REPL_SLUG") and os.getenv("REPL_OWNER") and                       # slug.fqdn fallback
                f"https://{os.getenv('REPL_SLUG')}.{os.getenv('REPL_OWNER')}.repl.co/schwab-login")
            or (os.getenv("RENDER_EXTERNAL_HOSTNAME") and                                   # Render
                f"https://{os.getenv('RENDER_EXTERNAL_HOSTNAME')}/schwab-login")
            or "https://127.0.0.1:8182"                                                     # localhost default
        )
        logging.info(f"[Schwab] Using redirect_uri: {redirect_uri}")

        # Get the current folder for token path
        current_folder = os.path.dirname(os.path.realpath(__file__))
        token_path = os.path.join(current_folder, 'token.json')
        try:
            # Ensure cloud login helper runs before easy_client
            self._ensure_cloud_login(redirect_uri, token_path, api_key, secret)
            # ⏳ Block until the user finishes OAuth (no arbitrary timeout—just a log)
            while not os.path.exists(token_path):
                logging.info("[Schwab] Waiting for OAuth completion… "
                             "open the link above, sign in, allow access.")
                time.sleep(5)
            # Now build the client – safe from the 127.0.0.1 fallback
    ...
    def _ensure_cloud_login(self, redirect_uri, token_path, api_key, secret):
        # Skip if token exists or we’re on localhost
        if os.path.exists(token_path) or "127.0.0.1" in redirect_uri:
            return
        ...
        @app.route("/")
        def root():
            return ("<h3>Schwab login helper</h3>"
                    "<p>Go to <a href='/schwab-login'>/schwab-login</a> "
                    "to start the authorization flow.</p>")
        ...
        def schwab_login():
            ...
            if not code:
                auth_url = (
                    ...
                    + urlencode({
                        ...
                        "state":        os.urandom(6).hex(),
                    })
                )
                # Pop out of iframe: open Schwab in new tab, fallback link for popup blockers
                return (
                    "<html><body>"
                    "<script>"
                    ...
                    "</script>"
                    "<p>Opening Schwab login… If nothing happens, "
                    f"<a href='{auth_url}' target='_blank'>click here</a>.</p>"
                    "</body></html>"
                )
            cli = Client(api_key, secret)
            ...
            with open(token_path, "w") as f:
                json.dump(token, f)
            logging.info("[Schwab] token.json saved — restarting bot now.")
            import os as _os
            _os._exit(0)  # force immediate restart on Replit/Render
            return "✅ Schwab token saved. Restart the bot; you’re all set."
        ...
        logging.info(colored(
            f"[Schwab] First-time setup → open {redirect_uri} in your browser, "
            "sign in once, then restart the bot.", "green"))
