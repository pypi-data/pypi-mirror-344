...
class Schwab(Broker):
    ...
    def __init__(
        ...
        redirect_uri = (
            ...
            or (os.getenv("REPLIT_APP_URL") + "/schwab-login"
                if os.getenv("REPLIT_APP_URL") else None)
            or (os.getenv("REPLIT_WEBVIEW_URL") + "/schwab-login"
                if os.getenv("REPLIT_WEBVIEW_URL") else None)
            or (f"https://{os.getenv('RENDER_EXTERNAL_HOSTNAME')}/schwab-login"
                if os.getenv("RENDER_EXTERNAL_HOSTNAME") else None)
            or "https://127.0.0.1:8182"
        )
        logging.info(colored(f"[Schwab] Using redirect_uri: {redirect_uri}", "cyan"))
        ...
        try:
            ...
            # 2️⃣ WAIT until it’s created  (max 300 s)
            for _ in range(300):
