...
class Schwab(Broker):
    ...
    def __init__(
        ...
        # Smart redirect URI detection (final version)
        redirect_uri = (
            os.getenv("SCHWAB_REDIRECT_URI") or
            os.getenv("REPLIT_APP_URL") or
            os.getenv("REPLIT_WEBVIEW_URL") or
            (f"https://{os.getenv('RENDER_EXTERNAL_HOSTNAME')}/schwab-login"
                if os.getenv('RENDER_EXTERNAL_HOSTNAME') else None) or
        ...
        try:
            # 1️⃣ ensure token exists (runs /schwab-login once if needed)
            self._ensure_cloud_login(redirect_uri, token_path, api_key, secret)
            # 2️⃣ WAIT until it’s created  (max 120 s)
            for _ in range(120):
                if os.path.exists(token_path):
                    break
                time.sleep(1)
            # 3️⃣ NOW build the client – safe from the 127.0.0.1 fallback
    ...
    def _ensure_cloud_login(self, redirect_uri: str, token_path: str, api_key: str, secret: str):
        """Create /schwab-login only for cloud URLs (not 127.0.0.1) and ensure token.json exists before continuing."""
        # Skip if token.json already exists OR we’re on localhost
        if os.path.exists(token_path) or "127.0.0.1" in redirect_uri:
            return

        app = Flask("schwab-login")

        # 1️⃣ First visit – redirect user to Schwab
        ...
        def initiate():
            ...
            return (
                ...
                f'Or copy this URL into your browser:<br>{auth_url}'
            )

        # 2️⃣ Schwab redirects back with ?code=...
        @app.route("/schwab-login")
        def finish():
            from schwab.client import Client
        ...
        logging.info(
            colored(f"[Schwab] First-time setup: open {redirect_uri} in your browser, "
                    "complete login, then restart the bot.", "green"))
