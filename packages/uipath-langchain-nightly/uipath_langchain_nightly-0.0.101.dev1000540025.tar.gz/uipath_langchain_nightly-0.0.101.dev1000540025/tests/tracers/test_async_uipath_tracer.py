...
class TestAsyncUiPathTracer:
    ...
    async def test_send_span(self, tracer):
        ...
        assert span_data["startTime"] == "2023-01-01T00:00:00"
        assert span_data["endTime"] == "2023-01-01T00:01:00"
    ...
    async def test_worker_functionality(self, mock_client, mock_response, mock_context):
        ...
        with patch("asyncio.create_task"):
            with patch.dict("os.environ", {"UIPATH_ACCESS_TOKEN": "test-token"}):
                ...
                tracer = AsyncUiPathTracer(context=mock_context, client=mock_client)
                tracer.running = False  # Set running to False to ensure worker exits

                # Configure test data
                expected_url = "https://test-url.com/api/spans"
                tracer._build_url = MagicMock(return_value=expected_url)
