...
class KafkaBaseSettings(BaseModel):
    ...
    Attributes:
        ...
        schema_registry_auto_register_schemas (bool): Auto-register new schemas flag.
        schema_registry_request_timeout_ms (int): Timeout for registry API requests (ms).
    ...
    def _build_config(self, for_schema_registry: bool = False) -> dict[str, Any]:
        ...
        for name, value in custom_props.items():
            if name.startswith("schema_registry") == for_schema_registry and name not in self.__class__.model_fields:
                if for_schema_registry:
                    name = name[:15]
                config[name.replace("_", ".")] = value
