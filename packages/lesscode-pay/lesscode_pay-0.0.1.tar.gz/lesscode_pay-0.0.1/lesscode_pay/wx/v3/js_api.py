...
class JsApi(CommonApi):
    ...
    def create_combine_order(self, combine_appid: str, combine_out_trade_no: str, sub_orders: list,
        ...
        """
            【jsapi合单下单】
            用户在商户前端选择微信支付后，商户需调用该接口在微信支付下单，生成用于调起支付的预支付交易会话标识(prepay_id)。
            注意
        ...
        """
        uri = "/v3/combine-transactions/jsapi"
        url = f"{self.base_url}{uri}"
