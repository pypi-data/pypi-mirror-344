...
class Grid:
    ...
    @staticmethod
    def get_attribute_values_by_coords(grid_data: np.ndarray, grid_xx: np.ndarray, grid_yy: np.ndarray, coords: np.ndarray)-> np.ndarray:
        """
        ...
        assert grid_data.ndim == 1 or grid_data.ndim == 2, "grid_data: Ожидался 1D или 2D массив"
        assert grid_xx.ndim == 2, "grid_xx: Ожидался 2D массив"
        assert grid_xx.shape == grid_yy.shape, "grid_xx, grid_yy: Ожидались массивы одинакового размера"
        assert coords.ndim == 1 and len(coords) == 2, "coords: Ожидался 1D массив длины 2"
        assert grid_data.shape[0] == grid_xx.shape[0] * grid_xx.shape[1], "grid_data: Ожидался массив длины grid_xx.shape[0] * grid_xx.shape[1]"
