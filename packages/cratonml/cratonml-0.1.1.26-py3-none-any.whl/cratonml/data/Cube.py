...
class Cube:
    ...
    def read(self,
             ...
             end_crossline_idx: int) -> datacontainers.CubeDataSlice:
        """
        ...
        cube = self.connection.get_cube_data_range(cube_id=cube_id,
    ...
    def get_info(self) -> dict:
        ...
        for cube in cube_list:
            ...
            horizons_dict = {'horizons': list(self.connection.get_cube_horizons(cube.cube_id))}
            transforms_dict = {'transform_matrix': self.connection.get_transform_matrix(cube.cube_id)}

            info_dict = dict(id_dict, **properties_dict, **horizons_dict, **transforms_dict)
            info[cube.cube_name] = info_dict
        return info

    @staticmethod
    def __get_distance(coord_1, coord_2):
        return np.sqrt((coord_1[0] - coord_2[0]) ** 2 + (coord_1[1] - coord_2[1]) ** 2)

    @staticmethod
    def __get_transform(transform_matrix, x, y):
    ...
    @staticmethod
    def parse_to_numpy(cube: datacontainers.CubeDataSlice,
                       cube_props: dict) -> tuple[np.ndarray, np.ndarray, np.ndarray, float]:
        """
        ...
        x_cube_coords = np.array([cube_props['x_min_inl_min_xl'],
                                  cube_props['x_max_inl_min_xl'],
                                  cube_props['x_min_inl_max_xl'],
                                  cube_props['x_max_inl_max_xl'], ])

        y_cube_coords = np.array([cube_props['y_min_inl_min_xl'],
        ...
        dx = Cube.__get_distance([x_cube_coords[0], y_cube_coords[0]],
                                 [x_cube_coords[1], y_cube_coords[1]]) / cube_props['inline_count']
        dy = Cube.__get_distance([x_cube_coords[0], y_cube_coords[0]],
                                 [x_cube_coords[2], y_cube_coords[2]]) / cube_props['xline_count']

        dx, dy = int(dx), int(dy)

        idx_dict = {(int(inlines[i]), int(xlines[i])): i for i in range(inlines.shape[0])}
        ...
        grid_values = np.reshape(grid_values,
                                 newshape=(grid_values.shape[0] * grid_values.shape[1],
                                           grid_values.shape[2]))
