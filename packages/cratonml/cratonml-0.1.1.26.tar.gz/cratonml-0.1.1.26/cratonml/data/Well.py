...
class Well:
    ...
    @staticmethod
    def parse_to_numpy_without_interpolation(well_curves_list: list[datacontainers.WellCurve | datacontainers.Curve],
                                             min_depth: float = -np.inf,
                                             max_depth: float = np.inf) -> tuple[list, list]:
        """
        ...
        for well_curve in np.array(well_curves_list):
            if np.isnan(well_curve.point_depths).all():
                ...
                assert curr_min_depth < curr_max_depth, "Неверно заданы глубины"
    ...
    @staticmethod
    def get_strat_depth(strat_dict: dict, strat_info: dict, else_number: float = np.nan):
        """
