...
def set_jwt_access_cookie(response, access_token):
    ...
    cookie_secure = app_settings.JWT_AUTH_SECURE
    cookie_httponly = app_settings.JWT_AUTH_HTTPONLY
    cookie_samesite = app_settings.JWT_AUTH_SAMESITE
    cookie_domain = app_settings.JWT_AUTH_COOKIE_DOMAIN
...
def set_jwt_refresh_cookie(response, refresh_token):
    ...
    refresh_cookie_name = app_settings.JWT_AUTH_REFRESH_COOKIE
    refresh_cookie_path = app_settings.JWT_AUTH_REFRESH_COOKIE_PATH
    cookie_secure = app_settings.JWT_AUTH_SECURE
    cookie_httponly = app_settings.JWT_AUTH_HTTPONLY
    cookie_samesite = app_settings.JWT_AUTH_SAMESITE
    cookie_domain = app_settings.JWT_AUTH_COOKIE_DOMAIN
...
class CookieTokenRefreshSerializer(TokenRefreshSerializer):
    ...
    def extract_refresh_token(self):
        ...
        cookie_name = app_settings.JWT_AUTH_REFRESH_COOKIE
    ...
    def authenticate(self, request):
        ...
        if header is None:
            if cookie_name:
                ...
                if (
                    app_settings.JWT_AUTH_COOKIE_ENFORCE_CSRF_ON_UNAUTHENTICATED
                ):  # True at your own risk
                    self.enforce_csrf(request)
                elif raw_token is not None and app_settings.JWT_AUTH_COOKIE_USE_CSRF:
                    self.enforce_csrf(request)
            else:
                return None
        else:
