...
def print_mpd_tree(
    ...
    # convert the range into an explicit index list
    if selected_periods is not None:
        ...
        if selected_periods.start < 0 and selected_periods.stop == 0:
            selected_periods = range(
                len(mpdi.periods) + selected_periods.start, len(mpdi.periods)
            )
        ...
        for period in mpdi.periods:
            ...
            for event_stream in period.event_streams:
                ...
                for event in event_stream.events:
                    if isinstance(event, Scte35BinaryEventInspector):
                        ...
                        descriptor_info = Text.from_markup(
                            "\n".join(summarize_xml(event.content))
                        )
                    else:
            ...
            if level >= 2:
                for adapset in period.adaptation_sets:
                    if selected_adaptation_set and (
                        selected_adaptation_set not in adapset.mime_type
                        and (
                            selected_adaptation_set.isdigit()
                            and int(selected_adaptation_set) != adapset.id
                        )
                    ):
                        continue
