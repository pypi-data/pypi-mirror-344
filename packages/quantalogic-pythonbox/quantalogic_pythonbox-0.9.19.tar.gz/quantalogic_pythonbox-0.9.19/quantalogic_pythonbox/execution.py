...
async def _async_execute_async(
    ...
    try:
        ...
        msg = str(e)
        # Handle StopIteration return in async generators
        if "coroutine raised StopIteration" in msg:
            import re
            matches = re.search(r'StopIteration\((.+?)\)', msg)
            if matches:
                rv = matches.group(1)
                if rv.startswith("'") and rv.endswith("'"):
                    ...
                    rv = rv[1:-1]
                return AsyncExecutionResult(result=rv, error=None, execution_time=time.time() - start_time, local_variables={})
        # Unwrap to original exception
        orig = e.original_exception
        while isinstance(orig, WrappedException):
            orig = orig.original_exception
        # For main entry, if TypeError, return its message; else propagate exception
        if entry_point == 'main':
            if isinstance(orig, TypeError):
                return AsyncExecutionResult(result=None, error=str(orig), execution_time=time.time() - start_time, local_variables={})
            return AsyncExecutionResult(result=None, error=orig, execution_time=time.time() - start_time, local_variables={})
        # Default: return WrappedException message
        return AsyncExecutionResult(result=None, error=msg, execution_time=time.time() - start_time, local_variables={})
    except Exception as exc_obj:
        result = str(exc_obj)
        local_vars = {}
        return AsyncExecutionResult(
