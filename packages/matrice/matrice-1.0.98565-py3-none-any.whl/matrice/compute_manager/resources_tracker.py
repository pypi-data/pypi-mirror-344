...
import docker
import logging
from matrice.compute_manager.instance_utils import get_gpu_info, calculate_time_difference
...
class ResourcesTracker:
        ...
        try:
            ...
            if stats:
                try:
                    ...
                    logging.error(f"Error calculating container metrics: {str(e)}")
            return 0, 0
        except Exception as e:
            logging.error(f"Error getting container stats: {str(e)}")
            return 0, 0

    def get_container_cpu_and_memory_with_container_id(self, container_id):
        """Get CPU and memory usage for a specific container"""
        try:
            # Get stats for each container (CPU and RAM)
            ...
            cpu_usage = float(stats[0].replace("% CPU", "").strip())  # Convert to float
            memory_usage = stats[1].split(" / ")[0]  # Only take the used memory portion
            # Convert memory usage to MB
            mem_value, mem_unit = (
                memory_usage[:-3],
                memory_usage[-3:],
            )  # Get the value and the unit (KiB, MiB, GiB)
            if mem_unit == "KiB":
                memory_usage_mb = float(mem_value) / 1024  # Convert KiB to MB
            elif mem_unit == "MiB":
                memory_usage_mb = float(mem_value)  # Already in MB
            elif mem_unit == "GiB":
                memory_usage_mb = float(mem_value) * 1024  # Convert GiB to MB
            else:
                memory_usage_mb = float(mem_value)  # Default case, if not recognized

            return cpu_usage, memory_usage_mb
        except subprocess.CalledProcessError as e:
            logging.error(f"Docker stats command failed: {str(e)}")
            return 0, 0
        except Exception as e:
            logging.error(f"Error getting container CPU and memory: {str(e)}")
            return 0, 0

    def get_container_gpu_info(self, container_id):
    ...
    def get_pid_id_by_container_id(self, container_id):
        # Fetch the PIDs of the container to track GPU usage
    ...
    def get_container_gpu_usage(self, container_pid):
        try:
            gpu_util = 0
            # Run nvidia-smi pmon to get process-level GPU usage for all GPUs
            ...
            for line in pmon_output[2:]:  # Skip the first two header lines
                parts = line.split()
                if len(parts) >= 8:  # Ensure the line contains the expected number of columns
                    pid = parts[1]
                    gpu_usage = parts[3]  # sm column indicates GPU usage

                    if pid == str(container_pid):
                        # Handle '-' values (which represent no usage) by defaulting to 0
    ...
    def get_container_gpu_memory_usage(self, container_pid):
        cmd = [
            "nvidia-smi",
        ...
        try:
            ...
            for line in result.stdout.splitlines():
                # Each line contains PID and used memory, separated by commas
                parts = line.strip().split(", ")
                if len(parts) == 2:
                    process_pid, used_memory = parts
                    # If the PID matches the input PID, add the memory usage to the total
    ...
    def _get_gpu_resources(self):
        ...
        try:
            ...
            logging.warning("nvidia-smi command not found - no NVIDIA GPU present")
        except FileNotFoundError:
            logging.warning("nvidia-smi command not found - GPU monitoring disabled")
        except Exception as e:
            logging.error(f"Error getting GPU resources: {str(e)}")
            ...
            if exited_containers:
                for container in exited_containers:
                    try:
                        ...
                        logging.error(f"Error processing exited container {container.id}: {str(e)}")
            ...
            if running_containers:
                for container in running_containers:
                    try:
                        ...
                        logging.error(f"Error processing running container {container.id}: {str(e)}")
        except Exception as e:
            logging.error(f"Error in _process_containers: {str(e)}")

    def get_sub_containers_by_label(self, label_key, label_value):
        ...
        try:
            ...
            logging.error(f"Error getting containers by label: {str(e)}")
            return []

    def _update_container_action_status(self, container, status):
        ...
        try:
            ...
            # Look for 24 character action ID in container args
            def remove_qutation_marks(args):
                new_args = []
                for arg in args:
                    new_args.extend(x.replace('"', '').replace("'", "") for x in arg.split(" "))
                return new_args
            action_record_id = next((arg for arg in remove_qutation_marks(inspect_data["Args"]) if len(arg)==24), None)
            if not action_record_id:
                raise ValueError("No valid action_id found for the container.")

            duration = calculate_time_difference(start_time, finish_time)

            # Get resource usage for main container
            current_gpu_utilization, current_gpu_memory, current_cpu_utilization, current_memory_utilization = (
                self.get_current_action_usage(container, status)
            )

            # Add resource usage from any sub-containers for the Nvidia Triton docker container sarted with the deploy_add action
            ...
            for sub_container in sub_containers:
                ...
                if status == "completed":
                    try:
                        ...
                        logging.error(f"Error removing sub-container {sub_container.id}: {str(e)}")

            # Update and get max usage values
            max_gpu_utilization, max_gpu_memory, max_cpu_utilization, max_memory_utilization = self.update_max_action_usage(
                action_record_id, 
            ...
            logging.info(
                "Updating action status: service_provider=%s, action_id=%s, running=%s, status=%s, duration=%s, start=%s, "
                "gpu_util=%.2f%%, cpu_util=%.2f%%, gpu_mem=%dMB, mem_util=%.2f%%, created=%s, updated=%s",
            ...
            logging.error(f"Error updating container {container.id} status: {str(e)}")

    def get_current_action_usage(self, container, status):
        ...
        try:
            ...
            if status == "running":
                try:
                    current_cpu_utilization, current_memory_utilization = (
                        self.resources_tracker.get_container_cpu_and_memory(container)
                    )
                    # or use this
                    # current_cpu_utilization, current_memory_utilization = ResourcesTracker().get_container_cpu_and_memory_with_container_id(container.id)
                    current_gpu_utilization, current_gpu_memory = self.resources_tracker.get_container_gpu_info(
                        container_id=container.id
                    )
                except Exception as e:
                    logging.error(f"Error getting container usage metrics: {str(e)}")

            return current_gpu_utilization, current_gpu_memory, current_cpu_utilization, current_memory_utilization

        except Exception as e:
            logging.error(f"Error in get_current_action_usage for container id {container.id}: {str(e)}")
            return 0, 0, 0, 0

    def update_max_action_usage(self, action_record_id, current_gpu_utilization, current_gpu_memory, current_cpu_utilization, current_memory_utilization):
        ...
        try:
            ...
            # Convert any None values to 0 before comparison
            current_values = {
            ...
            # Return the current maximum values
            return (
            ...
            logging.error(f"Error updating max action usage: {str(e)}")
            return 0, 0, 0, 0
...
class MachineResourcesTracker():
    ...
    def update_available_resources(self):
        ...
        try:
            ...
            if err is not None:
                logging.error("Error in updating available resources: %s", err)
        except Exception as e:
            logging.error(f"Error updating available resources: {str(e)}")
