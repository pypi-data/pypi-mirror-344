...
class ResourcesTracker:
        ...
        try:
            ...
            if stats:
                try:
                    ...
                    logging.error(
                        "Error calculating container metrics: %s",
                        str(err),
                    )
            return 0, 0
        except Exception as err:
            logging.error(
                "Error getting container stats: %s",
                str(err),
            )
            return 0, 0
    ...
    def get_container_cpu_and_memory_with_container_id(self, container_id):
        ...
        try:
            ...
            logging.error(
                "Docker stats command failed: %s",
                str(err),
            )
            return 0, 0
        except Exception as err:
            logging.error(
                "Error getting container CPU and memory: %s",
                str(err),
            )
            return 0, 0
...
class ActionsResourcesTracker:
    ...
    def update_actions_resources(self):
        ...
        try:
            ...
            logging.error(
                "Error in _process_containers: %s",
                str(err),
            )
    ...
    def get_sub_containers_by_label(self, label_key, label_value):
        ...
        try:
            ...
            logging.error(
                "Error getting containers by label: %s",
                str(err),
            )
            return []
    ...
    def _update_container_action_status(self, container, status):
        ...
        try:
            ...
            action_record_id = next(
                (arg for arg in remove_quotation_marks(inspect_data["Args"]) if len(arg) == 24 and "pypi" not in arg),
                None,
            )
            ...
            logging.error(
                "Error updating container %s status: %s",
                container.id,
                str(err),
            )
    ...
    def get_current_action_usage(self, container, status):
        ...
        try:
            ...
            logging.error(
                "Error in get_current_action_usage for container id %s: %s",
                container.id,
                str(err),
            )
            return 0, 0, 0, 0
    ...
    def update_max_action_usage(
        ...
        try:
            ...
            logging.error(
                "Error updating max action usage: %s",
                str(err),
            )
            return 0, 0, 0, 0
...
class MachineResourcesTracker:
    ...
    def update_available_resources(self):
        ...
        try:
            ...
            logging.error(
                "Error updating available resources: %s",
                str(err),
            )
