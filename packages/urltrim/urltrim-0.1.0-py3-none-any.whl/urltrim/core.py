...
class URLTrim:
    ...
    def _init_(self, url: str, mode: str = "default", validate: bool = True):
        """
    ...
    def _trim_url(self) -> str:
        ...
        # Smart e-commerce detection
        if "amazon." in domain:
            match = re.search(r"/dp/([A-Z0-9]{10})", path)
            if match:
                return f"{domain}/dp/{match.group(1)}"

        if "flipkart." in domain:
            match = re.search(r"/p/([^/?]+)", path)
            if match:
                return f"{domain}/p/{match.group(1)}"

        if "zomato." in domain:
            segments = path.split('/')
            if segments:
                return f"{domain}/{segments[0]}"
    ...
    def get_trimmed_url(self) -> str:
        """Return the trimmed (short) URL."""
