#-----------------------------------------------------------------------------------------
# Date: 20240728
# Updated: hotlittlewhitedog
# Desc: Pkg_resources was deprecated, was replaced by importlib
#
# Date: 20180821
# Updated: hotlittlewhitedog
# Project: sonofman, https://gitlab.com/hotlittlewhitedog/BibleMultiTheSonOfMan
# Desc: Full path to pdcurses.dll for Windows 10
# Version: Based on version 1.2 of UniCurses
# Ref: https://github.com/Chiel92/unicurses
# installation:
# unicurses.exe
# curses2.2 amd 64.whl with pip install
# in Console: chcp 65001, lucida font
# som > som_err  --> iniscr problem
#-----------------------------------------------------------------------------------------
#
...
try:
    PDCURSES_DLL_PATH = files('sonofman') / 'pdcurses.dll'
except:
    raise Exception("Unable to create full path to pdcurses.dll!")
...
try:
    ...
    if sys.platform.find('win') == -1: 
        ...
        if not os.access(PDCURSES_DLL_PATH, os.F_OK):
        ...
        pdlib = ctypes.CDLL(PDCURSES_DLL_PATH)   # we're on winXX, use pdcurses instead of native ncurses
...
def ucs_reconfigure(wrapper_ncurses, wrapper_pdcurses):
    ...
    if NCURSES_AVAILABLE:
        ...
        if wrapper_pdcurses == UCS_DEFAULT_WRAPPER:
            NCURSES = False
            try:
                pdlib = ctypes.CDLL(PDCURSES_DLL_PATH)
            except:
