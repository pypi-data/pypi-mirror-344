...
def process_option_chain(df: pd.DataFrame, currency: str) -> pd.DataFrame:
    ...
    df['mark_price'] = df['mark_price'] * df['spot_price']
    df['bid_price'] = df['best_bid_price'] * df['spot_price']
    df['ask_price'] = df['best_ask_price'] * df['spot_price']
    ...
    for idx, row in df.iterrows():
        ...
        if 'bids' in row and isinstance(row['bids'], list) and len(row['bids']) > 0:
            ...
            for bid in row['bids']:
                if len(bid) >= 3:
                    ...
                    price = float(bid[1]) if bid[0] == 'new' else float(bid[0]) * s
        ...
        if 'asks' in row and isinstance(row['asks'], list) and len(row['asks']) > 0:
            ...
            for ask in row['asks']:
                if len(ask) >= 3:
                    # Extract price and quantity, removing 'new' if present
                    price = float(ask[1]) if ask[0] == 'new' else float(ask[0]) * s
