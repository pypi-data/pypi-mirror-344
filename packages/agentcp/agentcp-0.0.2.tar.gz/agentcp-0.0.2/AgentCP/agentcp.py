...
class AgentCP(_AgentCP):
    ...
    def __on_invite_ack(self,data):
        ...
        if status == 404:
            message_list = []
            msg_block = AssistantMessageBlock(
                type="error",
                status="success",
            ...
            self.db_manager.insert_message("assistant",self.id,session_id,acceptor_id, "", self.id, json.dumps(message_list), "text", "success","")
    ...
    def __build_url(self, aid):
        ...
        self.ca_client = CAClient("https://ca."+end_str)
    ...
    def create_aid(self, ep_url: str,agent_id: str):
        self.ca_client = CAClient("https://ca."+ep_url)
