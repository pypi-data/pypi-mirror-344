...
class SuperCheetahTuner:
    def __init__(self, model_class, param_bounds, X, y, mode='classification', 
                 n_agents=20, max_iter=30, patience=5, cv=3, random_state=42, verbose=True):
        """
        ...
        if self.verbose:
            print("SuperCheetahTuner initialized with:")
            print(f"- {self.n_agents} agents")
            print(f"- {self.max_iter} maximum iterations")
    ...
    def get_best_score(self):
        """Returns the best validation score"""
        if self.mode == 'classification':
            return 1 - self.best_score  # Convert back to accuracy
        return -self.best_score  # For regression (RMSE)
        
    def get_best_model(self):
        """Returns model instance with best parameters"""
        if self.best_params is None:
            raise ValueError("Optimization not run yet. Call optimize() first.")
        return self.model_class(**self._clip_params(self.best_params))
