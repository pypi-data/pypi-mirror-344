...
class FunctionExtractor:
            ...
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(target, ast.Name) and target.id == var_name:
                        ...
                        # Collect dependencies from the right side of the assignment
    ...
    def _generate_main_entry_point(self):
        """Generate code for the main entry point with environment variables."""
        type_hints = get_type_hints(self.func)
        param_parsers = []
        ...
        for param_name, param in self.signature.parameters.items():
            ...
            # Handle Optional[List[X]] properly
            if origin_type is Union:
                args = get_args(py_type)
                if type(None) in args and len(args) == 2:
                    non_none_type = next(arg for arg in args if arg is not type(None))
                    inner_origin = get_origin(non_none_type)

                    if non_none_type is list or inner_origin is list:
                        # Reroute to list handling
                        list_type = non_none_type
                        if is_required:
                            param_parsers.append(
                                f'{param_name}_json = parse_env_var("{param_name}", "list", required=True)')
                        else:
                            param_parsers.append(
                                f'{param_name}_json = parse_env_var("{param_name}", "list", default={default_repr})')

                        if hasattr(list_type, "__args__") and list_type.__args__:
                            item_type = list_type.__args__[0]
                            item_type_name = getattr(item_type, "__name__", "Any")

                            if hasattr(item_type, "__annotations__"):
                                param_parsers.append(
                                    f"{param_name} = [{item_type_name}(**item) for item in {param_name}_json] if {param_name}_json else None"
                                )
                            else:
                                param_parsers.append(f"{param_name} = {param_name}_json")
                        else:
                            param_parsers.append(f"{param_name} = {param_name}_json")
                        continue  # Skip further handling since it's done

            # List handling
            if py_type is list or origin_type is list:
                if is_required:
                    param_parsers.append(f'{param_name}_json = parse_env_var("{param_name}", "list", required=True)')
                else:
                    param_parsers.append(
                        f'{param_name}_json = parse_env_var("{param_name}", "list", default={default_repr})')
            ...
            # Enum handling
                ...
                if is_required:
                    param_parsers.append(f'{param_name}_str = parse_env_var("{param_name}", "str", required=True)')
                else:
                    enum_default = f'"{param.default.value}"' if param.default else '""'
                    param_parsers.append(
                        f'{param_name}_str = parse_env_var("{param_name}", "str", default={enum_default})')
                param_parsers.append(
                    f"{param_name} = {py_type.__name__}({param_name}_str) if {param_name}_str else None")
            ...
            # Optional/simple Union
                ...
                if type(None) in args and len(args) == 2:
                    other_type = next(arg for arg in args if arg is not type(None))
                    type_name = getattr(other_type, "__name__", "Any")
                    if is_required:
                        param_parsers.append(
                            f'{param_name} = parse_env_var("{param_name}", "{type_name}", required=True)')
                    else:
                        param_parsers.append(
                            f'{param_name} = parse_env_var("{param_name}", "{type_name}", default={default_repr})')
                else:
                    first_type = args[0]
                    type_name = getattr(first_type, "__name__", "Any")
                    if is_required:
                        param_parsers.append(
                            f'{param_name} = parse_env_var("{param_name}", "{type_name}", required=True)')
                    else:
            ...
            # Fallback for standard types
                ...
                type_name = getattr(py_type, "__name__", "Any")
                if is_required:
                    param_parsers.append(f'{param_name} = parse_env_var("{param_name}", "{type_name}", required=True)')
                else:
                    param_parsers.append(
                        f'{param_name} = parse_env_var("{param_name}", "{type_name}", default={default_repr})')
