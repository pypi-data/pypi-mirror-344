...
# multi_lingual: accepts a list of Language enums. language field is ignored if this field is not None.
class AzureSTTService(STTService):
    def __init__(
        ...
        *,
        ...
        sample_rate: Optional[int] = None,
        multi_lingual: list[Language] = None,
        **kwargs,
    ):
        super().__init__(sample_rate=sample_rate, **kwargs)
        ...
        stream_format = AudioStreamFormat(samples_per_second=sample_rate, channels=1)
        self._audio_stream = PushAudioInputStream(stream_format)

        audio_config = AudioConfig(stream=self._audio_stream)
        # Configure auto-detection if multi_lingual is provided
        if multi_lingual:
            logger.debug("Multi-lingual mode enabled")
            self._speech_config.set_property(
                property_id=speechsdk.PropertyId.SpeechServiceConnection_LanguageIdMode,
                value="Continuous",
            )
            azure_languages = [language_to_azure_language(lang) for lang in multi_lingual]
            auto_detect_source_language_config = (
                speechsdk.languageconfig.AutoDetectSourceLanguageConfig(languages=azure_languages)
            )
