...
async def run_bot(
    ...
    # Create a local copy of base_tools for this bot instance
    local_base_tools = base_tools.copy()
    ...
    if call_config:
        ...
        if call_config.get("use_rag", False):
            ...
            local_base_tools.append(rag_tool)
    ...
    # Add the switch_language tool
    if "switch_language" in prompt:
        local_base_tools.append(switch_language_tool)
        llm.register_function(
            "switch_language",
            lambda fn,
            tool_call_id,
    ...
    if call_config and call_config.get("tools"):
        for tool in call_config.get("tools"):
            if tool["name"] != "end_call":  # Avoid duplicate registration
                ...
                local_base_tools.append(tool)

    # Convert to provider-specific format using local_base_tools
    tools = convert_tools_for_llm_provider(local_base_tools, llm_provider)
