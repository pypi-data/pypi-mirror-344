...
class VectorDatabase:
    def __init__(
        self,
        db_name: str,
        use_semantic_connections: bool = False,
        top_k: int = 5,
        db_file=None,
    ):
        ...
        if self.db_file and os.path.exists(self.db_file):
            self.load_from_disk(self.db_file)
    ...
    def save_to_disk(
        self, serialization_format: SerializationFormat = SerializationFormat.BINARY
    ) -> None:
        try:
            if serialization_format == SerializationFormat.BINARY:
                if not self.db_file:
    ...
    def __save_json(self) -> None:
        with open(self.db_file, "w") as file:
            json.dump(
                {"vectors": self.vectors.tolist(), "metadata": self.metadata}, file
            )
    ...
    def add_vectors_batch(
        self, records: List[Dict[str, Any]], normalize: bool = True
    ) -> None:
        vectors = []
    ...
    def _update_connections(self, index: int) -> None:
        ...
        similarities = self.top_cosine_similarity(
            vector, self.top_k + 1
        )  # +1 to account for self-similarity
        connections = [
            (sim[0]["id"], sim[1])
            for sim in similarities
            if sim[0]["id"] != self.metadata[index]["id"]
        ]
        ...
        for sim in similarities[1:]:  # Skip the first one as it's the vector itself
            other_index = next(
                i for i, meta in enumerate(self.metadata) if meta["id"] == sim[0]["id"]
            )
    ...
    def update_vector(
        self, id: int, new_record: Dict[str, Any], normalize: bool = True
    ) -> None:
        index = next(
            (i for i, meta in enumerate(self.metadata) if meta["id"] == id), None
        )
        if index is None:
        ...
        new_metadata["connections"] = self.metadata[index][
            "connections"
    ...
    def delete_vector(self, id: int) -> None:
        ...
        if self.use_semantic_connections:
            ...
            for meta in self.metadata:
                meta["connections"] = [
                    (conn_id, score)
                    for conn_id, score in meta["connections"]
                    if conn_id != id
                ]
    ...
    def get_connected_chunks(self, id: int, depth: int = 1) -> List[Dict[str, Any]]:
        ...
        return list(
            {chunk["id"]: chunk for chunk in result}.values()
        )  # Remove duplicates
    ...
    def semantic_search(
        self, query_embedding: np.ndarray | List[float], depth: int = 1
    ) -> List[Dict[str, Any]]:
        if len(self.vectors) == 0:
            print("The database is empty.")
            return []
        ...
        return list(
            {chunk["id"]: chunk for chunk in expanded_results}.values()
        )  # Remove duplicates
    ...
    def add_image_embedding(
        self, image_id: str, embedding: np.ndarray, metadata: Dict[str, Any]
    ) -> str:
    ...
    def image_semantic_search(
        self, query_embedding: np.ndarray, top_k: int = 5, depth: int = 1
    ) -> List[Dict[str, Any]]:
