                ...
                with open(cache_file, "rb") as f:
                    ...
                    if output_type is not None:
                        if not isinstance(result, output_type):
                            raise ValueError(f"Reading Error: cached result is not of type {output_type.__name__} it is {type(result).__name__}")
                    return result

            result = func(*args, **kwargs)
            if output_type is not None:
                if not isinstance(result, output_type) and result is not None:
                    raise ValueError(f"Writing Error: result is not of type {output_type.__name__} it is {type(result).__name__}")
