...
def _fetch_data(
    url: str, identifier_type: str = "CID", identifier: Any = None
) -> Optional[Any]:
    ...
    try:
        ...
        if not (
            ...
            print(
                f"Warning: API request failed for URL {url} ({identifier_type}: {identifier}): {e}",
                file=sys.stderr,
            )
        return None
    except Exception as e:  # Catch other potential errors
        print(
            f"Warning: An unexpected error occurred processing URL {url}: {e}",
            file=sys.stderr,
        )
        return None
...
def get_additional_properties(cid: int) -> Dict[str, Any]:
    ...
    if data and isinstance(data, dict) and "PropertyTable" in data:
        ...
        if (
            ...
            and isinstance(properties_list_data, list)
            and len(properties_list_data) > 0
        ):
...
def get_compound_description(cid: int) -> Optional[str]:
    ...
    if xml_content and isinstance(xml_content, str):
        try:
            ...
            print(
                f"Warning: Unexpected error parsing description XML string for CID {cid}: {e}",
                file=sys.stderr,
            )
...
def get_all_synonyms(cid: int) -> List[str]:
    ...
    if data and isinstance(data, dict) and "InformationList" in data:
        ...
        if info_list and isinstance(info_list, list) and len(info_list) > 0:
            ...
            if (
                syns
                and isinstance(syns, list)
                and all(isinstance(s, str) for s in syns)
            ):
                synonyms = syns
    return synonyms
...
def _get_batch_data_for_cids(
    cids: List[int], endpoint_segment: str, result_key: str, id_key: str = "CID"
) -> Dict[int, Any]:
    ...
    data = _fetch_data(
        url,
        identifier_type="Batch CID",
        identifier=f"{endpoint_segment} for {len(cids)} CIDs",
    )
    results = {}
    if data and isinstance(data, dict) and result_key in data:
        ...
        items_list_key = (
            "Properties" if result_key == "PropertyTable" else "Information"
        )
            ...
            for item in items:
                if isinstance(item, dict) and id_key in item:
                    try:  # Ensure CID is int for key
                        ...
                        results[item_cid] = (
                            item  # Store the whole item dict for this CID
                        )
                    except (ValueError, TypeError):
                        print(
                            f"Warning: Invalid CID '{item.get(id_key)}' found in batch response for {endpoint_segment}.",
                            file=sys.stderr,
                        )
                        continue
    return results
...
def get_batch_properties(
    ...
    if not properties:
        return {
            cid: {} for cid in cids
        }  # Return empty dicts if no properties requested
    properties_str = ",".join(properties)
    raw_results = _get_batch_data_for_cids(
        cids, f"property/{properties_str}", "PropertyTable", "CID"
    )
...
def get_batch_synonyms(cids: List[int]) -> Dict[int, List[str]]:
    ...
    for cid_key, data in raw_results.items():
        ...
        if isinstance(synonyms_raw, list) and all(
            isinstance(s, str) for s in synonyms_raw
        ):
...
def get_batch_descriptions(cids: List[int]) -> Dict[int, Optional[str]]:
    ...
    xml_content = _fetch_data(
        url, identifier_type="Batch CID", identifier=f"description for {len(cids)} CIDs"
    )
    results = {}
    ...
    if content_to_parse:
        try:
            ...
            print(
                f"Warning: Failed to parse batch XML description: {e}", file=sys.stderr
            )
        except Exception as e:
            print(
                f"Warning: Unexpected error parsing batch description XML: {e}",
                file=sys.stderr,
            )
