...
from turbo import command_name
...
def call_after_init(callback):
  """Calls the given callback only once TURBO has finished initialization.

  If TURBO has already finished initialization when ``call_after_init`` is
  ...
  Args:
    callback: a callable to be called once TURBO has finished initialization.
...
def _run_init(
  ...
  # Set up turbo logging handler.
  logging.use_turbo_handler()
