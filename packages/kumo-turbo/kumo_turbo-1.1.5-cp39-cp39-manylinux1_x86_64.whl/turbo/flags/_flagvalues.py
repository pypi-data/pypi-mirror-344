...
from turbo.flags import _validators_classes
from turbo.flags._flag import Flag
...
class FlagValues:
  """Registry of :class:`~turbo.flags.Flag` objects.
  ...
  The ``.value`` attribute of the registered :class:`~turbo.flags.Flag` objects
  ...
  :class:`~turbo.flags.Flag` objects can be used to access its value::

       FLAGS.longname  # parsed flag value
       FLAGS.x  # parsed flag value (short name)

  Command line arguments are scanned and passed to the registered
  :class:`~turbo.flags.Flag` objects through the ``__call__`` method.  Unparsed
  arguments, including ``argv[0]`` (e.g. the program name) are returned::

       argv = FLAGS(sys.argv)  # scan command line arguments

  The original registered :class:`~turbo.flags.Flag` objects can be retrieved
  through the use of the dictionary-like operator, ``__getitem__``::

       x = FLAGS['longname']   # access the registered Flag object

  The ``str()`` operator of a :class:`turbo.flags.FlagValues` object provides
  help for all of the registered :class:`~turbo.flags.Flag` objects.
  """
  ...
  def __copy__(self) -> Any:
    raise TypeError('FlagValues does not support shallow copies. '
                    'Use turbo.testing.flagsaver or copy.deepcopy instead.')
  ...
  def _set_is_retired_flag_func(self, is_retired_flag_func):
    ...
    Do not use it. This is a private turbo API used to check retired flags
    registered by the turbo C++ flags library.
  ...
  def _get_help_for_modules(self, modules, prefix, include_special_flags):
    ...
    Private to turbo.flags package.
  ...
  def read_flags_from_files(
    ...
    using turbo.flags DEFINE_flag() type functions.
