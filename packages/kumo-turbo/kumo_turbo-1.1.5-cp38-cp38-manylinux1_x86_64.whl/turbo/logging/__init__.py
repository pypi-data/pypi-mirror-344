...
from turbo.logging import converter
...
# Logging levels.
FATAL = converter.TURBO_FATAL
ERROR = converter.TURBO_ERROR
WARNING = converter.TURBO_WARNING
WARN = converter.TURBO_WARNING  # Deprecated name.
INFO = converter.TURBO_INFO
DEBUG = converter.TURBO_DEBUG

# Regex to match/parse log line prefixes.
TURBO_LOGGING_PREFIX_REGEX = (
    r'^(?P<severity>[IWEF])'
...
# Extra property set on the LogRecord created by TURBOLogger when its level is
# CRITICAL/FATAL.
_TURBO_LOG_FATAL = '_turbo_log_fatal'
# Extra prefix added to the log message when a non-turbo logger logs a
...
# The TURBO logger instance, initialized in _initialize().
_turbo_logger = None
# The TURBO handler instance, initialized in _initialize().
_turbo_handler = None
...
class _VerbosityFlag(flags.Flag):
  ...
  def _update_logging_levels(self):
    """Updates turbo logging levels to the current verbosity.

    Visibility: module-private
    """
    if not _turbo_logger:
      return

    if self._value <= converter.TURBO_DEBUG:
      standard_verbosity = converter.turbo_to_standard(self._value)
    else:
      # --verbosity is set to higher than 1 for vlog.
      standard_verbosity = logging.DEBUG - (self._value - 1)

    # Also update root level when turbo_handler is used.
    if _turbo_handler in logging.root.handlers:
      # Make turbo logger inherit from the root logger. turbo logger might have
      # a non-NOTSET value if logging.set_verbosity() is called at import time.
      _turbo_logger.setLevel(logging.NOTSET)
      logging.root.setLevel(standard_verbosity)
    else:
      _turbo_logger.setLevel(standard_verbosity)
...
class _LoggerLevelsFlag(flags.Flag):
  ...
  def _update_logger_levels(self):
    # Visibility: module-private.
    # This is called by turbo.app.run() during initialization.
...
def set_verbosity(v):
  ...
  try:
    ...
    new_level = converter.TURBO_NAMES[v.upper()]
...
def set_stderrthreshold(s):
  ...
  """
  if s in converter.TURBO_LEVELS:
    FLAGS.stderrthreshold = converter.TURBO_LEVELS[s]
  elif isinstance(s, str) and s.upper() in converter.TURBO_NAMES:
    FLAGS.stderrthreshold = s
  else:
    raise ValueError(
        'set_stderrthreshold only accepts integer turbo logging level '
...
def _fast_stack_trace():
  ...
  Compared to using `get_turbo_logger().findCaller(stack_info=True)`, this
...
def log_every_n(level, msg, n, *args, use_call_stack=False):
  ...
  Args:
    level: int, the turbo logging level at which to log.
  ...
  """
  caller_info = get_turbo_logger().findCaller()
  if use_call_stack:
...
def log(level, msg, *args, **kwargs):
  """Logs ``msg % args`` at turbo logging level ``level``.

  If no args are given just print msg, ignoring any interpolation specifiers.

  Args:
    level: int, the turbo logging level at which to log the message
  ...
  """
  if level > converter.TURBO_DEBUG:
    ...
    if level < converter.TURBO_FATAL:
      level = converter.TURBO_FATAL
    standard_level = converter.turbo_to_standard(level)

  # Match standard logging's behavior. Before use_turbo_handler() and
  # logging is configured, there is no handler attached on _turbo_logger nor
  # logging.root. So logs go no where.
  if not logging.root.handlers:
    logging.basicConfig()

  _turbo_logger.log(standard_level, msg, *args, **kwargs)
...
def vlog(level, msg, *args, **kwargs):
  ...
  Args:
    level: int, the C++ verbose logging level at which to log the message,
        e.g. 1, 2, 3, 4... While turbo level constants are also supported,
...
def vlog_is_on(level):
  ...
  if level > converter.TURBO_DEBUG:
    ...
    if level < converter.TURBO_FATAL:
      level = converter.TURBO_FATAL
    standard_level = converter.turbo_to_standard(level)
  return _turbo_logger.isEnabledFor(standard_level)


def flush():
  """Flushes all log files."""
  get_turbo_handler().flush()
...
def get_log_file_name(level=INFO):
  ...
  Args:
    level: int, the turbo.logging level.

  Raises:
    ValueError: Raised when `level` has an invalid value.
  """
  if level not in converter.TURBO_LEVELS:
    raise ValueError(f'Invalid turbo.logging level {level}')
  stream = get_turbo_handler().python_handler.stream
...
def get_turbo_log_prefix(record):
  """Returns the turbo log prefix for the log record.
  ...
  if _is_non_turbo_fatal_record(record):
    # When the level is FATAL, but not logged from turbo, lower the level so
...
def skip_log_prefix(func):
  """Skips reporting the prefix of a given function or name by :class:`~turbo.logging.TURBOLogger`.

  This is a convenience wrapper function / decorator for
  :meth:`~turbo.logging.TURBOLogger.register_frame_to_skip`.
  ...
  if callable(func):
    ...
    file_name = get_turbo_logger().findCaller()[0]
    func_name = func
    func_lineno = None
  else:
    raise TypeError('Input is neither callable nor a string.')
  TURBOLogger.register_frame_to_skip(file_name, func_name, func_lineno)
  return func


def _is_non_turbo_fatal_record(log_record):
  return (log_record.levelno >= logging.FATAL and
          not log_record.__dict__.get(_TURBO_LOG_FATAL, False))


def _is_turbo_fatal_record(log_record):
  return (log_record.levelno >= logging.FATAL and
          log_record.__dict__.get(_TURBO_LOG_FATAL, False))
...
class PythonHandler(logging.StreamHandler):
  ...
  def use_turbo_log_file(self, program_name=None, log_dir=None):
  ...
  def emit(self, record):
    ...
    if not FLAGS.is_parsed():  # Also implies "before flag has been defined".
      ...
      if ((FLAGS['alsologtostderr'].value or level >= stderr_threshold) and
          self.stream != sys.stderr):
        self._log_to_stderr(record)
    # Die when the record is created from TURBOLogger and level is FATAL.
    if _is_turbo_fatal_record(record):
...
class TURBOHandler(logging.Handler):
  ...
  def use_turbo_log_file(self, program_name=None, log_dir=None):
    self._current_handler.use_turbo_log_file(program_name, log_dir)
...
class PythonFormatter(logging.Formatter):
  """Formatter class used by :class:`~turbo.logging.PythonHandler`."""
  ...
  def format(self, record):
    ...
    if (not FLAGS['showprefixforinfo'].value and
        FLAGS['verbosity'].value == converter.TURBO_INFO and
        record.levelno == logging.INFO and
        _turbo_handler.python_handler.stream == sys.stderr):
      prefix = ''
    else:
      prefix = get_turbo_log_prefix(record)
    return prefix + super().format(record)


class TURBOLogger(logging.getLoggerClass()):
  ...
  TURBOLogger know which method from which file should be
  ...
  def findCaller(self, stack_info=False, stacklevel=1):
    ...
    TURBOLogger and any methods from this file, and whatever
    ...
    """
    f_to_skip = TURBOLogger._frames_to_skip
  ...
  def log(self, level, msg, *args, **kwargs):
    ...
    if level >= logging.FATAL:
      ...
      # This will be used by the TURBOHandler to determine whether it should
      # treat CRITICAL/FATAL logs as really FATAL.
      extra = kwargs.setdefault('extra', {})
      extra[_TURBO_LOG_FATAL] = True
  ...
  def handle(self, record):
    ...
    if it's not explicitly specified. Historically, turbo logging will not be
  ...
  def register_frame_to_skip(cls, file_name, function_name, line_number=None):
    ...
    The :class:`~turbo.logging.TURBOLogger` sometimes skips method calls on the
...
def get_turbo_logger():
  """Returns the turbo logger instance."""
  assert _turbo_logger is not None
  return _turbo_logger


def get_turbo_handler():
  """Returns the turbo handler instance."""
  assert _turbo_handler is not None
  return _turbo_handler
...
def use_python_logging(quiet=False):
  ...
  """
  get_turbo_handler().activate_python_handler()
  if not quiet:
...
def use_turbo_handler():
  """Uses the TURBO logging handler for logging.

  This method is called in :func:`app.run()<turbo.app.run>` so the turbo handler
  is used in turbo apps.
  """
  global _attempted_to_remove_stderr_stream_handlers
  if not _attempted_to_remove_stderr_stream_handlers:
    # The turbo handler logs to stderr by default. To prevent double logging to
    ...
    # logging.info/debug is called before calling use_turbo_handler().
    handlers = [
  ...
  turbo_handler = get_turbo_handler()
  if turbo_handler not in logging.root.handlers:
    logging.root.addHandler(turbo_handler)
...
def _initialize():
  ...
  global _turbo_logger, _turbo_handler

  if _turbo_logger:
    return

  original_logger_class = logging.getLoggerClass()
  logging.setLoggerClass(TURBOLogger)
  _turbo_logger = logging.getLogger('turbo')
  logging.setLoggerClass(original_logger_class)

  python_logging_formatter = PythonFormatter()
  _turbo_handler = TURBOHandler(python_logging_formatter)
