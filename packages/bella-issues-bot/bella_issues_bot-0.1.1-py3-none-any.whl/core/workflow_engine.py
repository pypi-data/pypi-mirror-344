...
class WorkflowEngine:
    ...
    def process_requirement(self, user_requirement: str) -> Optional[str]:
        ...
        try:
            # 初始化环境
            self.version_manager.init_environment(self.config.mode)
    ...
    def _run_code_generation_workflow(self, user_requirement: str) -> Optional[str]:
        ...
        # 提交更改
        if success:
            self.version_manager.apply_changes(mode=self.config.mode, response=response, is_code_change=True)
            return response
        else:
            self.CODE_TIMES += 1
    ...
    def _run_chat_workflow(self, user_requirement: str) -> Optional[str]:
        ...
        if response:
            self.version_manager.apply_changes(mode=self.config.mode, response=response, is_code_change=False)
            return response
        else:
            self.CHAT_TIMES += 1
