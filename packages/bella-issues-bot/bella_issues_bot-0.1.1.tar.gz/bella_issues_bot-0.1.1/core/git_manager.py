...
class GitManager:
    ...
    def reset_to_issue_branch(self, issue_id: int) -> str:
        ...
        try:
            ...
            # 构造issue分支名
            issue_branch = f"bella-issues-bot-{issue_id}"
            remote_issue_branch = f"{self.config.remote_name}/{issue_branch}"
            
            # 检查issue分支是否存在于远程仓库
            issue_branch_exists = any(remote_issue_branch in branch for branch in remote_branches)
            
            if issue_branch_exists:
                # 切换到issue分支
                self.switch_branch(issue_branch, create=True)
                self.pull()
                logger.info(f"重置到远程分支: {remote_issue_branch}")
                self.repo.git.reset(f"{remote_issue_branch}", hard=True)
                logger.info(f"成功切换到issue #{issue_id}的分支: {issue_branch}")
                return issue_branch
            else:
                # 如果没有找到issue分支，切换到默认分支，创建新的issue分支
                # 先确保默认分支是最新的
                ...
                # 创建并切换到issue分支
                self.switch_branch(issue_branch, create=True)
                logger.info(f"未找到issue #{issue_id}的分支，已创建新分支: {issue_branch}")
                return issue_branch
        except git.GitCommandError as e:
