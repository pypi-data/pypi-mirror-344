...
class PostgresManager(DatabaseManager):
    ...
    def get_adapter(self) -> ChainlitDataLayer:
        ...
        try:
            # Create the ChainlitDataLayer first
            data_layer = ChainlitDataLayer(database_url=self._database_url)

            # Create and set the storage provider separately
            storage_provider = LocalFileStorageClient()
            data_layer.storage_provider = storage_provider
