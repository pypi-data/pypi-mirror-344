...
from aki.tools.custom_tool_node import process_tool_message_content
...
class ChainlitCallback(AsyncCallbackHandler):
    ...
    def _modify_batch_tool_name(tool_input):
        ...
        # Extract tool names directly from invocations list
        tool_names = [
            item.get("name")
        ...
        return f"batch_tool[{', '.join(tool_names)}]", invocations
    ...
    def _format_output_for_display(output, tool_name=None):
        ...
        This method processes the unified response format and extracts the actual content
        ...
        if isinstance(output, ToolMessage):
            output = process_tool_message_content(output.content)

        # Check if output is in the unified format (list of dicts with 'type' field)
        if (
            isinstance(output, list)
            and len(output) > 0
            and all(isinstance(item, dict) and "type" in item for item in output)
        ):
            # Single text item - return just the text
            if len(output) == 1 and output[0]["type"] == "text":
                return output[0]["text"]

            # Single JSON item - return the JSON object
            elif len(output) == 1 and output[0]["type"] == "json":
                return output[0]["json"]

            # Image items - return as is
            elif len(output) == 1 and output[0]["type"] == "image_url":
                return output

            # Multiple items - handle based on content
            else:
                # Collect all text items
                text_items = [item["text"] for item in output if item["type"] == "text"]

                # If we only have text items, concatenate them
                if len(text_items) == len(output):
                    return "\n".join(text_items)

                # For mixed content, return as is
                return output

        # If not in unified format, return as is
        return output
    ...
    async def on_tool_end(self, output: str, **kwargs) -> None:
        ...
        try:
            ...
            # Format the output using our helper function which ensures the format
            # is identical to what CustomToolNode would have produced
            formatted_output = self._format_output_for_display(output, tool_name)

            # Make sure the output is serializable before setting it
