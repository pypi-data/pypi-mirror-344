...
from aki.chat.export_conversation import handle_export_command


async def register_commands():
    """
    Register the export command in the Chainlit UI.
    ICONs are avilalbe at https://lucide.dev/icons/
    """
    await cl.context.emitter.set_commands(
        [
            ...
            {
                "id": "Summarize",
                "description": "Trigger conversation summarization to free up memories. Type '.' to confirm.",
                "icon": "refresh-cw",
            },
        ]
    )
...
async def handle_summarize_command(message: cl.Message):
    """Handle the summarize command.

    This will set the need_summarize flag to true, which will cause
    the graph to route to the summary_node.
    """
    logging.info("Summarize command triggered")

    # Set the summarize flag
    cl.user_session.set("need_summarize", True)

    # Inform the user
    await cl.Message(content="Summarizing conversation...", author="System").send()
...
class EventHandler:
    ...
    async def handle_message(self, message: cl.Message):
        ...
        if message.command == "Export":
            ...
            return
        elif message.command == "Summarize":
            await handle_summarize_command(message)
            message = cl.Message(
                content="Please start summarize all conversations we have and keep important details"
            )
        ...
        try:
            ...
            fix_suggestion = "Sorry, please try a new conversation. Tips: you can export current conversation using /export m and then send the generated file to a new chat."
