...
async def test_batch_tool_execution(test_tools_dict: Dict[str, BaseTool]) -> None:
    ...
    # Create invocations
    invocations = [
        ToolInvocation(name="test_tool", parameters={"arg1": "hello", "arg2": 42}),
        ToolInvocation(name="async_only_tool", parameters={"param": "world"}),
    ]
    ...
    # Verify results
    # The _arun method now returns a list with a JSON content block
    # Extract the actual results from this structure
    actual_results = result[0]["json"]
    assert actual_results["test_tool"] == "Test tool ran with arg1=hello and arg2=42"
    assert actual_results["async_only_tool"] == "Async only: world"
...
async def test_batch_tool_handles_errors(test_tools_dict: Dict[str, BaseTool]) -> None:
    ...
    invocations = [
        ToolInvocation(name="test_tool", parameters={"arg1": "hello", "arg2": 42}),
        ToolInvocation(name="non_existent_tool", parameters={"param": "world"}),
        ToolInvocation(name="test_tool", parameters={}),
    ]
    ...
    actual_results = result[0]["json"]
    assert actual_results["test_tool"] == "Test tool ran with arg1=hello and arg2=42"
    assert "error" in actual_results["non_existent_tool"]
    assert (
        "Tool 'non_existent_tool' not found"
        in actual_results["non_existent_tool"]["error"]
    )
    assert "error" in actual_results["test_tool_1"]
    assert "Tool execution failed" in actual_results["test_tool_1"]["error"]
...
async def test_parameter_conversion(test_tools_dict: Dict[str, BaseTool]) -> None:
    ...
    with patch("aki.tools.batch_tool.convert_tool_args") as mock_convert:
        ...
        invocations = [
            ToolInvocation(name="test_tool", parameters={"camelCaseArg": "value"}),
        ]
        ...
        actual_results = result[0]["json"]
        assert (
            actual_results["test_tool"]
            == "Test tool ran with arg1=converted and arg2=100"
        )
