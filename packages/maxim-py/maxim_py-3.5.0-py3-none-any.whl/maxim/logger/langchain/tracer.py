...
class MaximLangchainTracer(BaseCallbackHandler):
    ...
    def __init__(
        self, logger: Logger, metadata: Optional[Dict[str, Any]] = None , eval_config: Optional[Dict[str, List[str]]] = None
    ) -> None:
    ...
    def on_chat_model_start(
        ...
        **kwargs: Any,
        ...
        if metadata is not None and 'langgraph_node' in metadata and metadata['langgraph_node'] in self.eval_config:
            eval_data = {
                'container_id': generation_container.id,
                'generation_container': generation_container,
                'evaluators': self.eval_config.get(metadata['langgraph_node'], []) if self.eval_config else [],
                'node_name': metadata['langgraph_node'],
                'input': last_input_message
            }
            self.to_be_evaluated_container_store.set(str(generation_container.id), eval_data, DEFAULT_TIMEOUT)
                    ...
                    if isinstance(result['choices'],list):
                        ...
                        if len(message['content'])>0:
                            ...
                            if tool_message['type'] == 'function':
                                tool_call_name = tool_message['function'].get('name','')
                                tool_call_args = tool_message['function'].get('arguments','')

                #only keeping non None type variables
                variable_dict = {k: v for k, v in {'input': obj['input'], 'output': output, 
                            'tool_call_name': tool_call_name, 'tool_call_args': tool_call_args}.items() 
                            if v is not None}

                generation_container.evaluate().with_evaluators(*obj['evaluators']).with_variables(variable_dict)
