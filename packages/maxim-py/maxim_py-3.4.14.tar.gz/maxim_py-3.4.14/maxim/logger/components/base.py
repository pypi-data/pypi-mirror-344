    ...
    def __init__(self, entity: Entity, config: BaseConfig, writer: LogWriter):        
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', config["id"]):            
            if writer.raise_exceptions:
                raise ValueError(f"Invalid ID: {config['id']}. ID must only contain alphanumeric characters, hyphens, and underscores. Event will not be logged.")
            else:
                logging.error(f"Invalid ID: {config['id']}. ID must only contain alphanumeric characters, hyphens, and underscores. Event will not be logged.")                
