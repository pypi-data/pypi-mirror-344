import json
import logging  # Added for logging potential inconsistencies
import sqlite3
...
import numpy as np
import pandas as pd  # Added for reading tables
...
class SDIFDatabase:
    def __init__(
        ...
        """
        self.path = Path(path).resolve()  # Resolve the path for consistency
        self.read_only = read_only
        self.schema_name = schema_name

        if read_only:
            # --- Read-Only Logic (Unchanged) ---
            ...
            with self.conn:  # Use context manager for transaction
                ...
                if not self.conn.execute("SELECT 1 FROM sdif_properties").fetchone():
                    self.conn.execute(
                        ...
                        ("1.0", datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")),
                    )
                ...
                # sdif_semantic_links table (optional)
                self.conn.execute("""
    ...
    def create_table(
        ...
        for col_name, col_props in columns.items():
            ...
            # Basic validation of SQLite types (can be expanded)
            valid_types = [
                "TEXT",
                "INTEGER",
                "REAL",
            ...
            if not any(col_type.startswith(valid) for valid in valid_types):
                log.warning(
                    f"Potentially non-standard SQLite type '{col_type}' used for column '{col_name}'."
                )
            ...
            if col_props.get("primary_key"):
                constraints.append(
                    "PRIMARY KEY"
                )  # Usually applied at table level if multi-column
            if col_props.get("not_null"):
                constraints.append("NOT NULL")
            if col_props.get("unique"):  # Added unique constraint
                constraints.append("UNIQUE")
            # Foreign key constraint is added at the table level later if specified

            # Wrap col_name in double quotes to handle special characters/keywords
            column_defs.append(f'"{col_name}" {col_type} {" ".join(constraints)}')

            # Prepare column metadata entry
            column_metadata_rows.append(
                (
                    table_name,
                    col_name,
                    col_props.get("description"),
                    col_props.get("original_format"),
                )
            )

        # Add table-level constraints (like multi-column PK or FKs)
        table_constraints = []
        pk_cols = [
        ...
        for col_name, col_props in columns.items():
            ...
            if isinstance(fk, dict) and "table" in fk and "column" in fk:
                ...
                # Add optional ON DELETE/ON UPDATE clauses
        ...
        try:
            with self.conn:  # Transaction
                # Check if table already exists physically
                cursor = self.conn.execute(
                ...
                if cursor.fetchone():
                    raise ValueError(
                        f"Metadata for table '{table_name}' already exists in sdif_tables_metadata."
                    )
                ...
                self.conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, 0) -- Initialize row_count to 0
                    """,
                    (table_name, source_id, description, original_identifier),
                )
                ...
                self.conn.executemany(
                    ...
                    (table_name, column_name, description, original_data_format)
                    VALUES (?, ?, ?, ?)
                    """,
                    column_metadata_rows,
                )
        except sqlite3.Error as e:
            log.error(f"Error creating table '{table_name}' or its metadata: {e}")
            # Rollback is handled by context manager 'with self.conn:'
            raise  # Re-raise the exception
    ...
    def get_schema(self) -> Dict[str, Any]:
        ...
        table_column_metadata = {}  # Cache column metadata for efficiency

        # Pre-fetch all column metadata
        try:
            cursor = self.conn.execute(
                "SELECT table_name, column_name, description, original_data_format FROM sdif_columns_metadata"
            )
            for row in cursor.fetchall():
                ...
                table_column_metadata[row["table_name"]][row["column_name"]] = {
                    ...
                    "original_data_format": row["original_data_format"],
                }
        except sqlite3.Error as e:
                        ...
                        table_schema["columns"].append(
                            {
                                ...
                                "original_data_format": sdif_col_meta.get(
    ...
    def write_dataframe(
        ...
        Handles table creation, metadata registration, and data insertion. Should be part of SDIFDatabase.
        """
        self._validate_connection()
        if self.read_only:
            raise PermissionError("Database is open in read-only mode.")
        ...
        try:
            ...
            if cursor.fetchone():
                ...
                if not cursor.fetchone():
                    log.warning(
                        f"Table '{table_name}' exists in metadata but not physically. Will attempt to recreate."
                    )
                    table_exists = False
        except sqlite3.Error as e:
            ...
            if if_exists == "fail":
                ...
                try:
                    with self.conn:
                        ...
                        self.conn.execute(f'DROP TABLE IF EXISTS "{table_name}"')
                        # Delete metadata (FKs might cascade, but be explicit)
                        self.conn.execute(
                            "DELETE FROM sdif_columns_metadata WHERE table_name = ?",
                            (table_name,),
                        )
        ...
        if not df.empty:
            try:
                data_list = df.replace({pd.NaT: None, np.nan: None}).to_dict("records")
