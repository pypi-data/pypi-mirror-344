...
class Jaal:
    ...
    def _callback_filter_nodes(self, graph_data, filter_nodes_text):
        ...
        try:
            ...
            nodes = [node for node in self.filtered_data["nodes"] if node["id"] in node_list]
            self.filtered_data["nodes"] = nodes
            graph_data = self.filtered_data
        except BaseException as err:
            graph_data = self.data
            _LOGGER.exception("wrong node filter query!!", exc_info=err)
        return graph_data
    ...
    def _callback_filter_edges(self, graph_data, filter_edges_text):
        ...
        try:
            ...
            edges = [edge for edge in self.filtered_data["edges"] if edge["id"] in edges_list]
            self.filtered_data["edges"] = edges
            graph_data = self.filtered_data
        except BaseException as err:
            graph_data = self.data
            _LOGGER.exception("wrong edge filter query!!", exc_info=err)
        return graph_data
