    ...
    def convert_excel_to_pdf(self,filename,file_storage_path):
        ...
        if file_extension == '.xls' or file_extension == '.xlsx':
            ...
            try:
                ...
                styleN.wordWrap = 'CJK'  # Allow smart wrapping without breaking words
                ...
                for row_idx, row in enumerate(data):
                    ...
                    for cell in row:
                        ...
                        if row_idx != 0:
                            # First row (header) — use Paragraph
                            wrapped_row.append(Paragraph(clean_cell, styleN))
                        else:
                            # Other rows — just plain text
                            wrapped_row.append(clean_cell)
                ...
                for col in range(num_cols):
                    max_len = max((len(row[col]) for row in data if row[col]), default=0)
                    max_lengths.append(max_len)

                # Assign base width according to max length
                col_widths = []
                for max_len in max_lengths:
                    if max_len > 50:
                        col_widths.append(usable_width * 0.2)  # Big text, big width
                    elif max_len > 20:
                        col_widths.append(usable_width * 0.1)  # Medium text
                    else:
                        col_widths.append(usable_width * 0.05)  # Small text

                # Scale widths to exactly fit usable_width
                total_width = sum(col_widths)
                scaling_factor = usable_width / total_width if total_width else 1
