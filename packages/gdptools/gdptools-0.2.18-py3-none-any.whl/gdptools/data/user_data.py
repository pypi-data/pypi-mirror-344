...
class UserTiffData(UserData):
    ...
    def __init__(
        ...
        """Data class used for zonal stats processing of geotiffs.

        Args:
            ds (Union[str, xr.Dataset]): Source XArray dataset or path to dataset. The values from this gridded data
                will be aggregated to the input target vector data features.
            proj_ds (Any): The crs info for the source gridded data.
                Any acceptable input to pyproj.crs.CRS.from_user_input().
            x_coord (str): Name of the x-coordinate of the source gridded data.
            y_coord (str): Name of the y-coordinate of the source gridded data.
            f_feature (Union[str, Path, gpd.GeoDataFrame]): Target GeoDataFrame or path to GeoDataFrame. These vector
                features will have the gridded data values aggregated to them.
            id_feature (str): ID of the feature of the vector features to identify the results with.
            bname (str): Name of the band; generally band.
            band (int): Band to use; generally 1.
            var (str) = A place holder name for the raster dataset. Required since Xarray is reading the data.
        """
