...
class RBACService:
    ...
    def create_role(
        ...
        Raises:
            DuplicateRoleError: If a role with the same name already exists
            PermissionNotFoundError: If any of the provided permission IDs don't exist
        """
        with self.db() as session:
            try:
    ...
    def get_role_with_permissions(self, role_uuid: str) -> Any:
        ...
        with self.db() as session:
            ...
            if not role:
                raise RoleNotFoundError(detail="Requested role does not exist")
    ...
    def get_user_permissions(self, user_uuid: str) -> List[Any]:
        ...
        with self.db() as session:
            ...
            if not user_roles:
                raise RoleNotFoundError(detail="User has no roles to get permissions.")
            ...
            try:
                ...
                if new_role_uuids:
                    ...
                    if len(new_roles) != len(new_role_uuids):
                        raise RoleNotFoundError(detail="One or more roles not found")
