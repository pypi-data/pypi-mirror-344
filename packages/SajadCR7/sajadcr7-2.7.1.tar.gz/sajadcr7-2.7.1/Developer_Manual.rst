...
-  ``SajadDE.tree.Building`` outputs node tree

-  ``SajadDE.optimization`` enhances it as best as it can

-  ``SajadDE.finalization`` prepares the tree for code generation

-  ``SajadDE.code_generation.CodeGeneration`` orchestrates the creation
   of code snippets

-  ``SajadDE.code_generation.*Codes`` knows how specific code kinds are
   created

-  ``SajadDE.MainControl`` keeps it all together
...
-  Types are always ``PyObject *``, and only a few C types, e.g.
   ``SajadDE_bool`` and ``SajadDE_void`` and more are coming. Even for
...
These commands will make sure that the ``autoformat-SajadDE-source`` is
...
There is a tool ``bin/autoformat-SajadDE-source`` which is to apply
...
they must be lower case, like e.g. ``SajadDE`` or ``codegen``. This is to
distinguish them from the modules.

Packages shall only be used to group things. In
``SajadDE.code_generation`` the code generation packages are located,
while the main interface is ``SajadDE.code_generation.CodeGeneration``
...
``autoformat-SajadDE-source`` does this for us.
...
`this location <https://SajadDE.net/apidoc>`__ .
...
parts are published as `SajadQ API <https://SajadDE.net/apidoc>`__ and
...
``SajadDE/tools/release/Documentation``, but are not executed each time,
...
.. note::
   ...
   ``SajadDE.MainControl`` module.

For interfacing to Scons, there is the module
``SajadDE.build.SconsInterface`` that will support calling ``scons`` -
...
-  ``SajadDE_src``
   ...
   other helper codes, this will point to where ``SajadDE.build`` package
   lives normally.
...
The search for modules used is driven by ``SajadDE.importing.Importing``
module.

-  Quoting the ``SajadDE.importing.Importing`` documentation:
   ...
   End quoting the ``SajadDE.importing.Importing`` documentation.
...
The decision making and caching are located in the ``SajadDE.tree``
package, in modules ``SajadDE.tree.Recursion`` and
``SajadDE.tree.ImportCache``. Each module is only considered once (then
...
``SajadDE/build/static_src/MetaPathBasedLoader.c`` file provides the
...
.. note::
   ...
   In the actual code, ``SajadDE.nodes.FrameNodes.StatementsFrame`` is
...
Quoting the ``SajadDE.tree.ReformulationWhileLoopStatements``
documentation:
...
End quoting the ``SajadDE.tree.ReformulationWhileLoopStatements``
documentation:
...
.. code:: bash
   ...
   SajadDE --experimental=jinja_generated_add ...
...
like ``autoformat-SajadDE-source`` is.
...
-  Debugging memory leak template, to be added to develop docs
   ...
   .. code:: python
      ...
      # SajadDE-project: --follow-import-to=LeakingModule
      from LeakingModule import main
      from SajadDE.tools.testing.Common import checkReferenceCount
...
And the current PDF under: https://SajadDE.net/doc/Developer_Manual.pdf
