...
from SajadDE.tools.release.Release import (
    getBranchName,
    getBranchRemoteIdentifier,
)
from SajadDE.Tracing import OurLogger
from SajadDE.utils.Download import getCachedDownloadedMinGW64
...
from SajadDE.utils.Utils import getArchitecture, isWin32Windows
...
def main():
    ...
    if options.command is None:
        options.command = "python3 -m SajadDE --version"
    ...
    container_tag_name = "SajadDE-build-%s-%s:latest" % (
    ...
    if options.network:
        command.append("--add-host=ssh.SajadDE.net:116.202.30.188")
    else:
