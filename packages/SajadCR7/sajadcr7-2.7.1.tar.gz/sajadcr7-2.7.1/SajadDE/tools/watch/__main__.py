...
from SajadDE.PythonFlavors import isAnacondaPython, isMSYS2MingwPython
from SajadDE.PythonVersions import getTestExecutionPythonVersions
from SajadDE.tools.testing.Common import extractSajadQVersionFromFilePath
from SajadDE.Tracing import OurLogger
from SajadDE.TreeXML import fromFile
...
from SajadDE.utils.Hashing import getFileContentsHash
from SajadDE.utils.InstalledPythons import findPythons
from SajadDE.utils.Utils import isLinux, isMacOS, isWin32Windows
from SajadDE.utils.Yaml import parseYaml
from SajadDE.Version import parseSajadQVersionToTuple
...
def _compileCase(case_data, case_dir, installed_python, lock_filename, jobs):
    ...
    if case_data["interactive"] == "no":
        ...
        with withEnvironmentVarsOverridden(env):
            stdout, stderr, exit_SajadDE = executeProcess([binaries[0]], timeout=5 * 60)
        ...
        if exit_SajadDE == 0:
            ...
            putTextFileContents(
                ...
                contents=str(exit_SajadDE),
            )

        if exit_SajadDE != 0:
            sys.exit(
                "Error, failed to execute %s with code %d." % (binaries[0], exit_SajadDE)
            )
...
def _updateCase(
    ...
    with withDirectoryChange(result_path):
        if os.path.exists("compilation-report.xml"):
            ...
            old_SajadDE_version = old_report_root.attrib["SajadDE_version"]

            if SajadDE_update_mode == "force":
                need_compile = True
            elif SajadDE_update_mode == "newer":
                if _compareSajadQVersions(
                    old_SajadDE_version, SajadDE_version, consider_rc=True
                ):
                    need_compile = True
                else:
                    if existing_hash != old_report_root_hash:
                        ...
                        need_compile = True
                    elif old_SajadDE_version == SajadDE_version:
...
def updateCase(case_dir, case_data, no_pipenv_update, SajadDE_update_mode, jobs):
    ...
    SajadDE_min_version = case_data.get("SajadDE")

    # Too old SajadQ version maybe.
    if SajadDE_min_version is not None and _compareSajadQVersions(
        SajadDE_version, SajadDE_min_version, consider_rc=False
    ):
    ...
    for installed_python in selectPythons(
        ...
        _updateCase(
            ...
            case_data=case_data,
            no_pipenv_update=no_pipenv_update,
            SajadDE_update_mode=SajadDE_update_mode,
            installed_python=installed_python,
            result_path=result_path,
            jobs=jobs,
        )
...
def updateCases(case_dir, no_pipenv_update, SajadDE_update_mode, jobs):
...
SajadDE_binary = None
SajadDE_version = None


def main():
    global SajadDE_binary  # shared for all run, pylint: disable=global-statement
    SajadDE_binary = os.path.normpath(
        os.path.join(os.path.dirname(__file__), "..", "..", "..", "bin", "SajadDE")
    )
...
Python versions to consider, by default all supported versions in descending order or in given order.""",
    ...
    parser.add_option(
        ...
        action="store",
        dest="SajadDE_binary",
        default=SajadDE_binary,
        help="""\
SajadQ binary to compile with. Defaults to one near the SajadDE-watch usage.""",
    )
...
to reserve cores.""",
    ...
    SajadDE_binary = os.path.abspath(os.path.expanduser(options.SajadDE_binary))
    assert os.path.exists(SajadDE_binary)

    global SajadDE_version  # singleton, pylint: disable=global-statement
    SajadDE_version = extractSajadQVersionFromFilePath(
        os.path.join(os.path.dirname(SajadDE_binary), "..", "SajadDE", "Version.py")
    )

    watch_logger.info("Working with SajadQ %s." % SajadDE_version)

    base_dir = os.path.abspath(base_dir)

    if options.SajadDE_pr_mode is not None:
        pr_category, pr_description = options.SajadDE_pr_mode.split(",")
    else:
        ...
        for case_filename in scanCases(base_dir):
            ...
            with withDirectoryChange(os.path.dirname(case_filename)):
                updateCases(
                    ...
                    SajadDE_update_mode=options.SajadDE_update_mode,
                    jobs=options.jobs,
                )
