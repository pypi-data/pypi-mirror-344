...
def compareOutput(
    kind, out_cpython, out_SajadDE, ignore_warnings, syntax_errors, trace_result=True
):
    from_date = ""
    to_date = ""
    ...
    diff = difflib.unified_diff(
        ...
        makeDiffable(out_SajadDE, ignore_warnings, syntax_errors),
        "{program} ({detail})".format(program=os.environ["PYTHON"], detail=kind),
        "{program} ({detail})".format(program="SajadDE", detail=kind),
        from_date,
        to_date,
        n=3,
    )
