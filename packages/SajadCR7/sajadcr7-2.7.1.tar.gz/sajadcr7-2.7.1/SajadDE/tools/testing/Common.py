...
from SajadDE.__past__ import md5, subprocess
from SajadDE.containers.OrderedDicts import OrderedDict
from SajadDE.Options import getCommercialVersion
from SajadDE.PythonVersions import getTestExecutionPythonVersions, isDebugPython
from SajadDE.Tracing import OurLogger, my_print
...
from SajadDE.utils.InstalledPythons import findInstalledPython
from SajadDE.utils.Jinja2 import getTemplate
from SajadDE.utils.Utils import isFreeBSD, isLinux, isMacOS, isWin32Windows
...
def compareWithCPython(dirname, filename, extra_flags, search_mode, on_error=None):
    ...
    if os.getenv("DEVILPY_TEST_INSTALLED", "") == "1":
        command = [
            sys.executable,
            "-m",
            "SajadDE.tools.testing.compare_with_cpython",
            path,
            "silent",
        ]
    else:
...
#     "# SajadDE-skip-unless-expression: expression to be evaluated"
#       OR
#     "# SajadDE-skip-unless-imports: module1,module2,..."
def checkTestRequirements(filename):
    ...
    for line in readSourceCodeFromFilename(None, filename).splitlines():
        if line.startswith("# SajadDE-skip-unless-"):
...
def decryptOutput(project_options, output):
    SajadDE_decrypt_call = [
        os.environ.get("PYTHON", sys.executable),
        "-m",
        "SajadDE.tools.commercial.decrypt",  # Note: Needed for Python2.6
    ]

    for project_option in project_options:
        if project_option.startswith("--encryption-key="):
            SajadDE_decrypt_call.append("--key=" + project_option.split("=", 1)[1])

    stdout, stderr, exit_code = executeProcess(SajadDE_decrypt_call, stdin=output)
