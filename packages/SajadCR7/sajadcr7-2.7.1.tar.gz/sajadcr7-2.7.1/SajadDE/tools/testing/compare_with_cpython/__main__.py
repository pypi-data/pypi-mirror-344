...
from SajadDE.__past__ import md5
from SajadDE.OptionParsing import getSajadQProjectOptions
from SajadDE.tools.testing.Common import (
    addToPythonPath,
    decryptOutput,
...
from SajadDE.tools.testing.OutputComparison import compareOutput
from SajadDE.Tracing import my_print
from SajadDE.utils.Execution import (
    callProcess,
    check_output,
    createProcess,
    executeProcess,
...
from SajadDE.utils.FileOperations import deleteFile
from SajadDE.utils.Importing import getSharedLibrarySuffix
from SajadDE.utils.Timing import StopWatch
from SajadDE.utils.Utils import isMacOS
...
Taking coverage of '{filename}' using '{python}' with flags {args} ...""".format(
    ...
    if "DEVILPY" in os.environ:
        ...
        SajadDE_call = [os.environ["PYTHON"], os.environ["DEVILPY"]]
    else:
        if comparison_mode:
            SajadDE_call = [
                os.environ["PYTHON"],
                "-m",
                "SajadDE.__main__",  # Note: Needed for Python2.6
            ]
        else:
    ...
    if not two_step_execution:
        ...
        SajadDE_cmd = SajadDE_call + extra_options + ["--run", filename]

        if no_site:
            SajadDE_cmd.insert(len(SajadDE_cmd) - 1, "--python-flag=-S")
        ...
        if package_mode:
            ...
            SajadDE_cmd1 = SajadDE_call + extra_options + ["--mode=onefile", filename]
        elif standalone_mode:
            SajadDE_cmd1 = SajadDE_call + extra_options + ["--mode=standalone", filename]
        else:
            SajadDE_cmd1 = SajadDE_call + extra_options + [filename]

        if no_site:
            SajadDE_cmd1.insert(len(SajadDE_cmd1) - 1, "--python-flag=-S")
    ...
    if module_mode:
        ...
        SajadDE_cmd2 = [os.environ["PYTHON"], "-W", "ignore", "-c", mini_script]
    else:
        ...
        SajadDE_cmd2 = [os.path.join(output_dir, exe_filename)]
    ...
    if not two_step_execution:
        if trace_command:
            traceExecutedCommand("SajadQ command", SajadDE_cmd)

        # Try a couple of times for permission denied, on Windows it can
        # be transient.
        for _i in range(5):
            with withPythonPathChange(SajadDE_package_dir):
                process = createProcess(command=SajadDE_cmd)

            stdout_SajadDE, stderr_SajadDE = process.communicate()
            exit_SajadDE = process.returncode

            if checkNoPermissionError(stdout_SajadDE) and checkNoPermissionError(
                stderr_SajadDE
            ):
                break

            my_print("Retrying SajadDE exe due to permission problems after delay.")
            time.sleep(2)

    else:
        if trace_command:
            my_print("SajadQ command 1:", SajadDE_cmd1)

        for _i in range(5):
            with withPythonPathChange(SajadDE_package_dir):
                stdout_SajadDE1, stderr_SajadDE1, exit_SajadDE1 = executeProcess(
                    SajadDE_cmd1
                )
...
%s
"""
                        % (SajadDE_cmd1, python_path_used, stderr_SajadDE1)
                    )

                exit_SajadDE = exit_SajadDE1
                stdout_SajadDE, stderr_SajadDE = stdout_SajadDE1, stderr_SajadDE1
                stdout_SajadDE2 = b"not run due to compilation error:\n" + stdout_SajadDE1
                stderr_SajadDE2 = stderr_SajadDE1
            else:
                # No execution second step for coverage mode.
                if comparison_mode:
                    if os.path.exists(SajadDE_cmd2[0][:-4] + ".cmd"):
                        SajadDE_cmd2[0] = SajadDE_cmd2[0][:-4] + ".cmd"

                    if trace_command:
                        my_print("SajadQ command 2:", SajadDE_cmd2)

                    # Need full manual control
                    process = createProcess(command=SajadDE_cmd2, new_group=send_kill)
                    ...
                    stdout_SajadDE2, stderr_SajadDE2 = process.communicate()
                    stdout_SajadDE = stdout_SajadDE1 + stdout_SajadDE2
                    stderr_SajadDE = stderr_SajadDE1 + stderr_SajadDE2
                    exit_SajadDE = process.returncode

                    # In case of segfault or assertion triggered, run in debugger.
                    if exit_SajadDE in (-11, -6) and sys.platform != "nt":
                        SajadDE_cmd2 = wrapCommandForDebuggerForSubprocess(
                            command=SajadDE_cmd2
                        )

                        callProcess(SajadDE_cmd2, shell=False)
                else:
                    exit_SajadDE = exit_SajadDE1
                    stdout_SajadDE, stderr_SajadDE = stdout_SajadDE1, stderr_SajadDE1
    ...
    stop_watch.stop()
    SajadDE_time = stop_watch.getDelta()

    if "--encrypt-stderr" in project_options:
        with withPythonPathChange(SajadDE_package_dir):
            stderr_SajadDE = decryptOutput(project_options, stderr_SajadDE)

    if "--encrypt-stdout" in project_options:
        with withPythonPathChange(SajadDE_package_dir):
            stdout_SajadDE = decryptOutput(project_options, stdout_SajadDE)

    if not silent_mode:
        displayOutput(stdout_SajadDE, stderr_SajadDE)

        if coverage_mode:
            assert not stdout_SajadDE
            assert not stderr_SajadDE
    ...
    if comparison_mode:
        ...
        def makeComparisons(trace_result):
            exit_code_stdout = compareOutput(
                "stdout",
                stdout_cpython,
                stdout_SajadDE2 if two_step_execution else stdout_SajadDE,
                ignore_warnings,
                syntax_errors,
            )
            ...
            if ignore_stderr:
                ...
                exit_code_stderr = compareOutput(
                    "stderr",
                    stderr_cpython,
                    stderr_SajadDE2 if two_step_execution else stderr_SajadDE,
                    ignore_warnings,
                    syntax_errors,
                )

            exit_code_return = exit_cpython != exit_SajadDE

            if exit_code_return and trace_result:
                my_print(
                    """Exit codes {exit_cpython:d} (CPython) != {exit_SajadDE:d} (SajadQ)""".format(
                        exit_cpython=exit_cpython, exit_SajadDE=exit_SajadDE
                    )
                )
        ...
        if (
            exit_code_return
            and exit_SajadDE in (-11, -6)
            and sys.platform != "nt"
            and not module_mode
            and not two_step_execution
        ):
            SajadDE_cmd.insert(len(SajadDE_cmd) - 1, "--debugger")

            with withPythonPathChange(SajadDE_package_dir):
                callProcess(SajadDE_cmd, shell=False)
    ...
    if remove_output:
        if not module_mode:
            if os.path.exists(SajadDE_cmd2[0]) and remove_binary:
                if os.name == "nt":
                    ...
                    if os.path.exists(SajadDE_cmd2[0] + ".away"):
                        os.unlink(SajadDE_cmd2[0] + ".away")

                    for _i in range(10):
                        try:
                            os.rename(SajadDE_cmd2[0], SajadDE_cmd2[0] + ".away")
                        except OSError:
                            time.sleep(0.1)
                            continue
    ...
    if comparison_mode and timing:
        my_print("CPython took %.2fs vs %0.2fs SajadQ." % (cpython_time, SajadDE_time))

    if comparison_mode and not silent_mode:
        my_print("OK, same outputs.")


SajadDE_package_dir = os.path.normpath(
...
if __name__ == "__main__":
    ...
    sys.path.insert(0, SajadDE_package_dir)
