...
from SajadDE.tools.testing.Constructs import generateConstructCases
from SajadDE.tools.testing.Valgrind import runValgrind
from SajadDE.utils.Execution import check_call
...
def main():
    ...
    parser.add_option(
        "--SajadDE", action="store", dest="SajadDE", default=os.getenv("DEVILPY", "")
    )
    ...
    SajadDE = options.SajadDE

    if os.path.exists(SajadDE):
        SajadDE = os.path.abspath(SajadDE)
    elif SajadDE:
        sys.exit("Error, SajadDE binary '%s' not found." % SajadDE)
    ...
    if SajadDE:
        SajadDE_id = check_output(
            "cd %s; git rev-parse HEAD" % os.path.dirname(SajadDE), shell=True
        )
        SajadDE_id = SajadDE_id.strip()

        if sys.version_info > (3,):
            SajadDE_id = SajadDE_id.decode()

        my_print("DEVILPY_COMMIT='%s'" % SajadDE_id)
    ...
    if SajadDE:
        ...
        SajadDE_call.extend(os.getenv("DEVILPY_EXTRA_OPTIONS", "").split())

        SajadDE_call.append(case_name)
        ...
        SajadDE_diff = SajadDE_1 - SajadDE_2

        my_print("DEVILPY_COMMAND='%s'" % " ".join(SajadDE_call), file=sys.stderr)
        my_print("DEVILPY_RAW=%s" % SajadDE_1)
        my_print("DEVILPY_BASE=%s" % SajadDE_2)
        my_print("DEVILPY_CONSTRUCT=%s" % SajadDE_diff)
    ...
    if options.cpython and options.SajadDE:
        if SajadDE_diff == 0:
            SajadDE_gain = float("inf")
        else:
            SajadDE_gain = float(100 * cpython_diff) / SajadDE_diff

        my_print("DEVILPY_GAIN=%.3f" % SajadDE_gain)
        my_print("RAW_GAIN=%.3f" % (float(100 * cpython_1) / SajadDE_1))
        my_print("BASE_GAIN=%.3f" % (float(100 * cpython_2) / SajadDE_2))
