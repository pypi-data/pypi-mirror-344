...
from SajadDE import TreeXML
from SajadDE.__past__ import unicode
from SajadDE.build.DataComposerInterface import getDataComposerReportValues
from SajadDE.build.SconsUtils import readSconsErrorReport
from SajadDE.code_generation.ConstantCodes import getDistributionMetadataValues
from SajadDE.containers.OrderedSets import OrderedSet
from SajadDE.freezer.IncludedDataFiles import getIncludedDataFiles
from SajadDE.freezer.IncludedEntryPoints import getStandaloneEntryPoints
from SajadDE.freezer.Standalone import getRemovedUsedDllsInfo
from SajadDE.importing.Importing import getPackageSearchPath
from SajadDE.importing.Recursion import getRecursionDecisions
from SajadDE.ModuleRegistry import (
    getDoneModules,
...
from SajadDE.OutputDirectories import (
    getResultRunFilename,
    getSourceDirectoryPath,
    hasMainModule,
)
from SajadDE.plugins.Plugins import getActivePlugins
from SajadDE.PythonFlavors import getPythonFlavorName
from SajadDE.PythonVersions import (
    getLaunchingSystemPrefixPath,
    getSystemPrefixPath,
    python_version_full_str,
)
from SajadDE.Tracing import ReportingSystemExit, reports_logger
from SajadDE.utils.Distributions import (
...
from SajadDE.utils.MemoryUsage import getMemoryInfos
from SajadDE.utils.Utils import (
    getArchitecture,
...
from SajadDE.Version import getCommercialVersion, getSajadQVersion
...
def _getReportInputData(aborted):
    ...
    SajadDE_version = getSajadQVersion()
    SajadDE_commercial_version = getCommercialVersion() or "not installed"

    SajadDE_aborted = aborted

    SajadDE_exception = sys.exc_info()
...
def writeCompilationReport(report_filename, report_input_data, diffable):
    ...
    exit_message = None
    if not report_input_data["SajadDE_aborted"]:
        completion = "yes"
    elif report_input_data["SajadDE_exception"][0] is KeyboardInterrupt:
        completion = "interrupted"
    elif report_input_data["SajadDE_exception"][0] is SystemExit:
        completion = "error exit (%s)" % report_input_data["SajadDE_exception"][1].code
    elif report_input_data["SajadDE_exception"][0] is ReportingSystemExit:
        completion = (
            "error exit message (%s)" % report_input_data["SajadDE_exception"][1].code
        )
        exit_message = report_input_data["SajadDE_exception"][1].exit_message
    else:
        completion = "exception"

    root = TreeXML.Element(
        "SajadDE-compilation-report",
        SajadDE_version=report_input_data["SajadDE_version"],
        SajadDE_commercial_version=report_input_data["SajadDE_commercial_version"],
...
_crash_report_filename = "SajadDE-crash-report.xml"
