...
from SajadDE.__past__ import Iterable, unicode
from SajadDE.containers.OrderedSets import OrderedSet
from SajadDE.importing.Importing import (
...
from SajadDE.reports.CompilationReportReader import (
    getEmbeddedDataFilenames,
    parseCompilationReport,
)
from SajadDE.Tracing import wheel_logger
from SajadDE.utils.Execution import check_call
from SajadDE.utils.FileOperations import deleteFile, getFileList, renameFile
from SajadDE.utils.ModuleNames import ModuleName


def setupSajadQDistutilsCommands(dist, keyword, value):
    # If the user project setup.py includes the key "build_with_SajadDE=True" all
    ...
    dist.cmdclass["bdist_wheel"] = bdist_SajadDE
    ...
    def _build(self, build_lib):
        ...
        for is_package, module_name in self._findBuildTasks():
            ...
            if toml_filename:
                ...
                for option, value in toml_options.get("SajadDE", {}).items():
                    command.extend(self._parseOptionsEntry(option, value))

                for option, value in (
                    toml_options.get("tool", {}).get("SajadDE", {}).items()
                ):
                    command.extend(self._parseOptionsEntry(option, value))
            ...
            if "SajadDE" in self.distribution.command_options:
...
# Required by distutils, used as command name, pylint: disable=invalid-name
class bdist_SajadDE(wheel.bdist_wheel.bdist_wheel):
