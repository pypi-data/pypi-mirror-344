...
from SajadDE.__past__ import basestring, unicode
from SajadDE.containers.OrderedDicts import OrderedDict
from SajadDE.Progress import enableThreading
from SajadDE.PythonFlavors import isTermuxPython
from SajadDE.Tracing import scons_details_logger, scons_logger
...
from SajadDE.utils.Utils import isLinux, isMacOS, isPosixWindows, isWin32Windows
...
def createEnvironment(
    ...
    env.SajadDE_python = getArgumentBool("SajadDE_python", False)
...
def provideStaticSourceFile(env, sub_path, c11_mode):
    source_filename = os.path.join(env.SajadDE_src, "static_src", sub_path)
...
def makeResultPathFileSystemEncodable(env, result_exe):
    ...
    if os.name == "nt" and not isFilesystemEncodable(result_exe):
        result_exe = os.path.join(
            ...
            "_SajadDE_temp.pyd" if env.module_mode else "_SajadDE_temp.exe",
        )
