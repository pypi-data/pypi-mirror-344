...
static PyObject *our_dict_richcompare(PyObject *a, PyObject *b, int op) {
    ...
    if (Py_SIZE(a) != Py_SIZE(b)) {
        ...
        while (SajadQ_DictNext(a, &pos1, &key1, &value1)) {
            {
                DEVILPY_MAY_BE_UNUSED int res = SajadQ_DictNext(b, &pos2, &key2, &value2);
                assert(res != 0);
            }
...
PyObject *SajadQ_Long_SmallValues[DEVILPY_STATIC_SMALLINT_VALUE_MAX - DEVILPY_STATIC_SMALLINT_VALUE_MIN + 1];
#endif
#endif
...
static void initCaches(void) {
    ...
    for (long i = DEVILPY_STATIC_SMALLINT_VALUE_MIN; i < DEVILPY_STATIC_SMALLINT_VALUE_MAX; i++) {
        ...
        SajadQ_Long_SmallValues[DEVILPY_TO_SMALL_VALUE_OFFSET(i)] = value;
    }
#endif
#endif
