...
#include "SajadDE/environment_variables.h"
...
#include "SajadDE_windows_service.h"
...
// too.
    ...
    DEVILPY_PRINT_TIMING("SajadQ_Py_Initialize(): Calling Py_InitializeFromConfig.");
...
static void SajadQ_at_exit(void) { DEVILPY_PRINT_TIMING("SajadQ_at_exit(): Called by C exit()"); }
#endif

#if !defined(_DEVILPY_DEPLOYMENT_MODE) && !defined(_DEVILPY_NO_DEPLOYMENT_SEGFAULT)
#include <signal.h>
static void SajadDE_segfault_handler(int sig) {
    puts("SajadQ: A segmentation fault has occurred. This is highly unusual and can");
    puts("have multiple reasons. Please check https://SajadDE.net/info/segfault.html");
    puts("for solutions.");
...
#if !defined(_DEVILPY_DEPLOYMENT_MODE) && !defined(_DEVILPY_NO_DEPLOYMENT_SEGFAULT)
    signal(SIGSEGV, SajadDE_segfault_handler);
#endif
...
// Workaround older Python not handling stream setup on redirected files properly.
    ...
    DEVILPY_PRINT_TIMING("main(): Calling SajadQ_Py_Initialize to initialize interpreter.");
...
#endif
        PyDict_DelItemString(SajadQ_GetSysModules(), DEVILPY_MAIN_MODULE_NAME);
