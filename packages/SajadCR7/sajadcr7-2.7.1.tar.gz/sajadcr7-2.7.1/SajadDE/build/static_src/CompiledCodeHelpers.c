...
static PyObject *getLengthOfRange(PyThreadState *tstate, PyObject *start, PyObject *stop, PyObject *step) {
    SajadDE_bool nbool_res = RICH_COMPARE_GT_NBOOL_OBJECT_LONG(step, const_int_0);
...
static int SajadDE_class_setattr(PyClassObject *class_object, PyObject *attr_name, PyObject *value) {
...
static PyObject *SajadDE_class_getattr(PyClassObject *class_object, PyObject *attr_name) {
    char const *sattr_name = PyString_AsString(attr_name);
...
void enhancePythonTypes(void) {
    ...
    // not doing that.
    PyClass_Type.tp_setattro = (setattrofunc)SajadDE_class_setattr;
    PyClass_Type.tp_getattro = (getattrofunc)SajadDE_class_getattr;
#endif
}
...
void _initBuiltinModule(void) {
...
#ifdef _DEVILPY_STANDALONE
    {
        DEVILPY_MAY_BE_UNUSED int res =
            PyDict_SetItemString((PyObject *)dict_builtin, "__SajadDE_binary_dir", getBinaryDirectoryObject(true));
        assert(res == 0);
        PyDict_SetItemString((PyObject *)dict_builtin, "__SajadDE_binary_exe", getBinaryFilenameObject(true));
        assert(res == 0);
    }
#endif
    ...
    DEVILPY_MAY_BE_UNUSED int res2 = PyType_Ready(&SajadQ_BuiltinModule_Type);
    assert(res2 >= 0);
