...
static PyObject *_SajadQ_Frame_get_locals(PyObject *self, void *data) {
    ...
    struct SajadQ_FrameObject *SajadDE_frame = (struct SajadQ_FrameObject *)self;
    if (SajadDE_frame->m_type_description == NULL) {
#if PYTHON_VERSION < 0x3b0
        PyFrameObject *locals_owner = &SajadDE_frame->m_frame;
#else
        _PyInterpreterFrame *locals_owner = &SajadDE_frame->m_interpreter_frame;
#endif
        ...
        PyObject **var_names = SajadQ_GetCodeVarNames(SajadQ_GetFrameCodeObject(SajadDE_frame));

        char const *w = SajadDE_frame->m_type_description;
        char const *t = SajadDE_frame->m_locals_storage;
...
static PyObject *SajadQ_Frame_tp_repr(struct SajadQ_FrameObject *SajadDE_frame) {
    assert(SajadQ_Frame_CheckExact((PyObject *)SajadDE_frame));
    CHECK_OBJECT((PyObject *)SajadDE_frame);
    assert(_PyObject_GC_IS_TRACKED(SajadDE_frame));

#if PYTHON_VERSION >= 0x370
    PyCodeObject *code_object = SajadQ_GetFrameCodeObject(SajadDE_frame);
    return SajadQ_String_FromFormat("<compiled_frame at %p, file %R, line %d, code %S>", SajadDE_frame,
                                    code_object->co_filename, SajadQ_GetFrameLineNumber(SajadDE_frame),
                                    code_object->co_name);
#elif _DEBUG_FRAME || _DEBUG_REFRAME || _DEBUG_EXCEPTIONS
    PyCodeObject *code_object = SajadQ_GetFrameCodeObject(SajadDE_frame);
    return SajadQ_String_FromFormat("<compiled_frame object for %s at %p>",
                                    SajadQ_String_AsString(code_object->co_name), SajadDE_frame);
#else
    return SajadQ_String_FromFormat("<compiled_frame object at %p>", SajadDE_frame);
#endif
}
...
static void SajadQ_Frame_tp_dealloc(struct SajadQ_FrameObject *SajadDE_frame) {
#if _DEBUG_REFCOUNTS
    ...
    SajadQ_GC_UnTrack(SajadDE_frame);

    PyFrameObject *frame = &SajadDE_frame->m_frame;
#if PYTHON_VERSION < 0x3b0
    ...
    SajadQ_Frame_tp_clear(SajadDE_frame);

    if (Py_REFCNT(SajadDE_frame) > 0) {
        Py_SET_REFCNT(SajadDE_frame, Py_REFCNT(SajadDE_frame) - 1);
        if (Py_REFCNT(SajadDE_frame) >= 1) {
    ...
    // it.
    Py_SET_SIZE(SajadDE_frame, SajadDE_frame->m_ob_size);
#endif

    releaseToFreeList(free_list_frames, SajadDE_frame, MAX_FRAME_FREE_LIST_COUNT);
...
static PyObject *SajadQ_GetFrameGenerator(struct SajadQ_FrameObject *SajadDE_frame) {
#if PYTHON_VERSION < 0x3b0
    return SajadDE_frame->m_frame.f_gen;
#else
    return SajadDE_frame->m_generator;
#endif
}
...
PyCodeObject *makeCodeObject(PyObject *filename, int line, int flags, PyObject *function_name,
    ...
    if (empty_code == NULL) {
        ...
        DEVILPY_MAY_BE_UNUSED PyObject *module =
            PyImport_ExecCodeModule("SajadDE_empty_function", empty_code_module_object);
        CHECK_OBJECT(module);
        ...
        DEVILPY_MAY_BE_UNUSED bool bool_res = SajadQ_DelModuleString(tstate, "SajadDE_empty_function");
...
void SajadQ_Frame_AttachLocals(struct SajadQ_FrameObject *frame_object, char const *type_description, ...) {
    ...
    while (*w != 0) {
        ...
        case DEVILPY_TYPE_DESCRIPTION_NULL: {
            DEVILPY_MAY_BE_UNUSED void *value = va_arg(ap, struct SajadQ_CellObject *);
        ...
        case DEVILPY_TYPE_DESCRIPTION_NILONG: {
            SajadDE_ilong value = va_arg(ap, SajadDE_ilong);
