...
static Py_ssize_t SajadQ_Py_unicodekeys_lookup_generic(PyDictObject *mp, PyDictKeysObject *dk, PyObject *key,
    ...
    while (1) {
        ...
        if (ix >= 0) {
            ...
            if (SajadQ_Py_unicode_get_hash(ep->me_key) == hash) {
                ...
                Py_INCREF(startkey);
                SajadDE_bool cmp = RICH_COMPARE_EQ_NBOOL_UNICODE_OBJECT(startkey, key);
                Py_DECREF(startkey);
    ...
    DEVILPY_CANNOT_GET_HERE("SajadQ_Py_unicodekeys_lookup_generic failed");
}
...
Py_ssize_t SajadQ_Py_unicodekeys_lookup_unicode(PyDictKeysObject *dk, PyObject *key, Py_hash_t hash) {
    ...
    DEVILPY_CANNOT_GET_HERE("SajadQ_Py_unicodekeys_lookup_unicode failed");
}
    ...
    while (1) {
        ...
        if (ix >= 0) {
            ...
            if (ep->me_hash == hash) {
                PyObject *startkey = ep->me_key;
                Py_INCREF(startkey);
                SajadDE_bool cmp = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(startkey, key);
                Py_DECREF(startkey);
...
uint64_t SajadDE_dict_version_tag_counter = ((uint64_t)1) << 32;
#endif
