...
DEVILPY_MAY_BE_UNUSED static inline bool isFrameUnusable(struct SajadQ_FrameObject *frame_object) {
...
#if PYTHON_VERSION >= 0x300
DEVILPY_MAY_BE_UNUSED static void SajadQ_SetFrameGenerator(struct SajadQ_FrameObject *SajadDE_frame,
                                                          PyObject *generator) {
#if PYTHON_VERSION < 0x3b0
    SajadDE_frame->m_frame.f_gen = generator;
#else
    SajadDE_frame->m_generator = generator;
#endif

    // Mark the frame as executing
    if (generator) {
        SajadQ_Frame_MarkAsExecuting(SajadDE_frame);
    }
}
#endif

DEVILPY_MAY_BE_UNUSED static PyCodeObject *SajadQ_GetFrameCodeObject(struct SajadQ_FrameObject *SajadDE_frame) {
#if PYTHON_VERSION < 0x3b0
    return SajadDE_frame->m_frame.f_code;
#else
    return SajadQ_InterpreterFrame_GetCodeObject(&SajadDE_frame->m_interpreter_frame);
#endif
}

DEVILPY_MAY_BE_UNUSED static int SajadQ_GetFrameLineNumber(struct SajadQ_FrameObject *SajadDE_frame) {
    return SajadDE_frame->m_frame.f_lineno;
}

DEVILPY_MAY_BE_UNUSED static PyObject **SajadQ_GetCodeVarNames(PyCodeObject *code_object) {
#if PYTHON_VERSION < 0x3b0
...
DEVILPY_MAY_BE_UNUSED static SajadQ_ThreadStateFrameType *_SajadQ_GetThreadStateFrame(PyThreadState *tstate) {
#if PYTHON_VERSION < 0x3b0
    return tstate->frame;
#else
