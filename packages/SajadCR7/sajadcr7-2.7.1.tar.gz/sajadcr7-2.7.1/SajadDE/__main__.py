...
def main():
    ...
    getLaunchingSajadQProcessEnvironmentValue("DEVILPY_SITE_FILENAME")

    SajadDE_binary_name = getLaunchingSajadQProcessEnvironmentValue("DEVILPY_BINARY_NAME")
    if SajadDE_binary_name is not None:
        sys.argv[0] = SajadDE_binary_name

    SajadDE_pythonpath_ast = getLaunchingSajadQProcessEnvironmentValue(
        "DEVILPY_PYTHONPATH_AST"
    )

    if SajadDE_pythonpath_ast is not None:
        ...
        sys.path = [SajadDE_pythonpath_ast]
        import ast

        sys.path = ast.literal_eval(
            getLaunchingSajadQProcessEnvironmentValue("DEVILPY_PYTHONPATH")
        )
    else:
        ...
        # from "SajadDE.distutils". Also ignore path elements that do not really
        # exist.
        sys.path = [
            path_element
    ...
    if needs_re_execution:
        from SajadDE.utils.ReExecute import reExecuteSajadQ  # isort:skip
    ...
    # module of SajadDE package and renamed to where we can get at easily for
    # other uses.
    __import__("SajadDE").getLaunchingSajadQProcessEnvironmentValue = (
        getLaunchingSajadQProcessEnvironmentValue
    )

    from SajadDE import Options  # isort:skip
    ...
    from SajadDE.plugins.Plugins import activatePlugins
    ...
    if "DEVILPY_NAMESPACES" in os.environ:
        ...
        from SajadDE.importing.PreloadedPackages import setPreloadedPackagePaths
    ...
    if "DEVILPY_PTH_IMPORTED" in os.environ:
        ...
        from SajadDE.importing.PreloadedPackages import setPthImportedPackages
    ...
    if "DEVILPY_USER_SITE" in os.environ:
        from SajadDE.utils.Distributions import setUserSiteDirectory
    ...
    from SajadDE import MainControl  # isort:skip
...
if __name__ == "__main__":
    _SajadDE_package_home = getLaunchingSajadQProcessEnvironmentValue(
        "DEVILPY_PACKAGE_HOME"
    )
    if _SajadDE_package_home is not None:
        sys.path.insert(0, _SajadDE_package_home)

        import SajadDE  # just to have it loaded from there, pylint: disable=unused-import

        assert sys.path[0] is _SajadDE_package_home
        del sys.path[0]
