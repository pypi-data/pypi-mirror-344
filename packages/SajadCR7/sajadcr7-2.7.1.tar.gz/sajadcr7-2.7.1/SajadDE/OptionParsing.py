...
do it, but updating and checking module "SajadDE.Options" values is not in
...
from SajadDE.utils.CommandLineOptions import SUPPRESS_HELP, makeOptionsParser
from SajadDE.utils.FileOperations import getFileContentByLine
...
# Indicator if we were called as "SajadDE-run" in which case we assume some
# other defaults and work a bit different with parameters.
_SajadDE_binary_name = os.path.basename(sys.argv[0])
if _SajadDE_binary_name == "__main__.py":
    _SajadDE_binary_name = "%s -m SajadDE" % os.path.basename(sys.executable)
is_SajadDE_run = _SajadDE_binary_name.lower().endswith("-run")

if not is_SajadDE_run:
    usage_template = (
...
parser = makeOptionsParser(usage=usage_template % _SajadDE_binary_name)
...
with "SajadDE-onefile-unpack" rather than a stream that only the onefile
...
def _considerPluginOptions(logger):
    ...
    from SajadDE.plugins.Plugins import (
...
def getSajadQProjectOptions(logger, filename_arg, module_mode):
    ...
    for line_number, line in enumerate(contents_by_line):
        match = re.match(b"^\\s*#(\\s*)SajadDE-project(.*?):(.*)", line)
        ...
        if match:
            ...
            if expect_block and level <= cond_level:
                sysexit(
                    line_number,
                    "Error, 'SajadDE-project-if|else' is expected to be followed by block start.",
                )
            ...
            if command == "-if":
                if not arg.endswith(":"):
                    sysexit(
                        line_number,
                        "Error, 'SajadDE-project-if' needs to start a block with a colon at line end.",
                    )
                ...
                if r is not True and r is not False:
                    sys.exit(
                        "Error, 'SajadDE-project-if' condition %r (expanded to %r) does not yield boolean result %r"
                        % (arg, expanded, r)
                    )
                ...
                if arg:
                    sysexit(
                        line_number,
                        "Error, 'SajadDE-project-else' cannot have argument.",
                    )

                if cond_level != level:
                    sysexit(
                        line_number,
                        "Error, 'SajadDE-project-else' not currently allowed after nested SajadDE-project-if.",
                    )
...
pass them to the compiled program execution."""
    ...
    return is_SajadDE_run, options, positional_args, extra_args


def runSpecialCommandsFromOptions(options):
    if options.plugin_list:
        from SajadDE.plugins.Plugins import listPlugins

        listPlugins()
        sys.exit(0)

    if options.list_package_dlls:
        from SajadDE.tools.scanning.DisplayPackageDLLs import displayDLLs

        displayDLLs(options.list_package_dlls)
        sys.exit(0)

    if options.list_package_exe:
        from SajadDE.tools.scanning.DisplayPackageDLLs import displayEXEs

        displayEXEs(options.list_package_exe)
        sys.exit(0)

    if options.list_package_data:
        from SajadDE.tools.scanning.DisplayPackageData import displayPackageData

        displayPackageData(options.list_package_data)
        sys.exit(0)

    if options.edit_module_code:
        from SajadDE.tools.general.find_module.FindModuleCode import (
            editModuleCode,
        )
    ...
    if options.create_environment_from_report:
        from SajadDE.tools.environments.CreateEnvironment import (
    ...
    if options.generate_mingw64_header:
        from SajadDE.tools.general.generate_header.GenerateHeader import (
            generateHeader,
        )
