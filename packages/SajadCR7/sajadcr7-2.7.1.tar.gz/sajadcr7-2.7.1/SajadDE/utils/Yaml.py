...
from SajadDE.Options import getUserProvidedYamlFiles
...
def getYamlPackageConfiguration():
    ...
    if _package_config is None:
        _package_config = parsePackageYaml(
            ...
            "standard.SajadDE-package.config.yml",
        )
        _package_config.update(
            parsePackageYaml(
                "SajadDE.plugins.standard",
                "stdlib2.SajadDE-package.config.yml",
            )
        )
        _package_config.update(
            parsePackageYaml(
                "SajadDE.plugins.standard",
                "stdlib3.SajadDE-package.config.yml",
            )
        )
        ...
        try:
            _package_config.update(
                parsePackageYaml(
                    "SajadDE.plugins.commercial", "commercial.SajadDE-package.config.yml"
                )
            )
        except IOError:
...
def getYamlPackageConfigurationSchemaFilename():
    ...
    return os.path.join(
        ...
        "..",
        "misc",
        "SajadDE-package-config-schema.json",
    )
