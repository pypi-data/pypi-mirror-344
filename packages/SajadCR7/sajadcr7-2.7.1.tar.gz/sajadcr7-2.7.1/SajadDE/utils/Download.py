...
from SajadDE.__past__ import urlretrieve
from SajadDE.Progress import withSajadQDownloadProgressBar
...
def getCachedDownload(
    ...
    SajadDE_download_dir = getDownloadCacheDir()

    SajadDE_download_dir = os.path.join(
        SajadDE_download_dir, os.path.basename(binary).replace(".exe", "")
    )

    if is_arch_specific:
        SajadDE_download_dir = os.path.join(SajadDE_download_dir, is_arch_specific)

    if specificity:
        SajadDE_download_dir = os.path.join(SajadDE_download_dir, specificity)

    download_path = os.path.join(SajadDE_download_dir, os.path.basename(url))
    exe_path = os.path.join(SajadDE_download_dir, binary)

    makePath(SajadDE_download_dir)
        ...
        if not os.path.isfile(exe_path) and os.path.isfile(download_path):
            ...
            try:
                ...
                for zip_info in zip_file.infolist():
                    ...
                    zip_file.extract(zip_info, SajadDE_download_dir)
