...
def reExecuteSajadQ(pgo_filename):
    ...
    if "SajadDE.__main__" in sys.modules:
        our_filename = sys.modules["SajadDE.__main__"].__file__
    else:
    ...
    setLaunchingSajadQProcessEnvironmentValue(
        ...
        os.path.dirname(os.path.abspath(sys.modules["SajadDE"].__path__[0])),
    )

    if pgo_filename is not None:
        args.append("--pgo-python-input=%s" % pgo_filename)
    else:
        setLaunchingSajadQProcessEnvironmentValue("DEVILPY_SYS_PREFIX", sys.prefix)

    # Same arguments as before.
    args += sys.argv[1:]

    from SajadDE.importing.PreloadedPackages import (
    ...
    if "site" in sys.modules:
        ...
        setLaunchingSajadQProcessEnvironmentValue("DEVILPY_SITE_FILENAME", site_filename)
    ...
    setLaunchingSajadQProcessEnvironmentValue("DEVILPY_PYTHONPATH", repr(sys.path))
    ...
    setLaunchingSajadQProcessEnvironmentValue("DEVILPY_RE_EXECUTION", "1")
