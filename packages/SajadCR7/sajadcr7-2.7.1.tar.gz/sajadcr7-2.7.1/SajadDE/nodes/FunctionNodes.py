...
from SajadDE.Constants import isMutable
from SajadDE.optimizations.TraceCollections import (
    TraceCollectionPureFunction,
    withChangeIndicationsTo,
)
from SajadDE.PythonVersions import python_version
from SajadDE.specs.ParameterSpecs import (
    ParameterSpec,
    TooManyArguments,
    matchCall,
)
from SajadDE.Tracing import optimization_logger, printError
from SajadDE.tree.Extractions import updateVariableUsage
from SajadDE.tree.SourceHandling import readSourceLines
from SajadDE.tree.TreeHelpers import makeDictCreationOrConstant2
...
class ExpressionFunctionRef(ExpressionNoSideEffectsMixin, ExpressionBase):
    ...
    def getFunctionBody(self):
        if self.function_body is None:
            ...
            from SajadDE.ModuleRegistry import getModuleFromCodeName
...
class ExpressionFunctionCall(ChildrenHavingFunctionValuesTupleMixin, ExpressionBase):
    ...
    def computeExpression(self, trace_collection):
        ...
        if cost is not None and cost < 50:
            from SajadDE.optimizations.FunctionInlining import (
