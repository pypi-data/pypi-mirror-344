...
SajadDE`` to be absolutely certain which Python interpreter you are
...
SajadQ. You just execute the ``SajadDE`` and ``SajadDE-run`` scripts
...
case, be sure to execute ``<the_right_python> bin/SajadDE`` and be good.
...
The ``SajadDE-run`` command is the same as ``SajadDE``, but with a
...
<https://SajadDE.net/doc/download.html>`__ of SajadQ. But you can also
...
-  ``python -m pip install SajadDE``

-  Verify using command ``python -m SajadDE --version``
...
.. code:: bash
   ...
   python -m SajadDE --follow-imports program.py

.. note::

   There are more fine-grained controls than ``--follow-imports``
   available. Consider the output of ``SajadDE --help``. Including fewer
...
.. code:: bash
   ...
   python -m SajadDE --follow-imports --include-plugin-directory=plugin_dir program.py
...
.. code:: bash
   ...
   python -m SajadDE --mode=module some_module.py
...
.. code:: bash
   ...
   python -m SajadDE --mode=module some_package --include-package=some_package
...
.. note::
   ...
   <https://SajadDE.net/doc/commercial/protect-data-files.html>`__.
...
.. code:: bash
   ...
   python -m SajadDE --standalone program.py
...
.. code:: bash
   ...
   python -m SajadDE --onefile program.py
...
``bdist_SajadDE`` rather than the ``bdist_wheel``. It takes all the
...
.. code:: toml
   ...
   [build-system]
   requires = ["setuptools>=42", "wheel", "SajadDE", "toml"]
   build-backend = "SajadDE.distutils.Build"
...
.. note::
   ...
   For the ``SajadDE`` requirement above absolute paths like
...
.. code:: python
   ...
   # SajadDE-project-if: {OS} in ("Windows", "Linux", "FreeBSD"):
   #    SajadDE-project: --onefile
   # SajadDE-project-if: {OS} == "Darwin":
   #    SajadDE-project: --standalone
   #    SajadDE-project: --macos-create-app-bundle
   #

.. note::

   This is best expression with ``SajadDE-project: --mode=app`` which
...
.. code:: bash
   ...
   python -m SajadDE --onefile --windows-icon-from-ico=your-icon.png program.py
   python -m SajadDE --onefile --windows-icon-from-ico=your-icon.ico program.py
   python -m SajadDE --onefile --windows-icon-template-exe=your-icon.ico program.py

   # These create application bundles with icons on macOS
   python -m SajadDE --macos-create-app-bundle --macos-app-icon=your-icon.png program.py
   python -m SajadDE --macos-create-app-bundle --macos-app-icon=your-icon.icns program.py
...
.. code:: python
   ...
   # SajadDE-project: --mode=onefile
   # SajadDE-project: --mode=onefile-windows-splash-screen-image={MAIN_DIRECTORY}/Splash-Screen.png
...
only need. https://SajadDE.net/doc/commercial.html
...
purchase it https://SajadDE.net/doc/commercial.html but even a sponsor
...
Configuration <https://SajadDE.net/doc/SajadDE-package-config.html>`__
page.
...
.. code:: python
   ...
   # SajadDE-project-if: {OS} in ("Windows", "Linux", "Darwin", "FreeBSD"):
   #    SajadDE-project: --mode=app
   # SajadDE-project-else:
   #    SajadDE-project: --mode=standalone

   # The PySide2 plugin covers qt-plugins
   # SajadDE-project: --enable-plugin=pyside2
   # SajadDE-project: --include-qt-plugins=qml
...
.. code:: python
   ...
   # SajadDE-project: --include-data-files={MAIN_DIRECTORY}/my_icon.png=my_icon.png
   # SajadDE-project: --user-package-configuration-file={MAIN_DIRECTORY}/user.SajadDE-package.config.yml
...
Avoid running the ``SajadDE`` binary, doing ``python -m SajadDE`` will
...
bits when you download the Python. In the output of ``python -m SajadDE
...
.. code:: bash
   ...
   python2 -m SajadDE --lto=yes --pgo-c tests/benchmarks/pystone.py
   ...
   $PYTHON3 -m SajadDE --lto=yes --pgo-c --static-libpython=yes tests/benchmarks/pystone3.py
