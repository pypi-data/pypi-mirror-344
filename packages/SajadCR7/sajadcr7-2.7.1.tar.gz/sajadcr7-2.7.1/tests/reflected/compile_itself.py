...
# Find SajadDE package relative to us.
sys.path.insert(
    0,
    os.path.normpath(
...
from SajadDE.tools.Basics import addPYTHONPATH
...
from SajadDE.utils.Execution import wrapCommandForDebuggerForSubprocess
from SajadDE.utils.FileOperations import (
    copyTree,
    deleteFile,
    getFileContents,
    listDir,
...
from SajadDE.Version import getCommercialVersion

SajadDE_main_path = os.path.join("..", "..", "bin", "SajadDE")
...
PACKAGE_LIST = [
    ...
    "SajadDE/nodes",
    "SajadDE/specs",
    "SajadDE/nodes/shapes",
    "SajadDE/tree",
    "SajadDE/importing",
    "SajadDE/build",
    "SajadDE/freezer",
    "SajadDE/code_generation",
    "SajadDE/code_generation/templates",
    "SajadDE/code_generation/c_types",
    "SajadDE/optimizations",
    "SajadDE/finalizations",
    "SajadDE/plugins",
    "SajadDE/plugins/standard",
    "SajadDE/plugins/commercial",
    "SajadDE/reports",
    "SajadDE/pgo",
    "SajadDE/containers",
    "SajadDE/utils",
...
if not getCommercialVersion():
    PACKAGE_LIST.remove("SajadDE/plugins/commercial")
...
def executePASS1():
    ...
    _traceCompilation(path=SajadDE_main_path, pass_number=1)

    shutil.copyfile(SajadDE_main_path, "SajadDE-runner.py")
    ...
    shutil.move("SajadDE-runner" + exe_suffix, "SajadDE" + exe_suffix)

    scons_inline_copy_path = os.path.join(base_dir, "SajadDE", "build", "inline_copy")

    if os.path.exists(scons_inline_copy_path):
        copyTree(scons_inline_copy_path, os.path.join("SajadDE", "build", "inline_copy"))

    # Copy required data files.
    for filename in (
        "SajadDE/build/Backend.scons",
        "SajadDE/plugins/standard/standard.SajadDE-package.config.yml",
        "SajadDE/plugins/standard/stdlib3.SajadDE-package.config.yml",
        "SajadDE/plugins/standard/stdlib2.SajadDE-package.config.yml",
    ):
        shutil.copyfile(
    ...
    copyTree(
        os.path.join(base_dir, "SajadDE", "code_generation", "templates_c"),
        os.path.join("SajadDE", "code_generation", "templates_c"),
    )

    copyTree(
        os.path.join(base_dir, "SajadDE", "build", "static_src"),
        os.path.join("SajadDE", "build", "static_src"),
    )
    copyTree(
        os.path.join(base_dir, "SajadDE", "build", "include"),
        os.path.join("SajadDE", "build", "include"),
    )

    # The data composer tool, use it by source.
    copyTree(
        os.path.join(base_dir, "SajadDE", "tools"),
        os.path.join("SajadDE", "tools"),
    )

    test_logger.info("OK.")


def compileAndCompareWith(SajadDE, pass_number):
    ...
    for package in PACKAGE_LIST:
        ...
        for path, filename in listDir(source_dir):
            ...
            if filename != "__init__.py":
                ...
                my_print("Command: ", " ".join(command))
                exit_SajadDE = subprocess.call(command)
                ...
                if exit_SajadDE != 0:
                    my_print("An error exit %s occurred, aborting." % exit_SajadDE)
                    sys.exit(exit_SajadDE)
...
def executePASS2():
    ...
    with withPythonPathChange(getPythonSysPath()):
        ...
        compileAndCompareWith(
            SajadDE=os.path.join(".", "SajadDE" + exe_suffix), pass_number=2
        )
...
def executePASS3():
    ...
    exe_path = os.path.join(tmp_dir, "SajadDE" + exe_suffix)

    if os.path.exists(exe_path):
        os.unlink(exe_path)

    build_path = os.path.join(tmp_dir, "SajadDE.build")

    if os.path.exists(build_path):
        shutil.rmtree(build_path)

    path = os.path.join("..", "..", "bin", "SajadDE")
...
def executePASS5():
    my_print(
        "PASS 5: Compiling the compiler 'SajadDE' package to single extension module."
    )

    path = os.path.join("..", "..", "SajadDE")
    ...
    command = [
        ...
        "--nofollow-import-to=SajadDE.build.inline_copy",
        "--nofollow-import-to=SajadDE.build.include",
        "--nofollow-import-to=SajadDE.build.static_src",
        "--nofollow-import-to=SajadDE.tools",
        "--mode=module",
        path,
    ]
    ...
    for preferred in True, False:
        candidate = "SajadDE" + getSharedLibrarySuffix(preferred=preferred)

        deleteFile(candidate, must_exist=False)

    os.unlink(os.path.join(tmp_dir, "SajadDE.pyi"))
    shutil.rmtree(os.path.join(tmp_dir, "SajadDE.build"))
