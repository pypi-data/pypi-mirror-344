...
def lint_or_fix_sql(sql, mode, config, file_path=None):
    ...
    Returns:
        str: The fixed SQL string (if mode is 'fix') or the original SQL string (if mode is 'lint').
    """
    try:
        if mode == 'fix':
...
class SQLStringTransformer(cst.CSTTransformer):
    ...
    def leave_SimpleString(self, original_node: cst.SimpleString, updated_node: cst.SimpleString) -> Union[cst.SimpleString, cst.FormattedString]:
        ...
        try:
            ...
            processed_normalized_sql = lint_or_fix_sql(normalized_sql, self.mode, self.config, self.file_path)
            ...
            return cst.SimpleString(value=quote_value + final_sql + quote_value)
        except Exception as e:
...
def modify_file_in_place(file_path, mode, config, pattern=None):
    ...
    try:
        ...
        # Count issues only if in lint mode
        if mode == 'lint':
            lint_result = sqlfluff.lint(source_code, config=config)
            if lint_result:
                issues_count += len(lint_result)
                print(f"Error: {Path(file_path)}")
                for error in lint_result:
                    print(f"  - Code: {error['code']}")
                    print(f"    Description: {error['description']}")
                    print(f"    Line: {error['start_line_no']}, Position: {error['start_line_pos']}")
                    print('-' * 40)
        
        return issues_count
    except Exception as e:
        print(f"Error processing file {file_path}: {e}")
        # Return 0 issues but the error will propagate up
        return 0