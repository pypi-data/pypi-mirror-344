...
export class ActionHandler extends DynDep implements ActionHandler {
    ...
    executeAction = ({
        ...
        /**
         * If status has param_values we keep the param_values.
         * Otherwise, if actorId and URLContext.static.actorData.id is the
         * same I keep the param_values.
         * Otherwise, when the param_values is taken from the URLContext and
         * action's actorId is NOT the same, param_values's number of keys may
         * differ between the URLContext actor and the action actor, so, we
         * throw away the param_values.
         */
        if ((!status || !this.context.filled(status.param_values)) &&
            actorId != (this.context.static.actorData || {}).id)
            delete dataParams[constants.URL_PARAM_PARAM_VALUES];
                ...
                this.context.dataContext.set(data, false, {loading: false},
                    (mutableData) => {
                        ...
                        if (context.contextType === constants.CONTEXT_TYPE_SINGLE_ROW) {
                            context.history.replace({sr: [mutableData.id]});
                            ...
                            mutableData.rows.forEach(row => {
                                if (context.value.sr.includes(row[pki]))
                                    sr.push(row[pki]);
                            });
                            context.history.replace({sr: sr});
                        }
                        if (reload) {
