...
class Renderer(JsCacheRenderer):
    ...
    def panel2json(self, p):
        ...
        # def do(klass, cd):
        #     cd.update(**getattr(klass, "_choosers_dict", {}))
        #     for cls in klass.__subclasses__():
        #         do(cls, cd)

        if isclass(p._datasource) and issubclass(p._datasource, Actor):

            # for cls in p._datasource.__subclasses__():
            #     do(cls, chooser_dict)

            chooser_dict.update(**getattr(
                p._datasource.model, "_choosers_dict", {}))
        if chooser_dict:
