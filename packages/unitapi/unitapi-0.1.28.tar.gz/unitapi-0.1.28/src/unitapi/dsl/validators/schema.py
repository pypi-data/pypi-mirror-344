...
class ExtensionSchema(BaseModel):
    ...
    def validate_name(cls, v):
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', v):
            raise ValueError("Extension name must contain only letters, numbers, underscores, and hyphens")
        return v
...
class DeviceSchema(BaseModel):
    ...
    def validate_id(cls, v):
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', v):
            raise ValueError("Device ID must contain only letters, numbers, underscores, and hyphens")
        return v
    ...
    def validate_device_type(cls, v):
        ...
        valid_types = [
            "computer", "raspberry_pi", "arduino", "esp32", "esp8266",
            "jetson", "smartphone", "tablet", "camera", "microphone",
            "speaker", "display", "keyboard", "mouse", "gamepad",
            "touchscreen", "gpio", "sensor", "actuator", "robot",
            "custom"
        ]
        if v not in valid_types and not v.startswith("custom_"):
            raise ValueError(f"Invalid device type. Must be one of {valid_types} or start with 'custom_'")
        return v
...
class PipelineStepSchema(BaseModel):
    ...
    def validate_action(cls, v):
        ...
        valid_actions = [
            "capture", "filter", "transform", "forward", "publish",
            "subscribe", "read", "write", "detect", "recognize",
            "analyze", "process", "on", "trigger", "alert", "log",
            "custom"
        ]
        if v not in valid_actions and not v.startswith("custom_"):
            raise ValueError(f"Invalid action. Must be one of {valid_actions} or start with 'custom_'")
        return v
...
class PipelineSchema(BaseModel):
    ...
    def validate_name(cls, v):
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', v):
            raise ValueError("Pipeline name must contain only letters, numbers, underscores, and hyphens")
        return v
    ...
    def validate_pipeline_references(cls, v, values):
        ...
        for pipeline in v:
            if pipeline.source and pipeline.source not in device_ids:
                raise ValueError(f"Pipeline '{pipeline.name}' references non-existent source device '{pipeline.source}'")
            
            if pipeline.target and pipeline.target not in device_ids:
                raise ValueError(f"Pipeline '{pipeline.name}' references non-existent target device '{pipeline.target}'")
