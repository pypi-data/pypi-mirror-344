...
class StarlarkParser(IDSLParser):
    ...
    def __init__(self):
        ...
        self._starlark_available = self._check_starlark_available()

    def _check_starlark_available(self):
        """Check if starlark package is available"""
        try:
            import starlark
    ...
    def parse(self, content: str) -> Dict[str, Any]:
        ...
        """
        if not self._starlark_available:
            # Fallback to a simple parsing mechanism if starlark is not available
    ...
    def _fallback_parse(self, content: str) -> Dict[str, Any]:
        """
        Simple fallback parser for Starlark content when the starlark package is not available.
        ...
        # Extract extensions
        extension_matches = re.finditer(
            r'(\w+)\s*=\s*extension\s*\(\s*name\s*=\s*["\']([^"\']+)["\'](?:.*?)\)',
            content,
            re.DOTALL,
        )
        for match in extension_matches:
            ...
            result["extensions"].append(
                Extension(type="extension", name=name, version=">=1.0.0")
            )

        # Extract devices
        device_matches = re.finditer(
            r'(\w+)\s*=\s*device\s*\(\s*id\s*=\s*["\']([^"\']+)["\'](?:.*?)\)',
            content,
            re.DOTALL,
        )
        for match in device_matches:
            ...
            device_type = type_match.group(1) if type_match else "generic"
            result["devices"].append(
                Device(type="device", id=device_id, device_type=device_type)
            )

        # Extract pipelines
        pipeline_matches = re.finditer(
            r'(\w+)\s*=\s*pipeline\s*\(\s*name\s*=\s*["\']([^"\']+)["\'](?:.*?)\)',
            content,
            re.DOTALL,
        )
        for match in pipeline_matches:
            pipeline_name = match.group(2)
            pipeline_obj = Pipeline(type="pipeline", name=pipeline_name)

            # Try to extract source and target
            source_match = re.search(
                r'source\s*=\s*["\']([^"\']+)["\']', match.group(0)
            )
            if source_match:
                pipeline_obj.source = source_match.group(1)

            target_match = re.search(
                r'target\s*=\s*["\']([^"\']+)["\']', match.group(0)
            )
            if target_match:
                pipeline_obj.target = target_match.group(1)

            result["pipelines"].append(pipeline_obj)
