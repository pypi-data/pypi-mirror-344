...
class DSLExecutor:
    ...
    async def initialize_device(self, device: Device):
        ...
        try:
            ...
            device_instance = self.unitapi.create_device(
                device.id,
                device.device_type
            )
        ...
        try:
            ...
            if pipeline.source:
                if pipeline.source not in self.initialized_devices:
                    logger.warning(f"Source device {pipeline.source} not initialized, initializing now")
                    source_device = next((d for d in self.unitapi.devices if d.id == pipeline.source), None)
                    if source_device:
            ...
            if pipeline.target:
                if pipeline.target not in self.initialized_devices:
                    logger.warning(f"Target device {pipeline.target} not initialized, initializing now")
                    target_device = next((d for d in self.unitapi.devices if d.id == pipeline.target), None)
                    if target_device:
    ...
    async def cleanup(self):
        ...
        for device_id, device_instance in self.initialized_devices.items():
            if hasattr(device_instance, 'cleanup') and callable(device_instance.cleanup):
                try:
