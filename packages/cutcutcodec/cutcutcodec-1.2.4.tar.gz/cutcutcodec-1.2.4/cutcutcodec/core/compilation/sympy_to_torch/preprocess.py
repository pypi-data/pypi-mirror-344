...
from sympy.core.numbers import Float, NumberSymbol
...
def evalf(expr: Basic) -> Basic:
    ...
    expr = expr.evalf(n=25)  # the float128 mantissa is 80 bits => n = ceil(log10(2**80)) = 25
    if sub := {s: round(s) for s in expr.atoms(Float)}:  # float to int
        sub = {s: i for s, i in sub.items() if float(s) == int(i)}
