...
def add_addl_features_online(df,tickers):
    ...
    return df
def add_addl_features_feed(feed,tickers,drop_ta=False,addTa=True):
    print(f'addTa {addTa}')
    add_ta_features_feed(feed,drop_ta=drop_ta,addTa=addTa)
    # add_sym_feature_feed(feed,tickers)
def add_ta_features_feed(feed,drop_ta=False,addTa=True):
    dfaL=[]
    feed.ndata={}
    for t in feed.tickers:
        ...
        for d in dfa['Date'].unique():
            try:
                ...
                df=add_vol_n(df,sdx)
                df=norm_add_ta(df,drop_ta=drop_ta,addTa=addTa)
