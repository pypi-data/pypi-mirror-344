...
async def async_matrix_rain(
    ...
    """
    Asynchronous Matrix-style rain animation. If width/height are None, autodetect terminal size.
    ...
    msg_width = max((len(line) for line in multiline), default=0)
    ...
    use_unicode = any(ord(c) > 127 for c in charset)
        ...
        while True:
            ...
            if all_revealed_twice():
                if msg_fully_revealed_time is None:
                    msg_fully_revealed_time = asyncio.get_event_loop().time()
                elif asyncio.get_event_loop().time() - msg_fully_revealed_time >= duration:
                    final_message_done = True
                    continue
            elif asyncio.get_event_loop().time() - start > 60 * 60:  # safety timeout
                break
