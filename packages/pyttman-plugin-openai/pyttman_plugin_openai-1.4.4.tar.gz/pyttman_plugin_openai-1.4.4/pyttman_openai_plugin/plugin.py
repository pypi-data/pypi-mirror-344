...
class RagMemoryBank:
    ...
    def _load_memories_from_file(self):
        ...
        with open(self.file_path, "r", encoding="utf-8") as f:
            ...
            self.memories = set(data["memories"])
...
class OpenAIPlugin(PyttmanPlugin):
    ...
    detect_memory_prompt = (
        "You are a filtering layer in a multi-modal AI system. Your"
        " *only* job is to decide if the user’s latest message "
        "contains a new, useful long-term memory. No analysis, "
        "no reply. If yes: summarize in this format: "
        "`[MEMORY]: <memory content> <tags>`. If not: return `0`."
        " Store if the message shares life details, routines, "
        "values, people, events, goals, preferences, or direct orders "
        "to save info, and it’s not already stored. Be brief and "
        "precise. "
        ...
        "Avoid duplicates. No assumptions or elaboration."
        " Store in the message’s language. Add 1–3 short tags "
        "(comma-separated) about the memory’s topic. Re-use tags where "
        "possible. Output ONLY `[MEMORY]: ... <tags>` or `0`. You do not "
        "answer. Current date/time: {DATE_TIME_DATA}."
    )
    ...
    def create_memory_if_applicable(self,
        ...
        try:
            ...
            pyttman.logger.log(f" - [OpenAIPlugin]: Memory detection took {time() - now:.2f} seconds.")
