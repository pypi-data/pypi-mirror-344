...
def spin_density(mo, orbital_file, show_spin_density, structure):
    ...
    try:
        _fig = structure.create_fig_with_isosurface(
            _density, isovalue=0.01, colors=("#24FF51", "#FA7496")
        )
    except ValueError:
        _fig = mo.md(
            "/// error | Isovalue too extreme, the surface would protrude the grid volume. Is this an open-shell calculation?"
        )
...
def active_space_analysis(
    ...
    _active_space = parse_active_space_orbitals(file)
    _loewdin_orbital_compositions = parse_loewdin_orbital_compositions(file)
...
def _(Path, mo):
    ...
    return file_arg, file_text, sys


@app.cell
def _(Path, co, file_text, mo, parse_orbital_energies):
...
def _(orbital_energies, pl):
    ...
    return LUMO_RANGE, homo, orbitals_df
...
def _(homo, mo, orbital_energies, orbitals_df):
    ...
    return is_unrestricted, orbital_dropdown
...
def _(
    ...
    try:
        loewdin_compositions = parse_loewdin_orbital_compositions(file)
