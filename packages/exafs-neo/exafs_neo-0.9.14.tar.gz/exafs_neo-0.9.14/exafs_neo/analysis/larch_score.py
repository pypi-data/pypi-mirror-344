...
This module is used for modeling the score for each dataset
"""
...
def latex_table(paths, best_Fit_r, err_full):
    ...
    for i in range(len(full_paths)):
        ...
        latex_table_str += ("                        " + str(full_paths[i]) + " & " +
                            ...
                            convert_to_str(best_Fit_r[i][0], prec_arr[0]) + r"$\pm$" + convert_to_str(err_full[i, 0],
                                                                                                      prec_arr[
                                                                                                          0]) + " & " +
                            convert_to_str(best_Fit_r[i][1], prec_arr[1]) + r"$\pm$" + convert_to_str(err_full[i, 1],
                                                                                                      prec_arr[
                                                                                                          1]) + " & " +
                            str(np.round(best_Fit_r[i][2], 4)) + r"$\pm$" + str(np.round(err_full[i, 2], 4)) + " & " +
                            str(np.round(best_Fit_r[i][3], 3)) + r"$\pm$" + str(
                    temp_round_deltaR(err_full[i, 3])) + " &  " +
