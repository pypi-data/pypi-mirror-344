...
class BKG_plot:
    ...
    def initial_parameters(self, file, rbkg, bkg_kw, bkg_kmax, k_min, k_max, delta_k, kweight):
        """Initalize the parameters

        Args:
            file (tkinter str): File path
            rbkg (float): rbkg value
            bkg_kw (float): background kweight
            bkg_kmax (float): background kmax
            k_min (float): Minimum k value
            k_max (float): Maximum k value
            delta_k (float): Delta k value
            kweight (float): kweight value
        """
