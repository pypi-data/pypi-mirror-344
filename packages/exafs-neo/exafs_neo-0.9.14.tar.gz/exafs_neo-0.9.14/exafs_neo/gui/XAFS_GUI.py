...
class App:
        ...
        try:
            ...
            if not isinstance(ini_file, tuple):
                if len(ini_file) != 0:
                    ...
                    messagebox.showinfo("", f"Ini file written to {ini_file}")
    ...
    def Build_inputs_tab(self):
        ...
        def select_data_file():
            try:
                filepath = filedialog.askopenfilenames(initialdir=os.getcwd(),
                                                       title="Choose xmu/csv",
                                                       filetypes=(("xmu files", "*.xmu"), ("csv files", "*.csv"),
                                                                  ("all files", "*.*")))
                if filepath is None:
                    ...
                    if isinstance(filepath, tuple):
                        if len(filepath) == 1:
                            self.data_file.set(str(filepath[0]))
                            self.temp_data_file.set(str(filepath[0]))
                        else:
                            file_list = list(self.root.splitlist(filepath))
                            separator = ' '
        ...
        def select_output_file():
            try:
                file_path = filedialog.asksaveasfilename(initialdir=os.getcwd(), title="Choose data_file",
    ...
    def Build_analysis_tab(self):
        ...
        def run_analysis():
            params = {'base': '', 'Kmin': self.kmin.get(), 'Kmax': self.kmax.get(), 'kweight': self.k_weight.get(),
                      'deltak': self.delta_k.get(), 'rbkg': self.r_bkg.get(), 'bkgkw': self.bkg_kw.get(),
                      'bkgkmax': self.bkg_kmax.get(), 'front': self.feff_file.get().split(','),
                      'CSV': self.data_file.get(), 'optimize': self.path_optimize.get(),
                      'series_index': series_index.get(), 'series': self.series.get()}
            # Set up the params
