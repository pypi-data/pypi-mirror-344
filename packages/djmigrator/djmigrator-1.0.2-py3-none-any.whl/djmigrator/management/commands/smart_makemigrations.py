...
class Command(BaseCommand):
    help = "Smart DB sync: fully auto-fixes DB-model mismatches and broken migrations."
    ...
    def handle(self, *args, **options):
            ...
            with connection.schema_editor() as schema_editor:
                for table_name, field in columns_to_add:
                    try:
                        ...
                        self.stdout.write(self.style.SUCCESS(f"✅ Column `{field.column}` added to table `{table_name}`"))
        ...
        # 🛠 Handling missing or broken migrations
        if missing_changes_detected or options['repair']:
            ...
            try:
                ...
                if 'It is impossible to add a non-nullable field' in str(e):
                    ...
                    # Detect broken apps dynamically
                    loader = MigrationLoader(connection, ignore_no_migrations=True)
                    broken_apps = set()
                    ...
                    for app_label in broken_apps:
                        self.stdout.write(self.style.WARNING(f"🧹 Removing migration records for app: {app_label}"))
                        recorder.Migration.objects.filter(app=app_label).delete()

                        # Also delete all migration .py files
                        try:
                    ...
                    self.stdout.write(self.style.MIGRATE_HEADING(f"🔁 Retrying makemigrations after cleaning apps: {broken_apps}"))
                    call_command('makemigrations', interactive=False)
