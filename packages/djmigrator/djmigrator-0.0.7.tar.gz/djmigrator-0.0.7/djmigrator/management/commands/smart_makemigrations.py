...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--repair', action='store_true', help="Attempt to auto-fix missing columns via raw SQL.")
        ...
        if options['repair'] and columns_to_add:
            ...
            with connection.schema_editor() as schema_editor:
                for table_name, field in columns_to_add:
                    try:
                        schema_editor.add_field(apps.get_model(field.model._meta.app_label, field.model._meta.model_name), field)
            ...
            try:
                ...
                for app_label in fake_apps:
                    ...
                    if initial_key in loader.disk_migrations:
                        if initial_key in applied_migrations:
                            self.stdout.write(f"🔁 Unapplying previously faked migration: {initial_key}")
                            recorder.Migration.objects.filter(app=app_label, name="0001_initial").delete()
                        self.stdout.write(f"🛠️ Applying initial migration for: {app_label}")
                        call_command('migrate', app_label)
                    else:
                        self.stdout.write(self.style.WARNING(f"⚠️ No initial migration found for app: {app_label}"))
