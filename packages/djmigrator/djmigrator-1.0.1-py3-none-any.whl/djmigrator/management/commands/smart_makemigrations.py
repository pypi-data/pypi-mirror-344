        ...
        if missing_changes_detected or options['repair']:
            ...
            try:
                ...
                if 'It is impossible to add a non-nullable field' in str(e):
                    ...
                    # Dynamically detect broken apps
                    loader = MigrationLoader(connection, ignore_no_migrations=True)
                    broken_apps = set()

                    for (app_label, migration_name), migration in loader.disk_migrations.items():
                        for dependency_app, dependency_migration in migration.dependencies:
                            if (dependency_app, dependency_migration) not in loader.applied_migrations:
                                broken_apps.add(app_label)
                                broken_apps.add(dependency_app)
                    ...
                    self.stdout.write(self.style.WARNING(f"🔁 Retrying makemigrations after cleaning broken apps: {broken_apps}"))
                    call_command('makemigrations', interactive=False)
