...
from luann.server.rest_api.optimistic_json_parser import OptimisticJSONParser
...
class ChatCompletionsStreamingInterface(AgentChunkStreamingInterface):
    ...
    Key Behaviors:
    - process_chunk: Accepts ChatCompletionChunkResponse objects (e.g. from luann.an
    ...
    async def _create_generator(
        self,
    ) -> AsyncGenerator[Union[LuannMessage, MessageStreamStatus], None]:
        """
    ...
    def _push_to_buffer(
        ...
        """
        Add an item (a luannMessage, status marker, or partial chunk)
    ...
    def _process_chunk_to_openai_style(self, chunk: ChatCompletionChunkResponse) -> Optional[ChatCompletionChunk]:
        ...
        content (especially from luann.a 'send_message' tool) is exposed as text
