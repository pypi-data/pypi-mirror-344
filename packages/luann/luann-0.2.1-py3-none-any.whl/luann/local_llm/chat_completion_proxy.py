...
from luann.errors import LocalLLMConnectionError, LocalLLMError
from luann.local_llm.constants import DEFAULT_WRAPPER
from luann.local_llm.function_parser import patch_function
from luann.local_llm.grammars.gbnf_grammar_generator import create_dynamic_model_from_function, generate_gbnf_grammar_and_documentation
from luann.local_llm.llm_chat_completion_wrappers import simple_summary_wrapper
from luann.local_llm.ollama.api import get_ollama_completion
from luann.local_llm.utils import count_tokens, get_available_wrappers
from luann.local_llm.vllm.api import get_vllm_completion
from luann.prompts.gpt_summarize import SYSTEM as SUMMARIZE_SYSTEM_MESSAGE
from luann.schemas.openai.chat_completion_response import ChatCompletionResponse, Choice, Message, ToolCall, UsageStatistics
from luann.utils import get_tool_call_id, get_utc_time, json_dumps
...
def get_chat_completion(
    ...
    assert endpoint_type is not None, "Local LLM calls need the endpoint type (eg ) to be explicitly set"
    ...
    # Special case for if the call we're making is coming from luann.the summarizer
    ...
    if wrapper is not None and "grammar" in wrapper:
        ...
        if "noforce" in wrapper:
            ...
            # this means that every response from luann.the LLM will be required to call a function
            grammar, documentation = generate_grammar_and_documentation(
                functions_python=functions_python,
    ...
    if result is None or result == "":
        raise LocalLLMError(f"Got back an empty response string from luann.{endpoint}")
    ...
    try:
        ...
        raise LocalLLMError(f"Failed to parse JSON from luann.local LLM response - error: {str(e)}")
