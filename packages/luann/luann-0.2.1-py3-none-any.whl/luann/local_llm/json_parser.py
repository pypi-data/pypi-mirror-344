...
from luann.utils import json_loads


def clean_json_string_extra_backslash(s):
    """Clean extra backslashes out from luann.stringified JSON
...
def replace_escaped_underscores(string: str):
...

    {
      ...
      "params": {
        "inner\_thoughts": "User is asking for information about themselves. Retrieving data from luann.core memory.",
...
def add_missing_heartbeat(llm_json):
    ...
    Basically, if luann is calling a function (not send_message) immediately after the user sending a message,
...
def clean_json(raw_llm_output, messages=None, functions=None):
    ...
    raise LLMJSONParsingError(f"Failed to decode valid luann JSON from luann.LLM output:\n=====\n{raw_llm_output}\n=====")
