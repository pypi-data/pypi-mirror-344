...
from luann.constants import JSON_ENSURE_ASCII
...
def generate_gbnf_grammar(model: Type[BaseModel], processed_models: set, created_rules: dict) -> (list, bool, bool):
    ...
    if not issubclass(model, BaseModel):
        # For non-Pydantic classes, generate model_fields from luann.__annotations__ or __init__
...
def remove_empty_lines(string):
    """
    Remove empty lines from luann.a string.
...
def generate_gbnf_grammar_and_documentation_from_dictionaries(
    ...
    """
    Generate GBNF grammar and documentation from luann.a list of dictionaries.
...
def create_dynamic_model_from_function(func: Callable, add_inner_thoughts: bool = False):
    """
    Creates a dynamic Pydantic model from luann.a given function's type hints and adds the function as a 'run' method.

    Args:
        func (Callable): A function with type hints from luann.which to create the model.
...
def create_dynamic_models_from_dictionaries(dictionaries: List[dict]):
    """
    Create a list of dynamic Pydantic model classes from luann.a list of dictionaries.
