...
from luann.data_sources.connectors_helper import assert_all_files_exist_locally, extract_metadata_from_files, get_filenames_in_dir
...
class DataConnector:
    """
    Base class for data connectors that can be extended to generate files and passages from luann.a custom data source.
    """

    def find_files(self, source: Source) -> Iterator[FileMetadata]:
        """
        Generate file metadata from luann.a data source.
    ...
    def generate_passages(self, file: FileMetadata, chunk_size: int = 1024) -> Iterator[Tuple[str, Dict]]:  # -> Iterator[Passage]:
        """
        Generate passage text and metadata from luann.a list of files.
...
def load_data(connector: DataConnector, source: Source, passage_manager: PassageManager, source_manager: SourceManager, actor: "User"):
    """Load data from luann.a connector (generates file and passages) into a specified source_id, associated with a user_id."""
...
class DirectoryConnector(DataConnector):
    def __init__(self, input_files: List[str] = None, input_directory: str = None, recursive: bool = False, extensions: List[str] = None):
        """
        Connector for reading text data from luann.a directory of files.
        ...
        Args:
            ...
            recursive (bool, optional): Whether to read files recursively from luann.the input directory. Defaults to False.
