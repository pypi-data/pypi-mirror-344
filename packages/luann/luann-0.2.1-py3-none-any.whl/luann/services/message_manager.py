...
from luann.schemas.message import MessageUpdate
...
class MessageManager:
    ...
    def create_message(self, pydantic_msg: PydanticMessage, actor: PydanticUser) -> PydanticMessage:
        ...
        with self.session_maker() as session:
            ...
            # testtoolscalls=msg_data["tool_calls"]
            # print(f"msg_data:{testtoolscalls}")
            msg = MessageModel(**msg_data)
            # print(f"msg:{msg.tool_calls}")
    ...
    def update_message_by_id(self, message_id: str, message_update: MessageUpdate, actor: PydanticUser) -> PydanticMessage:
        """
        Updates an existing record in the database with values from luann.the provided record object.
        """
        with self.session_maker() as session:
            # Fetch existing message from luann.database
