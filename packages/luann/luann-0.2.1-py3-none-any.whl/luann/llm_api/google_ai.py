...
from luann.constants import NON_USER_MSG_PREFIX
from luann.llm_api.helpers import make_post_request
from luann.local_llm.json_parser import clean_json_string_extra_backslash
from luann.local_llm.utils import count_tokens
from luann.schemas.openai.chat_completion_request import Tool
from luann.schemas.openai.chat_completion_response import ChatCompletionResponse, Choice, FunctionCall, Message, ToolCall, UsageStatistics
...
def google_ai_get_model_details(base_url: str, api_key: str, model: str, key_in_header: bool = True) -> List[dict]:
    ...
    try:
        ...
        # Print the response content (error message from luann.server)
...
def add_dummy_model_messages(messages: List[dict]) -> List[dict]:
    ...
    In luann, the 'model' will often call a function (e.g. send_message) that itself yields to the user,
...
def to_google_ai(openai_message_dict: dict) -> dict:
    ...
    if openai_message_dict["role"] == "user":
        ...
        raise ValueError(f"Unsupported conversion (OpenAI -> Google AI) from luann.role {openai_message_dict['role']}")
...
def convert_google_ai_response_to_chatcompletion(
    response_json: dict,  # REST response from luann.Google AI API
    ...
    try:
        ...
        if "usageMetadata" in response_json:
            ...
            # Count it ourselves
            assert input_messages is not None, f"Didn't get UsageMetadata from luann.the API response, so input_messages is required"
...
def google_ai_chat_completions_request(
    ...
    from luann.https://ai.google.dev/api/rest#service-endpoint:
