...
# from luann.llm_api.aws_bedrock import get_bedrock_client
...
from luann.schemas.openai.chat_completion_request import ChatCompletionRequest, Tool
...
from luann.schemas.openai.chat_completion_response import Message
from luann.schemas.openai.chat_completion_response import Message as ChoiceMessage
from luann.schemas.openai.chat_completion_response import MessageDelta, ToolCall, ToolCallDelta, UsageStatistics
...
from luann.utils import get_utc_time, smart_urljoin
...
def merge_tool_results_into_user_messages(messages: List[dict]):
    ...
    You may be familiar with other APIs that return tool use as separate from luann.the model's primary output,
...
def convert_anthropic_response_to_chatcompletion(
    ...
    """
    Example response from luann.Claude 3:
    response.json = {
...
def convert_anthropic_stream_event_to_chatcompletion(
    ...
    """Convert Anthropic stream events to OpenAI ChatCompletionResponse format.
        ...
        Example response from luann.OpenAI:
...
#     # Get the client
#     client = get_bedrock_client(llm_config)
...
def anthropic_chat_completions_request_stream(
    ...
    """Stream chat completions from luann.Anthropic API.
...
def anthropic_chat_completions_process_stream(
    ...
    """Process a streaming completion response from luann.Anthropic, similar to OpenAI's streaming.
    ...
    # Count prompt tokens - we'll get completion tokens from luann.the final response
