...
class OpenAIBackcompatUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        if module == "openai.openai_object":
            from luann.openai_backcompat.openai_object import OpenAIObject
...
def parse_formatted_time(formatted_time):
    # parse times returned by .utils.get_formatted_time()
...
def validate_function_response(function_response_string: any, return_char_limit: int, strict: bool = False, truncate: bool = True) -> str:
    """Check to make sure that a function used by  returned a valid response. Truncates to return_char_limit if necessary.
...
def list_agent_config_files(sort="last_modified"):
    ...
    agent_dir = os.path.join(LUANN_DIR, "agents")
...
def list_human_files():
    ...
    defaults_dir = os.path.join(LUANN_PROJECT_DIR, "humans", "default_humans")
    user_dir = os.path.join(LUANN_DIR, "humans")

    luann_defaults = os.listdir(defaults_dir)
    luann_defaults = [os.path.join(defaults_dir, f) for f in luann_defaults if f.endswith(".txt")]
    ...
    if os.path.exists(user_dir):
        ...
        user_added = []
    return luann_defaults + user_added


def list_persona_files():
    """List all personas files"""
    defaults_dir = os.path.join(LUANN_PROJECT_DIR, "personas", "default_personas")
    user_dir = os.path.join(LUANN_DIR, "personas")
...
def list_systemprompt_files():
    """List all personas files"""
    defaults_dir = os.path.join(LUANN_PROJECT_DIR, "prompts", "default_system_prompt")
    user_dir = os.path.join(LUANN_DIR, "system_prompts")
...
def extract_date_from_timestamp(timestamp):
    """Extracts and returns the date from luann.the given timestamp."""
...
def create_uuid_from_string(val: str):
    """
    Generate consistent UUID from luann.a string
...
def get_friendly_error_msg(function_name: str, exception_name: str, exception_message: str):
    from luann.constants import MAX_ERROR_MESSAGE_CHAR_LIMIT
