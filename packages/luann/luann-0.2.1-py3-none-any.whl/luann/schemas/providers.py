...
from luann.constants import LLM_MAX_TOKENS, MIN_CONTEXT_WINDOW
from luann.llm_api.azure_openai import get_azure_chat_completions_endpoint, get_azure_embeddings_endpoint
from luann.llm_api.azure_openai_constants import AZURE_MODEL_TO_CONTEXT_LENGTH
...
# from luann.schemas.llm_config_overrides import LLM_HANDLE_OVERRIDES

# from luann.schemas.llm_config_overrides import LLM_HANDLE_OVERRIDES

class ProviderBase(LuannBase):
...
class Provider(ProviderBase):
    ...
    base_url: Optional[str] = Field(None, doc="base url used for requests to the provider.")


    # Azure openai
    api_version: Optional[str] = Field(None, description="api version used for requests to the Azure provider.")
    # api_version: Mapped[str] = mapped_column(nullable=False, doc="api version used for requests to the Azure provider.")


    # aws 
    aws_region: Optional[str] = Field(None, description="aws region used for requests to the aws provider.")
    ...
    default_prompt_formatter: Optional[str] = Field(None, description="Default prompt formatter (aka model wrapper) to use on vLLM /completions API.")


     #whether to enable
    # is_enabled:Optional[str] = Field(None, doc="whether  to enable the provider")
    is_enabled: bool = Field(False, description="whether  to enable the provider")
...
class ProviderCreate(ProviderBase):
    name: str = Field(..., description="The name of the provider.")
    api_key: Optional[str] = Field(..., description="API key used for requests to the provider.")
...
class LuannProvider(Provider):
    ...
    def list_llm_models(self) -> List[LLMConfig]:
        return [
            LLMConfig(
                model="luann-free",  # NOTE: renamed
                model_endpoint_type="openai",
                model_endpoint="https://inference.luann.ai",
                context_window=16384,
                handle=self.get_handle("luann-free"),
            )
        ]
    ...
    def list_embedding_models(self):
        return [
            EmbeddingConfig(
                embedding_model="luann-free",  # NOTE: renamed
...
class OpenAIProvider(Provider):
    ...
    def list_llm_models(self) -> List[LLMConfig]:
        from luann.llm_api.openai import openai_get_model_list
...
class AnthropicProvider(Provider):
    ...
    def list_llm_models(self) -> List[LLMConfig]:
        from luann.llm_api.anthropic import anthropic_get_model_list
...
class GoogleAIProvider(Provider):
    ...
    def list_llm_models(self):
        from luann.llm_api.google_ai import google_ai_get_model_list
    ...
    def get_model_context_window(self, model_name: str) -> Optional[int]:
        from luann.llm_api.google_ai import google_ai_get_model_context_window
...
class AzureProvider(Provider):
    ...
    base_url: str = Field(
        ..., description="Base URL for the Azure API endpoint. This should be specific to your org, e.g. `https://luann.openai.azure.com`."
    )
    ...
    def list_llm_models(self) -> List[LLMConfig]:
        from luann.llm_api.azure_openai import azure_openai_get_chat_completion_model_list
        

        # print(f"self.base_url:{self.base_url}")
        # print(f"self.api_key:{self.api_key}")
        # print(f"self.api_version:{self.api_version}")
        ...
        for model_option in model_options:
            ...
            configs.append(
                LLMConfig(model=model_name, model_endpoint_type="azure", 
                          model_endpoint=model_endpoint,
                            context_window=context_window_size,
                            handle=self.get_handle(model_name),
                            azure_api_key=self.api_key,
                            azure_version=self.api_version,
                            )
                
            )
        # print(f"llm configs:{configs}")
        return configs

    def list_embedding_models(self) -> List[EmbeddingConfig]:
        from luann.llm_api.azure_openai import azure_openai_get_embeddings_model_list
...
#     def list_llm_models(self):
#         from luann.llm_api.aws_bedrock import bedrock_get_model_list
...
#         from luann.llm_api.aws_bedrock import bedrock_get_model_context_window
