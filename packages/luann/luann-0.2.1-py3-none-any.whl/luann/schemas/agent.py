...
from luann.constants import DEFAULT_EMBEDDING_CHUNK_SIZE
from luann.schemas.block import CreateBlock
from luann.schemas.embedding_config import EmbeddingConfig
# from luann.schemas.environment_variables import AgentEnvironmentVariablelu
...
from luann.utils import create_random_username
...
class CreateAgent(BaseModel, validate_assignment=True):  #
    ...
    include_base_tools: bool = Field(
        True, description="If true, attaches the luann core tools (e.g. archival_memory and core_memory related functions)."
    )
    include_multi_agent_tools: bool = Field(
        False, description="If true, attaches the luann multi-agent tools (e.g. sending a message to another agent)."
    )
    description: Optional[str] = Field(None, description="The description of the agent.")
    metadata_: Optional[Dict] = Field(None, description="The metadata of the agent.", alias="metadata_")
