...
from luann.schemas.enums import MessageContentType
# luann API style responses (intended to be easier to use vs getting true Message types)


class LuannMessage(BaseModel):
    """
    Base class for simplified Luann message response type. This is intended to be used for developers who want the internal monologue, function calls, and function returns in a simplified format that does not include additional information other than the content and timestamp.
...
class SystemMessage(LuannMessage):
    """
...
class UserMessage(LuannMessage):
    """
...
class ReasoningMessage(LuannMessage):
    """
...
class ToolCallDelta(BaseModel):
    ...
    # NOTE: this is a workaround to exclude None values from luann.the JSON dump,
...
class ToolCallMessage(LuannMessage):
    """
...
class ToolReturnMessage(LuannMessage):
    """
    A message representing the return value of a tool call (generated by luann executing the requested tool).
    ...
    Attributes:
        ...
        stdout (Optional[List(str)]): Captured stdout (e.g. prints, logs) from luann.the tool invocation
        stderr (Optional[List(str)]): Captured stderr from luann.the tool invocation
    """
...
# Legacy luann API had an additional type "assistant_message" and the "function_call" was a formatted string


class AssistantMessage(LuannMessage):
    message_type: Literal["assistant_message"] = "assistant_message"
    content: Union[str, List[MessageContentUnion]]


class LegacyFunctionCallMessage(LuannMessage):
    function_call: str


class LegacyFunctionReturn(LuannMessage):
    """
    A message representing the return value of a function call (generated by luann executing the requested function).
    ...
    Attributes:
        ...
        stdout (Optional[List(str)]): Captured stdout (e.g. prints, logs) from luann.the function invocation
        stderr (Optional[List(str)]): Captured stderr from luann.the function invocation
    """
...
class LegacyInternalMonologue(LuannMessage):
    """
...
LegacyLuannMessage = Union[LegacyInternalMonologue, AssistantMessage, LegacyFunctionCallMessage, LegacyFunctionReturn]


LuannMessageUnion = Annotated[
    Union[SystemMessage, UserMessage, ReasoningMessage, ToolCallMessage, ToolReturnMessage, AssistantMessage],
    Field(discriminator="message_type"),
]

def create_luann_message_union_schema():
    return {
        "oneOf": [
