...
class LuannResponse(BaseModel):
    """
    Response object from luann.an agent interaction, consisting of the new messages generated by the agent and usage statistics.
    The type of the returned messages can be either `Message` or `luannMessage`, depending on what was specified in the request.

    Attributes:
        messages (List[Union[Message, LuannMessage]]): The messages returned by the agent.
        usage (LuannUsageStatistics): The usage statistics
    """

    messages: List[LuannMessageUnion] = Field(
        ...,
        description="The messages returned by the agent.",
        json_schema_extra={
            "items": {
                "$ref": "#/components/schemas/LuannMessageUnion",
            }
        },
    )
    usage: LuannUsageStatistics = Field(..., description="The usage statistics of the agent.")
    def __str__(self):
        return json_dumps(
            {
                "messages": [message.model_dump() for message in self.messages],
                # Assume `Message` and `luannMessage` have a `dict()` method
                "usage": self.usage.model_dump(),  # Assume `luannUsageStatistics` has a `dict()` method
            },
            indent=4,
        )
...
# The streaming response is either [DONE], [DONE_STEP], [DONE], an error, or a luannMessage
LuannStreamingResponse = Union[LuannMessage, MessageStreamStatus, LuannUsageStatistics]