...
class LuannBase(BaseModel):
    """Base schema for Luann schemas (does not include model provider schemas, e.g. OpenAI)"""
    ...
    #    return f"{self.__id_prefix__}-{uuid.uuid4()}"
     # TODO: generate ID from luann.regex pattern?
    @classmethod
    ...
    def allow_bare_uuids(cls, v, values):
        ...
        return v
class OrmMetadataBase(LuannBase):
    # metadata fields
