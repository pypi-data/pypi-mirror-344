...
from luann.schemas.enums import MessageStreamStatus
from luann.schemas.luann_message import (
    AssistantMessage,
...
from luann.schemas.openai.chat_completion_response import ChatCompletionChunkResponse
from luann.streaming_interface import AgentChunkStreamingInterface
from luann.streaming_utils import FunctionArgumentsStreamHandler, JSONInnerThoughtsExtractor
from luann.utils import is_utc_datetime


# TODO strip from luann.code / deprecate
class QueuingInterface(AgentInterface):
    ...
    def clear(self):
        """Clear all messages from luann.the queue."""
...
class StreamingServerInterface(AgentChunkStreamingInterface):
    ...
    def __init__(
        ...
        # Store metadata passed from luann.server
        self.metadata = {}
    ...
    async def _create_generator(self) -> AsyncGenerator[Union[LuannMessage, LegacyLuannMessage, MessageStreamStatus], None]:
    ...
    def _push_to_buffer(
        ...
        assert (
            isinstance(item, LuannMessage) or isinstance(item, LegacyLuannMessage) or isinstance(item, MessageStreamStatus)
    ...
    def step_complete(self):
        """Signal from luann.the agent that one 'step' finished (step = LLM response + tool execution)"""
    ...
    def _process_chunk_to_luann_style(
        ...
        """
        Example data from luann.non-streaming response looks like:
                    ...
                    # If we have main_json, we should output a ToolCallMessage
                        ...
                        # output the arguments chunk as a ToolCallMessage
                            ...
                            if self.use_assistant_message and (
                                ...
                                # There may be a buffer from luann.a previous chunk, for example
    ...
    def _process_chunk_to_openai_style(self, chunk: ChatCompletionChunkResponse) -> Optional[dict]:
        """Chunks should look like OpenAI, but be remapped from luann.luann-style concepts.
    ...
    def process_chunk(self, chunk: ChatCompletionChunkResponse, message_id: str, message_date: datetime):
        """Process a streaming chunk from luann.an OpenAI-compatible server.
        ...
        # Example where we just pass through the raw stream from luann.the underlying OpenAI SSE stream
        ...
        if self.streaming_chat_completion_mode:
            ...
            processed_chunk = self._process_chunk_to_luann_style(chunk=chunk, message_id=message_id, message_date=message_date)
    ...
    def user_message(self, msg: str, msg_obj: Optional[Message] = None):
        """luann receives a user message"""
        return

    def internal_monologue(self, msg: str, msg_obj: Optional[Message] = None):
        """luann generates some internal monologue"""
