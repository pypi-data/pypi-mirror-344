...
from luann.server.rest_api.utils import get_luann_server, sse_async_generator
...
async def create_chat_completions(
    ...
    luann_agent = server.load_agent(agent_id=agent_id, actor=actor)
    ...
    if model != llm_config.model:
        warning_msg = f"The requested model {model} is different from luann.the model specified in this agent's ({agent_id}) llm_config: \n{llm_config.model_dump_json(indent=4)}"
...
async def send_message_to_agent_chat_completions(
    ...
    try:
        ...
        luann_agent.interface = ChatCompletionsStreamingInterface()
        ...
        asyncio.create_task(
            asyncio.to_thread(
                ...
                actor=actor,
                agent_id=luann_agent.agent_state.id,
                messages=messages,
