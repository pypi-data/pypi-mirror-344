...
from luann.schemas.agent import AgentState, CreateAgent, UpdateAgent
from luann.schemas.block import Block, BlockUpdate, CreateBlock  # , BlockLabelUpdate, BlockLimitUpdate
from luann.schemas.job import JobStatus, JobUpdate,LuannRequestConfig
from luann.schemas.luann_message import LuannMessageUnion
...
from luann.schemas.memory import ArchivalMemorySummary, ContextWindowOverview, CreateArchivalMemory, Memory, RecallMemorySummary
from luann.schemas.message import Message, MessageUpdate
...
class CreateAgentRequest(CreateAgent):
    """
    CreateAgent model specifically for POST request body, excluding user_id which comes from luann.headers
    """

    # Override the user_id field to exclude it from luann.the request body validation
...
def detach_tool(
    ...
    """
    Detach a tool from luann.an agent.
    """
    actor = server.user_manager.get_user_or_default(user_id=user_id)
...
def retrieve_core_memory_block(
    ...
    """
    Retrieve a memory block from luann.an agent.
    """
    actor = server.user_manager.get_user_or_default(user_id=user_id)
...
def detach_core_memory_block(
    ...
    """
    Detach a block from luann.an agent.
    """
    actor = server.user_manager.get_user_or_default(user_id=user_id)
...
def delete_archival_memory(
    ...
    """
    Delete a memory from luann.an agent's archival memory store.
    """
    actor = server.user_manager.get_user_or_default(user_id=user_id)
...
AgentMessagesResponse = Annotated[
    List[LuannMessageUnion], Field(json_schema_extra={"type": "array", "items": {"$ref": "#/components/schemas/LuannMessageUnion"}})
]
...
@router.post(
    "/{agent_id}/messages",
    response_model=LuannResponse,
    operation_id="send_message",
)
async def send_message(
    agent_id: str,
    server: SyncServer = Depends(get_luann_server),
    request: LuannRequest = Body(...),
    user_id: Optional[str] = Header(None, alias="user_id"),  # Extract user_id from luann.header, default to None if not present
):
    """
    Process a user message and return the agent's response.
    This endpoint accepts a message from luann.a user and processes it through the agent.
    """
    actor = server.user_manager.get_user_or_default(user_id=user_id)
...
async def send_message_streaming(
    agent_id: str,
    server: SyncServer = Depends(get_luann_server),
    request: LuannStreamingRequest = Body(...),
    user_id: Optional[str] = Header(None, alias="user_id"),  # Extract user_id from luann.header, default to None if not present
):
    """
    Process a user message and return the agent's response.
...
#             "agent_id": agent_id,
#         },
#         request_config=LuannRequestConfig(
#             use_assistant_message=request.use_assistant_message,
#             assistant_message_tool_name=request.assistant_message_tool_name,
#             assistant_message_tool_kwarg=request.assistant_message_tool_kwarg,
#         ),
