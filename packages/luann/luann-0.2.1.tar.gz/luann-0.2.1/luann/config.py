...
@dataclass
class LuannConfig:
    config_path: str = os.getenv("LUANN_CONFIG_PATH") or os.path.join(LUANN_DIR, "config")
    ...
    metadata_storage_path: str = LUANN_DIR
    ...
    @classmethod
    def load(cls, llm_config: Optional[LLMConfig] = None, embedding_config: Optional[EmbeddingConfig] = None) -> "LuannConfig":
        ...
        if os.getenv("MEMGPT_CONFIG_PATH"):
            ...
            config_path = LuannConfig.config_path

        # insure all configuration directories exist
        cls.create_config_dir()
        printd(f"Loading config from luann.{config_path}")
        if os.path.exists(config_path):
            ...
            # llm_config_dict = {
            #    # Extract relevant LLM configuration from luann.the config file
            ...
            #    # Extract relevant Embedding configuration from luann.the config file
            ...
            config_dict = {
                ...
                # "luann_version": get_field(config, "version", "luann_version"),
            }
    ...
    def create_config_dir():
        if not os.path.exists(LUANN_DIR):
            os.makedirs(LUANN_DIR, exist_ok=True)
        ...
        for folder in folders:
            if not os.path.exists(os.path.join(LUANN_DIR, folder)):
                os.makedirs(os.path.join(LUANN_DIR, folder))
