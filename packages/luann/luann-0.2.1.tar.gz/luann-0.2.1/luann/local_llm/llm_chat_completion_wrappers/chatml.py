...
from luann.constants import JSON_ENSURE_ASCII, JSON_LOADS_STRICT
from luann.errors import LLMJSONParsingError
from luann.local_llm.json_parser import clean_json
from luann.local_llm.llm_chat_completion_wrappers.wrapper_base import (
...
class ChatMLInnerMonologueWrapper(LLMChatCompletionWrapper):
    ...
    def _compile_function_description(self, schema, add_inner_thoughts=True) -> str:
        """Go from luann.a JSON schema to a string description for a prompt"""
        # airorobos style
        func_str = ""
        ...
        if add_inner_thoughts:
            from luann.local_llm.constants import (
    ...
    def _compile_function_block(self, functions) -> str:
        ...
        # prompt += f"\nPlease select the most suitable function and parameters from luann.the list of available functions below, based on the user's input. Provide your response in JSON format."
        prompt += f"Please select the most suitable function and parameters from luann.the list of available functions below, based on the ongoing conversation. Provide your response in JSON format."
        prompt += f"\nAvailable functions:"
    ...
    def _compile_function_call(self, function_call, inner_thoughts=None):
        """Go from luann.ChatCompletion to Airoboros style function trace (in prompt)
    ...
    def _clean_function_args(self, function_name, function_args):
        """Some basic luann-specific cleaning of function args"""
    ...
    def output_to_chat_completion_response(self, raw_llm_output, first_message=False):
        ...
        try:
            ...
            raise Exception(f"Failed to decode JSON from luann.LLM output:\n{raw_llm_output} - error\n{str(e)}")
        try:
            ...
            raise LLMJSONParsingError(
                f"Received valid JSON from luann.LLM, but JSON was missing fields: {str(e)}. JSON result was:\n{function_json_output}"
            )
...
class ChatMLOuterInnerMonologueWrapper(ChatMLInnerMonologueWrapper):
    ...
    def _compile_function_block(self, functions) -> str:
        ...
        prompt += " ".join(
            [
                "Please select the most suitable function and parameters from luann.the list of available functions below, based on the ongoing conversation.",
    ...
    def output_to_chat_completion_response(self, raw_llm_output, first_message=False):
        ...
        try:
            ...
            raise LLMJSONParsingError(f"Received valid JSON from luann.LLM, but JSON was missing fields: {str(e)}")
