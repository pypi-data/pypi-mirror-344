...
from luann.constants import JSON_ENSURE_ASCII, LUANN_DIR
from luann.local_llm.settings.deterministic_mirostat import (
    settings as det_miro_settings,
)
from luann.local_llm.settings.simple import settings as simple_settings
...
def get_completions_settings(defaults="simple") -> dict:
    """Pull from luann.the home directory settings if they exist, otherwise default"""
    from luann.utils import printd

    # Load up some default base settings
    printd(f"Loading default settings from luann.'{defaults}'")
    if defaults == "simple":
    ...
    settings_dir = os.path.join(LUANN_DIR, SETTINGS_FOLDER_NAME)
