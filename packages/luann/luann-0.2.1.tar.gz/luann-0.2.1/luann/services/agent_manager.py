...
from luann.constants import BASE_MEMORY_TOOLS, BASE_TOOLS, MAX_EMBEDDING_DIM, MULTI_AGENT_TOOLS
from luann.embeddings import embedding_model
from luann.log import get_logger
from luann.orm import Agent as AgentModel
from luann.orm import AgentPassage, AgentsTags
from luann.orm import Block as BlockModel
from luann.orm import Source as SourceModel
from luann.orm import SourcePassage, SourcesAgents
from luann.orm import Tool as ToolModel
from luann.orm.errors import NoResultFound
# from luann.orm.sandbox_config import AgentEnvironmentVariable as AgentEnvironmentVariableModel
from luann.orm.sqlite_functions import adapt_array
from luann.schemas.agent import AgentState as PydanticAgentState
from luann.schemas.agent import AgentType, CreateAgent, UpdateAgent
...
# from luann.schemas.tool_rule import ToolRule as PydanticToolRule
from luann.schemas.user import User as PydanticUser
from luann.services.block_manager import BlockManager
from luann.services.helpers.agent_manager_helper import (
...
from luann.utils import enforce_types, get_utc_time, united_diff
...
class AgentManager:
    ...
    def _update_agent(self, agent_id: str, agent_update: UpdateAgent, actor: PydanticUser) -> PydanticAgentState:
        ...
        with self.session_maker() as session:
            ...
            scalar_fields = {"name", "system", "llm_config", "embedding_config", "message_ids", "description", "metadata_"}
    ...
    # TODO: 2) These messages are ordered from luann.oldest to newest
    ...
    def rebuild_system_prompt(self, agent_id: str, actor: PydanticUser, force=False, update_timestamp=True) -> PydanticAgentState:
        ...
        if update_timestamp:
            ...
            # NOTE: a bit of a hack - we pull the timestamp from luann.the message created_by
    ...
    def reset_messages(self, agent_id: str, actor: PydanticUser, add_default_initial_messages: bool = False) -> PydanticAgentState:
        ...
        with self.session_maker() as session:
            ...
            # this relationship to an empty list will physically remove them from luann.the DB.
            agent.messages = []
    ...
    def detach_source(self, agent_id: str, source_id: str, actor: PydanticUser) -> None:
       """
        Detaches a source from luann.an agent.
       ...
       with self.session_maker() as session:
            ...
            # Remove the source from luann.the relationship
            ...
            if len(remaining_sources) == len(agent.sources):  # Source ID was not in the relationship
                logger.warning(f"Attempted to remove unattached source id={source_id} from luann.agent id={agent_id} by actor={actor}")
    ...
    def detach_block(
        ...
        """Detaches a block from luann.an agent."""
        with self.session_maker() as session:
            agent = AgentModel.read(db_session=session, identifier=agent_id, actor=actor)
    ...
    def detach_block_with_label(
        ...
        """Detaches a block with the specified label from luann.an agent."""
        with self.session_maker() as session:
            agent = AgentModel.read(db_session=session, identifier=agent_id, actor=actor)
            original_length = len(agent.core_memory)
    ...
    def _build_passage_query(
        ...
        with self.session_maker() as session:
            ...
            # Build main query from luann.combined CTE
    ...
    def detach_tool(self, agent_id: str, tool_id: str, actor: PydanticUser) -> PydanticAgentState:
        """
        Detaches a tool from luann.an agent.
        ...
        with self.session_maker() as session:
            ...
            if len(remaining_tools) == len(agent.tools):  # Tool ID was not in the relationship
                logger.warning(f"Attempted to remove unattached tool id={tool_id} from luann.agent id={agent_id} by actor={actor}")
