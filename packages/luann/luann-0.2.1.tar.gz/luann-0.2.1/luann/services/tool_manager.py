...
from luann.constants import BASE_FUNCTION_RETURN_CHAR_LIMIT, BASE_MEMORY_TOOLS, BASE_TOOLS, MULTI_AGENT_TOOLS
from luann.functions.functions import derive_openai_json_schema, load_function_set
...
from luann.orm.tool import Tool as ToolModel
from luann.schemas.tool import Tool as PydanticTool
from luann.schemas.tool import ToolUpdate
...
class ToolManager:
    ...
    def upsert_base_tools(self, actor: PydanticUser) -> List[PydanticTool]:
        ...
        for module_name in module_names:
            full_module_name = f"luann.functions.function_sets.{module_name}"
            try:
        ...
        for name, schema in functions_to_schema.items():
            if name in BASE_TOOLS + BASE_MEMORY_TOOLS + MULTI_AGENT_TOOLS:
                if name in BASE_TOOLS:
                    tool_type = ToolType.LUANN_CORE
                    tags = [tool_type.value]
                elif name in BASE_MEMORY_TOOLS:
                    tool_type = ToolType.LUANN_MEMORY_CORE
                    tags = [tool_type.value]
                elif name in MULTI_AGENT_TOOLS:
                    tool_type = ToolType.LUANN_MULTI_AGENT_CORE
                    tags = [tool_type.value]
                else:
                    raise ValueError(
