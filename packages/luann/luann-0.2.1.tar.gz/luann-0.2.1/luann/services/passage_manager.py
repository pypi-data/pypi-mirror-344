...
from luann.embeddings import embedding_model, parse_and_chunk_text
from luann.orm.errors import NoResultFound
from luann.orm.passage import AgentPassage, SourcePassage
...
class PassageManager:
            ...
            for text in parse_and_chunk_text(text, embedding_chunk_size):
                ...
                if isinstance(embedding, dict):
                    try:
                        ...
                        raise TypeError(
                            f"Got back an unexpected payload from luann.text embedding function, type={type(embedding)}, value={embedding}"
                        )
    ...
    def update_passage_by_id(self, passage_id: str, passage: PydanticPassage, actor: PydanticUser, **kwargs) -> Optional[PydanticPassage]:
        ...
        with self.session_maker() as session:
            ...
            # Update the database record with values from luann.the provided record
    ...
    def delete_passage_by_id(self, passage_id: str, actor: PydanticUser) -> bool:
        """Delete a passage from luann.either source or archival passages."""
        if not passage_id:
            raise ValueError("Passage ID must be provided.")
