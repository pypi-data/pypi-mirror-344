...
from luann.constants import MULTI_AGENT_SEND_MESSAGE_MAX_RETRIES, MULTI_AGENT_SEND_MESSAGE_TIMEOUT
from luann.functions.helpers import async_send_message_with_retries
from luann.orm.errors import NoResultFound
from luann.server.rest_api.utils import get_luann_server

if TYPE_CHECKING:
    from luann.agent import Agent


def send_message_to_specific_agent(self: "Agent", message: str, other_agent_id: str) -> Optional[str]:
    """
    Send a message to a specific luann agent within the same organization.

    Args:
        message (str): The message to be sent to the target luann agent.
        other_agent_id (str): The identifier of the target luann agent.

    Returns:
        Optional[str]: The response from luann.the luann agent. It's possible that the agent does not respond.
    """
    server = get_luann_server()
...
def send_message_to_agents_matching_all_tags(self: "Agent", message: str, tags: List[str]) -> List[str]:
    ...
    Returns:
        List[str]: A list of responses from luann.the agents that match all tags.
