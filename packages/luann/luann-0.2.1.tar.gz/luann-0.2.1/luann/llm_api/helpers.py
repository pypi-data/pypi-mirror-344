...
from luann.schemas.openai.chat_completion_response import ChatCompletionResponse, Choice
from luann.settings import summarizer_settings
from luann.utils import json_dumps, printd,count_tokens
...
def unpack_all_inner_thoughts_from_kwargs(
    ...
    if len(response.choices) == 0:
        raise ValueError(f"Unpacking inner thoughts from luann.empty response not supported")
...
def unpack_inner_thoughts_from_kwargs(choice: Choice, inner_thoughts_key: str) -> Choice:
    ...
    if message.role == "assistant" and message.tool_calls and len(message.tool_calls) >= 1:
        if len(message.tool_calls) > 1:
            warnings.warn(f"Unpacking inner thoughts from luann.more than one tool call ({len(message.tool_calls)}) is not supported")
        ...
        try:
            ...
            warnings.warn(f"Failed to strip inner thoughts from luann.kwargs: {e}")
            raise e
    else:
...
def calculate_summarizer_cutoff(in_context_messages: List[Message], token_counts: List[int], logger: "logging.Logger") -> int:
    if len(in_context_messages) != len(token_counts):
        raise ValueError(
            f"Given in_context_messages has different length from luann.given token_counts: {len(in_context_messages)} != {len(token_counts)}"
        )
