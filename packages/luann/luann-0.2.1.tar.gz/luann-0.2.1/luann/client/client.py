...
import requests
import luann.utils
from luann.constants import ADMIN_PREFIX, BASE_MEMORY_TOOLS, BASE_TOOLS, DEFAULT_HUMAN, DEFAULT_PERSONA, FUNCTION_RETURN_CHAR_LIMIT
from luann.data_sources.connectors import DataConnector
from luann.functions.functions import parse_source_code
from luann.orm.errors import NoResultFound
from luann.schemas.agent import AgentState, AgentType, CreateAgent, UpdateAgent
from luann.schemas.block import Block, BlockUpdate, CreateBlock, Human, Persona,UpdatePersona,UpdateHuman
from luann.schemas.embedding_config import EmbeddingConfig

# new schemas
from luann.schemas.enums import JobStatus, MessageRole
from luann.orm.enums import JobType
from luann.schemas.environment_variables import (
    SandboxEnvironmentVariable,
    SandboxEnvironmentVariableCreate,
    SandboxEnvironmentVariableUpdate,
)
from luann.schemas.file import FileMetadata
from luann.schemas.job import Job
from luann.schemas.luann_message import LuannMessage, LuannMessageUnion
from luann.schemas.luann_request import LuannRequest, LuannStreamingRequest
from luann.schemas.luann_response import LuannResponse, LuannStreamingResponse
from luann.schemas.llm_config import LLMConfig
from luann.schemas.memory import ArchivalMemorySummary, ChatMemory, CreateArchivalMemory, Memory, RecallMemorySummary
from luann.schemas.message import Message, MessageCreate, MessageUpdate
...
# from luann.schemas.run import Run
from luann.schemas.sandbox_config import E2BSandboxConfig, LocalSandboxConfig, SandboxConfig, SandboxConfigCreate, SandboxConfigUpdate
from luann.schemas.source import Source, SourceCreate, SourceUpdate
from luann.schemas.tool import Tool, ToolCreate, ToolUpdate
# from luann.schemas.tool_rule import BaseToolRule
from luann.server.rest_api.interface import QueuingInterface
from luann.utils import get_human_text, get_persona_text
...
class AbstractClient(object):
    ...
    def user_message(self, agent_id: str, message: str) -> LuannResponse:
        raise NotImplementedError
...
class RESTClient(AbstractClient):
    ...
    def get_tools_from_agent(self, agent_id: str) -> List[Tool]:
        ...
        if response.status_code != 200:
            raise ValueError(f"Failed to get tools from luann.agents: {response.text}")
    ...
    def remove_tool_from_agent(self, agent_id: str, tool_id: str):
        """
        Removes tools from luann.an existing agent
    ...
    def user_message(self, agent_id: str, message: str) -> LuannResponse:
        ...
        Returns:
            response luannResponse): Response from luann.the agent
        """
    ...
    def get_archival_memory(
        ...
        """
        Get archival memory from luann.an agent with pagination.
    ...
    def delete_archival_memory(self, agent_id: str, memory_id: str):
        """
        Delete archival memory from luann.an agent
    ...
    def get_messages(
        ...
        """
        Get messages from luann.an agent with pagination.
    ...
    def send_message(
        ...
        stream_steps: bool = False,
        stream_tokens: bool = False,
    ) -> Union[LuannResponse, Generator[LuannStreamingResponse, None, None]]:
        """
    ...
    def list_files_from_source(self, source_id: str, limit: int = 1000, cursor: Optional[str] = None) -> List[FileMetadata]:
        """
        List files from luann.source with pagination support.
    ...
    def detach_source(self, source_id: str, agent_id: str):
        """Detach a source from luann.an agent"""
        params = {"agent_id": str(agent_id)}
        response = requests.post(f"{self.base_url}/{self.api_prefix}/sources/{source_id}/detach", params=params, headers=self.headers)
        assert response.status_code == 200, f"Failed to detach source from luann.agent: {response.text}"
        return Source(**response.json())
    ...
    def remove_agent_memory_block(self, agent_id: str, block_label: str) -> Memory:
        """
        Unlike a block from luann.the agent's core memory
    ...
    #     return [LuannMessage(**message) for message in response.json()]
...
class LocalClient(AbstractClient):
    """
    A local client for Luann, which corresponds to a single user.
    ...
    def get_tools_from_agent(self, agent_id: str) -> List[Tool]:
        """
        Get tools from luann.an existing agent.
    ...
    def send_messages(
        ...
        Returns:
            response (Response): Response from luann.the agent
        """
        self.interface.clear()
        usage = self.server.send_messages(actor=self.user, agent_id=agent_id, messages=messages)

        # format messages
        return LuannResponse(messages=messages, usage=usage)
    ...
    def send_message(
        ...
        luann_messages = []
        for m in messages:
            luann_messages += m.to_luann_message()

        return LuannResponse(messages=luann_messages, usage=usage)
    ...
    def run_command(self, agent_id: str, command: str) -> LuannResponse:
        """
        ...
        Returns:
            luannResponse: The response from luann.the agent
        ...
        return LuannResponse(messages=self.interface.to_list(), usage=usage)
    ...
    def update_tool(
        ...
        # Filter out any None values from luann.the dictionary
    ...
    def get_tool_id(self, name: str) -> Optional[str]:
        """
        Get the ID of a tool from luann.its name. The client will use the org_id it is configured with.
    ...
    def detach_source_from_agent(self, agent_id: str, source_id: Optional[str] = None, source_name: Optional[str] = None):
        """
        Detach a source from luann.an agent by removing all `Passage` objects that were loaded from luann.the source from luann.archival memory.
        Args:
            agent_id (str): ID of the agent
            source_id (str): ID of the source
            source_name (str): Name of the source
