...
from luann.orm.base import Base
...
class ToolMixin(Base):
    """Mixin for models that belong to an tool."""

    __abstract__ = True

    tool_id: Mapped[str] = mapped_column(String, ForeignKey("tools.id"))
...
class PassageMixin(Base):
    """Mixin for models that belong to a file."""

    __abstract__ = True

    passage_id: Mapped[str] = mapped_column(String, ForeignKey("passages.id"))
...
class MessageMixin(Base):
    """Mixin for models (e.g. file) that belong to a message."""
