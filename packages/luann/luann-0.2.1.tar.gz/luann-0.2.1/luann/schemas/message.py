...
from luann.constants import DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG, TOOL_CALL_ID_MAX_LEN
from luann.local_llm.constants import INNER_THOUGHTS_KWARG
from luann.schemas.enums import MessageContentType, MessageRole
...
from luann.system import unpack_message
from luann.utils import get_utc_time, is_utc_datetime, json_dumps
...
class Message(BaseMessage):
    """
    luann's internal representation of a message. Includes methods to convert to/from luann.LLM provider formats.
    ...
    @staticmethod
    def to_luann_messages_from_list(
        ...
        # Convert messages to luannMessages
        return [
            msg
            for m in messages
            for msg in m.to_luann_message(
    ...
    def to_luann_message(
        self,
        use_assistant_message: bool = False,
        assistant_message_tool_name: str = DEFAULT_MESSAGE_TOOL,
        assistant_message_tool_kwarg: str = DEFAULT_MESSAGE_TOOL_KWARG,
    ) -> List[LuannMessage]:
        """Convert message object (in DB format) to the style used by the original luann API"""
            ...
            if self.tool_calls is not None:
                ...
                for tool_call in self.tool_calls:
                    ...
                    if use_assistant_message and tool_call.function.name == assistant_message_tool_name:
                        # We need to unpack the actual message contents from luann.the function call
                        try:
                            func_args = json.loads(tool_call.function.arguments)
    ...
    def dict_to_message(
        ...
        # If we're going from luann.deprecated function form
        if openai_message_dict["role"] == "function":
            ...
            # Convert from luann.'function' response to a 'tool' response
            if id is not None:
                return Message(
                    agent_id=agent_id,
                    model=model,
            ...
            # Convert a function_call (from luann.an assistant message) into a tool_call
            ...
            # If we're going from luann.tool-call style
            if id is not None:
                return Message(
                    agent_id=agent_id,
                    model=model,
    ...
    def to_openai_dict(
        ...
        """Go from luann.Message class to ChatCompletion message object"""
    ...
    def to_google_ai_dict(self, put_inner_thoughts_in_kwargs: bool = True) -> dict:
        """
        Go from luann.Message class to Google AI REST message object
        """
    ...
    def to_cohere_dict(
        ...
        # TODO: update this prompt style once guidance from luann.Cohere on
