...
from luann.constants import EMBEDDING_TO_TOKENIZER_DEFAULT, EMBEDDING_TO_TOKENIZER_MAP, MAX_EMBEDDING_DIM
from luann.schemas.embedding_config import EmbeddingConfig
from luann.utils import is_valid_url, printd
...
class EmbeddingEndpoint:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not is_valid_url(base_url):
            raise ValueError(
                f"Embeddings endpoint was provided an invalid URL (set to: '{base_url}'). Make sure embedding_endpoint is set correctly in your Luann config."
            )
        # TODO: find a neater solution - re-mapping for luann endpoint
        if model == "luann-free":
    ...
    def _call_api(self, text: str) -> List[float]:
        if not is_valid_url(self._base_url):
            raise ValueError(
                f"Embeddings endpoint does not have a valid URL (set to: '{self._base_url}'). Make sure embedding_endpoint is set correctly in your luann config."
            )
        import httpx
        ...
        if isinstance(response_json, list):
            ...
            try:
                ...
                raise TypeError(f"Got back an unexpected payload from luann.text embedding function, response=\n{response_json}")
        else:
...
def embedding_model(config: EmbeddingConfig, user_id: Optional[uuid.UUID] = None):
    ...
    # TODO: refactor to pass in settings from luann.server
