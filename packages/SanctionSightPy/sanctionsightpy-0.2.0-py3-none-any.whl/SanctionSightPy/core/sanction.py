...
class SanctionService:
    ...
    async def get_filtered(
        ...
        Example:
            ...
            >>> filters = [DateRangeFilter(min_price=500000), NameFilter(min_bedrooms=3)]
            >>> filtered_data = await service.get_filtered('canada_sanction', filters)
        """
    ...
    async def to_pandas(self, state: str, columns: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        Example:
            >>> service = SanctionService(client)
            >>> df = await service.to_pandas('canada_sanction', columns=['entity', 'last_name'])
            >>> df.head()
        """
