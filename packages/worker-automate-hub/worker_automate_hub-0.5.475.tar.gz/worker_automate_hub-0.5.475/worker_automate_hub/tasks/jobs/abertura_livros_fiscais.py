...
async def abertura_livros_fiscais(task: RpaProcessoEntradaDTO) -> RpaRetornoProcessoDTO:
    try:
        ...
        if return_login.sucesso:
            ...
            # Clicando nao na tela de somar icms
            try:
                console.print("clicando em nao na janela de somar icms..")
                await emsys.verify_warning_and_error("Gerar Registros", "&No")
                await worker_sleep(5)
            except:
                # Tentando novamente clicar em nao na tela de somar
                console.print(
                    "tentando clicar novamente em nao na janela de somar icms.."
                )
                await emsys.verify_warning_and_error("Gerar Registros", "&Nao")
                await worker_sleep(5)
