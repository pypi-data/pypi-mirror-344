...
async def wait_aguarde_window_closed(app, timeout=60):
    ...
    while time.time() - start_time < timeout:
        ...
        console.print(f"Titulo da janela top:  {janela_topo.window_text()}")
        await emsys.verify_warning_and_error("Aviso", "&Ok")
        await worker_sleep(2)
        all_windows_uia = Desktop(backend="uia").windows()
        all_windows_win = Desktop(backend="win32").windows()

        aguarde_ou_travada_uia = any(
            (
                "aguarde" in w.window_text().lower()
                or "respondendo" in w.window_text().lower()
            )
            and w.is_visible()
            for w in all_windows_uia
        )
        aguarde_ou_travada_win = any(
            (
                "aguarde" in w.window_text().lower()
                or "respondendo" in w.window_text().lower()
            )
            and w.is_visible()
            for w in all_windows_win
        )

        if aguarde_ou_travada_uia or aguarde_ou_travada_win:
            console.print(
                "Janela 'Aguarde...' ou travada ainda aberta", style="bold yellow"
            )
        else:
            console.print(
                "Nenhuma janela 'Aguarde...' ou travada detectada", style="bold green"
            )
            return
    console.log("Timeout esperando a janela 'Aguarde...' ou travada fechar.")
