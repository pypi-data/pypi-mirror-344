...
class ClientConsole:
    ...
    def _acompanhamento_carga_inicial_presenter(self, view: acompanhamento_window.Ui_FormAcompanhamento):
        ...
        def on_show_event(event):
            ...
            QTimer.singleShot(0, lambda: self.executar_carga_inicial([], self._env, self._tenant_espelho))
    ...
    def _acompanhamento_integracao_presenter(self, view: acompanhamento_window.Ui_FormAcompanhamento):
        ...
        def on_show_event(event):
            event.accept()
            _previous_show(event)
            QTimer.singleShot(0, lambda: self.executar_integracao([], self._env, self._tenant_espelho))
    ...
    def _acompanhamento_verificacao_integridade_presenter(self, view: acompanhamento_window.Ui_FormAcompanhamento):
        ...
        def on_show_event(event):
            ...
            QTimer.singleShot(0, lambda: self.executar_verificacao_integridade(args))
    ...
    def _do_desativar_integracao(self):
        if msg_window.confirmar_acao(None, "Deseja desativar a integração?"):
            try:
                _symmetrics_instalado = self.integracao_dao.symmetrics_instalado()
    ...
    def modo_janela(self):
        ...
        try:
            with InjectorFactory() as injector:
                ...
                _existem_nodes_symmetrics_ativos = self.integracao_dao.existem_nodes_symmetrics_ativos()
