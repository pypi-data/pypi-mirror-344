...
def do_ai_auto(Testk_data):
    ...
    try:
        ...
        if "https://" in Testk_data:
            ...
            num = 0
            for i in Testk_data:
                aa = json.loads(i['ext']['input'])['user_input']
                if startAt == 0:
                    startAt = int(time.time())
                # 创建会话
                seseion_id = AilyLarkClient().create_ailysession(tenant_access_token)
...
if __name__ == '__main__':
    url = "http://localhost:8000/FlowApp/get_collection_cases_by_collection?collection_id=27"
    ...
    cases = response_res['data']['results']
