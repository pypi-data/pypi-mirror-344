...
def abacus_dataset(config):
    ...
    if config["abacus"]["dataset_path"]:
        if os.path.exists(config["abacus"]["dataset_path"]):
            ...
            for root, _, files in os.walk(config["abacus"]["dataset_path"]):
                for file in files:
                    if file == 'STRU':
                        dataset_roots.append(os.path.join(root, file))
                        break
    ...
    if not os.path.exists(config['init_train_xyz']):
        myprint(f"未找到 {config['init_train_xyz']} 请检查配置文件", 'RED')
        exit()
    # 自动生成 abacus dataset
    home_path = os.getcwd()
    dataset_roots = []
