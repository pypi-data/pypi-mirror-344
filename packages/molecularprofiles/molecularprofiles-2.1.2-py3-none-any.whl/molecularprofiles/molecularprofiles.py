...
class MolecularProfile:
    ...
    def create_molecular_extinction_profile(
        self,
        rayleigh_extinction_table: Table,
        molecular_absorption_table: Table,
        mep_file: str | None = None,
    ) -> Table:
        """
        Create a Molecular Extinction Profile (MEP) by combining Rayleigh scattering and molecular absorption.

        This function combines Rayleigh scattering and molecular
        absorption data provided in an Astropy Tables. The combined extinction data is then saved to a new
        file if a file name is provided, or returned as an Astropy Table. This combined extinction profile
        represents the molecular extinction due to both Rayleigh scattering and molecular absorption for
        each altitude bin and wavelength bin specified in the input data.

        Parameters
        ----------
        rayleigh_extinction_table : Table
            An Astropy Table containing the Rayleigh scattering data. This table should contain
            absolute optical depth per altitude bin per wavelength bin.
        molecular_absorption_table : Table
            An Astropy Table containing the molecular absorption data. This table should include the optical
            depth per altitude bin per wavelength bin due to molecular absorption.
        mep_file : Optional[str], default=None
            The path where the combined molecular extinction profile should be saved. If None, the profile
        ...
        """
        molecular_extinction_table = rayleigh_extinction_table.copy()
        for col_ma, col_rs in zip(
            ...
            if col_ma != "altitude_max" and col_ma != "altitude_min":
                molecular_extinction_table[col_rs] = (
                    molecular_absorption_table[col_ma]
                    + molecular_extinction_table[col_rs]
                )
        if mep_file:
    ...
    def convert_to_simtel_compatible(
        self, input_ecsv_file: str, output_file: str, observation_altitude: Quantity
    ) -> None:
        """
    ...
    def timeseries_analysis(
        ...
        parameter (e.g., number density, temperature, pressure, relative humidity) at a specified altitude level.
        The analysis is focused between specified lower and upper altitude bounds (m_floor and m_ceiling).
        The result is an Astropy table containing the provided atmospheric parameter at the specified altitude level
        as a function of the Modified Julian Date (MJD). This table is then saved to a specified output file.
        The timeseries analysis can be used for various purposes, such as identifying seasons or validating data
        assimilation systems (DAS) against local weather station observations.
        ...
        altitude_level : Quantity
            The specific altitude level at which the timeseries analysis will be conducted. This is typically
            chosen based on where significant seasonal differences are observed or where validation against
            local observations is desired.
        atmospheric_parameter : str
        ...
        m_floor : Quantity
            The lowest altitude to consider in the analysis. This defines the lower bound of the altitude range.
        m_ceiling : Quantity
            The highest altitude to consider in the analysis. This defines the upper bound of the altitude range.
        altitude_list : Quantity, optional
            A sequence of altitudes at which the atmospheric parameters will be calculated. This list should
            cover the range between m_floor and m_ceiling. Defaults to a standard CORSIKA altitude profile.
        ...
        altitude level across the provided altitude list within the bounds of m_floor and m_ceiling.
        """
        tables = []
        altitude = altitude_list[
            (altitude_list.to_value() >= m_floor.to_value(altitude_list.unit))
