...
class MasterKey:
    def __init__(self, openai_key: str, generation_model: str = "deepseek-chat", evaluation_model: str = "deepseek-chat"):
        """
        ...
        self.client = OpenAI(
            ...
            base_url="https://api.deepseek.com",  # 新增DeepSeek的base_url
    ...
    def llm_completion(self, prompt: str, model: str) -> str:
        """
        Calls the Deepseek API to get a completion for the given prompt using the specified model.
        ...
        response = self.client.chat.completions.create(  # 使用预初始化的客户端
            model=model,
            messages=[
                {"role": "user", "content": prompt},
            ],
            stream=False  # 根据DeepSeek示例添加stream参数
        )
