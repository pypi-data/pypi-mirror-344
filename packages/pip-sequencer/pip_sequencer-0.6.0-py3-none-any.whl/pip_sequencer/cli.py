...
def main():
    # Main parser for ps/pip-sequencer
    parser = argparse.ArgumentParser(
        prog='ps', # Use 'ps' as the program name in help messages
        ...
        epilog="""Examples:
  ps install requests              # Install requests using pip backend (default)
  ps uv install flask              # Install flask using uv backend
  ps pip uninstall django         # Uninstall django using pip backend explicitly
  ps replay --from-export        # Replay exported sequence using pip backend (default)
  ps uv replay --start 5         # Replay steps 5+ from history using uv backend
  ps export -o my_sequence.json  # Export current state sequence
        """
    )
    parser.add_argument(
        "--file", "-f",
...
Example: 'ps uv install ...' or 'ps install ...' (uses pip).""",
        help="Action command or explicit backend selection."
    )

    # --- Define actions (install, uninstall, replay) ---
    # This function will be called to add these actions under the main parser
    # (for default 'pip' backend) and under the explicit 'uv' and 'pip' subparsers.
