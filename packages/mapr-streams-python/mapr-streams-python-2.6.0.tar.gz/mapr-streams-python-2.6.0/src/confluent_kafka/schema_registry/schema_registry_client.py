...
from confluent_kafka.auth_utils.mapr_auth_utils import get_challenge
...
class _RestClient(object):
    """
    HTTP client for MapR Schema Registry.
    ...
    def _getRequestHeaders(self, body):
    ...
    def send_request(self, url, method, body=None, query=None):
        ...
        headers = self._getRequestHeaders(body)
        if body is not None:
            body = json.dumps(body)

        response = self.session.request(
            method, url="/".join([self.base_url, url]),
            headers=headers, data=body, params=query, verify=True)
...
class _MaprRestClient(_RestClient):
    """
    ...
    def __init__(self, conf):
        # Call the superclass constructor to handle the core setup
        super().__init__(conf)
        self.challenge_str = get_challenge(self.base_url)

    def _getAuthrorizationHeader(self):
        return "MAPR-Negotiate {}".format(self.challenge_str)

    def _getRequestHeaders(self, body):
        headers = {'Accept': "application/vnd.schemaregistry.v1+json,"
                        " application/vnd.schemaregistry+json,"
                        " application/json",
                    'Authorization': self._getAuthrorizationHeader()}
...
class SchemaRegistryClient(object):
    ...
    def __init__(self, conf):
        self._rest_client = _MaprRestClient(conf)
