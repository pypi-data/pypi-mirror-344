...
**mapr-streams-python** provides a high-level Producer, Consumer and AdminClient compatible with all
...
for data in some_data_source:
    ...
    p.produce('/streamName:topicName', data.encode('utf-8'), callback=delivery_report)
...
c.subscribe(['/streamName:topicName'])
...
new_topics = [NewTopic(topic, num_partitions=3, replication_factor=1) for topic in ["/stream1:topic1", "/stream1:topic2"]]
...
MapR streams python client does not support broker.
