            ...
            try:
                ...
                try:
                    ...
                    if ws_endpoint_cls is None:
                        ex_str: str = (f"WebSocket URL({lines[0].split()[1]})\r\n"
                                       "cannot match endpoint signature\r\n\r\n")
                        socket_client.send(ex_str.encode())
                        exception(ValueError(f"WebSocket URL({lines[0].split()[1]}) cannot match endpoint signature"))
                        continue
