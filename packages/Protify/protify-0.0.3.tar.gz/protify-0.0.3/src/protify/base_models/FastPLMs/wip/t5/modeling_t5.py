...
from .t5_flex_attention import T5FlexAttention

try:
    from torch.nn.attention.flex_attention import flex_attention
    FLEX_ATTENTION_AVAILABLE = True
    print(f"[Flex Attention] PyTorch {torch.__version__} is supported")
except ImportError:
    print(f"[Warning] flex attention needs PyTorch 2.5.0+ but your version is {torch.__version__}")
    print('Defaulting to native transformers implementation')
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, layer_idx: Optional[int] = None):
        ...
        if FLEX_ATTENTION_AVAILABLE:
            self.SelfAttention = T5FlexAttention(
                config, has_relative_attention_bias=has_relative_attention_bias, layer_idx=layer_idx, compile_flex=True
            )
        else:
            self.SelfAttention = T5AttentionTransformers(
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config, layer_idx: Optional[int] = None):
        super().__init__()
        if FLEX_ATTENTION_AVAILABLE:
            self.EncDecAttention = T5FlexAttention(config, has_relative_attention_bias=False, layer_idx=layer_idx, compile_flex=True)
        else:
            self.EncDecAttention = T5AttentionTransformers(config, has_relative_attention_bias=False, layer_idx=layer_idx)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
            ...
            if hasattr(module.wo, "bias") and module.wo.bias is not None:
                module.wo.bias.data.zero_()
        elif isinstance(module, T5AttentionTransformers) or isinstance(module, T5FlexAttention):
...
if __name__ == '__main__':
    ...
    outputs = model(inputs_embeds=random_input)
    print(outputs)
